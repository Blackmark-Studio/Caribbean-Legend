...
void CreateCitizens(ref loc)
{
	...
	if(isMainCharacterPatented() && int(Items[int(pchar.EquipedPatentId)].TitulCur) > 4) generateAffairOfHonor = false;
	// нищие -->
	if (loc.type == "town")
	{
	...
	if(CheckAttribute(pchar,"GenQuest.NightAdventure") && loc.type == "town" && bNightAdventure)
	{
		switch(rand(3)) 
		{
			case 0: // матрос
				...
				if (int(Colonies[iColony].HeroOwn) == true) LAi_group_MoveCharacter(chr, LAI_GROUP_PLAYER_OWN);
                    ...
                    case 2:
										{
											...
											GiveItem2Character(chr, blade);
											EquipCharacterbyItem(chr, blade);
											ForceAutolevel(chr, GEN_TYPE_ENEMY, GEN_COMMONER, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, true, 0.6);
											pchar.GenQuest.NightAdventureVar = "NightAdventure_PirateBad";
										}
										break; // плохой исход
                }
	...
	{
		if(iNation != PIRATE || int(Colonies[iColony].HeroOwn) == true || bHasCallback)//колония
		{
			...
			for(i=0; i<iSailorQty; i++)//матросы
			{
				...
				ForceAutolevel(chr, GEN_TYPE_ENEMY, GEN_COMMONER, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.6); // RB Мирные граждане
			}
			...
			for(i=0; i<iCapQty; i++)//капитаны кораблей
			{
				...
				ForceAutolevel(chr, GEN_TYPE_ENEMY, GEN_ELITE, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.6); // RB Мирные граждане
			}
			for(i=0; i<iCitizQty; i++)//мещане-бюргеры + посетители церкви
			{
				...
				if(bAh || bBettaTestMode)
				{
					if(generateAffairOfHonor)
					{
						...
						{
							if(int(PChar.QuestTemp.AffairOfHonor.FinishCount) < 8 && !CheckAttribute(PChar, "QuestTemp.AffairOfHonor.CoatHonor") && GetCharacterIndex("AffairOfHonor_QuestMan") == -1)
							{
								if(chr.sex == "man" || !CheckAttribute(PChar, "QuestTemp.AffairOfHonor.Cavalier"))
								{
									// Jason: дохляки нам не нужны - делаем кулфайтера
									int Rank = int(pchar.rank)+MOD_SKILL_ENEMY_RATE+10;
									...
									ForceAutolevel(chr, GEN_TYPE_ENEMY, GEN_MINIBOSS, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.6); // RB Одиночные кулфайтеры
									}
			...
			for(i=0; i<iMonkQty; i++)//монахи
			{
				...
				if (rand(4) == 4 && !CheckAttribute(pchar, "questTemp.ShipCapellan") && pchar.ship.type != SHIP_NOTUSED && 6-int(RealShips[int(Pchar.Ship.Type)].Class) > 0) //корабельный капеллан
				{
	...
	if (checkAttribute(loc, "soldiers") && CheckAttribute(loc, "locators.soldiers"))
	{
		...
		for (i=0; i<iCitizensQuantity; i++)
		{
            ...
            SetFantomParamFromRank(chr, int(pchar.rank)+MOD_SKILL_ENEMY_RATE, true); // бравые орлы
	...
	if(CheckAttribute(loc,"locators.fisher"))
	{
		...
		for (i = 1; i <= iSailorQty; i++)
		{
			...
			chr = &characters[iChar];
			SetNPCModelUniqForLoc(chr, sType, MAN, loc);
			if(!HasSubStr(chr.model, "fisherman")) chr.model = "fisherman_"+int(rand(10)+1);
			chr.City = Colonies[iColony].id;
			RemoveAllCharacterItems(chr, true);
			chr.CityType = sType;
			chr.chr_ai.hp_max = 1;
	...
	if(CheckAttribute(loc,"locators.benchsit"))
	{
		...
		for (i = 1; i <= iSailorQty; i++)
		{
			...
			if(iSex == MAN)
			{
				if(rand(3) == 2)
				{
					CreateModel(iChar, "drinker", iSex, loc);
				}
			}
			chr.City = Colonies[iColony].id;
			RemoveAllCharacterItems(chr, true);
...
void CreateIslatesoroEng(ref loc)//Jason, Исла Тесоро при провале Саги
{
		...
		if (CheckAttribute(loc, "fastreload"))
		{
			...
			{ 
				...
				if (CheckFreeLocator(loc.id, "sit_base"+i, -1))
				{
					...
					iChar = NPC_GeneratePhantomCharacterForLoc(sModelGS, iHeroNation, MAN, 1, loc);
					if (iChar != -1)
					{
						chr = &characters[iChar];
						...
						{
							chr.model = "sold_" + NationShortName(int(chr.nation)) + "_" + (1 + rand(7));
							FaceMaker(chr);
							CirassMaker(chr);
			...
			{ 
				...
				if(CheckFreeLocator(loc.id, "sit_base"+i, -1))
				{
					iChar = NPC_GeneratePhantomCharacterForLoc("sailor", iHeroNation, MAN, 1, loc);
					if (iChar != -1)
					{
						chr = &characters[iChar];
						SetNPCModelUniqForLoc(chr, "sailor", MAN, loc);
			...
			if (!CheckAttribute(pchar, "questTemp.Sharlie.Lock"))//не идёт старт квеста Шарля
			{
				...
				for (i = 0; i <iCitizensQuantity; i++)
				{
					...
					sTemp = PlaceCharacter(chr, "sit", "random_free");
					ReSitCharacterOnFree(chr, loc.id, sTemp);
					ForceAutolevel(chr, GEN_TYPE_OFFICER, GEN_ELITE, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.5); // RB Генераторные офицеры
			...
			if (hrand(99, Colonies[iColony].id) < i || TestRansackCaptain)
			{
				...
				if (iChar != -1)
				{
					...
					if (GetCharacterItem(pchar, "map_full") == 0) // нет карты - генерим, если купит
					{
						...
						chr.TreasureMoney = 45 + hrand(75, sTemp); // Addon-2016 Jason
					}
					if (int(Colonies[iColony].HeroOwn) == true)
					{
						LAi_group_MoveCharacter(chr, LAI_GROUP_PLAYER_OWN);
            ...
            else TEV.TavernMapBonus = int(TEV.TavernMapBonus) + 2;
            // <-- Кладмен
...
void CreateMaltains(ref loc)//Jason, заполнение базы мальтийцев в Сен-Пьере
{	
	...
	if (CheckAttribute(loc, "Maltains"))
	{
		...
		if (!CheckAttribute(loc, "Maltains_date") || GetNpcQuestPastDayParam(loc, "Maltains_date") >= 2)
		{
			...
			FantomMakeCoolFighter(sld, int(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 100, 100, "blade_07", "pistol5", "bullet", 150);//спецназ
			sld.greeting = "patrol";
			...
			for (i=1; i<=4; i++)
			{
				...
				FantomMakeCoolFighter(sld, int(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 90, 90, "blade_08", "pistol6", "bullet", 100);//спецназ
				sld.greeting = "protector";
				sld.Dialog.Filename = "Quest\Sharlie\OtherNPC.c";
			...
			for (i=1; i<=4; i++)
			{
				...
				FantomMakeCoolFighter(sld, int(pchar.rank)+MOD_SKILL_ENEMY_RATE+10, 70, 70, "blade_08", "pistol7", "bullet", 100);//спецназ
				sld.Dialog.Filename = "Quest\Sharlie\OtherNPC.c";
				sld.dialog.currentnode = "Maltie_soldier";	
				sld.greeting = "soldier";
			...
			for (i=1; i<=5; i++)
			{
				...
				LAi_SetLoginTime(sld, 0.0, 24.0);
				LAi_SetPatrolType(sld);
				ForceOldGenerateToNew(sld, int(pchar.rank)+MOD_SKILL_ENEMY_RATE+10);
				sld.city = "FortFrance";
				sld.citytype = "soldier";
...
void CreateHWICOffice(ref loc)//Jason, заполнение офиса ГВИК. Belamour legendary edition переехали в новое здание
{	
...
void CreateLSCGuardClan(ref loc)
{	
	if (loc.id == "LostShipsCity_town")
	{
		...
		int i;
		int iRank = int(pchar.rank)+MOD_SKILL_ENEMY_RATE+15;
...
void CreateLSCInsideClan(ref loc) // создаем кланы внутри их кораблей
{
...
void CreateKsochitamSkeletons(ref loc) //Jason
{	
	if (CheckAttribute(loc, "KsochitamSkeletons"))
	{
		ref sld;
		int i;
		int n = int(loc.KsochitamSkeletons);
		string sIdf = loc.id;
		int iRank = int(pchar.rank)+MOD_SKILL_ENEMY_RATE+8;
		...
		if (!CheckAttribute(loc, "Ksochitam_date") || GetNpcQuestPastDayParam(loc, "Ksochitam_date") >= 2)
		{
			...
			for (i=1; i<=n; i++)
			{
				...
				ForceAutolevel(sld, GEN_TYPE_ENEMY, GEN_ELITE, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.6); // RB Скелеты Ксочитем
...
void CreateKhaelRoaSkeletons(ref loc) //Jason
{	
			...
			if (!CheckAttribute(loc, "KhaelRoa_date") || GetNpcQuestPastDayParam(loc, "KhaelRoa_date") >= 1)
			{
				...
				for (i=0; i<n; i++)
				{
					if (rand(1) == 0 || loc.id.label == "Alcove") // чавинави
					{
						...
						ForceAutolevel(sld, GEN_TYPE_ENEMY, GEN_MINIBOSS, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.6); // RB Скелеты Калеуче чавинави
					...
					else // скелеты
					{
						...
						ForceAutolevel(sld, GEN_TYPE_ENEMY, GEN_ELITE, GEN_ARCHETYPE_RANDOM, GEN_ARCHETYPE_RANDOM, GEN_RANDOM_PIRATES, 0.6); // RB Скелеты Калеуче обычные
...
void CreateIncquisitio(ref loc)
{	
	...
	if (CheckAttribute(loc, "Incquisitio"))
	{
		...
		if (CheckNPCQuestDate(loc, "Guard_date"))
		{
			...
			for (i=1; i<=4; i++)
			{
				...
				sld.CityType = "soldier";
				LAi_LoginInCaptureTown(sld, true);
				FantomMakeCoolFighter(sld, int(pchar.rank)+MOD_SKILL_ENEMY_RATE+15, 100, 90, BLADE_LONG, "pistol3", "grapeshot", 100);//спецназ
				LAi_SetLoginTime(sld, 0.0, 24.0);
				LAi_SetGuardianType(sld);
				LAi_group_MoveCharacter(sld, "SPAIN_CITIZENS");	
		...
		if (!CheckAttribute(loc, "Mayak_date") || GetNpcQuestPastDayParam(loc, "Mayak_date") > 2)
		{
				...
				for (i=0; i<iCitizensQuantity; i++)
				{
					...
					if (findsubstr(locatorName, "protector" , 0) != -1)
					{
						...
						if (iChar == -1)
							continue;
						chr = &characters[iChar];
						SetNPCModelUniqForLoc(chr, "soldier", MAN, loc);
					}
					else
					{
						iChar = NPC_GeneratePhantomCharacterForLoc("mushketer", iNation, MAN, 2, loc);
						if (iChar == -1)
							continue;
						chr = &characters[iChar];
						SetNPCModelUniqForLoc(chr, "mushketer", MAN, loc);
						chr.MusketerDistance = 0;
					}
					chr.City = Colonies[iColony].id;
					chr.CityType = "soldier";
					chr.greeting = "soldier_common";
					SetFantomParamFromRank(chr, int(pchar.rank)+MOD_SKILL_ENEMY_RATE+6, true); // бравые орлы
					LAi_SetLoginTime(chr, 0.0, 24.0);
					solderLoc = GetAttributeN(st, i);
					locatorName = GetAttributeName(solderLoc);
					ChangeCharacterAddressGroup(chr, pchar.location, "soldiers", locatorName);
			...
			if (checkAttribute(loc, "soldiers") && CheckAttribute(loc, "locators.patrol"))
			{
				...
				for (i=0; i<iCitizensQuantity; i++)
				{
					...
					LAi_SetLoginTime(chr, 0.0, 24.0);
					LAi_SetPatrolType(chr);
					ForceOldGenerateToNew(chr, int(pchar.rank)+MOD_SKILL_ENEMY_RATE);
					LAi_group_MoveCharacter(chr, slai_group);
					chr.greeting = "soldier";
					PlaceCharacter(chr, "patrol", "random_free");
					chr.dialog.filename = "Common_Soldier.c";
...
void CreatePearlVillage(ref loc) // Jason: деревни мискито, браво, араваков
{
...
void CreateIndianVillage(ref loc) // Jason: деревня карибов
{
...
void CreateLosTequesMine(ref loc) // Jason: шахта на руднике
{
...
void CreateMineBandits(ref loc) // Jason: бандиты у рудника у Блювельда
{
	ref chr;
	int i;
	int iRank = int(pchar.rank)+MOD_SKILL_ENEMY_RATE+5;
...
void CreateInsideHouseEncounters(ref loc)
{
...
void CreateInsideResidenceEncounters(ref loc)
{
	string sLabel = loc.id.label$string("");
	bool bOk;
	if (CheckAttribute(loc, "MustSetReloadBack"))
	{ //если заглушка
		if (sLabel == "TownhallRoom" || sLabel == "Townhall")
		{
...
void CreateResidenceNpc(ref loc)
{
	ref chr;
	int Rank;
	...
	case 1:
		...
		break;
	case 2:
		Rank = int(pchar.rank) + MOD_SKILL_ENEMY_RATE;
...
bool CreateQuestResidenceNPC(ref loc)
{
	bool bOk;
...
void CreateSkladInsideEncounters(ref loc)
{
	if (CheckAttribute(loc, "packhouse") && !IsLocationCaptured(loc.id))
	{
		...
		if (GetCharacterIndex(sName) > 0) return; //если есть
		int Rank = int(pchar.rank) + rand(6);
		if (Rank < 10) Rank += 10;
		if (Rank > 30) Rank -= 10;
		...
		if (checkAttribute(loc, "soldiers") && CheckAttribute(loc, "locators.soldiers"))
		{
			...
			for (i=0; i<iCitizensQuantity; i++)
			{
				...
				if (findsubstr(locatorName, "protector" , 0) != -1 && iNation != PIRATE) 
				{
					...
					chr.greeting = "protector";
					// belamour cle
					if (!CheckAttribute(pchar, "questTemp.Sharlie.FastStart") && int(pchar.rank) < 4
...
void CreateAmmo(ref loc) // Jason, оружейная
{
	if (loc.type == "ammo")
	{
		...
		if (!CheckAttribute(loc, "Ammo_date") || GetNpcQuestPastDayParam(loc, "Ammo_date") > 2)
		{
			...
			iNation = GetCityNation(loc.parent_colony);
			string slai_group = GetNationNameByType(iNation)  + "_citizens";
			iRank = int(pchar.rank)+5+MOD_SKILL_ENEMY_RATE*2;
			// комендант
...
void CreatIslaMonaNPC(ref loc)
{
	...
	//if(int(pchar.questTemp.IslaMona.TownStage) < 2) return;
	int iColony = -1;
	...
	if(loc.id == "islamona_town")
	{
		...
		if(CheckAttribute(pchar,"questTemp.IslaMona.TownStage") && int(pchar.questTemp.IslaMona.TownStage) > 1)
		{
			...
			if(CheckAttribute(rTown,"Ship.crew.quantity") && rTown.Ship.crew.quantity > 0) // матросы у костра
			{
				...
				int j = int(rTown.Ship.crew.quantity)/100;
				if(j < 1) j=1;
				for(i=1; i<=j; i++)
				{
		...
		if(int(pchar.questTemp.IslaMona.TownStage) > 1)
		{
		...
		if(int(pchar.questTemp.IslaMona.TownStage) > 0)
		{
			if(CheckAttribute(loc,"locators.blacksmith"))
			{
				makearef(locatorGroup, loc.locators.blacksmith);
				...
				for (i = 1; i <= iSailorQty; i++)
				{
					...
					if (iChar == -1)
						continue;
					chr = &characters[iChar];
					SetNPCModelUniqForLoc(chr, "blacksmith", MAN, loc);
	...
	if(iColony != -1 && loc.id == "IslaMona_Jungle_01")
	{
		...
		if(CheckAttribute(pchar,"questTemp.IslaMona.TownStage") && int(pchar.questTemp.IslaMona.TownStage) > 1)
		{
			...
			if(CheckAttribute(rTown,"Ship.crew.quantity") && rTown.Ship.crew.quantity > 0) // матросы у костра
			{
				log_testinfo("crew.quantity Ok...");
				arrayNPCModelHow = 0;
				j = int(rTown.Ship.crew.quantity)/75;
				if(j < 1) j=1;
				for(i=1; i<=j; i++)
				{
					iChar = NPC_GeneratePhantomCharacterForLoc("sailor", PIRATE, MAN, 1, loc);
	...
	// пирс
	if(FindColony("islamona") != -1 && int(pchar.questTemp.IslaMona.TownStage) > 1 && loc.id == "Shore75" && loc.models.always.locators == "Shore07_locators_pier")
	{
...
int GetCityNation(string _sColony) // boal метод вернет нацию по ид города
{
	...
	return int(Colonies[iColony].nation);
}
...
void ReSitCharacterOnFree(ref chr, string _locId, string sTemp)
{
	...
	n = 0;
	while (!CheckFreeLocator(_locId, sTemp, int(chr.index)) && n < 10)
	{
	    sTemp = PlaceCharacter(chr, "sit", "random_free");
	    n++;
	}
	...
	{    // чтоб не занимали место напротив
	    sTemp = PlaceCharacter(chr, "sit", "random_free");
	    k = 0;
	    while (!CheckFreeLocator(_locId, sTemp, int(chr.index)) && k < 10)
		{
		    sTemp = PlaceCharacter(chr, "sit", "random_free");
		    k++;
		}
...
void SetNPCModelUniqForLoc(ref chr, string sType, int iSex, ref rLoc)
{
	int i = 0;
	...
	while (chr.model == "" || !CheckNPCModelUniq(chr))
	{
		...
		while (j < 10)
		{
			bResult = CreateModel(int(chr.index), sType, iSex, rLoc);
			if (bResult)
			{
