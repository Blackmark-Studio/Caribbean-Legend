...
void RemoveAllCharacterItems(ref _ch, bool _removemoney, bool removeMusket = false)
{
	// сносим нафик всю экипировку
	RemoveCharacterEquip(_ch, BLADE_ITEM_TYPE);
	RemoveCharacterEquip(_ch, GUN_ITEM_TYPE);
	if (removeMusket) RemoveCharacterEquip(_ch, MUSKET_ITEM_TYPE);
	...
	{		
		sName = GetAttributeName(GetAttributeN(arItems, a));
		TakeNItems(_ch, sName, int(arItems.(sName)));
	}		
...
{
	int iShipClass = int(RealShips[int(_ch.Ship.Type)].Class);
	int iShipType;
...
bool IsPCharHaveTreasurer()
{
	...
	for(int i=0; i<MAX_CHARACTERS; i++)
	{
		...
		if(CheckAttribute(rOfficer, "treasurer") && int(rOfficer.Treasurer) == 1)
		{
...
ref GetPCharTreasurerRef()
{
	...
	for(int i=0; i<MAX_CHARACTERS; i++)
	{
		if(CheckAttribute(&characters[i], "treasurer") && int(characters[i].Treasurer) == 1)
		{
			return &characters[i];
		}
	}
...
string QuestGetColony(string _city) // Получить рандомную колонию по аттрибуту NPChar.city, не равную _city
{
	...
	{
		if (colonies[n].nation != "none" && int(colonies[n].nation) != PIRATE && GetRelation2BaseNation(int(colonies[n].nation)) != RELATION_ENEMY && colonies[n].id != "Panama" && colonies[n].id != _city)
		{
			storeArray[howStore] = n;
			howStore++;
		}
...
string GetColonyExpect2Colonies(string _city1, string _city2) // Исключая две колонии
{
	...
	for(int n=0; n<MAX_COLONIES; n++)
	{
		...
		if(!CheckAttribute(rColony, "HasNoFort") && rColony.nation != "none" && int(rColony.nation) != PIRATE && GetRelation2BaseNation(int(rColony.nation)) != RELATION_ENEMY && rColony.id != "Panama" && colonies[n].id != "LosTeques" && colonies[n].id != "SanAndres" && rColony.id != _city1 && rColony.id != _city2)
		{
			storeArray[howStore] = n;
			howStore++;
		}
...
void QuestActions()
{
	...
	{
		ShipSituation_CheckConsequence(int(pchar.GenQuest.ShipSituation.Epidemy));
	}
	...
	if (GetDataDay() == 5 || GetDataDay() == 20)
	{
		if (CheckAttribute(&NullCharacter, "capitainBase"))
		{
    		...
    		for (a=0; a<Qty; a++)
    		{
				...
				{
					if (GetNpcQuestPastDayParam(arCapBase, "checkTime") >= int(arCapBase.checkTime))
					{
	...
	if (!CheckAttribute(pchar, "questTemp.ChickenGod") && GetDLCenabled(DLC_APPID_2) && int(pchar.rank) >= 5) {
...
void Cap_SetMapAgain(ref sld, aref arCapBase) 
{
	...
	case "mushket": 
		...
		Map_CreateTrader(sld.cityShore, sld.quest.targetShore, sld.id, int(arCapBase.checkTime)-5);
		break
	case "citizen": 
		...
		Map_CreateTrader(sld.city, sld.quest.targetCity, sld.id, int(arCapBase.checkTime)-5);
		break
	case "robber": 
		if (sld.city == sld.quest.targetCity)
		{
	...
	case "jornal": 
		...
		Map_CreateTrader(sld.quest.baseShore, sld.quest.targetCity, sld.id, int(arCapBase.checkTime)-5);
		break
	case "Danielle": 
...
string GetQuestNationsCity(int _nation)
{
	...
	{
		if (colonies[n].nation != "none"  && colonies[n].id != "Panama" && colonies[n].id != "LosTeques" && colonies[n].id != "SanAndres" && int(colonies[n].nation) == _nation && GiveArealByLocation(loadedLocation) != colonies[n].island) //не на свой остров
		{
...
string SelectNotEnemyColony(ref NPChar)
{
	...
	for(n=0; n<MAX_COLONIES; n++)
	{
		...
		if (nation != RELATION_ENEMY && colonies[n].id != "Panama" && colonies[n].id != "LosTeques" && colonies[n].id != "SanAndres" && colonies[n].id != "FortOrange" && colonies[n].nation != "none" && int(colonies[n].nation) != PIRATE && GetIslandByCityName(npchar.city) != GetIslandByColony(&colonies[n])) //не на свой остров
		{
			storeArray[howStore] = n;
			howStore++;
		}
...
string SelectAnyColony(string _City)
{
	...
	for(n=0; n<MAX_COLONIES; n++)
	{
		...
		if (colonies[n].id != "Panama" && colonies[n].id != "LosTeques" && colonies[n].id != "SanAndres" && colonies[n].id != "FortOrange" && colonies[n].nation != "none" && int(colonies[n].nation) != PIRATE && !bIsSameCity) //не на свой остров
		{
			storeArray[howStore] = n;
			howStore++;
		}
...
string SelectAnyColony2(string _City1, string _City2)
{
	...
	for(n=0; n<MAX_COLONIES; n++)
	{
		...
		if (colonies[n].id != "Panama" && colonies[n].id != "LosTeques" && colonies[n].id != "SanAndres" && colonies[n].id != "FortOrange" && colonies[n].nation != "none" && int(colonies[n].nation) != PIRATE && !bIsSameCity1 && !bIsSameCity2) //не на свой остров
		{
			storeArray[howStore] = n;
			howStore++;
		}
...
void SetCapitainFromSeaToCity(string sChar)
{
	...
	AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_3", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name, GetFullName(sld)),
		StringFromKey("LSC_Q2Utilite_4", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name),
		StringFromKey("LSC_Q2Utilite_5", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
}
...
void SetRobberFromMapToSea(string sChar)
{
	...
	AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_7", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld)),
		StringFromKey("LSC_Q2Utilite_8", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name),
		StringFromKey("LSC_Q2Utilite_9", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
}
...
void PortmansSeekShip_writeQuestBook(ref rid)
{
	...
	if (iTemp != -1)
	{
		...
		AddQuestUserData(sTitle, "sShipTypeName", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName)));
		AddQuestUserData(sTitle, "sShipName", sld.Ship.name);
		AddQuestUserData(sTitle, "sCity", XI_ConvertString("Colony" + rid.city + "Gen"));
		iTemp = int(RealShips[int(sld.Ship.Type)].basetype);
...
void CitizCapFromMapToCity(string sChar)
{
	...
	{	//ходит в городе
		...
		LAi_SetCitizenType(sld);
		string slai_group = GetNationNameByType(int(colonies[iColony].nation))  + "_citizens";
		LAi_group_MoveCharacter(sld, slai_group);
		//даем слух, что кэп на рейде
		AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_11", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld)),
			StringFromKey("LSC_Q2Utilite_12", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name),
			StringFromKey("LSC_Q2Utilite_13", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
	}
	else
	{	//стоит на рейде в порту
		...
		Group_SetTaskNone(sGroup);
		//даем слух, что кэп на рейде
		AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_14", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld)),
			StringFromKey("LSC_Q2Utilite_15", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name),
			StringFromKey("LSC_Q2Utilite_16", GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
	}
	//таймер через сколько опять выйти на карту
