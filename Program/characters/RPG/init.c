...
void initNewMainCharacter()//инициализация главного героя
{
    ...
    MOD_EXP_RATE =  int(MOD_EXP_RATE + MOD_SKILL_ENEMY_RATE * MOD_EXP_RATE / 1.666666666); // разные уровни для всех
    if (MOD_EXP_RATE < 10) MOD_EXP_RATE = 10; // иначе будет развал целостности данных, порог релиховой версии бля всех сложностей.

	// куда плывем	
	switch (int(ch.nation))
	{
		case ENGLAND : 
		...
		case HOLLAND : ch.HeroParam.ToCityId = FindAlliedColonyForNation(int(ch.nation), true); break;
		case PIRATE :  ch.HeroParam.ToCityId = "Pirates"; break;
	}

	// откуда плывем
    ch.HeroParam.FromCityId  = Colonies[FindNonEnemyColonyForAdventure(int(ch.nation), ch.HeroParam.ToCityId, true)].id;
	...
	ReloadProgressUpdate();
 	...
 	SetNationRelations();
 	// от кого драпаем
	ch.HeroParam.EnemyNation  = FindEnemyNation2Nation(int(ch.nation));
	...
	sGlobalTemp   = "Flag" + NationShortName(int(ch.nation));
	...
	{
		if(startHeroType == 4) ch.Ship.Type = GenerateShipExt(SHIP_SCHOONER_W, false, ch);//элен
		else ch.Ship.Type = GenerateShipExt(int(ch.StartShip), false, ch);//остальные
		SetBaseShipData(ch);
		DeleteAttribute(ch, "StartShip");
		ch.Ship.Cannons.Type = CANNON_TYPE_CANNON_LBS3;
		if(startHeroType == 4)
		{
			RealShips[int(ch.Ship.Type)].MaxCaliber = 18;
			//RealShips[int(ch.Ship.Type)].CannonsQuantity = 10;
			RealShips[int(ch.Ship.Type)].MinCrew = 13;
			RealShips[int(ch.Ship.Type)].OptCrew = 96;
			RealShips[int(ch.Ship.Type)].MaxCrew = 120;
			RealShips[int(ch.Ship.Type)].SpeedRate = 15.00;
			RealShips[int(ch.Ship.Type)].TurnRate = 60.00;
			RealShips[int(ch.Ship.Type)].Capacity = 1500;
			RealShips[int(ch.Ship.Type)].HP = 1600;
		...
		/* RealShips[int(ch.Ship.Type)].MinCrew = 3;
		RealShips[int(ch.Ship.Type)].OptCrew = 12;
		RealShips[int(ch.Ship.Type)].MaxCrew = 15;
		RealShips[int(ch.Ship.Type)].SpeedRate = 11.50;
		RealShips[int(ch.Ship.Type)].TurnRate = 74.00;
		RealShips[int(ch.Ship.Type)].HP = 400; */
		...
		/* if(CheckAttribute(&RealShips[int(ch.Ship.Type)], "Tuning.Capacity"))
		{
			DegradeShipParameter(ch, "Capacity");
			DeleteAttribute(&RealShips[int(ch.Ship.Type)],"DontTuning.Capacity");
			DeleteAttribute(&RealShips[int(ch.Ship.Type)],"Tuning.Capacity");
		}
		RealShips[int(ch.Ship.Type)].Capacity = 334;
		...
		// коцаем корабль
		// ch.ship.SP = int(ch.ship.SP) - 10; <-- этот код не имеет смысла
		//ch.ship.HP = int(ch.ship.HP) - int(int(ch.ship.HP)/2);
...
{
    return int(GetNewMainCharacterParam("heroFace_" + startHeroType));
}
...
void setNewMainCharacter(ref ch, int num)
{
    ...
    heroFaceId  = string(int(totalInfo));
...
void InitStartParam(ref _chref)
{
    ...
    for (i=1; i <= SKILL_QTY; i++)
    {
        ...
        _chref.skill.(skillName) = int(MOD_EXP_RATE / GetCharacterExpRate(_chref, skillName) + 0.5);
    }
...
int GetHPIncrease(ref chr) {
	...
	int ret = int(2 + Ers * 0.55 + 0.5);
	
	return ret * (int(chr.rank) - 1);
}