...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Que tipo de perguntas, "+GetSexPhrase("rapaz","moça")+"?","O que você precisa, "+GetSexPhrase("bonito","bonitinha")+"? Pode perguntar."),"De novo com perguntas?","Tee-hee, se não se importa - perguntas...",""+GetSexPhrase("Hum, por que você não escolhe uma bela para si? Estou começando a ficar desconfiada de você...","Hm, por que você não escolhe uma beleza para si? Receio que não temos nenhum rapaz por aqui, hihi...")+"","block",1,npchar,Dialog.CurrentNode);
