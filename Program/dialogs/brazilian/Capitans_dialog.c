...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "quests":
			...
			{
                Dialog.text = "Não estou autorizado a tratar com você. Fale com o comandante do nosso esquadrão. O nome dele é  "+GetFullName(characterFromID(NPChar.MainCaptanId))+", você pode encontrá-lo no "+XI_ConvertString(RealShips[int(characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Type)].BaseName)+" '"+characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Name+"'.";
        ...
        case "Talk_board":
            ...
            else
            {
                ...
                else
                {
                    ...
                    Pchar.GenQuest.MoneyForCaptureShip = (70 + hrand(60, "&CapSur"+NPChar.id)) * iEncPow;
					if(HasShipTrait(pchar, "trait14")) Pchar.GenQuest.MoneyForCaptureShip = int(int(Pchar.GenQuest.MoneyForCaptureShip) * 1.35);
                    Dialog.text = RandSwear()+"Sim, já ouvi muito sobre suas atrocidades. Que seja, mas lembre-se "+XI_ConvertString(NationShortName(int(NPChar.nation))+"hunter")+" não vou deixar isso impune!";
		...
		case "MerchantTrap_Abordage":
			...
			Dialog.Text = "Em nome de "+NationKingsName(NPChar)+" se renda, "+GetSexPhrase("pirata imundo","vadia imunda!")+"! Então sua vida será poupada até um tribunal justo, onde você será julgado por tudo "+NationNameSK(int(NPChar.nation))+"navios que você saqueou"+GetSexPhrase("","")+".";
		...
		case "Siegehelp":		
            ...
            {
                Dialog.text = "Não tenho autorização para tratar com você. Fale com o comandante do nosso esquadrão. O nome dele é  "+GetFullName(rchar)+", você pode encontrá-lo em "+XI_ConvertString(RealShips[int(rchar.Ship.Type)].BaseName)+" ''"+rchar.Ship.Name+"''.";
                link.l1 = "Obrigado. Vou fazer como você disse.";
                link.l1.go = "exit";
		...
		case "Talk_Capture_City":
            ...
            dialog.text = "Olá, capitão! Foi realmente uma batalha difícil, mas conseguimos quebrar a feroz resistência dos defensores da cidade. Saqueamos "+int(aData.loot)+" pesos. Conforme nosso acordo, sua parte é "+ilt+". Please accept this money; you certainly deserve every last coin of it. And now please excuse me, but I must return to my business.";
		...
		case "GoldSquadron":
            ...
            {
                Dialog.text = "Não estou autorizado a negociar com você. Fale com o comandante do nosso esquadrão. O nome dele é "+GetFullName(rch)+", você pode encontrá-lo em "+XI_ConvertString(RealShips[int(rch.Ship.Type)].BaseName)+" ''"+rch.Ship.Name+"''.";
                link.l1 = "Obrigado. Vou fazer como você disse.";
                link.l1.go = "exit";
		...
		case "WM_Captain_3":
			...
			dialog.text = "Eu já entendi tudo isso. Considerando o que já temos, eu preciso "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Planks))+" tábuas, "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Sailcloth))+" tecido de vela e "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Linen))+" algodão. Com esses materiais, conseguiríamos consertar o navio em uma semana, e então poderíamos zarpar.";
		...
		case "VSPShip_final":
			...
			dialog.text = "Quero te agradecer mais uma vez, capitão – você ajudou muito a mim e à minha tripulação a sair dessa enrascada. Aqui estão"+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+". Toda a tripulação contribuiu, então nem pense em recusar. E este é meu presente pessoal para você.";
		...
		case "SShip_final":
			...
			dialog.text = "Quero te agradecer de novo, capitão, por ter nos tirado dessa enrascada. Aqui, aceite essas moedas - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+"  e aqui, aceite este presente. Não é muito, mas é tudo o que posso fazer para agradecer pelo que você fez. Por favor, não recuse minha oferta\nMinha antiga tripulação está impressionada com você e suas batalhas contra índios e piratas. Todos eles querem se juntar a você. Espero que não recuse a oferta de serviço leal de marinheiros que acabam de perder seu navio?";
		...
		case "Количество вбитого провианта":
			...
			{
				dialog.text = "Fechado! "+int(iQty*iMoney)+" pesos.";
		...
		case "Количество вбитых гарпунов":
			...
			if(iQty > 0 && iQty <= iTemp)
			{
				dialog.text = "Fechado! "+int(iQty*100)+" pesos.";
