	...
	//<-- Бремя гасконца
		...
		case "ship_tunning_start" :		
			...
			{				
				...
				if(!TuningAvailable())
				{
					dialog.Text = "Mais ou menos... vamos ver o que temos aqui... Sim - "+XI_ConvertString(RealShips[int(Pchar.Ship.Type)].BaseName)+". Seu navio já possui o número máximo de melhorias, receio que qualquer trabalho adicional seria inútil e poderia até prejudicá-lo.";
					Link.l1 = "Entendi...";
					Link.l1.go = "ship_tunning_not_now_1";
					break;
				}
		...
		case "ship_tunning_Capacity_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_Capacity_again";
				dialog.Text = "Ainda preciso de: madeira de ferro - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "Certo.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_SpeedRate_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_SpeedRate_again";
				dialog.Text = "Ainda preciso de: seda para o navio - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "Certo.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_TurnRate_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_TurnRate_again";
				dialog.Text = "Ainda preciso de: cordas - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "Certo.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_MaxCrew_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_MaxCrew_again";
				dialog.Text = "Ainda preciso de: pau-ferro - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "Certo.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: ironwood - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "ship_tunning_HP_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_HP_again";
				dialog.Text = "Ainda preciso de: resina - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "Certo.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "Findship_complete":
			...
			dialog.text = "Aqui está o seu dinheiro - "+FindRussianMoneyString(int(pchar.GenQuest.Findship.Shipyarder.Money))+". Obrigado pelo serviço. Não se esqueça de voltar para me ver. Até logo!";
		...
		case "Device_complete_1":
			...
			dialog.text = "Pelo seu esforço, vou lhe pagar "+FindRussianMoneyString(int(pchar.GenQuest.Device.Shipyarder.Money))+". Espero que isso seja uma recompensa à altura.";
			link.l1 = "Obrigado!";
		...
		case "SailsTypeChoose":
			dialog.text = "Quais velas você gostaria de instalar? No momento você tem "+GetSailsType(int(RealShips[int(Pchar.Ship.Type)].ship.upgrades.sails))+".";
		...
		case "SailsTypeChoose2":
			...
			dialog.text = "No momento você tem "+GetSailsType(int(RealShips[int(Pchar.Ship.Type)].ship.upgrades.sails))+", você selecionou "+GetSailsType(int(NPChar.SailsTypeChooseIDX))+". O custo da substituição é "+FindRussianMoneyString(int(NPChar.SailsTypeMoney))+". Prosseguir?";
		...
		case "SailsColorDone":
			...
			dialog.text = "Combinado. Vamos pintar as velas "+GetStrSmallRegister(XI_ConvertString(SailsColors[int(NPChar.SailsColorIdx)].name))+" cor.";
		    Link.l9 = "Obrigado.";
			Link.l9.go = "exit";
...
//<-- -----------------------------------------Бремя гасконца-----------------------------------------------------
		...
		case "IslaMona_7":
            dialog.text = "Então, você está me devendo "+int(pchar.questTemp.IslaMona.Shipyarder.Money)+" dobrões.";
		...
		case "IslaMona_money":
            ...
            dialog.text = "Não sinta pena de mim, você tem um armário cheio de ouro na cabine. Você me deve "+FindRussianDublonString(int(pchar.questTemp.IslaMona.Shipyarder.Money))+".";
			link.l1 = "";
			link.l1.go = "IslaMona_money_exit";
		break;
