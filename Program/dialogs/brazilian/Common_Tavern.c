	...
	////////////////////////////////////////////////////////=============================================///////////////////////////////////////////////////////	
        case "int_quests":
			...
			if(CheckAttribute(pchar,"GenQuest.CaptainComission") && NPChar.city == pchar.GenQuest.CaptainComission.City && CheckAttribute(pchar,"GenQuest.CaptainComission.Speak_Tavern"))
			{
				...
				{
					link.l9 = "Você sabe onde posso encontrar o capitão de uma patrulha "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+"?";
				...
				if(pchar.GenQuest.CaptainComission.variant == "A3" && pchar.GenQuest.CaptainComission == "Begin_1")
				{
					...
					{					
						link.l9 = "Você sabe algo sobre o capitão da patrulha "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+"?";
		...
		case "work":
			...
			{
				dialog.text = RandPhraseSimple("Não me importa a quem eu sirvo, já que dinheiro não tem nacionalidade. Mas não vou te ajudar, já que você veio sob a bandeira de "+NationNameGenitive(int(pchar.nation))+".","Sou apenas uma simples dona de taverna, mas colaborar com "+NationNameAblative(int(pchar.nation))+" não me parece atraente.");
