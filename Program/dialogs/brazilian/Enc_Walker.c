...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("Meu marido não me permite conversar com estranhos!","Não há nada sobre o que possamos conversar.");
				Link.l1 = "Não é nada demais.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Me conte, quais são os boatos populares na taverna local?","Prefere me contar o que está acontecendo por aqui?","Ei, camarada, você pode me contar como é a vida em terra firme?");
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("Ah sim, estou feliz"+NPCharSexPhrase(NPChar,"","")+" te ver é quase como tomar um gole de bebida. Pergunte o que quiser.","Você gosta de conversar, capitão? Eu também... Ainda mais com uma caneca de rum.","Sim, "+PChar.name+"?"),LinkRandPhrase("O que você precisa?","Entendi, Capitão "+PChar.name+", você fala bastante, não é?","Perguntas cansativas, Capitão. Seria melhor se você me oferecesse um pouco de rum.")),PCharRepPhrase(LinkRandPhrase("O que você queria saber "+GetAddress_Form(NPChar)+"?","Estou ouvindo você, capitão.","Eu preciso ir, então por favor seja rápido, capitão."),LinkRandPhrase("Sempre bom ter uma companhia agradável, "+GetAddress_Form(NPChar)+" "+PChar.lastname+". Diga o que pensa.","Sim, "+GetAddress_Form(NPChar)+"?","Você gosta de conversar, capitão? Pois é, eu também...")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("Que tipo de fofoca é popular na taverna local?","O que está acontecendo nessas terras?","O que há de novo em terra por aqui?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("Não, você está ouvindo coisas. Vou indo agora.","Não, nada. Só fui dar uma volta."),RandPhraseSimple("Não, nada. Boa sorte pra você!","Só estou dando uma volta. Até logo."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Ouça, como cidadão desta cidade, preciso pedir que você não ande por aí com a lâmina desembainhada.");
				link.l1 = LinkRandPhrase("Certo.","Certo.","Como preferir...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"Calma aí, "+GetSexPhrase("companheiro","moça")+", quando você anda por aí com uma arma na mão. Isso pode me deixar nervoso...","Eu não gosto quando "+GetSexPhrase("homens","pessoas")+" andam por aí com as armas em punho. Isso me assusta...");
				link.l1 = RandPhraseSimple("Entendi.","Eu vou remover isso.");
			}
			link.l1.go = "exit";
