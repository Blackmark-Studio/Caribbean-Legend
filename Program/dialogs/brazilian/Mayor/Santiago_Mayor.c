...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			link.l1 = HeroStringReactionRepeat(RandPhraseSimple("Mudei de ideia, desculpe.","Agora não. Lugar e momento errados."),"É verdade, não tenho nada a dizer no momento, desculpe.","Vou perguntar depois.","Me desculpe, "+GetAddress_FormToNPC(NPChar)+"...",npchar,Dialog.CurrentNode);
			link.l1.go = "exit";
		break;

		case "Cupture_after":
            dialog.text = RandPhraseSimple("Você já levou tudo. O que mais você quer?","Ficou alguma coisa que você ainda não pegou?");
            link.l1 = RandPhraseSimple("Só estou dando uma olhada...","Só estou conferindo, talvez tenha esquecido de pegar alguma coisa...");
            link.l1.go = "exit";
