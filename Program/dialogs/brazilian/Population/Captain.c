	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Pare. Você navega sob a bandeira de "+NationNameGenitive(int(pchar.nation))+", marujo. Não quero conversar com você e não quero confusão... Cai fora!";
			...
			else
			{
				...
				{
					dialog.text = "Preciso repetir? Não quero que pensem que sou amigo de "+NationNameAblative(int(pchar.nation))+"! Cai fora ou vou chamar os guardas!";
		...
		//квестовая часть
			...
			case "march_1":
				...
				dialog.text = "Meu navio é "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.shiptype),"Name")))+" '"+pchar.GenQuest.MarchCap.shipname+"Ela está no porto agora mesmo. Não se atrase, capitão, vou partir em seis horas!";
