...
//----------------------------------------захват пассажиров для выкупа------------------------------------------
			...
			case "passenger_2":
				...
				dialog.text = "Seria muito imprudente tentar te enganar, lobo do mar. Eu moro nesta cidade e não preciso de problemas. Por apenas "+int(pchar.GenQuest.Marginpassenger.Dublon)+" dobrões e eu te dou todas as informações. Você vai lucrar muito mais.";
			...
			case "passenger_4"://установка параметров
				...
				dialog.text = "Mantenha sua pólvora seca, você vai precisar dela em breve, capitão. Agora escute, em "+FindRussianDaysString(int(pchar.GenQuest.Marginpassenger.Days))+" um navio vai partir da nossa colônia, o navio está "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Marginpassenger.ShipType),"Name")))+" chamado de '"+pchar.GenQuest.Marginpassenger.ShipName+"' e estará indo para "+XI_ConvertString("Colony"+pchar.GenQuest.Marginpassenger.Targetcity+"Gen")+". Haverá um passageiro -  "+pchar.GenQuest.Marginpassenger.Text+", chamado de "+pchar.GenQuest.Marginpassenger.q1Name+"\nO capitão "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Marginpassenger.ShipType),"Name")+"Gen"))+" é um covarde de marca maior, igual a todo comerciante, ha-ha-ha! Talvez você nem precise lutar. Só meter um bom susto nele que ele mesmo entrega o alvo, he-he\nComo você entende, "+pchar.GenQuest.Marginpassenger.Text1+", você provavelmente vai receber suas moedas, sim, mas as autoridades locais vão te caçar depois disso. É melhor você trazer sua presa para "+XI_ConvertString("Colony"+pchar.GenQuest.Marginpassenger.Targetcity)+". Você vai encontrar um homem lá chamado "+pchar.GenQuest.Marginpassenger.q2Name+"\n"+pchar.GenQuest.Marginpassenger.Text2+" e você nem vai precisar dizer seu nome. A decisão de onde resgatar seu homem é sua, mas não tente dar um passo maior que a perna... Mas "+int(pchar.GenQuest.Marginpassenger.Dublon)*4+" Dúbloons você pode pedir, claro. Entendeu?";
				link.l1 = "Sim. Vou voltar para o meu navio e registrar isso no diário de bordo.";
				link.l1.go = "passenger_5";
			break;
		
			case "passenger_5":
				dialog.text = "Exatamente! A escolha certa. Muito bem, Capitão, boa sorte com essa tarefa fácil. Ah, sim, quase me esqueci: "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Marginpassenger.ShipType),"Name")))+" não vai partir do porto, mas de uma das baías próximas. Então fique de olho. Boa sorte e obrigado pelo ouro!";
