...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			dialog.text = "Vamos registrar: Capitão "+GetFullName(pchar)+", navio é "+pchar.Ship.Name+"... a duração da regata em horas é "+int(pchar.questTemp.Regata.FirstTransitionTime)+". Pronto, seu resultado foi registrado, você pode seguir seu caminho.";
