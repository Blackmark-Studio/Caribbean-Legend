    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Talvez tenha notado, capitão, que a cidade está sitiada. Esperamos um ataque dos espanhóis a qualquer momento e estamos mobilizando nossas forças. Mas a regata continua acontecendo\nVamos nos inscrever, capitão "+GetFullName(pchar)+", o navio está - "+pchar.Ship.Name+"... Tempo da regata em horas é "+int(pchar.questTemp.Regata.FourthTransitionTime)+". Pronto, seu resultado foi registrado, você pode seguir seu caminho.";
