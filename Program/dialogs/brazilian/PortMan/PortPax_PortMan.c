...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			{
			dialog.text = "Senhor! Você quebrou as regras da regata e preciso desclassificá-lo. Sinto muito. Você não participará da próxima viagem. Já preparei um despacho para Port Royal.";
			...
			dialog.text = "Vamos registrar você, Capitão "+GetFullName(pchar)+", navio é "+pchar.Ship.Name+"... Tempo da regata em horas é "+int(pchar.questTemp.Regata.SecondTransitionTime)+". Pronto, seu resultado foi registrado, pode seguir seu caminho.";
