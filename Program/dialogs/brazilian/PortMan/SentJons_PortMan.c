    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			if (CheckAttribute(pchar, "questTemp.Regata.Go") && !CheckAttribute(npchar, "quest.regatatalk"))
			{
				...
				link.l1 = "Isso mesmo, senhor. Preciso me registrar aqui conforme as regras.";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			{
			dialog.text = "Hm... Você quebrou as regras da regata e eu preciso desclassificá-lo. Sinto muito. Você não poderá participar da próxima viagem. Já preparei um despacho para Port Royal.";
			...
			{
			pchar.questTemp.Regata.ThirdTransitionTime = GetPastTime("hour", int(pchar.questTemp.Regata.StartYear), int(pchar.questTemp.Regata.StartMonth), int(pchar.questTemp.Regata.StartDay), float(pchar.questTemp.Regata.StartTime), GetDataYear(), GetDataMonth(), GetDataDay(), GetTime());//истратил ГГ в часах на 1+2+3 переход
			dialog.text = "Vamos registrar: Capitão "+GetFullName(pchar)+", navio é "+pchar.Ship.Name+"... O tempo da regata em horas é "+int(pchar.questTemp.Regata.ThirdTransitionTime)+". Pronto, seu resultado foi registrado, pode seguir seu caminho.";
		...
		case "Regata_info":
			...
			else
			{
				if (n==6)
				{
				dialog.text = "Você é o último homem. É melhor se apressar.";
