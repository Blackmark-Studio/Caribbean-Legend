...
void ProcessDialogEvent()
{
	...
	// --------------------------------------вернулся на Остров---------------------------------------------
		...
		case "return_2":
			...
			{
				dialog.text = "Não, não estou com frio. Eu só... não quero falar sobre isso aqui. Vem na minha cabine depois. Quero conversar com você a sós, né? Você vai ver o Nathaniel agora?";
				link.l1 = "Sim. Preciso levar a Danielle até ele. Não vou demorar. Te vejo logo, Mary.";
			}
			link.l1.go = "return_3";
		break;
	...
	// --> Мэри вне LSC
		...
		case "LongHappy_9c":
			...
			dialog.text = "Acho que, "+int(pchar.questTemp.LongHappy.MarryMoney)+" pesos vão ser suficientes, né. "+int(pchar.questTemp.LongHappy.MarryRum)+" barris de rum - não tem nada pior do que ficar sem bebida! Assim que juntar tudo, venha para a taverna. O resto eu resolvo.";
