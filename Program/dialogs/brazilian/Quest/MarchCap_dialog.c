	...
	switch(Dialog.CurrentNode)
	{
		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				case 0: //конвой у острова, установка параметров
					...
					dialog.text = "Consegui descobrir que em "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" um pequeno comboio comercial sob a bandeira de "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", dois navios mercantes e um navio de escolta, vão navegar próximos a "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+", isso não está longe de "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+". Os comerciantes têm muito "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" nos porões deles. Seria tolice não aproveitar essa informação, capitão\nNós dois podemos lidar com os guardas facilmente e pegar a carga para nós. Então, você está dentro?";
					link.l1 = "Parece tentador. Eu diria que sim!";
					link.l1.go = "MarchCap_1_1";
					link.l2 = "Alguns"+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" não é o bastante para eu arruinar minha relação com "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+". Não, eu não vou fazer isso! Adeus, senhor, e nem pense em tentar me impedir!";
				...
				case 1: //энкаунтер на глобалке, установка параметров
					...
					dialog.text = "Consegui descobrir que em "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" um pequeno comboio comercial sob a bandeira de "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", dois navios mercantes e um navio de escolta, partirão de "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Startcity+"Gen")+" to "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Finishcity+"Acc")+". Os comerciantes têm de sobra "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" nos porões deles. Seria tolice não aproveitar essa informação, capitão.\nVocê e eu podemos lidar com os guardas facilmente e pegar a carga para nós. O mais difícil é encontrá-los em alto-mar entre essas duas colônias. Então, vai entrar nessa?";
					link.l1 = "Parece tentador. Eu diria que sim!";
				...
				case 2: //одиночный пиратский кулсейлор у бухты, установка параметров
					...
					dialog.text = "Fui informado sobre um pirata chamado "+GetName(NAMETYPE_ORIG,pchar.GenQuest.MarchCap.PirateName,NAME_NOM)+". Este bandido recentemente roubou "+RandPhraseSimple(RandPhraseSimple("Inglês","French"),RandPhraseSimple("Spanish","Dutch"))+" e encheu o porão de carga com "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+". Mas que pena, o navio dele "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.ShipType),"Name")+"Acc"))+" estava muito danificada, então ele precisou de um tempo para consertá-la.\nAtualmente ele está em "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+" de "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+". Acho que vai levar cerca de "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" para ele consertar os mastros e os buracos no casco. Podemos chegar a tempo se nos apressarmos e todos "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" será nosso\nEu não vou conseguir lidar com esse pirata sozinho, ele é um marinheiro e lutador muito habilidoso, mas nós dois juntos podemos derrotá-lo, apesar da experiência dele. Então, vai topar ou não?";
					link.l1 = "Parece tentador. Eu diria que sim!";
		...
		case "MarchCap_AfterBattle":
			...
			if (iTemp > 50 && iTemp < 500)
			{
				...
				link.l1 = "Se você tivesse me ajudado em vez de ficar contando pássaros no céu, o ataque teria dado certo... Pegue sua parte - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" e suma daqui!";
				link.l1.go = "MarchCap_Talk_exit";
			...
			{
				dialog.text = "Muito bem, capitão! Como eu disse - foi bem fácil esvaziar os porões daqueles mercadores, ha-ha... O prêmio é "+FindRussianQtyString(iQty)+" moedas. Minha parte é "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "Excelente, capitão! Como eu disse - foi relativamente fácil esvaziar os porões daqueles mercadores, ha-ha... O prêmio é enorme, muito maior do que eu esperava. É "+FindRussianQtyString(iQty)+" moedas. Minha parte é "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
		...
		case "MarchCap_AfterBattleGold":
			...
			if (iTemp > 10 && iTemp < 100)
			{
				...
				link.l1 = "Se você tivesse me ajudado em vez de ficar contando pássaros no céu, o ataque teria dado certo... Pegue a sua parte - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" e suma daqui!";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "poor";
				break;
			}
			...
			{
				dialog.text = "Muito bem, Capitão! O prêmio é "+FindRussianQtyString(iQty)+" moedas. Minha parte é "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "Excelente, capitão! Como eu disse – foi até fácil, ha-ha... O prêmio é enorme, muito maior do que eu esperava. É "+FindRussianQtyString(iQty)+" moedas. Minha parte é "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
