...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(CheckAttribute(pchar, "questTemp.Saga"))
			{
				...
				if(pchar.questTemp.Saga == "calendar") // квесты баронов завершены
				{
					...
					if (CheckAttribute(pchar, "questTemp.Saga.Late"))
					{
						dialog.text = "Ouvi falar dos seus feitos heroicos! Dannie me contou o resumo. Aliás, agora somos amigos. E o Nathan mudou completamente! Toda aquela tristeza sumiu quando ele viu o 'Centurion'.\nMas tenho más notícias pra você, "+pchar.name+". Perdemos tanto tempo que os direitos à herança do testamento de Sharp já se foram há muito, e Isla Tesoro virou uma base militar inglesa. Não há mais nada que eu possa fazer para te ajudar com Levasseur.";
						link.l1 ="Estou completamente sem palavras... Passamos por tantos problemas à toa. Será que realmente não há nada que possamos fazer?";
						link.l1.go = "saga_l2";
					}
					else
					{
		...
		case "saga_55":
			...
			link.l1 = "Eu encontrei "+FindRussianQtyString(int(npchar.quest.bakaut))+" no 'Ostra'.";
			link.l1.go = "saga_56";
		break;
		...
		case "saga_60":
			...
			link.l1 = "No geral, tenho dois mil em ouro para Loxley e "+int(npchar.quest.bakaut_sum)*2+" dobrões além disso.";
			link.l1.go = "saga_61";
		break;
		...
		case "saga_61":
			...
			if (PCharDublonsTotal() > 0)
			{
				...
				link.l1 = "Aqui está. Eu trouxe "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
		...
		case "saga_61_1":
			...
			{
				npchar.quest.bakaut_pay = func_min(int(npchar.quest.bakaut_sum), PCharDublonsTotal()); // дублоны в кармане
				link.l1 = "Aqui está. Eu tenho "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
				link.l1.go = "bakaut_pay";
			}
			else
			{
		...
		case "bakaut_pay": // оплата
			...
			{
				dialog.text = "Ótimo. Vai trazer o resto "+FindRussianQtyString(int(npchar.quest.bakaut_sum))+" dobrões depois?";
