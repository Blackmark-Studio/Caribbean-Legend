	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "Sem negócios hoje. Tem patrulhas em todas as baías, nosso governador está procurando uma carga que foi confiscada por algum capitão de"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" e ele escondeu isso em algum lugar.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "Por que não? Talvez eu encontre uma utilidade para a sua 'carga'. Leve "+int(pchar.questTemp.different.GiveShipLetters.price2)+" moedas e esqueça isso.";
				...
				else
				{
					...
					{
						dialog.text = "Exatamente! Agora fiquei bem curioso. Suponho que se eu te pagar "+int(pchar.questTemp.different.GiveShipLetters.price3)+" moedas, você não vai ligar nem um pouco para esses papéis.";
						link.l1 = "Não, obrigado.";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "Agora, essa é uma chance de pegar aquele sujeito insolente! Estou realmente, mas realmente curioso. Olha, eu te pago "+int(pchar.questTemp.different.GiveShipLetters.price4)+" em ouro, e você vai esquecer completamente dos documentos em cima da mesa?";
						link.l1 = "Eu não vou.";
						link.l1.go = "SM_ShipLetters_2";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "Sem negócios hoje. Tem patrulhas em todas as baías, nosso governador está procurando uma carga que foi confiscada por certo capitão de"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" e ele escondeu isso em algum lugar.";
					link.l1 = "E onde está esse capitão"+pchar.GenQuest.CaptainComission.Name+"?";
					link.l1.go = "CapComission_1";
					break;
				}
		...
		case "Meeting_2":
			...
			Link.l1 = "Talvez você se interesse por algumas mercadorias especiais? Ou quem sabe tenha algo para me oferecer em troca?"
			Link.l1.go ="Meeting_3";				
		break;
...
//navy <--
		...
		case "Travel_pay":
			...
			{
				Dialog.Text = "Parece que você não tem dinheiro suficiente.";
				Link.l1 ="Que pena, volto mais tarde.";
			}
			Link.l1.go = "Exit";
		break;  
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "É mesmo? Tudo bem. O senhor Rosenkraft precisa de um lote de pau-ferro, mas não menos que "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+". Você tem carga suficiente?";
