...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Perguntas, "+GetAddress_Form(NPChar)+"?","Como posso ajudar você, "+GetAddress_Form(NPChar)+"?"),""+GetSexPhrase("Hm, qual é a grande ideia, "+GetAddress_Form(NPChar)+"? ","De novo com essas perguntas estranhas? Querido, vai tomar um rum ou algo assim...")+"","No decorrer deste dia inteiro, é a terceira vez que você faz essa pergunta..."+GetSexPhrase(" Isso são sinais de carinho?","")+"","Mais perguntas, imagino eu, "+GetAddress_Form(NPChar)+"?","block",1,npchar,Dialog.CurrentNode);
