...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "Preciso ser levado até a taverna de "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Gen")+", que em "+XI_ConvertString(GetIslandByCityName(npchar.GenQuest.GetPassenger_Destination)+"Dat")+", por "+FindRussianDaysString(int(npchar.GenQuest.GetPassenger_Time))+", e por isso vou te pagar "+FindRussianMoneyString(int(npchar.GenQuest.GetPassenger_Money))+". Você concorda?";
