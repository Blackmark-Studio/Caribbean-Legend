...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest":
			...
			{
				dialog.text = RandPhraseSimple("Ahhh, inferno! Eu sei quem você é! Só um completo idiota contrataria você para escoltar o navio dele. Adeus!","Ah, eu sei quem você é! Me contratar como escolta, é isso? Não sou idiota...");
				link.l1 = RandPhraseSimple("Heh, que pena!","As pessoas andam mesmo assustadas à toa hoje em dia!");
				link.l1.go = "convoy_refused";
			}
			else
			{
				dialog.text = TimeGreeting()+", "+GetAddress_Form(NPChar)+"! Eu sou "+GetFullName(NPChar)+", um comerciante. Ouvi dizer que você está procurando trabalho?";
				link.l1 = "Algo assim. E você, pelo que ouvi, está procurando um capitão para escoltar você e seu navio até o seu destino?";
				link.l1.go = "prepare_convoy_quest_2";
			}
		break;
		
		case "prepare_convoy_quest_2":
			dialog.text = "Exatamente. Além disso, acredito que você seja a pessoa certa para me acompanhar. O que me diz?";
			link.l1 = "Bem, faça uma oferta e talvez a gente feche negócio.";
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "Preciso ser escoltado até "+XI_ConvertString("Colony"+pchar.quest.destination+"Gen")+", que fica em "+XI_ConvertString(GetIslandByCityName(pchar.quest.destination)+"Dat")+", em "+FindRussianDaysString(int(pchar.ConvoyQuest.iDay))+", e por isso vou te pagar "+FindRussianMoneyString(int(pchar.ConvoyQuest.convoymoney))+". Então, qual é a sua decisão?";
			link.l1 = "Estou dentro.";
		...
		case "complete_convoy_quest":
			dialog.text = "Oh! Obrigada. Sob sua proteção, me senti mais segura do que nunca. Aqui está sua recompensa merecida.";
			Link.l1 = "De nada.";
			link.l1.go = "exit";
			//слухи
