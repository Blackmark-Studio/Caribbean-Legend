...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("我丈夫不允许我和陌生人说话! ",
                         "我们没什么可聊的。 ");
				Link.l1 = "没什么大不了的。 ";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase ("告诉我, 当地酒馆里流行的八卦是什么? ",
									"你愿意告诉我这附近发生了什么吗? ",
									"嘿, 伙计, 你能告诉我陆地上的生活怎么样吗? ");
			link.l10.go = "rumours_citizen"; //挂在市民身上, 如果需要单独的谣言, 就编写相应的实现
		...
		case "sovet":
			if (NPChar.sex != "man") //fix eddy。 第二次老太太像男人一样说话
			{
                dialog.text = RandPhraseSimple("我丈夫不允许我和陌生人说话! ",
                         "我们没什么可聊的。 ");
				Link.l1 = "没什么大不了的。 ";
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,
		                PCharRepPhrase(LinkRandPhrase("哦, 是的, 我很高兴" + NPCharSexPhrase(NPChar, "", "") + "见到你, 就像见到一杯酒一样。 有什么想问的尽管问。 ", "你喜欢聊天吗, 船长? 嗯, 我也是... 尤其是边喝朗姆酒边聊。 ", "是的, " +pchar.name+ "? "),
                                        LinkRandPhrase("你需要什么? ", "我明白了, " +pchar.name+ "船长, 你很健谈吗? ", "问题真烦, 船长, 你最好请我喝杯朗姆酒。 ")),
		                PCharRepPhrase(LinkRandPhrase("你想知道什么, " + GetAddress_Form(NPChar) + "? ", "我在听你说, 船长。 ", "我得走了, 所以请快点说, 船长。 "),
                                        LinkRandPhrase("总是很高兴有你这样的好伙伴, " + GetAddress_Form(NPChar) + " " + PChar.lastname + "。 有话直说吧。 ",
                                                        "是的, " + GetAddress_Form(NPChar) + "? ",
                                                        "你喜欢聊天吗, 船长? 嗯, 我也是... "))
                            );

            // homo 25/06/06
			link.l1 = LinkRandPhrase ("当地酒馆里流行什么样的八卦? ",
                                    "这些地方发生了什么事? ",
                                    "陆地上有什么新鲜事? ");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("不, 你听错了。 我现在要走了。 ", "不, 没什么。 只是出去散步。 "),
                                        RandPhraseSimple("不, 没什么。 祝你好运! ", "只是散散步。 再见。 "));
			link.l5.go = "exit";
		break;
		//关于露出武器的说明
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar, "听着, 作为这个城镇的市民, 我必须请你不要带着出鞘的刀刃到处走。 ");
				link.l1 = LinkRandPhrase("好的。 ", "好的。 ", "如你所说... ");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar, "等等, " + GetSexPhrase("伙计", "姑娘") + ", 手里拿着武器跑来跑去的时候要小心。 这会让我紧张... ", "我不喜欢" + GetSexPhrase("男人", "人们") + "拿着武器走来走去。 这让我害怕... ");
				link.l1 = RandPhraseSimple("我知道了。 ", "我会收起来的。 ");
			}
			link.l1.go = "exit";
