	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PZ_LongwayRazgovorOProshlom_6":
			...
			if (sti(pchar.reputation.nobility) <= 60)
			{
				...
				notification("声誉过低! ("+XI_ConvertString(GetReputationName(61))+")", "None");
			}
			else
			{
				link.l1 = "你是怎么做到的, 朗威? ";
		...
		case "PZ_Longway_78":
			...
			RefreshWeather();
			RefreshLandTime();
			pchar.GenQuest.AbordageInStorm = true;	//允许在风暴中登船
