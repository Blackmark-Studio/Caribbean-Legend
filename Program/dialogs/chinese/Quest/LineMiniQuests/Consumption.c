...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Sergio_24":
			dialog.text = "船长, 你看起来是个可以信任的人。 另一个非常重要的事情是你是"+GetSexPhrase(""+NationNameMan(int(pchar.baseNation))+"",""+NationNameWoman(int(pchar.baseNation))+"")+"人, 有自己的船。 我建议我们达成一项贸易协议。 ";
		...
		case "oil_trade_1":
			...
			if(int(pchar.questTemp.SergioOil) >= 1 && !CheckAttribute(pchar, "questTemp.SergioOilBlock")) // 增加树脂供应量
			{
				link.l4 = "塞尔吉奥先生, 我想买更多树脂。 你能提供五倍大的批次吗? ";
				link.l4.go = "UpgradeOil";
			}
			if(int(pchar.questTemp.SergioOil) >= 1 && CheckAttribute(pchar, "questTemp.SergioOilPotom") && PCharDublonsTotal() >= 3000) // 增加树脂供应量, 如果第一次没带来
			{
		...
		case "oil_trade_4":
			...
			pchar.questTemp.SergioOil = int(pchar.questTemp.SergioOil) + 1; // 购买计数器
		break;
