...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "noel_93":
			dialog.text = "你真是名不虚传。 我向你发誓, 副舰队司令, 你会收回这笔钱。 我相信我们会在潘西的箱子和他的放贷人那里找到一大笔钱。 我只请你等六个月, 直到我带着委员会从欧洲回来。 ";
		...
		case "noel_105":
			dialog.text = "在卡普斯特维尔从潘西那里找到的所有钱都被布尔丹男爵没收了。 但事实证明, 潘西还有一笔由英国放贷人藏匿的 stash。 我会和他谈谈归还投资的事, 然后想办法补偿你。 ";
			link.l1 = "恐怕你要从英国放贷人手里拿到东西不容易。 ";
			link.l1.go = "noel_106";
		break;
		...
		case "noel_114":
			dialog.text = "正是。 据我所知, 你进入法国服务时, 得到了重型护卫舰 'Gryffondor' 供你使用。 这无疑是艘很棒的船, 但它只适合军官, 不适合副舰队司令和现在的总督。 你是法国在这里的代表。 你的旗舰应该与之匹配。 所以我们决定将在加勒比经历首次战斗的船加入你的中队... ";
			link.l1 = "弗尔热先生, 你是想说... ";
			link.l1.go = "noel_115";
		break;
		...
		case "BladeUpg":
			...
			for(n=0; n<iItemsNum; n++)
			{
				...
				{
					attrL = "l" + i;
					Link.(attrL) = GetItemName(rItem) + ": 攻击力 - " + FloatToString(float(rItem.attack),1) + ", 平衡性 - " + FloatToString(float(rItem.Balance),1) + ", 重量 - " + FloatToString(float(rItem.Weight),1);
