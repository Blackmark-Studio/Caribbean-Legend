void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SharlieEpilog_Cureer":
			dialog.text = "请原谅. 你是"+GetFullName(pchar)+GetTitle(NPChar, false)+"吗?";
			link.l1 = "正是本人. 有何贵干?";
			link.l1.go = "SharlieEpilog_Cureer_1";
		break;

		case "SharlieEpilog_Cureer_1":
			dialog.text = "啊, 终于找到你了…… 要找到你, "+GetFullName(pchar)+GetTitle(NPChar, true)+", 可真不容易, 我几乎跑遍了整个群岛. 我这里有一封来自本努瓦神父的信件要交给你.";
			link.l1 = "嗯…… 来自神父的信? 真是出乎意料.";
			link.l1.go = "SharlieEpilog_Cureer_2";
		break;

		case "SharlieEpilog_Cureer_2":
			dialog.text = "我被吩咐必须亲手交给你. 关于报酬你无需担心——本努瓦神父已经提前支付了我所有的花费.";
			link.l1 = "那就交给我吧. 非常感谢.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Dassen":
			dialog.text = "有何吩咐, "+GetTitle(NPChar, true)+"? ";
			link.l1 = ""+npchar.name+"，我就直说了——我将离开这片群岛。 家族事务需要我回到父 亲的庄园亲自处理。";
			link.l1.go = "SharlieEpilog_Dassen_1";
		break;
		
		case "SharlieEpilog_Dassen_1":
			dialog.text = "如果我理解得没错, 我们这是要分别了? ";
			link.l1 = "遗憾的是……我别无选择。";
			link.l1.go = "SharlieEpilog_Dassen_2";
		break;
		
		case "SharlieEpilog_Dassen_2":
			dialog.text = "那么, 能与您在同一面旗帜下服役, 对我而言是一种荣幸; 若命运允许, 我也很乐意再次如此。";
			link.l1 = "我希望我们之间的 友谊不会就此中断, 而是愈发牢固, 毕竟我们一同经历了许多冒险。";
			link.l1.go = "SharlieEpilog_Dassen_nothing";
			link.l2 = "鉴于您的忠诚与专业, 我希望以遣散费的形式, 向您支付一个月的薪酬。";
			link.l2.go = "SharlieEpilog_Dassen_salary";
			link.l3 = "您的服役无可挑剔, 我认为有必要给予您应得的回报。 请收下这个——这里是三个月的薪酬。 您完全配得上这份奖赏。";
			link.l3.go = "SharlieEpilog_Dassen_salaryX3";
		break;
		
		case "SharlieEpilog_Dassen_nothing":
			dialog.text = "我由衷地认同这一希望, "+GetTitle(NPChar, true)+"。";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "直接叫我夏尔就好。 顺便一提, 我还有些东西要给你。";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
				link.l1 = "那么, 就此告别吧, "+npchar.lastname+" 先生。 再会。";
				link.l1.go = "SharlieEpilog_Dassen_Leave";
			}
		break;
		
		case "SharlieEpilog_Dassen_salary":
			dialog.text = "感谢您, "+GetTitle(NPChar, true)+"。";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "直接叫我夏尔就好。 顺便一提, 我还有些东西要给你。";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			...
			{
				link.l1 = "那么, 期待下次再见, "+npchar.lastname+" 先生。 告辞。";
				link.l1.go = "SharlieEpilog_Dassen_Leave";
			}
		...
		case "SharlieEpilog_Dassen_salaryX3":
			dialog.text = "万分感激, "+GetTitle(NPChar, true)+", 您对我实在是太慷慨了。";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "直接叫我夏尔就好。 顺便一提, 我还有些东西要给你。";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			...
			{
				link.l1 = "不必拘泥于礼节, 直接叫我夏尔吧。 为庆祝启航, 我打算在酒馆里办一场小小的宴会。 希望你能来参加? ";
		...
		case "SharlieEpilog_Dassen_Governor":
			dialog.text = "你引起了我的兴趣, "+pchar.name+"。";
			link.l1 = "我已推荐你出任伊斯拉莫纳的总督。 不久之后, 相关的任命函件将从官署送达, 我非常希望你不要予以拒绝。 这座岛屿对我意义非凡, 若能由你执掌其治理, 我将深感欣慰。";
			link.l1.go = "SharlieEpilog_Dassen_Governor_1";
		break;
		
		case "SharlieEpilog_Dassen_Governor_1":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+"…… "+pchar.name+"…… 这实在是出乎意料……";
			link.l1 = "即便如此, 你的答复会是? ";
			link.l1.go = "SharlieEpilog_Dassen_Governor_2";
		break;
		
		case "SharlieEpilog_Dassen_Governor_2":
			dialog.text = "这是一个无法拒绝的提议。 我欣然接受, 并承诺竭尽所能, 促进这座岛屿的繁荣与居民的福祉。";
			link.l1 = "太好了, 我的朋友! 那就请你到酒馆来, 参加为我启航而举行的庆祝宴会。 阿隆索会在一切准备就绪时通知你。";
			link.l1.go = "SharlieEpilog_Dassen_Governor_3";
		break;
		
		case "SharlieEpilog_Dassen_Governor_3":
			dialog.text = "我绝不会错过这次机会, 蒙塞尼尔。";
			link.l1 = "";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		
		case "SharlieEpilog_Dassen_Tavern":
			dialog.text = "我绝不会错过, "+pchar.name+"。";
			link.l1 = "太好了, 我的朋友。 那我们就在那里见面吧。 等我们准备好开始时, 阿隆索会通知你。";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		...
		case "SharlieEpilog_PikarOnUliss_D_1":
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarAlive") || CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarResurrected")){dialog.text = "你说得对。";}
		...
		case "SharlieEpilog_HenriDeMonper_Mary_4":
			if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "你会把我介绍给其他人";
			else sStr = "你会把我介绍给我们的客人";
		...
		case "SharlieEpilog_InKitchenDeMonpe_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer") || CheckAttribute(pchar, "questTemp.LSC.Mary_officer"))
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "好朋友们";
				else sStr = "战友";
				dialog.text = "";
			...
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "你的好朋友们";
				else sStr = "你的朋友";
				dialog.text = "";
		...
		case "SharlieEpilog_KomendantFort_5":
			dialog.text = "这是个绝妙的计划, 阁下! 不过…… 您无疑是一位久经海战的勇士, 但以您的身份, 并不适合亲自冒险。 舰队完全可以由…… 来率领。";
			link.l1 = "正如我无法忍受欧洲那种平静的生活, 我也无法在岸边无所事事 地等待战局的结果。 命运无法逃避, "+GetAddress_Form(NPChar)+" "+npchar.lastname+"。";
			link.l1.go = "SharlieEpilog_KomendantFort_6";
		break;

		case "SharlieEpilog_KomendantFort_6":
			dialog.text = "既然如此, 便依您的意愿行事。 是否允许我下令开始准备?";
			link.l1 = "开始吧。 愿上帝与我们同在。";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Mishelle_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer")) sStr = " 和你的妻子";
			dialog.text = "你还是不明白吗? 你已经死了。和你的船员一起"+sStr+"。你想要抢劫的某个商人引爆了火药舱——只为了不让一分钱落到你这种人手里。\nSpectra sunt memoria peccatorum。你才是那个幽灵, "+pchar.name+"…… 一个被贪婪和盲目虚荣吞噬的加勒比传奇。为了黄金, 你杀了多少人?";
