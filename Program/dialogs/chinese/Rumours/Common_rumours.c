...
// Addon 2016-1 Jason 海盗系列
			...
			{
				AddMoneyToCharacter(pchar, -1000);
				Dialog.Text = "您的名声在外, 船长。 我敢打赌, 您经常在烦恼该把那些小玩 意儿和宝石卖到哪里去吧…… ";
				link.l1 = "你知道谁可以帮我解决这个问题?";
...
//Jason ------------------------------------全职业2级邮政生成器-------------------------------------------
		case "Postcureer_LevelUp_ForAll":
			...
			if (int(pchar.questTemp.WPU.Postcureer.AgentChance) == 4)// 揭穿骗局
			{
			...
			if (pchar.questTemp.WPU.Postcureer == "late")// 迟到
			{
				...
				pchar.questTemp.WPU.Postcureer.Money = int(int(pchar.questTemp.WPU.Postcureer.Money) / (int(iTime) + 1));// 大幅降低报酬
				dialog.text = "我一直在等你, 但你晚了" + FindRussianDaysString(iTime) + "天! 你携带的信息已经变得没那么有用了... 当然还是有一些价值, 所以我会付给你" + FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money)) + "。 请拿钱离开吧。 ";
	        	link.l1 = "谢谢, 我想是吧。 ";
	        	link.l1.go = "Postcureer_LevelUp_complete";
				pchar.quest.PostcureerTime_FullOver.over = "yes";// 移除第二个计时器
				pchar.questTemp.WPU.Postcureer.count = int(pchar.questTemp.WPU.Postcureer.count) - 1;// 迟到的行程不计入
			...
			dialog.text = "我一直在等你, 你来得正是时候, 你带来的信息会很有用。 请接受这份工作报酬 --" + FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money)) + "。 感谢你的 timing。 ";
        	link.l1 = "不客气, 再见! ";
        	link.l1.go = "Postcureer_LevelUp_complete";
		...
		case "Postcureer_LevelUp_complete_fail":
			...
			ChangeCharacterComplexReputation(pchar,"nobility", -2);
			ChangeCharacterNationReputation(pchar, int(pchar.questTemp.WPU.Postcureer.Nation), -2);
			pchar.questTemp.WPU.Postcureer.count = int(pchar.questTemp.WPU.Postcureer.count)-3;//减少计数器
			DeleteAttribute(pchar, "questTemp.WPU.Postcureer.TargetPortmanID");
			DeleteAttribute(pchar, "questTemp.WPU.Current");
			DeleteAttribute(pchar, "questTemp.WPU.Postcureer.LevelUp");
			pchar.questTemp.WPU.Postcureer = "complete";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case "About_Gaspar":
			dialog.text = "我知道。 他叫加斯帕·帕尔芒捷, 但朋友们都叫他金牙。 他是个可靠的人, 最重要的是, 他的采购量之大, 连任何放贷人都无法想象。 我相信你们会很快找到共同语言。 记住: 他不喜欢陌生人。 所以如果你去找他, 一定要提到是我介绍的。 他的房子在港口管理处附近。";
			link.l1 = "好吧, 你应得这笔钱——拿着吧。 你还知道什么值得注意的吗?";
			link.l1.go = "rumours_tavern";
