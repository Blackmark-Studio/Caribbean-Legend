    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "You might have noticed, captain, that the city is under siege. We are expecting the Spanish to attack us at any moment and are mobilizing our forces. But the regatta still goes on\nLet's register, captain "+GetFullName(pchar)+", ship is - "+pchar.Ship.Name+"... Regatta time in hours is "+int(pchar.questTemp.Regata.FourthTransitionTime)+". Done, your result has been recorded, you may continue on your way.";
