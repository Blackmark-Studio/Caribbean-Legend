...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			dialog.text = "Let's register: Captain "+GetFullName(pchar)+", ship is "+pchar.Ship.Name+"... Regatta's time in hours is "+int(pchar.questTemp.Regata.ThirdTransitionTime)+". Done, your result has been recorded, you may continue on your way.";
