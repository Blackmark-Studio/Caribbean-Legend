    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			if (CheckAttribute(pchar, "questTemp.Mtraxx"))
            {
				...
				{
					link.l1 = "I've heard this shipyard specialises in improving a ship's sailing speed. It requires some silk sailcloth, which is very rare. Would you like... to discuss this?";
					link.l1.go = "mtraxx";
				}
			}
			if (CheckAttribute(npchar, "quest.upgradeship")) {
				link.l2 = "I'm from Miss MacArthur, about...";
				link.l2.go = "helen_upgradeship";
			}
			
			if (CheckAttribute(npchar, "quest.upgradeship.money") && HelenCanUpgrade()) {
				link.l2 = "Begin the overhaul of the ship!";
		...
		// Addon 2016-1 Jason пиратскаЯ линейка
		case "mtraxx":
            dialog.text = "Could you be more clear, young man? What exactly brought you here? If you want to improve your ship - let's get down to business right away.";
			link.l1 = "Not entirely. The type of services you perform requires goods which are not freely available in the store. I would like to offer you the same kind of materials you might require.";
			link.l1.go = "mtraxx_1";
		break;
		...
		case "mtraxx_2":
            dialog.text = "I acquire all of the silk sailcloth directly from my clients, or purchase limited quantities with the special permission of the governor. If you don't have any further questions, then please, let me get back to my job.";
			link.l1 = "Hmm... Very well. Have a good day, sir.";
			link.l1.go = "mtraxx_3";
		break;
		...
		case "helen_upgradeship_2":
			dialog.text = "It depends on the size of your flagship, Captain. A first-rate ship will cost you five thousand doubloons, and a fifth-rate - only a thousand.";
		...
		case "helen_upgradeship_money_1":
			...
			dialog.text = "... I suppose that's it... Quality is guaranteed.";
