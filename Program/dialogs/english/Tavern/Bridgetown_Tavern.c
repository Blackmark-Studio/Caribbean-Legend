...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("What questions do you have?","How can I help you, "+GetAddress_Form(NPChar)+"?"),"You tried to ask me a question not long ago, "+GetAddress_Form(NPChar)+"...","This is the third time you've bothered me now...","More questions, I presume?","block",1,npchar,Dialog.CurrentNode);
            ...
            {
                link.l1 = "I'd like a drink. Pour me your finest rum.";
			...
			{
                link.l1 = "Do you happen to have any work? Or perhaps you could suggest something?";
		...
		case "TavernDone_1":
			dialog.text = "Really? Well, there's no shame in drinking for a good deal! So, you say you're going to do business in Barbados? That's great! Cheers to you, captain!";
			link.l1 = "Thank you kindly, mate. Alright... I'll go look for a place where I can sit down.";
			link.l1.go = "exit";	
		...
		case "Tonzag_check"://начинаем проверять нашего ГГ по всем статьям
			if(sti(pchar.reputation.nobility) > 48)//высокая репа
			{
				...
				dialog.text = "I don't need the services of a scoundrel. I don't have any tasks I can offer you.";
			...
			if(!bOk)//слабое фехтование
			{
				...
				// <--
				dialog.text = "Captain, you're too soft. I need a fighter a little more reputable than you. Come back when you learn how to handle that steel dangling from your belt. And a little accuracy in shooting wouldn't hurt either.";
				link.l1 = "I see. Then I'll pay you a visit later.";
				link.l1.go = "exit";
				break;
			}
			PlaySound("Voice\English\hambit\Ercule Tongzag-02.wav");
			dialog.text = "Hm... You're actually on time. I've got one errand that must be fulfilled right away. If you show yourself in a good light, I'll introduce you to some influential people. Now listen, here's what you have to do.";
		...
		case "Tonzag_task":
			...
			dialog.text = "A hidalgo has recently arrived from Seville, seeking to avoid the unwanted consequences of a duel back in Europe. But vengeance knows no bounds, and one Spanish noble family wants this hidalgo dead. Fulfil their request by any means necessary.\nBring the man's finger with his ring as proof. Also, bring any other items you find on his corpse. Are you ready to take the job?";
			link.l1 = "If the payment is good, then I'm in.";
			link.l1.go = "Tonzag_task_1";
			link.l2 = "Work as an assassin? No way!";
		...
		case "Tonzag_exit":
			dialog.text = "Then bugger off and forget what we've talked about.";
			link.l1 = "I'm leaving.";
			link.l1.go = "exit";	
		...
		case "Tonzag_task_1":
			dialog.text = "Your reward will be 30,000 pieces of eight, good money. On top of that, you may take every coin you find on his body. Now, let's discuss the details. Name's Don Fernando Rodriguez, 35 years old, tall, swarthy, dressed like a military officer. He's a good sailor and an experienced fencer\nCan't tell where to find him, all I know is that he is somewhere in the Caribbean\nSearch every Spanish city until you get him. You have two months. One last thing: I am aware of what he is carrying on him, so don't even think about hiding any items from me. Questions?";
			link.l1 = "I have no questions. I'm on my way!";
		...
		case "Task_check":
			...
			{
				link.l1 = "No. Fernando sank into the water along with his ship, and the finger sank with Fernando.";
				link.l1.go = "Task_fail";	
			}
		break;
		
		case "Task_fail":
			dialog.text = "The finger with the ring was a mandatory condition in the contract. It was meant to serve as proof of the client's death. Who knows if Rodriguez made it to shore from the sunken ship? Did you check? No. You've essentially failed the assignment, so our agreement is over. All the best.";
			link.l1 = "But I did away with him. He's gone! Oh, damn you... There's plenty of other things to do in the Caribbean. Bye.";
			link.l1.go = "exit";
			if(!CheckAttribute(pchar, "questTemp.HWIC.Fail1"))
			{
				AddMapQuestMarkCity("Villemstad", false);
		...
		case "Task_check_1":
			...
			dialog.text = "Wonderful! I'll keep the finger myself. Now, let's see what's in his pockets. Let's have a look at what you've brought.";
		...
		case "Task_check_2":
			if (CheckCharacterItem(pchar, "jewelry7") && CheckCharacterItem(pchar, "totem_05") && CheckCharacterItem(pchar, "amulet_1"))
			{
				...
				link.l1 = "Thank you! It's always nice to be praised for a job well done.";
				link.l1.go = "Task_complete";	
			}
			else
			{
				dialog.text = "Hm... I warned you, I know what Rodriguez would never do without. And I don't see what I need here. Have you pocketed those things? Have you lost them? It doesn't matter anyway. I have no desire to be involved with you anymore. Get out of here.";
				link.l1 = "Oh, damn you...";
				link.l1.go = "exit";
				pchar.questTemp.HWIC.Fail3 = "true";
				if(!CheckAttribute(pchar, "questTemp.HWIC.Fail1"))
		...
		case "Fleetwood_house":
			...
			dialog.text = "This time, you are going to hunt not a man, but an item. You must sneak into Richard Fleetwood's house on Antigua and steal his logbook. He is an English captain. The place is closely guarded, and Richard himself rarely visits his house\nThe plan is simple. The governor of St. John's will be organizing a private party in ten days, and Fleetwood will attend. You must get inside his house at night between one and three o'clock. There will be only one soldier inside\nGet rid of him. Search for the journal inside Richard's apartment. Take this key.";
			link.l1 = "Hm... Interesting. I understand what I need to do. I'm ready to get going!";
			link.l1.go = "Fleetwood_house_1";	
		break;
		
		case "Fleetwood_house_1":
			GiveItem2Character(pchar, "key3");
			dialog.text = "Be careful. Don't sneak into the house before the date I've indicated to you. Otherwise, in the best case, they'll just kick you out, or in the worst case, you'll end up behind bars. Repeat the date and time to me.";
		...
		case "guardoftruth":
			dialog.text = LinkRandPhrase("No, I haven't. We have herbalists and physicians, but none of them by that name.","This is the first time I've heard such a strange name. No, we've never had a visit from the man you speak of.","We don't have any kind of alchemists at all. We've got physicians, but none with such a strange name.");
