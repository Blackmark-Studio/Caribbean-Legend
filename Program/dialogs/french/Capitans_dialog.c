...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "quests":
            ...
            if (!CheckAttribute(NPChar, "MainCaptanId")  || NPChar.MainCaptanId == pchar.GenQuest.CaptainId)
            {
                ...
                if (CheckAttribute(NPChar, "EncGroupName") && !bOk) // только для фантомов грабеж
                {
                    ...
                    link.l2 = "(Chance d’intimidation: " + iCapBattleDifficulty + "%%) " + RandPhraseSimple("Tu sais qui je suis ? Mes canons pourraient transformer ta coque en passoire. Réglons ça paisiblement – une rançon me suffira.", "La mer... rien que nous... Et si, pour la sécurité de ton équipage, tu faisais un petit geste pour calmer mes hommes ?");
			...
			{
                Dialog.text = "Je ne suis pas autorisé à traiter avec vous. Parlez au commandant de notre escadron. Son nom est  "+GetFullName(characterFromID(NPChar.MainCaptanId))+", vous pouvez le trouver sur le "+XI_ConvertString(RealShips[int(characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Type)].BaseName)+" '"+characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Name+".'.";
        ...
        case "Talk_board":
            ...
            {
                Dialog.text = "Ha-ha ! Bonne blague. D’autant plus que j’ai clairement l’avantage. Retourne sur ton navire et coule avec lui.";
                link.l1 = "...";
                link.l1.go = "Boarding";
            }
            else
            {
                ...
                else
                {
                    ...
                    Pchar.GenQuest.MoneyForCaptureShip = (70 + hrand(60, "&CapSur"+NPChar.id)) * iEncPow;
					if(HasShipTrait(pchar, "trait14")) Pchar.GenQuest.MoneyForCaptureShip = int(int(Pchar.GenQuest.MoneyForCaptureShip) * 1.35);
                    Dialog.text = RandSwear()+"Oui, j'ai entendu parler de vos outrages. Soit, mais souvenez-vous "+XI_ConvertString(NationShortName(int(NPChar.nation))+"chasseur")+" ne le laissera pas impuni !";
		...
		case "MerchantTrap_Abordage":
			...
			Dialog.Text = "Au nom de "+NationKingsName(NPChar)+" capitule, "+GetSexPhrase("sale pirate","garce répugnante!")+"! Alors, votre vie sera épargnée jusqu'à un tribunal équitable, où vous serez jugé pour tout "+NationNameSK(int(NPChar.nation))+"navires que vous avez pillés"+GetSexPhrase("","")+".";
		...
		case "Siegehelp":		
            ...
            {
                Dialog.text = "Je ne suis pas autorisé à traiter avec vous. Parlez au commandant de notre escadron. Son nom est  "+GetFullName(rchar)+", vous pouvez le trouver sur "+XI_ConvertString(RealShips[int(rchar.Ship.Type)].BaseName)+"''"+rchar.Ship.Name+"''.";
                link.l1 = "Merci. Je ferai comme tu dis.";
                link.l1.go = "exit";
		...
		case "GoldSquadron":
            ...
            {
                Dialog.text = "Je ne suis pas autorisé à traiter avec vous. Parlez au commandant de notre escadron. Son nom est "+GetFullName(rch)+", vous pouvez le trouver sur "+XI_ConvertString(RealShips[int(rch.Ship.Type)].BaseName)+" ''"+rch.Ship.Name+"''.";
                link.l1 = "Merci. Je ferai comme tu dis.";
                link.l1.go = "exit";
		...
		case "WM_Captain_3":
			...
			dialog.text = "J'ai déjà tout compris. Considérant ce que nous avons déjà, j'ai besoin "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Planks))+" planches, "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Sailcloth))+" toile à voile et "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Linen))+" coton. Avec ces matériaux, nous pourrions réparer le navire en une semaine et ensuite nous pourrions mettre les voiles.";
		...
		case "VSPShip_final":
			...
			dialog.text = "Je veux vous remercier encore une fois, capitaine - vous nous avez grandement aidés, moi et mon équipage, à sortir de ce pétrin. Voici"+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+". Tout l'équipage a contribué, alors ne pense même pas à y renoncer. Et ceci est mon cadeau personnel pour toi.";
			link.l1 = "J'étais heureux de vous aider!";
			link.l1.go = "WMShip_final_1";
		break;
		...
		case "Количество вбитого провианта":
			...
			{
				dialog.text = "Marché conclu ! "+int(iQty*iMoney)+" pesos.";
		...
		case "Количество вбитых гарпунов":
			...
			if(iQty > 0 && iQty <= iTemp)
			{
				dialog.text = "Marché conclu! "+int(iQty*100)+" pesos.";
