...
//			if(rand(1) == 0) PChar.GenQuest.ChurchQuest_1.CapWaitOnTavern = true;
		...
		case "ShipLetters_Portman1_1":
			...
			dialog.text = s1+"Dis-moi, accepterais-tu une somme de "+int(pchar.questTemp.different.GiveShipLetters.price1)+" pesos une récompense convenable ?";
...
///Jason-----------------------новые генераторы накопительного типа для портмана----------------------------
		case "Work_check"://фейс-контроль - общая проверка профпригодности ГГ и выбор задания
			...
			if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") >= 2)//проверка повтора
				{
					...
					{
						dialog.text = RandPhraseSimple("Il n'y a pas de travail pour ceux qui naviguent sous le pavillon de "+NationNameGenitive(int(pchar.nation))+"Quittez mon bureau sur-le-champ !","Je ne collaborerai pas avec "+NationNameAblative(int(pchar.nation))+"Va-t'en !");
					...
					link.l2 = "Je veux gagner de l'argent supplémentaire en escortant des marchands.";
					Link.l2.go = "escort_begin";	//эскорт
					link.l3 = "Je veux gagner de l'argent supplémentaire en livrant du courrier.";
					Link.l3.go = "cureer_begin";	//доставка почты
					Link.l4 = "Eh bien, peut-être pourriez-vous proposer quelque chose ?";
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Autant venir protéger un convoi avec une tartane. Pour ce travail il faut un navire doté d’une puissance de feu convenable — un guerrier ou un navire de raid.";
				link.l1 = "OK, j'ai compris.";
				link.l1.go = "exit";
			}
		}
		else
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Désolé, mais je n'ai rien pour vous. Ce genre de mission demande un navire de raid agile. Avec le vôtre il vaut mieux accepter du fret.";
				link.l1 = "OK, j'ai compris.";
				link.l1.go = "exit";
			}
		}
...
///--> ------фрахт со свободным выбором пункта назначения, оплаты и вида груза из предложенного списка---------
		...
		case "Fraht_choise":
			...
			{
				dialog.text = "Les variantes suivantes sont disponibles :\n"+"cargaison "+GetGoodsNameAlt(iFrahtGoods1)+" en quantité de "+FindRussianQtyString(iFrahtGoodsQty1)+" à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+", dans "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\ncargo "+GetGoodsNameAlt(iFrahtGoods2)+" en montant de "+FindRussianQtyString(iFrahtGoodsQty2)+" à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+", dans "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"\ncargaison "+GetGoodsNameAlt(iFrahtGoods3)+" en montant de "+FindRussianQtyString(iFrahtGoodsQty3)+" à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City3)+", à "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty3)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money3))+" \nQuel est votre choix ?";
			...
			{
				dialog.text = "Il y a les variantes suivantes disponibles:\n"+"cargaison "+GetGoodsNameAlt(iFrahtGoods1)+" à hauteur de "+FindRussianQtyString(iFrahtGoodsQty1)+" à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+", dans "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\ncargo "+GetGoodsNameAlt(iFrahtGoods2)+" en quantité de "+FindRussianQtyString(iFrahtGoodsQty2)+"  à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+", dans "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+" \nQuel est votre choix ?";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (amount > 0)
			{
				dialog.text = "Voyons voir, alors. Vous étiez censé livrer une cargaison de "+GetGoodsNameAlt(iFrahtGoods)+" en montant de "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+". Mais comme je peux le voir, vous n'avez pas livré l'ensemble de la cargaison - vous êtes "+FindRussianQtyString(int(amount))+" court.";
		...
		case "Fraht_complete_bad_1":
			...
			dialog.text = "Trois fois le coût des marchandises. Cela s'élève à "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+".";
		...
		case "Fraht_complete_fail":
			dialog.text = "Oh, je vois... Eh bien, dans ce cas, tu ne devrais plus jamais montrer ta frimousse à aucune autorité portuaire de "+NationNameGenitive(int(pchar.questTemp.WPU.Fraht.Nation))+"! Va-t'en !";
...
/// -------------------------------работа курьером по доставке почты-------------------------------------
		...
		case "Postcureer":
			...
			dialog.text = "Ce paquet doit être livré à l'autorité portuaire de la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City)+", pour "+FindRussianDaysString(pchar.questTemp.WPU.Postcureer.DaysQty)+", et dès que possible. Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
		...
		case "Postcureer_complete":
			if (pchar.questTemp.WPU.Postcureer == "late")//опоздали
			{
				...
				dialog.text = "Hm... Mais vous étiez "+FindRussianDaysString(iTime)+" en retard. Par conséquent, je dois réduire votre paiement pour le travail. Selon le contrat, votre pénalité est de cinq pour cent du montant total pour chaque jour de retard. Maintenant, s'il vous plaît, prenez votre "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+", et essayez de vous en tenir au contrat à l'avenir.";
				link.l1 = "Eh bien, c'est déjà quelque chose.";
				link.l1.go = "Postcureer_complete_1";
				break;
			}
			dialog.text = "Merci pour votre travail. Veuillez prendre votre paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
			link.l1 = "Merci !";
			link.l1.go = "Postcureer_complete_1";
		break;
...
/// ---------------------------работа в качестве эскорта для торговых кораблей----------------------------
		...
		case "escort":
			...
			dialog.text = "Alors, tu devras escorter des navires '"+pchar.questTemp.WPU.Escort.ShipName1+"' et '"+pchar.questTemp.WPU.Escort.ShipName2+" à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+", pour "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"Les deux navires doivent atteindre leur destination sains et saufs. Vous remettrez les navires au fonctionnaire de l'autorité portuaire locale. C'est lui qui vous paiera également pour le travail.";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				...
				if (amount > 0)
				{
					dialog.text = "D'accord, alors. Vous étiez censé escorter des navires et livrer la cargaison de "+GetGoodsNameAlt(iGoods)+" en montant de "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+" . Mais je vois que vous n'avez pas livré toute la cargaison - vous êtes "+FindRussianQtyString(int(amount))+" court.";
			...
			if (pchar.questTemp.WPU.Escort == "late")//опоздали
			{
				...
				dialog.text = "Hm... Mais vous étiez "+FindRussianDaysString(iTime)+" en retard. Par conséquent, je dois réduire votre paiement pour le travail. Selon le contrat, votre amende est de cinq pour cent du montant total pour chaque jour de retard. Maintenant, s'il vous plaît, prenez votre "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+", et essayez de respecter le contrat à l'avenir.";
				link.l1 = "Eh bien, c'est déjà quelque chose.";
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				dialog.text = "D'accord, alors. Vous deviez escorter des navires et livrer la cargaison de "+GetGoodsNameAlt(iGoods)+" en quantité de "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+". Merci pour votre travail. Prenez votre paiement, je vous prie - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			else
			{
				dialog.text = "Merci pour votre travail. S'il vous plaît, prenez votre paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			link.l1 = "Merci !";
			link.l1.go = "Escort_complete_1";
		}
		...
		case "escort_bonus":
			...
			dialog.text = "Alors, vous aurez besoin d'escorter des navires '"+pchar.questTemp.WPU.Escort.ShipName1+"' et '"+pchar.questTemp.WPU.Escort.ShipName2+" à la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+", pour "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+". Paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"Aussi sur votre navire, je place la cargaison de "+GetGoodsNameAlt(iGoods)+" en quantité de "+FindRussianQtyString(iGoodsQty)+". Paiement pour la livraison - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+".";
			link.l1 = "Tout semble clair. Dois-je procéder ?";
		...
		case "EscortBonus_complete_fail":
			...
			if (pchar.questTemp.WPU.Escort == "begin")
			{
				...
				if (amount > 0 && amount <= 100)
				{
					...
					dialog.text = "Incroyable ! Et pourquoi avions-nous besoin de vous ? Comment avez-vous pu laisser cela arriver ?! J'espère que vous comprenez que toute récompense pour l'escorte est maintenant hors de question... Non seulement cela, mais vous êtes également à court de cargaison - "+FindRussianQtyString(int(amount))+"\nEn général, je ne vous paierai que pour le fret des marchandises livrées en bon état, moins la valeur des articles manquants. Vaquer à vos occupations "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+" et vous pouvez aller où bon vous semble.";
					link.l1 = "Eh bien, c'est déjà quelque chose.";
		...
		case "EscortBonus_complete_fail_3"://потеряли корабль и груз, время норма или нет, война
			dialog.text = "Oh, je vois... Eh bien, dans ce cas, tu ne devrais jamais montrer ta frimousse à aucune autorité portuaire de "+NationNameGenitive(int(pchar.questTemp.WPU.Escort.Nation))+"Va-t'en !";
			link.l1 = "Je n'ai sûrement pas manqué grand-chose...";
			link.l1.go = "exit";
		...
		case "Escort_fail_complete":
			...
			if (amount > 0)
			{
				dialog.text = "Voyons voir, alors. Vous deviez livrer une cargaison de "+GetGoodsNameAlt(iFrahtGoods)+" en montant de "+FindRussianQtyString(pchar.questTemp.WPU.Escort.GoodsQty)+"Mais je vois que vous n'avez pas livré l'ensemble de la cargaison - vous êtes "+FindRussianQtyString(int(amount))+" court.";
...
//-------------------------------------задания 2 уровня для курьера---------------------------------------------
		case "Postcureer_LevelUp":
			...
			{
				case 0:
					dialog.text = "Vous devrez livrer une dépêche officielle urgente au gouverneur de la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" pas plus tard que "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Soyez très prudent - nos ennemis pourraient également être à la recherche de ce message. Paiement pour la livraison - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Je suis d'accord !";
				...
				case 1:
					dialog.text = "Vous devrez livrer ces papiers au propriétaire du magasin de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+"au plus tard que"+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+" . Voici le dernier résumé des prix dans les magasins de tout l'archipel - un document commercial précieux. Il faut être prudent - les concurrents pourraient organiser une chasse. Paiement pour la livraison - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Je suis d'accord !";
					link.l1.go = "Postcureer_LevelUp_Go";
...
//					pchar.questTemp.WPU.Current.Add = "сводку цен для хозяина магазина";
				...
				case 2:
					dialog.text = "Vous devrez livrer ce paquet de documents à l'usurier dans la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" pas plus tard que "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". C'est une information très importante sur les taux de change et les cours des actions. Soyez très prudent - quelques personnages louches sont déjà intéressés par ce colis. Paiement pour la livraison - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Je suis d'accord !";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "Non, je suppose que je vais passer.";
					link.l2.go = "Postcureer_LevelDown";
				...
				case 3:
					dialog.text = "Vous devriez livrer ce paquet au maître de quai dans la ville de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" pas plus tard que "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Il contient les plans pour le nouveau "+RandPhraseSimple(LinkRandPhrase("brigantin","corvette","frégate"),LinkRandPhrase("flûte","pinace","galion"))+", employant des conceptions technologiques uniques. J'espère que vous comprenez qu'il y a d'autres... parties intéressées, donc vous devrez agir discrètement. Paiement pour la livraison - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Je suis d'accord !";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "Non, je suppose que je vais passer.";
					link.l2.go = "Postcureer_LevelDown";
...
//----------------------------------------задания 2 уровня фрахт------------------------------------------
	...
	case "Fraht_LevelUp":
		...
		switch (rand(4))//для тестов
		{
			case 0:
				...
				dialog.text = "Une demande pour une certaine marchandise a récemment augmenté de manière spectaculaire dans notre colonie. Les magasins locaux n'en ont plus en stock. Par conséquent, je voudrais que vous me fournissiez "+GetGoodsNameAlt(iGoods)+" en montant de "+FindRussianQtyString(iGoodsQty)+", et je suis prêt à payer le double - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" par article. Cela se montera à "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+" Oh, et essayez de respecter le délai d'un mois - je ne pourrai pas attendre les marchandises plus longtemps.";
			...
			case 1:
				...
				pchar.questTemp.WPU.Current.Add = "double";
				// 08Mar17 "month" was "moths"
				dialog.text = "Une demande pour une certaine marchandise a récemment augmenté de manière spectaculaire dans notre colonie. Les magasins locaux n'en ont plus en stock. Par conséquent, je voudrais que vous me fournissiez "+GetGoodsNameAlt(iGoods)+" d'un montant de "+FindRussianQtyString(iGoodsQty)+", et je suis prêt à payer le double - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" par article. Cela s'élèvera à "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, et essayez de respecter le délai d'un mois - je ne pourrai pas attendre les marchandises plus longtemps.";
				link.l1 = "D'accord, j'accepte cette mission.";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "Je suis désolé de vous décevoir, mais je ne peux pas le faire en ce moment.";
				link.l2.go = "Fraht_LevelDown";
			...
			case 2:
				...
				// 08Mar17 "month" was "moths"
				dialog.text = "L'arsenal de notre colonie manque cruellement "+GetGoodsNameAlt(iGoods)+". Les magasins locaux n'en ont plus en stock. Par conséquent, j'aimerais que vous me fournissiez "+GetGoodsNameAlt(iGoods)+" en montant de "+FindRussianQtyString(iGoodsQty)+", et je suis prêt à payer triple - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" par article. Cela s'élèvera à "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, et essayez de respecter le délai d'un mois - je ne pourrai pas attendre les marchandises plus longtemps.";
				link.l1 = "D'accord, j'accepte cette mission.";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "Je suis désolé de vous décevoir mais je ne peux pas le faire en ce moment.";
				link.l2.go = "Fraht_LevelDown";
			...
			case 3:
				...
				pchar.questTemp.WPU.Current.Add = "triple";
				// 08Mar17 "month" was "moths"
				dialog.text = "Après la récente épidémie de fièvre dans notre colonie, nous manquons de fournitures médicales - et il ne reste rien dans les magasins. Le gouverneur craint pour la sécurité des habitants. Par conséquent, j'aimerais que vous me fournissiez "+GetGoodsNameAlt(iGoods)+" en montant de "+FindRussianQtyString(iGoodsQty)+", et je suis prêt à payer le triple - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" par paquet. Cela s'élèvera à "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, et essayez de respecter le délai d'un mois - je ne pourrai pas attendre les marchandises plus longtemps.";
				link.l1 = "D'accord, j'accepte cette mission.";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "Je suis désolé de vous décevoir, mais je ne peux pas le faire en ce moment.";
				link.l2.go = "Fraht_LevelDown";
			...
			case 4:
				...
				pchar.questTemp.WPU.Current.Add = "double";
				// 08Mar17 "month" was "moths"
				dialog.text = "Apres une mauvaise récolte, notre colonie a constamment besoin de nourriture importée. En ce moment, notre stock est presque vide, et tout a été balayé des étagères des magasins depuis un bon moment. Je voudrais vous demander de me livrer "+GetGoodsNameAlt(iGoods)+" en quantité de "+FindRussianQtyString(iGoodsQty)+", et je suis prêt à payer le double - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+"par paquet. Cela s'élèvera à "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+"Ah, et essayez de respecter le délai d'un mois - je ne pourrai pas attendre la marchandise plus longtemps.";
				link.l1 = "D'accord, j'accepte cette mission.";
				link.l1.go = "Fraht_LevelUp_Go";
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		{
			dialog.text = "D'accord, vous deviez livrer une cargaison de "+GetGoodsNameAlt(iGoods)+" en quantité de "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+"Excellent, merci beaucoup. Voici votre paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
...
//----------------------------------------задания 2 уровня эскорт ------------------------------------------
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		dialog.text = "Excellent ! Encore une fois, vous avez prouvé que l'on peut compter sur vous. Merci pour ce travail bien fait. Veuillez accepter votre paiement - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"  Et n'oublie pas de me rendre visite de temps en temps - il y a toujours une ou deux missions pour un capitaine comme toi.";
...
//-------------------------------- квест доставки судового журнала до рассеяного кэпа ---------------------
		...
		case "SeekShip_good_1":
			dialog.text = "Je suis prêt à vous payer votre récompense. Elle se compose de " + int(npchar.quest.money) + " in doubloons. Malheureusement, je ne peux pas te payer plus que ça.";
...
*/		
		case "ShipStock_2":
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName)+" ' "+chref.Ship.Name+", classe "+RealShips[int(chref.Ship.Type)].Class+", le coût d'amarrage est "+FindRussianMoneyString(int(NPChar.MoneyForShip))+" par mois, paiement d'un mois à l'avance.";
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName) + " '" + chref.Ship.Name + "', classe " + RealShips[int(chref.Ship.Type)].Class +
								 ", coût d'amarrage " + FindRussianMoneyString(int(NPChar.MoneyForShip)) + " par mois, paiement anticipé.";
			dialog.Text = dialog.Text + " Si vous la laissez avec un officier et un équipage, alors avec eux " + FindRussianMoneyString(int(NPChar.MoneyForShip) + int(NPChar.MoneyForCrew));
        ...
        case "ShipStockManBack":
			...
			{
			    dialog.Text = "Tu veux récupérer ton navire ? Pour l'amarrage, tu dois encore "+FindRussianMoneyString(int(NPChar.MoneyForShip))+".";
			}
			else
			{
