	...
	switch(Dialog.CurrentNode)
	{
	  ...
	  // to_do
		...
		case "SitySpy_1":
			...
			dialog.text = "N'importe qui dans cette ville ! "+NPCharSexPhrase(npchar,GetFullName(&characters[int(pchar.GenQuest.SeekSpy.Idx)])+" peut le faire, par exemple, maintenant qu'"+NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.Idx)],"il","elle")+" est juste là.",
				"Oh, Seigneur, eh bien, par exemple, "+NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.Idx)],GetAddress_FormToNPC(NPChar)+" "+characters[int(pchar.GenQuest.SeekSpy.Idx)].lastname,GetFullName(&characters[int(pchar.GenQuest.SeekSpy.Idx)]))+" peut le faire.");
		...
		case "SeekSpy_Checking":
			dialog.text = "Oui, "+NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.BaseIdx)]," est bien une personne du coin.","elle est bien une personne du coin.");
		...
		case "CitizenFindItem_2":
			dialog.text = ""+GetSexPhrase("Capitaine","Madame")+", c'est vraiment génial ! S'il vous plaît, trouvez cette bague pour moi, mon mari arrive dans seulement "+int(NPChar.LifeDay)+" jours, et je ne voudrais pas qu'il me voie sans ma bague de mariage.";
		...
		case "CitizenFindItem_4":
			dialog.text = "Oui, s'il vous plaît. Et dépêchez-vous - rappelez-vous que vous avez seulement "+int(NPChar.LifeDay)+" jours.";
