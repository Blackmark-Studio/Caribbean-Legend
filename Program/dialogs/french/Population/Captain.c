	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Avast. Vous naviguez sous le drapeau de "+NationNameGenitive(int(pchar.nation))+", moussaillon. Je n'ai aucune envie de parler avec toi et je ne veux pas d'ennuis... Déguerpis !";
			...
			else
			{
				...
				{
					dialog.text = "Dois-je me répéter ? Je ne veux pas être soupçonné d'être un ami de "+NationNameAblative(int(pchar.nation))+"Fiche le camp ou j'appelle la garde !";
		...
		//квестовая часть
			...
			case "march_1":
				...
				dialog.text = "Mon navire est "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.shiptype),"Nom")))+" ' "+pchar.GenQuest.MarchCap.shipname+"Elle est au port en ce moment. Ne soyez pas en retard capitaine, je partirai dans six heures !";
