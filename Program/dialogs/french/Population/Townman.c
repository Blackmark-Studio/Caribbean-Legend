...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "passenger":
			...
			{
				dialog.text = "Capitaine, j'ai besoin d'aller à la colonie nommée "+XI_ConvertString("Colony"+pchar.GenQuest.Townpassenger.City)+", dès que possible, c'est sur "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Townpassenger.City)+"Voc")+", dans "+FindRussianDaysString(int(pchar.GenQuest.Townpassenger.DaysQty))+". Votre navire a l'air solide comparé à la majorité de ces petites embarcations qui naviguent ici. Je peux vous payer "+FindRussianMoneyString(int(pchar.GenQuest.Townpassenger.Money))+"Que dis-tu ?";
			}
			else
			{
				dialog.text = "Capitaine, il me faut aller à la colonie "+XI_ConvertString("Colony"+pchar.GenQuest.Townpassenger.City)+", c'est sur un "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Townpassenger.City)+"Voc")+", dans un "+FindRussianDaysString(int(pchar.GenQuest.Townpassenger.DaysQty))+"Je sais que cela semble extrêmement risqué, mais peut-être "+FindRussianDublonString(int(pchar.GenQuest.Townpassenger.Money))+" serait un bon coup de pouce pour vous ?";
			}
