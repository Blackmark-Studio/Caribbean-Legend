...
void ProcessDialogEvent()
{
	...
	// --------------------------------------вернулся на Остров---------------------------------------------
		...
		case "return_2":
			...
			{
				dialog.text = "Non, je n'ai pas froid. Je suis juste... Je ne veux pas en parler ici. Viens à ma cabine quand tu auras fini. Je veux rester seule avec toi, ouais, hein ? Tu ne vas pas voir Nathaniel maintenant ?";
				link.l1 = "Oui. Je dois emmener Danielle à lui. Cela ne prendra pas trop de temps. Je te verrai bientôt, Mary.";
			}
			link.l1.go = "return_3";
		break;
	...
	// --> Мэри вне LSC
		...
		case "LongHappy_9c":
			...
			dialog.text = "Je pense, "+int(pchar.questTemp.LongHappy.MarryMoney)+" pesos suffiront, ouais, hein ? "+int(pchar.questTemp.LongHappy.MarryRum)+"barils de rhum, il n’y a rien de pire que d’être privé de boissons. Dès que tu auras tout rassemblé, viens à la taverne, je m’occuperai du reste.";
