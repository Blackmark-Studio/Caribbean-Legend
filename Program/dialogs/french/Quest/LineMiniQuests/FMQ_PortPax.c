...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "officer_5":
			PlaySound("Voice\English\z_arest_02.wav");
			dialog.text = "Que diable se passe-t-il ici, messieurs ? J'ai entendu des combats.";
			link.l1 = "Ouf... Officier, je suis ici à la demande de la serveuse de la taverne — elle affirmait que quelqu’un se faisait tuer ici, et cela s’est malheureusement confirmé — ces deux vauriens ont attaqué ce noble gentilhomme. Hélas, je n’ai pas pu le sauver. Ces misérables l’ont achevé.";
			link.l1.go = "officer_6";
		break;
		
		case "officer_6":
			dialog.text = "Que le Diable m'emporte... C'est un Espagnol ! Que ferait-il ici ? Hm. Et qui êtes-vous alors, "+GetAddress_Form(NPChar)+" ?";
			link.l1 = ""+GetFullName(pchar)+". Capitaine "+GetFullName(pchar)+".";
		...
		case "noble":
			dialog.text = "Permettez-moi d'expliquer tout, officier ! Moi, Don Domingo Ortiz Albalate de Santo Domingo, suis arrivé à Port-au-Prince à l’invitation d’une jeune demoiselle. Alors que j’attendais cette entrevue, je me suis arrêté dans une taverne. Mais à peine deux heures s’étaient écoulées lorsque dos canallas malditas ont fait irruption dans ma chambre, et si ce n’avait été pour l’aide opportune de "+GetSexPhrase("ce bon señor qui est apparu","cette belle señorita qui est apparue")+", j’aurais été dans un pétrin.";
		...
		case "noble_5":
			dialog.text = "Capitaine "+pchar.lastname+"?! Oh Madre de Dios ! Quel désastre ! C'est un massacre ! Que s'est-il passé ici, "+GetSexPhrase("mi buen amigo","mi salvadora")+" ? Qui sont ces hommes ?";
		...
		case "noble_9":
			dialog.text = "Seulement des soupçons... d'affreux soupçons "+GetSexPhrase("mi buen amigo","mi salvadora")+" ! Comment le frère de ma mère pourrait-il faire une telle chose ?";
		...
		case "noble_14":
			dialog.text = "Tu devrais choisir de meilleures fréquentations, Carlos. Mon cher Ugo a mentionné les papiers de mon oncle dans sa dernière lettre... C'est un indice direct. Oh, je ne veux pas y croire ! Mais... ma cousine, Margarita Ortiz, qui vit en Espagne, a été fiancée il n'y a pas longtemps. Cela pourrait-il en être la cause, "+GetAddress_Form(NPChar)+" capitaine ?";
		...
		case "noble_22":
			dialog.text = "Suis-je maintenant autorisé à monter à bord de ton navire, capitaine ?";
		...
		case "noble_27":
			dialog.text = "Qu'as-tu là, "+GetAddress_Form(NPChar)+" ?";
		...
		case "noble_36":
			...
			dialog.text = "Tonton Luis est à sa place légitime réservée à son âme - en enfer... "+pchar.name+", tu m'as été envoyé par mon ange gardien ! Je suis heureux que tu n'aies pas cru aux paroles de ce vaurien. Pour être honnête, je craignais que tu ne me laisses ici seul à combattre ce canalla\nOui "+GetAddress_Form(NPChar)+", je ne suis pas parfait moi-même mais ça... ce mensonge éhonté sur moi engageant des pirates pour tuer mes propres parents ! M'accuser de ça... ça ! Bah ! Un destin bien mérité.";
			link.l1 = "Content que nous ayons réussi, Don Domingo. Et maintenant ? J'espère que nous n'avons pas enfreint la loi espagnole.";
			link.l1.go = "noble_37";
		break;
		
		case "noble_37":
			dialog.text = "Avec la loi ? Ha-ha ! Non "+pchar.name+", je vais montrer ce livre de comptes impeccable avec toutes les opérations de contrebande de mon oncle dedans. N'oublie pas les nombreuses tentatives sur ma vie, nous avons un grand nombre de témoins après sa dernière tentative à l'instant. Ne t'en fais pas, "+GetAddress_Form(NPChar)+". C'est ma ville, il est temps de leur montrer à tous !";
			link.l1 = "Veuillez avancer prudemment, señor. Laissez-les penser mieux de vous qu'ils ne le faisaient de votre défunt oncle.";
			link.l1.go = "noble_38";
		break;
		
		case "noble_38":
			dialog.text = "C'est le plan, capitaine. Mais je ne montrerai aucune pitié à quiconque ose me défier. Permettez-moi de vous exprimer ma sincère gratitude, "+pchar.name+" ! Allez à la taverne, reposez-vous, prenez une ou deux coupes. Je serais ravi de vous rejoindre, mais vous aviez raison. Il est temps de devenir plus sage et de prendre moi-même le contrôle de la plantation. Venez me voir ici demain, capitaine !";
		...
		case "noble_39":
			...
			PlaySound("VOICE\English\LE\Domingo\Domingo_05.wav");
			dialog.text = "Ah, te voilà "+GetAddress_Form(NPChar)+" !";
		...
		case "noble_45":
			dialog.text = "C'est vrai. Ce fut un plaisir, "+GetAddress_Form(NPChar)+" ! Viens me voir un de ces jours. Bon vent en mer !";
		...
		case "uncle":
			...
			if (pchar.sex == "woman") { dialog.text = "On dirait que je suis arrivé juste à temps. Mon neveu dévoyé et sa nouvelle conquête, la capitaine "+GetFullName(pchar)+"... Oui, oui, "+GetSexPhrase("jeune homme","señorita")+", ne soyez pas surprise – je me suis renseigné sur vous."; }
			else { dialog.text = "On dirait que je suis arrivé juste à temps. Mon neveu dévoyé et son ange gardien, le capitaine "+GetFullName(pchar)+"... Oui, oui, "+GetSexPhrase("jeune homme","señorita")+", ne soyez pas surpris – je me suis renseigné sur vous."; }
			link.l1 = "Luis Ortiz ?";
			link.l1.go = "uncle_1";
		break;
		...
		case "uncle_3":
			if (pchar.sex == "woman") { dialog.text = "Mon cher neveu, fidèle à lui-même. Tant de bruit, de lames qui s'entrechoquent et de verres qui tintent, mais si peu de résultats. Je ne sais pas ce que vous lui trouvez, señorita, il est..."; }
			else { dialog.text = "Mon cher neveu, fidèle à lui-même. Tant de bruit, de lames qui s'entrechoquent et de verres qui tintent, mais si peu de résultats. Je ne comprends pas ce qu’il vous a fait pour que vous aidiez un garnement aussi indigne..."; }
		...
		case "uncle_5":
			dialog.text = "Ah Ugo ! Le pire fripon de tous ! Ugo aimait son maître vaurien. Tous les crimes de son maître n'étaient pour lui que des enfantillages. Souviens-toi, "+GetSexPhrase("jeune homme","señorita")+", les affaires de famille sont réservées à la famille. Ceux qui fouinent et se mêlent se retrouveront dans de beaux draps.";
		...
		case "uncle_6":
			...
			dialog.text = "Avertissement. Écoutez, capitaine : "+GetSexPhrase("vous êtes allé trop loin, mais ","")+"je ne vous considère pas comme un ennemi, seulement comme quelqu’un d’égaré. Voici donc mon offre : faites demi-tour et partez. Je ne veux pas de bain de sang. Les affaires familiales seront réglées sans vous. "+GetSexPhrase("Je suis prêt à vous dédommager pour les frais causés par mon neveu :","Et pour que vous n’ayez plus à y penser, je suis prêt à vous payer :")+" "+FindRussianMoneyString(iTotalTemp)+" en liquide. Prenez-les et oubliez cet endroit. Sinon, mes gardes vous tueront tout simplement.";
		...
		case "uncle_8":
			if (pchar.sex == "woman") {dialog.text = "Je vois clair maintenant. Vous êtes comme mon neveu — une fille stupide et irréfléchie. Parfaite pour lui. Il est temps de vous donner une leçon à tous les deux !";}
			else {dialog.text = "Je vois clair maintenant. Vous ne valez pas mieux que mon neveu — un garçon idiot et présomptueux ! Il est temps de vous donner une leçon à tous les deux !";}
			link.l1.go = "exit";
		...
		case "ugo":
			...
			dialog.text = TimeGreeting()+" "+GetAddress_Form(NPChar)+". Comment puis-je vous être utile ?";
		...
		case "ugo_2":
			dialog.text = "Hélas, "+GetSexPhrase("caballero","señorita")+", Don Domingo n'est pas chez lui en ce moment. Personnellement, j'aimerais bien savoir moi-même où se trouve ce garnement... eh, je vous prie de m'excuser, jeune señor. Puis-je vous demander ce que vous avez besoin de lui ?";
		...
		case "ugo_3":
			dialog.text = "Madre de Dios ! Quelles histoires épouvantables vous racontez ici ! Je dois voir le commandant immédiatement - pour le prévenir des dangers auxquels Don Domingo fait face en ce moment ! "+UpperFirst(GetAddress_Form(NPChar))+", dovete andare e io devo sbrigarmi!, vous devez partir et je dois me dépêcher !";
		...
		case "ugo_4":
			dialog.text = "Oh! Comment avez-vous pu ? Ce garçon naïf - et entre les mains de ces mangeurs de grenouilles français ? Sans parler des ladrones menaçant sa vie ? Non, je dois voir le commandant ! "+UpperFirst(GetAddress_Form(NPChar))+", je vous en prie, allez-y !";
		...
		case "ugo_5":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+", quittez cette maison sur-le-champ !";
