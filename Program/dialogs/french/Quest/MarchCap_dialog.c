	...
	switch(Dialog.CurrentNode)
	{
		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				case 0: //конвой у острова, установка параметров
					...
					dialog.text = "J'ai réussi à découvrir que dans "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" un petit convoi de commerce sous le pavillon de "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", deux navires marchands et un navire de garde, navigueront près de "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+", ce n'est pas loin de "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+"Les marchands ont beaucoup de "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" dans leurs cales. Ce serait idiot de ne pas utiliser cette information, capitaine\nVous et moi pouvons facilement nous occuper des gardes et prendre la cargaison pour nous-mêmes. Alors, vous êtes partant ?";
					link.l1 = "Ça sonne tentant. Je dirais oui !";
					link.l1.go = "MarchCap_1_1";
					link.l2 = "Quelques-uns"+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" n'est pas suffisant pour moi pour gâcher mes relations avec "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+"Non, je ne vais pas faire cela ! Adieu, monsieur, et n'essayez même pas de m'arrêter !";
				...
				case 1: //энкаунтер на глобалке, установка параметров
					...
					dialog.text = "J'ai réussi à découvrir que dans "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" un petit convoi marchand sous le pavillon de "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", deux navires marchands et un navire de garde, vont partir de "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Startcity+"Gen")+" à "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Finishcity+"Acc")+" Les marchands ont beaucoup de "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" dans leurs cales. Ce serait idiot de ne pas utiliser cette information, capitaine.\nVous et moi pouvons facilement nous occuper des gardes et prendre la cargaison pour nous. Le plus difficile est de les trouver en pleine mer entre ces deux colonies. Alors, vous êtes partant ?";
					link.l1 = "Cela semble tentant. Je dirais oui!";
					link.l1.go = "MarchCap_2_1";
					link.l2 = "Certain "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" n'est pas suffisant pour que je gache mes relations avec "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+"Non, je ne vais pas faire cela ! Adieu, monsieur, et n'essayez même pas de m'arrêter !";
					link.l2.go = "MarchCap_exit";
				break;
				...
				case 2: //одиночный пиратский кулсейлор у бухты, установка параметров
					...
					dialog.text = "On m'a informé d'un pirate nommé "+GetName(NAMETYPE_ORIG,pchar.GenQuest.MarchCap.PirateName,NAME_NOM)+"Ce bandit a récemment chapardé "+RandPhraseSimple(RandPhraseSimple("Anglais","Français"),RandPhraseSimple("Espagnol","Neerlandais"))+" et a chargé sa cale de "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+". Mais, quel dommage, son navire "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.ShipType),"Nom")+"Accord"))+" fut gravement endommagé, il dut donc prendre le temps de la réparer.\nIl est actuellement à "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Général")+" de "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+". Je pense que cela prendra environ "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" pour qu'il répare les mâts et les trous dans la coque. Nous pouvons y arriver à temps si nous nous dépêchons et que tout le monde "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" sera à nous\nJe ne pourrai pas affronter ce pirate seul, c'est un très bon marin et combattant, mais à deux, nous pourrons le défier avec succès malgré son expérience. Alors, tu es partant ou pas?";
		...
		case "MarchCap_AfterBattle":
			...
			if (iTemp > 50 && iTemp < 500)
			{
				...
				link.l1 = "Si tu m'avais aidé au lieu de compter les oiseaux dans le ciel, le raid aurait été un succès... Prends ta part - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" et va te faire foutre !";
				link.l1.go = "MarchCap_Talk_exit";
			...
			{
				dialog.text = "Bien joué, capitaine ! Comme je l'ai dit - il était plutôt facile de vider les cales de ces marchands, ha-ha... Le butin est "+FindRussianQtyString(iQty)+" pièces. Ma part est "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "Excellent, capitaine ! Comme je l'ai dit - il était relativement facile de vider les cales de ces marchands, ha-ha... La prise est énorme, bien plus que ce que j'avais espéré. C'est "+FindRussianQtyString(iQty)+" pièces. Ma part est "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
		...
		case "MarchCap_AfterBattleGold":
			...
			if (iTemp > 10 && iTemp < 100)
			{
				...
				link.l1 = "Si tu m'aidais au lieu de compter les oiseaux dans le ciel, le raid aurait été un succès... Prends ta part - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" et fiche le camp !";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "poor";
				break;
			}
			...
			{
				dialog.text = "Bien joué, capitaine ! Le butin est "+FindRussianQtyString(iQty)+"  pieces. Ma part est "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "Splendide, capitaine ! Comme je l'ai dit - c'était plutôt facile, ha-ha... Le butin est énorme, bien plus que je ne l'avais espéré. Il est "+FindRussianQtyString(iQty)+" pieces. Ma part est "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
