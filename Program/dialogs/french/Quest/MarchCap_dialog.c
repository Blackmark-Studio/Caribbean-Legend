		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				...
				case 1: //энкаунтер на глобалке, установка параметров
					...
					dialog.text = "J'ai réussi à découvrir que dans "+FindRussianDaysString(sti(pchar.GenQuest.MarchCap.DaysQty))+" un petit convoi marchand sous le pavillon de "+NationNameGenitive(sti(pchar.GenQuest.MarchCap.Nation))+", deux navires marchands et un navire de garde, vont partir de "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Startcity+"Gen")+" à "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Finishcity+"Acc")+" Les marchands ont beaucoup de "+GetGoodsNameAlt(sti(pchar.GenQuest.MarchCap.Goods))+" dans leurs cales. Ce serait idiot de ne pas utiliser cette information, capitaine.\nVous et moi pouvons facilement nous occuper des gardes et prendre la cargaison pour nous. Le plus difficile est de les trouver en pleine mer entre ces deux colonies. Alors, vous êtes partant ?";
