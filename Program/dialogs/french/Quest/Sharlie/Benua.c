	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
			...
			if (!CheckAttribute(pchar, "questTemp.SharlieEpilog_Patent") && CheckCharacterItem(pchar, "patent_fra") || CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && !CheckAttribute(pchar, "questTemp.SharlieEpilog_Governor"))
			{
				if (CheckAttribute(pchar, "questTemp.Patria.GenGovernor"))
				{
					link.l3 = "Je souhaite informer le baron Forger ainsi que la chancellerie de Capsterville que je suis contraint de quitter le poste de gouverneur général.";
					link.l3.go = "SharlieEpilog_Benua_Governor";
				}
				else
				{
					link.l3 = "Je détiens un brevet français en cours de validité — je souhaite notifier la chancellerie de Capsterville de ma décision.";
			...
			{
				link.l6 = "Je dois faire parvenir un message à Fadey le Moscovite, à Bas-Ter.";
			...
			{
				link.l7 = "Je souhaite faire savoir à la famille Houk de Maroon Town que je pars en voyage.";
				link.l7.go = "SharlieEpilog_Benua_Houk";
			}
			if (!CheckAttribute(pchar, "questTemp.SharlieEpilog_Jino") && CharacterIsAlive("Jino"))
			{
				link.l8 = "Je veux prévenir Gino Guineili de Saint John's de mon départ pour l’Ancien Monde.";
		...
		case "SharlieEpilog_Benua_Governor":
			dialog.text = "Je regrette sincèrement, "+pchar.name+", que, pour rencontrer ton père, tu doives renoncer à une charge si élevée. Il est toutefois réconfortant de voir que, pour toi, la famille passe avant tout.";
			link.l1 = "Ce qui me tourmente, c’est que durant tout mon séjour ici, aux Caraïbes, je n’ai pas pris la peine de lui envoyer la moindre lettre ; et j’entends réparer cela par une visite en personne. Aussi étourdissant que fût mon succès, je ne dois pas oublier le devoir envers mon parent. Quant au baron — je pense qu’il saura pardonner un départ aussi brusque : après tout, j’ai beaucoup fait pour le bien de notre État commun.";
			link.l1.go = "SharlieEpilog_Benua_Governor_2";
			pchar.questTemp.SharlieEpilog_Governor = true;
		break;

		case "SharlieEpilog_Benua_Governor_2":
			dialog.text = "Tu as fait un choix difficile, mais digne, mon fils. Le pouvoir et les honneurs sont passagers, tandis que le devoir envers la famille et la lignée est éternel. Crois-moi, le Seigneur voit tes intentions et récompense non les titres, mais la fidélité du cœur.";
			link.l1 = "Je vous remercie, mon père. Vos paroles comptent beaucoup pour moi.";
		...
		case "SharlieEpilog_Benua_Governor_4":
			dialog.text = "Parfait. Je prendrai toutes les mesures nécessaires afin que les lettres soient remises dans les plus brefs délais.";
			link.l1 = "...";
			link.l1.go = "SharlieEpilog_Benua_Delivery";
