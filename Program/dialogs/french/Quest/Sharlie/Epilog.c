void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SharlieEpilog_Cureer":
			dialog.text = "Je vous prie de m'excuser. Êtes-vous " + GetTitle(NPChar, false) + " " + GetFullName(pchar) + " ?";
			link.l1 = "En personne. À quoi ai‑je l’honneur ?";
			link.l1.go = "SharlieEpilog_Cureer_1";
		break;

		case "SharlieEpilog_Cureer_1":
			dialog.text = "Ah, enfin... Vous trouver n’a pas été facile, j’ai dû presque parcourir tout l’archipel. J’ai pour vous une correspondance de l’abbé Benoît.";
			link.l1 = "Hm... De l’abbé ? Inattendu.";
			link.l1.go = "SharlieEpilog_Cureer_2";
		break;

		case "SharlieEpilog_Cureer_2":
			dialog.text = "On m’a ordonné de vous la remettre en main propre. Ne vous inquiétez pas pour le paiement — l’abbé a déjà couvert toutes mes dépenses à l’avance.";
			link.l1 = "Eh bien, donnez‑la ici. Je vous remercie.";
			link.l1.go = "exit";
			AddDialogExitQuestFunction("SharlieEpilog_Benua_activation");
		break;
		...
		// Увольняем Дассена
		case "SharlieEpilog_Dassen":
			dialog.text = "Quels sont vos ordres, "+GetTitle(NPChar, true)+" ?";
			link.l1 = ""+npchar.name+", je ne vais pas tergiverser - je quitte l’archipel. Des affaires familiales exigent ma présence au domaine de mon père.";
			link.l1.go = "SharlieEpilog_Dassen_1";
		break;
		
		case "SharlieEpilog_Dassen_1":
			dialog.text = "Si je comprends bien, nous nous séparons ?";
			link.l1 = "Hélas... Je ne peux agir autrement.";
			link.l1.go = "SharlieEpilog_Dassen_2";
		break;
		
		case "SharlieEpilog_Dassen_2":
			dialog.text = "Eh bien, ce fut pour moi un honneur de servir sous le même pavillon que vous, et je le ferai de nouveau avec plaisir, si le destin le permet.";
			link.l1 = "J’espère que notre amitié ne se rompra pas, mais qu’elle n’en sera que renforcée, car nous avons partagé bien des aventures.";
			link.l1.go = "SharlieEpilog_Dassen_nothing";
			link.l2 = "Pour votre loyauté et votre professionnalisme, je souhaite vous verser, à titre d’indemnité de départ, un mois de solde.";
			link.l2.go = "SharlieEpilog_Dassen_salary";
			link.l3 = "Vous avez servi de manière irréprochable, et je juge nécessaire de vous récompenser comme il se doit. Acceptez ceci - voici trois mois de solde. Vous avez amplement mérité cette récompense.";
			link.l3.go = "SharlieEpilog_Dassen_salaryX3";
		break;
		
		case "SharlieEpilog_Dassen_nothing":
			dialog.text = "Je partage sincèrement cet espoir, "+GetTitle(NPChar, true)+".";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "Appelez-moi simplement Charles. Et, soit dit en passant, j’ai quelque chose pour vous.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
				link.l1 = "Eh bien, jusqu’à notre prochaine rencontre, monsieur "+npchar.lastname+". Adieu.";
				link.l1.go = "SharlieEpilog_Dassen_Leave";
			}
		break;
		
		case "SharlieEpilog_Dassen_salary":
			dialog.text = "Je vous remercie, "+GetTitle(NPChar, true)+".";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "Appelez-moi simplement Charles. Et, soit dit en passant, j’ai encore quelque chose pour vous.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
		...
		case "SharlieEpilog_Dassen_salaryX3":
			dialog.text = "Je vous suis fort reconnaissant, "+GetTitle(NPChar, true)+", vous êtes bien trop généreux envers moi.";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "Appelez-moi simplement Charles. Et, soit dit en passant, j’ai encore quelque chose pour vous.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			...
			{
				link.l1 = "Assez de formalités, appelez-moi simplement Charles. Pour célébrer mon départ, je compte organiser une petite fête à la taverne. J’espère que vous vous joindrez à nous ?";
		...
		case "SharlieEpilog_Dassen_Governor":
			dialog.text = "Vous avez piqué ma curiosité, "+pchar.name+".";
			link.l1 = "Je vous ai recommandé au poste de gouverneur d’Isla Mona. Une lettre officielle parviendra bientôt de la chancellerie, et j’espère sincèrement que vous n’y répondrez pas par un refus. Cette île compte énormément pour moi, et je serais heureux de vous voir à la tête de son administration.";
			link.l1.go = "SharlieEpilog_Dassen_Governor_1";
		break;
		
		case "SharlieEpilog_Dassen_Governor_1":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+"... "+pchar.name+"... C’est pour le moins inattendu...";
			link.l1 = "Et pourtant. Quelle sera votre réponse ?";
			link.l1.go = "SharlieEpilog_Dassen_Governor_2";
		break;
		
		case "SharlieEpilog_Dassen_Governor_2":
			dialog.text = "C’est une offre que l’on ne refuse pas. Je l’accepte avec plaisir et je promets de tout faire pour la prospérité de cette île et le bien-être de ses habitants.";
			link.l1 = "Parfait, mon ami ! Je vous attends donc à la taverne, pour la fête donnée en l’honneur de mon départ. Alonso vous préviendra lorsque tout sera prêt.";
			link.l1.go = "SharlieEpilog_Dassen_Governor_3";
		break;
		
		case "SharlieEpilog_Dassen_Governor_3":
			dialog.text = "Je ne manquerai cela pour rien au monde, monseigneur.";
			link.l1 = "";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		
		case "SharlieEpilog_Dassen_Tavern":
			dialog.text = "Je ne manquerai cela pour rien au monde, "+pchar.name+".";
			link.l1 = "Parfait, mon ami. Nous nous y retrouverons donc. Alonso vous fera savoir quand nous serons prêts à commencer.";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		
		case "SharlieEpilog_Dassen_Leave":
			DialogExit();
			AddDialogExitQuestFunction("SharlieEpilog_Dassen_exit");
		break;
		...
		case "SharlieEpilog_Alonso_11": // этап 1
			...
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_gold_L")) {Achievment_Set("ach_CL_205");}
		break;
		...
		case "SharlieEpilog_PikarOnUliss_D_1":
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarAlive") || CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarResurrected")){dialog.text = "Tu as raison.";}
		...
		case "SharlieEpilog_HenriDeMonper_Mary_4":
			if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "du wirst mich den anderen vorstellen";
			else sStr = "du wirst mich unserem gast vorstellen";
		...
		case "SharlieEpilog_InKitchenDeMonpe_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer") || CheckAttribute(pchar, "questTemp.LSC.Mary_officer"))
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "bons amis";
				else sStr = "compagnon d’armes";
				dialog.text = "";
			...
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "tes bons amis";
				else sStr = "ton ami";
				dialog.text = "";
		...
		case "SharlieEpilog_KomendantFort_5":
			dialog.text = "Un plan remarquable, Votre Grâce ! Toutefois… vous êtes, sans nul doute, un homme aguerri aux combats navals, et pourtant il ne sied guère à une personne de votre rang de risquer sa vie. La flotte pourrait fort bien être commandée par…";
			link.l1 = "Tout comme je n’ai pu supporter la vie paisible en Europe, je ne saurais rester oisif sur le rivage tandis que se décide l’issue de la bataille. On n’échappe pas à son destin, "+GetAddress_Form(NPChar)+" "+npchar.lastname+". ";
			link.l1.go = "SharlieEpilog_KomendantFort_6";
		break;

		case "SharlieEpilog_KomendantFort_6":
			dialog.text = "Qu’il en soit ainsi. M’autorisez-vous à donner l’ordre de commencer les préparatifs ?";
			link.l1 = "Faites. Et que Dieu soit avec nous.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Mishelle_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer")) sStr = " et ta femme";
			dialog.text = "Tu n’as donc toujours pas compris ? Tu es mort. Avec tout ton équipage"+sStr+". Un simple marchand que tu avais décidé de détrousser a fait sauter la soute à poudre — plutôt que de laisser le moindre sou à des gens comme toi.\nSpectra sunt memoria peccatorum. C’est toi le fantôme, "+pchar.name+"... La légende des Caraïbes parle d’un homme dévoré par la cupidité et une vanité aveugle. Combien as-tu tué pour de l’or ?";
