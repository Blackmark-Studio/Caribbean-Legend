...
//*************************** Генератор - "Найденные судовые документы" **************
		...
		case "Al_ShipLetters_2":
			...
			s1 = s1 + "I actually needed a packet just like this. I, in turn, will offer you " + int(pchar.questTemp.different.GiveShipLetters.price5) + " pesos. What do you say?";
			dialog.text = s1;
...
//*************************** Открывание двери ***************************
	...
	// <-- хронометр Алекса
		...
		case "FDM_cabin":
			dialog.text = "Pourquoi pas ? Mes mains sont encore habiles.";
			link.l1 = "...";
		...
		case "FDM_cabin_s":
			dialog.text = "Sept coffres de doublons et ce sera fait en cinq jours.";
			link.l1 = "Marché conclu !";
		...
		case "FDM_cabin_pay":
			...
			if (amount <= 0) // всё принес
			{
				...
				dialog.text = "Alors, tu devais m'apporter "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" coffres, vous avez apporté "+FindRussianQtyString(iTemp)+"J'ai le paiement. Maintenant, c'est à mon tour. Va te promener. Ne t'inquiète pas, je ferai de mon mieux.";
				link.l1 = "Bien...";
			...
			else
			{
				...
				dialog.text = "Alors, tu devais m'apporter "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" coffres, vous avez apporté "+FindRussianQtyString(iTemp)+". "+FindRussianQtyString(amount)+" coffres restants.";
		...
		case "FDM_hull_01":
			dialog.text = "Bon, faisons les comptes... Pour une reconstruction complète de la coque du navire, il me faudra 150 unités de gaïac, de cordages, de soie navale et de résine pour chacune. Et 10 000 doublons d’or par-dessus le marché. Et ne me regardez pas comme ça — la plus grande partie de l’argent passera dans les matériaux. Ce travail revient à démonter le navire pièce par pièce pour le reconstruire entièrement depuis zéro.";
			link.l1 = "Non, je ne crois pas. Je ne suis pas prêt à apporter autant de ressources et à verser une montagne d'or pour la reconstruction. Je resterai avec ce que j'ai.";
		...
		case "FDM_hull_waitmaterials":
			...
			dialog.text = "Pas peur de te promener dans une colonie de pirates avec une telle somme, hehe ? Bon, je plaisante, ce sont tous des gens honnêtes, à leur manière. Maintenant, j'attends les matériaux. Pour rappel, tu devras apporter un total de 150 bois de fer, 150 rouleaux de cordages, 150 rouleaux de soie de navire, et 150 barils de résine.";
		...
		case "FDM_hull_givemoney":
			...
			if(PCharDublonsTotal() >= 10000)
			{
    			link.l2 = "Oui, je l'ai apporté. 10 000 doublons pleins, comme convenu.";
