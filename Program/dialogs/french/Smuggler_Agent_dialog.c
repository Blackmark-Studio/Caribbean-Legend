	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "Aucune affaire aujourd'hui. Il y a des patrouilles à chaque baie, notre gouverneur cherche une cargaison qui a été confisquée par un certain capitaine de"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Nom")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" et il l'a caché quelque part.";
				...
				{
					Link.l5 = "Un collègue à vous et mon bon ami de  "+XI_ConvertString("Colony"+PChar.GenQuest.ChurchQuest_2.QuestTown+"Gen")+" a dit que vous êtes au courant de certaines affaires.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "Pourquoi pas? Peut-être que je trouverai une utilité pour ton 'chargement'. Prends "+int(pchar.questTemp.different.GiveShipLetters.price2)+" pièces et oubliez ça.";
					link.l1 = "Non, merci.";
				...
				else
				{
					...
					{
						dialog.text = "Exactement! Je suis vraiment curieux maintenant. Je suppose que si je te paye "+int(pchar.questTemp.different.GiveShipLetters.price3)+" pièces et tu n'en auras rien à faire de ces documents.";
						link.l1 = "Non, merci.";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "Maintenant, c'est une chance de attraper ce insolent! Je suis vraiment, vraiment curieux. Ecoute, je te paierai "+int(pchar.questTemp.different.GiveShipLetters.price4)+" en or et tu oublieras complètement les documents sur la table ?";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "Aucune affaire aujourd'hui. Il y a des patrouilles à chaque baie, notre gouverneur recherche une cargaison qui a été confisquée par un certain capitaine de"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Nom")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" et il l'a caché quelque part.";
					link.l1 = "Et où est ce capitaine"+pchar.GenQuest.CaptainComission.Name+"?";
					link.l1.go = "CapComission_1";
					break;
				}
			...
			if(CheckAttribute(PChar, "GenQuest.ChurchQuest_2.AskContra_2") && NPChar.location == PChar.GenQuest.ChurchQuest_2.Contra_Colony + "_tavern")
			{
				Link.l3 = "Un collègue à vous et mon bon ami de  "+XI_ConvertString("Colony"+PChar.GenQuest.ChurchQuest_2.QuestTown+"Gen")+" a dit que vous êtes au courant de certaines affaires.";
		...
		case "Meeting_2":
			...
			Link.l1 = "Peut-être, vous serez intéressé par des marchandises spéciales ? Ou peut-être, avez-vous quelque chose à me proposer à la place ?";
			Link.l1.go = "Meeting_3";
		break;
                ...
                if (npchar.quest.trade_date != lastspeak_date)
    			{
                    ...
                    if (ChangeContrabandRelation(pchar, 0) > 5)
                    {
                        ...
                        if (Pchar.quest.contraband.CurrentPlace != "None")//boal fix
                        {
                            ...
                            {
                                Dialog.Text = "Je sais que nous pouvons faire des affaires avec vous. Nous vous attendrons à "+GetLocationNameByID(Pchar.quest.contraband.CurrentPlace)+".";
                            }
                            else
                            {
            				    Dialog.Text = "Hm... Peut-être, nous pouvons vous trouver un acheteur. Nous vous attendrons à "+GetLocationNameByID(Pchar.quest.contraband.CurrentPlace)+".";
            				}
				...
				else
				{
					...
					if (nRel > 0 && Statistic_AddValue(PChar, "contr_TravelKill", 0) < 20)
					{
						...
						if (bOk && !bOk2)
						{
							...
							Dialog.Text = "Eh bien, nous pouvons vous emmener à "+GetLocationNameByRef(&locations[FindLocation(pchar.GenQuest.contraTravel.destination.loc)])+" près "+XI_ConvertString("Colony"+pchar.GenQuest.contraTravel.destination+"Gen")+" pour "+pchar.GenQuest.contraTravel.price+" or. Apporte l'argent dans "+FindRussianDaysString(nDay)+". Le navire vous attendra à "+GetLocationNameByRef(&locations[FindLocation(Pchar.GenQuest.contraTravel.CurrentPlace)])+"exactement 24 heures.";
...
////////////////////////////////////////////////////////////////////////////////
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "C'est vrai ? Très bien. Mynheer Rosenkraft a besoin d'un lot de bois de fer, mais pas moins de "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+". Avez-vous assez de cargaison?";
		...
		case "bakaut_1":
			dialog.text = "Son galion naviguera près des côtes de "+XI_ConvertString(pchar.GenQuest.Bakaut.Island+"Gen")+" pendant plusieurs jours. Alors ne soyez pas en retard - il pourrait naviguer vers un autre endroit de l'archipel.";
