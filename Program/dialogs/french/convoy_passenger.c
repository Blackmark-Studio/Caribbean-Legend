...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "J'ai besoin d'être livré à la taverne de "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Gen")+", que sur "+XI_ConvertString(GetIslandByCityName(npchar.GenQuest.GetPassenger_Destination)+"Cela")+", pour "+FindRussianDaysString(int(npchar.GenQuest.GetPassenger_Time))+", et pour cela je te payerai "+FindRussianMoneyString(int(npchar.GenQuest.GetPassenger_Money))+". Êtes-vous d'accord ?";
		...
		case "convoy_DeskTalk":
			dialog.text = NPCStringReactionRepeat(LinkRandPhrase(RandSwear()+"Capitaine, le temps est écoulé. Quand, nom de dieu, vais-je enfin arriver à "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Acc")+"?",RandSwear()+"Capitaine, combien de temps encore allez-vous errer Dieu sait où ?! Quand arriverons-nous à "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Gen")+"?",RandSwear()+"Ecoutez, capitaine, je dois me rendre à "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Gen")+", et je veux obtenir une réponse claire et simple à quand cela se produira!"),RandPhraseSimple("Capitaine, une fois de plus je dois ouvrir une question concernant vos obligations. Quand arriverons-nous à "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Gen")+"?","Capitaine, c'est la deuxième fois que je dois soulever la question de mon voyage. Quand, enfin, verrons-nous la côte de "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Gen")+"?"),RandPhraseSimple(RandSwear()+"Capitaine, je demande pour la troisième fois - quand arriverons-nous à "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Acc")+"?","Capitaine, nous avons perdu un enfer de temps ! Quand allez-vous m'emmener à "+XI_ConvertString("Colony"+npchar.GenQuest.GetPassenger_Destination+"Acc")+"?"),"Écoutez Capitaine, c'est vraiment trop!","bloc",0,npchar,Dialog.CurrentNode);
