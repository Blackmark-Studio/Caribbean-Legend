...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "J'ai besoin d'être escorté à "+XI_ConvertString("Colony"+pchar.quest.destination+"Gen")+", qui est situé sur "+XI_ConvertString(GetIslandByCityName(pchar.quest.destination)+"Cela")+", dans "+FindRussianDaysString(int(pchar.ConvoyQuest.iDay))+", et pour cela je vais te payer "+FindRussianMoneyString(int(pchar.ConvoyQuest.convoymoney))+". Alors, votre décision?";
