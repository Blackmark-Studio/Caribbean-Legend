   ...
   /*NPChar.FileDialog2 = "DIALOGS\" + LanguageGetLanguage() + "\Prison\" + NPChar.City + "_Prison.c";
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "GiveTaskGun_1":
			...
			dialog.Text = "Ich brauche "+pchar.questTemp.PrisonGun.Text+", in der Höhe von "+int(pchar.questTemp.PrisonGun.Qty)+"  genau Einheiten. Ich werde in goldenen Dublonen bezahlen, "+int(pchar.questTemp.PrisonGun.Price)+" für ein Stück. Das wird sich belaufen auf "+FindRussianDublonString(int(pchar.questTemp.PrisonGun.Sum))+" insgesamt. Was wirst du sagen? Ach, und noch etwas - Ich werde die gesamte Lieferung akzeptieren, aber nicht früher als in einem Monat - das Geld ist noch nicht eingetroffen.";
