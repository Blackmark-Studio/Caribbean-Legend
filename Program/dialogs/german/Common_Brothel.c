	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "Hostess_NotChoice":
			...
			else
			{
				...
				// <-- legendary edition
				dialog.text = ""+GetSexPhrase("Nun, ausgezeichnet, mein Hengst!","Sie sind alle sehr geschickt in dem, was sie tun, zweifellos.")+" Ich kann Ihnen ein sehr nettes Mädchen anbieten, ihr Name ist "+GetFullName(sld)+", und sie ist im Moment frei. Dieses Vergnügen wird dich kosten "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+". Einverstanden?";
        ...
        case "Hostess_NotChoice_agree":
			...
			{
				dialog.text = "Oh, Sie sehen, das Problem ist, dass "+sld.name+" ist kein billiges Mädchen, ihr Preis ist "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+". Und ich sehe, dass du dir sie momentan einfach nicht leisten kannst. Komm zurück, wenn du reich bist"+GetSexPhrase(", Lieber","")+"..., Freund.";
		...
		// belamour ночной приключенец
		case "NA_Girl":
            dialog.text = "Oh, also haben wir hier einen Kenner, hee hee ... Nun, nun, ich habe so ein Mädchen. Aber sie kommt nicht billig: "+int(pchar.GenQuest.NightAdventure_money)+" Pesos.";
			link.l1 = "Abgemacht.";
