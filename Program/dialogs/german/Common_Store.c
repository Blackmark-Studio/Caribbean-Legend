...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "storage_0":
			...
			{
				dialog.text = "Was die Miete angeht, schuldest du mir immer noch "+FindRussianMoneyString(int(NPChar.MoneyForStorage))+".";
		...
		case "storage_4":
			...
			if(int(NPChar.MoneyForStorage) > 0)
			{
				dialog.text = "Und für die Miete, du schuldest mir immer noch "+FindRussianMoneyString(int(NPChar.MoneyForStorage))+".";
				if(int(pchar.money) >= int(NPChar.MoneyForStorage))
				{
					link.l1 = "Gut.";
		...
		case "business":
			...
			if (CheckAttribute(pchar, "GenQuest.Sunplace.Trader") && pchar.GenQuest.Sunplace.Trader == "complete" && npchar.location == pchar.GenQuest.Sunplace.Trader.City + "_store") 
			{
				...
				link.l1 = "Heh! Das ist sicher. "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Sunplace.Trader.ShipType),"Name")))+" "+pchar.GenQuest.Sunplace.Trader.ShipName+" und die Ladung - "+GetGoodsNameAlt(int(pchar.GenQuest.Sunplace.Trader.Goods))+", gehören nicht länger deinem Rivalen.";
		...
		case "generate_quest":
			if (npchar.quest.trade_date != lastspeak_date || bBettaTestMode)
			{
				...
				{
					dialog.text = "Nein, ich brauche deine Dienste nicht! Zumindest solange du Feinde mit "+NationNameAblative(int(NPChar.nation))+".";
					...
					else
					{
                        ...
                        if (storeMan > 0)
                        {
    						...
    						else
    						{
                                ...
                                dialog.text = "Oh! Ich wollte dich gerade um einen Gefallen bitten. Siehst du, ich muss die Fracht liefern von "+GetGoodsNameAlt(iTradeGoods)+" in der Menge von "+FindRussianQtyString(iQuantityGoods)+" zur Stadt von "+sNation+sTemp+", und je eher, desto besser. Wenn es Ihnen gelingt, es für "+FindRussianDaysString(int(pchar.CargoQuest.iDaysExpired))+", dann bei der Ankunft erhalten Sie "+FindRussianMoneyString(iMoney)+" als deine Belohnung. Was sagst du dazu?";
                        ...
                        {
                            dialog.text = "Politische Realitäten im Archipel erlauben es mir nicht zu handeln. "+XI_ConvertString(NationShortName(int(NPChar.nation))+"hunter")+" ist feindselig gegenüber jedem und hat nur mich im Laden gelassen.";
		...
		case "generate_quest_2":
			...
			else
			{
                ...
                dialog.text = "Tatsächlich! Ich warte schon eine ganze Weile darauf. Du musst mir eine Ladung liefern "+GetGoodsNameAlt(iTradeGoods)+" in Höhe von "+FindRussianQtyString(iQuantityGoods)+" und erhalten dafür "+FindRussianMoneyString(int(pchar.CargoQuest.iMoney))+".";
				link.l1 = "Genau.";
		...
		case "generate_quest_not_closed":
            ...
            dialog.text = "Hmm. Ich habe gehört, dass du deine Verpflichtungen aus dem vorherigen Vertrag nicht erfüllt hast - und du fragst nach einem anderen? Du musstest die Fracht liefern von "+GetGoodsNameSeaSection(&Goods[iTradeGoods])+" zu "+XI_ConvertString("Colony"+pchar.CargoQuest.iTradeColony)+".";
...
//Jason --> мини-квест Дефицитный товар
		...
		case "Wine_Qty":
			...
			dialog.text = ""+int(iQty)+" Flaschen? Gut. Das kostet dich "+FindRussianMoneyString(int(pchar.questTemp.Wine.Summ))+".";
...
// Jason --> -------------------------генератор Сомнительное предложение----------------------------------------
		case "Contraoffer":
			...
			dialog.text = "Ich hatte vor, ein gutes Geschäft zu machen, aber ich brauche noch eine bestimmte Ware dafür - "+GetGoodsNameAlt(iCGood)+". Das Problem ist, dass diese Ware in unserer Kolonie verboten ist, daher kann ich nicht darauf zählen, sie von Handelskapitänen zu kaufen\nAlles, was ich brauche, sind "+FindRussianQtyString(int(pchar.GenQuest.Contraoffer.Trader.Qty))+" Einheiten davon. Vielleicht könnten Sie es mir liefern? Ich werde Sie gut bezahlen, "+FindRussianMoneyString(int(pchar.GenQuest.Contraoffer.Trader.Price))+" pro Einheit, was sich auf "+FindRussianMoneyString(int(pchar.GenQuest.Contraoffer.Trader.Summ))+". Oh, und ich brauche es nicht später als in "+FindRussianDaysString(int(pchar.GenQuest.Contraoffer.Trader.Days))+". Also, wirst du mir helfen?";
		...
		case "Contraoffer_pay":
			dialog.text = "Danke für Ihre Arbeit. Bitte, nehmen Sie Ihre Bezahlung - "+FindRussianMoneyString(int(pchar.GenQuest.Contraoffer.Trader.Summ))+".";
		...
		case "Contraoffer_slaves"://единичный вариант за всю игру, ака 'пасхалка'
			...
			dialog.text = "Ich brauche dringend eine Gruppe Sklaven - "+int(pchar.GenQuest.Contraoffer.Slaves.Qty)+" Köpfe. Ich bin bereit zu zahlen "+int(pchar.GenQuest.Contraoffer.Slaves.Price)+" Münzen pro Seele, was sich auf "+int(pchar.GenQuest.Contraoffer.Slaves.Money)+" pesos in total; term - "+FindRussianDaysString(int(pchar.GenQuest.Contraoffer.Slaves.Days))+".";
		...
		case "Contraoffer_slaves_pay":
			dialog.text = "Danke für Ihre Arbeit. Bitte nehmen Sie Ihre Bezahlung - "+FindRussianMoneyString(int(pchar.GenQuest.Contraoffer.Slaves.Money))+". Also, bitte akzeptieren Sie dies. Ich bin sicher, Sie werden es nützlich finden.";
			link.l1 = "Danke! Es war ein Vergnügen, Geschäfte mit Ihnen zu machen.";
		...
		case "Sunplace_1":
			...
			dialog.text = "Sehr gut. Also, der Name Ihres Mannes ist "+pchar.GenQuest.Sunplace.Trader.Enemyname+". Vor kurzem hatte er ausgesandt "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Sunplace.Trader.ShipType),"Name")+"Konto"))+" '"+pchar.GenQuest.Sunplace.Trader.ShipName+"mit einer Ladung von "+GetGoodsNameAlt(int(pchar.GenQuest.Sunplace.Trader.Goods))+". Er hat ziemlich viel Geld für diese Ladung bezahlt, also wird der Verlust ganz schön an seinem Geldbeutel kratzen.";
			link.l1 = "Und könnten Sie mir sagen, wo genau ich diesen 'Freund' von Ihnen suchen soll?";
			link.l1.go = "Sunplace_2";
		break;
		
		case "Sunplace_2":
			dialog.text = "Mein 'Freund' ist derzeit in seinem Zuhause in "+XI_ConvertString("Colony"+pchar.GenQuest.Sunplace.Trader.CityT+"Voc")+". Du brauchst ihn nicht - du brauchst sein Schiff, das, wie ich herausgefunden habe, nach etwa "+FindRussianDaysString(int(pchar.GenQuest.Sunplace.Trader.DaysQty))+" wird in der Nähe vorbeifahren "+XI_ConvertString(pchar.GenQuest.Sunplace.Trader.Shore+"Gen")+". Sie können es versenken, können es durch Entern nehmen - das spielt keine Rolle. Die Hauptsache ist, sicherzustellen, dass das Schiff und die Ladung nicht mehr im Besitz von "+pchar.GenQuest.Sunplace.Trader.Enemyname+". Und es ist mir egal, wer es beansprucht - du oder das Meer. Für diese Arbeit werde ich dich bezahlen "+FindRussianMoneyString(int(pchar.GenQuest.Sunplace.Trader.Money)) + ".";
		...
		case "SharkGoldFleet_01":
			...
			dialog.text = "So lala... Also, dreihundert Einheiten Kaffee... Und die gleiche Menge Kakao... Das wird dich kosten..."+int(GetStoreGoodsPrice(&stores[int(rColony.StoreNum)],GOOD_COFFEE,PRICE_TYPE_BUY,pchar,300)+GetStoreGoodsPrice(&stores[int(rColony.StoreNum)],GOOD_CHOCOLATE,PRICE_TYPE_BUY,pchar,300))+" Pesos!";
