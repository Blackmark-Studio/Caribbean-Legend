...
// --> продажа мышьяка
		...
		case "get_poison_2" :
			dialog.text = "Oh, stattlicher so ein tapferer Mann! (flüstert) Bezahle mich "+int(npchar.quest.poison_price)+" Dublonen.";
...
// <-- продажа мышьяка
	...
	// --> Мангароса
		...
		case "dwh_ne_ta":
			...
			dialog.text = "Nein, " + GetSexPhrase("mein Lieber", "meine Schöne") + ", ich bin nicht diejenige, die du suchst, es ist " + sld.name + ". Sie ist gerade hier in der Stadt. Ich habe sie kürzlich gesehen.";
			link.l1 = "Danke.";
			link.l1.go = "exit";
		break;
		// <-- Тёмные воды исцеления
