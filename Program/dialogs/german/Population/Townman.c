...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "passenger":
			...
			{
				dialog.text = "Kapitän, ich muss in die Kolonie namens "+XI_ConvertString("Colony"+pchar.GenQuest.Townpassenger.City)+", so schnell wie möglich, es ist an "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Townpassenger.City)+"Dat")+", in "+FindRussianDaysString(int(pchar.GenQuest.Townpassenger.DaysQty))+". Ihr Schiff sieht solide aus im Vergleich zu der Mehrheit dieser kleinen Boote, die hier segeln. Ich kann Ihnen zahlen "+FindRussianMoneyString(int(pchar.GenQuest.Townpassenger.Money))+". Was sagst du dazu?";
			}
			else
			{
				dialog.text = "Kapitän, ich muss zur Kolonie "+XI_ConvertString("Colony"+pchar.GenQuest.Townpassenger.City)+", es ist auf einem "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Townpassenger.City)+"Voc")+", in einem "+FindRussianDaysString(int(pchar.GenQuest.Townpassenger.DaysQty))+". Ich weiß, das klingt äußerst riskant, aber vielleicht "+FindRussianDublonString(int(pchar.GenQuest.Townpassenger.Money))+" wäre ein netter Schub für dich?";
			}
