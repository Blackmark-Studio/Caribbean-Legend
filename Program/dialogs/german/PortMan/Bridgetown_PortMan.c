    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Vielleicht ist es Ihnen, Kapitän, aufgefallen, dass die Stadt belagert wird. Wir erwarten jeden Moment einen Angriff der Spanier und mobilisieren unsere Kräfte. Aber die Regatta geht trotzdem weiter\nLassen Sie uns registrieren: Kapitän "+GetFullName(pchar)+", Schiff ist - "+pchar.Ship.Name+"... Regatta's Zeit in Stunden ist "+int(pchar.questTemp.Regata.FourthTransitionTime)+". Fertig, Ihr Ergebnis ist registriert, Sie können Ihren Weg fortsetzen.";
		...
		case "caleuche":
			...
			link.l5 = "Das Problem ist, dass ich den Namen seiner alten Prahme nicht kenne. Es ist seltsam, eine Vogelfrau, geboren...";
		...
		case "caleuche_2":
			...
			link.l4 = "Furie."; 
			link.l4.go = "caleuche_wrong_name";
			link.l5 = "Ich muss noch etwas nachdenken.";
			link.l5.go = "exit";
		break;
