void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Escort_choice"://первые развилки - выбираем между городом, бухтой и необитайкой
			...
			switch (hrand(2))
			{
				case 0://город дружественный
					...
					dialog.text = "Ich möchte Sie bitten, mich zu begleiten zu "+XI_ConvertString("Colony"+pchar.GenQuest.Escort.Trader.City+"Gen")+" innerhalb "+FindRussianDaysString(int(pchar.GenQuest.Escort.Trader.DaysQty))+". Ich werde dich bezahlen "+FindRussianMoneyString(int(pchar.GenQuest.Escort.Trader.Money))+".";
		...
		case "EscortTrader_complete_2":
			dialog.text = "Ja, eine sehr weise Beobachtung... Hier, nimm deinen "+FindRussianMoneyString(int(pchar.GenQuest.Escort.Trader.Money))+" und Lebewohl.";
