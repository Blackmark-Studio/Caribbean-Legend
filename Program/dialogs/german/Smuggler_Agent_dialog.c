	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "Kein Geschäft heute. Es gibt Patrouillen in jeder Bucht, unser Gouverneur sucht nach einer Fracht, die von einem Kapitän konfisziert wurde."+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Akk"))+" "+pchar.GenQuest.CaptainComission.Name+" und er hat es irgendwo versteckt.";
				...
				{
					Link.l8 = "Hör zu, mein Freund, ich brauche eine Lieferung Wein und Rum. Viel und schnell.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "Warum nicht? Vielleicht finde ich eine Verwendung für deine 'Fracht'. Nimm "+int(pchar.questTemp.different.GiveShipLetters.price2)+" Münzen und vergiss es.";
					link.l1 = "Nein, danke.";
				...
				else
				{
					...
					{
						dialog.text = "Genau! Jetzt bin ich wirklich neugierig. Ich vermute, dass wenn ich dich bezahle "+int(pchar.questTemp.different.GiveShipLetters.price3)+" Münzen, da werden diese Papiere dir völlig egal sein.";
						link.l1 = "Nein, danke.";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "Nun, das ist eine Chance, diesen frechen Kerl zu fangen! Ich bin wirklich, wirklich neugierig. Schau, ich werde dich bezahlen "+int(pchar.questTemp.different.GiveShipLetters.price4)+" in Gold und du wirst die Dokumente auf dem Tisch völlig vergessen?";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "Kein Geschäft heute. Es gibt Patrouillen in jeder Bucht, unser Gouverneur sucht nach einer Fracht, die von irgendeinem Kapitän konfisziert wurde."+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Konto"))+" "+pchar.GenQuest.CaptainComission.Name+" und er hat es irgendwo versteckt.";
					link.l1 = "Und wo ist dieser Kapitän"+pchar.GenQuest.CaptainComission.Name+"?";
					link.l1.go = "CapComission_1";
					break;
				}
		...
		case "Meeting_2":
			...
			Dialog.Text = "Wofür?";
			Link.l1 = "Vielleicht sind Sie an einigen besonderen Waren interessiert? Oder vielleicht haben Sie mir etwas anzubieten?";
			Link.l1.go = "Meeting_3";
		break;
                ...
                if (npchar.quest.trade_date != lastspeak_date)
    			{
                    ...
                    if (ChangeContrabandRelation(pchar, 0) > 5)
                    {
                        ...
                        if (Pchar.quest.contraband.CurrentPlace != "None")//boal fix
                        {
                            ...
                            {
                                Dialog.Text = "Ich weiß, dass wir Geschäfte mit Ihnen machen können. Wir werden auf Sie warten bei "+GetLocationNameByID(Pchar.quest.contraband.CurrentPlace)+".";
                            }
                            else
                            {
            				    Dialog.Text = "Hm... Vielleicht können wir einen Käufer für dich finden. Wir warten auf dich bei "+GetLocationNameByID(Pchar.quest.contraband.CurrentPlace)+".";
            				}
...
//navy <--
				...
				else
				{
					...
					if (nRel > 0 && Statistic_AddValue(PChar, "contr_TravelKill", 0) < 20)
					{
						...
						if (bOk && !bOk2)
						{
							...
							Dialog.Text = "Nun, wir können dich zu "+GetLocationNameByRef(&locations[FindLocation(pchar.GenQuest.contraTravel.destination.loc)])+" in der Nähe "+XI_ConvertString("Colony"+pchar.GenQuest.contraTravel.destination+"Gen")+" für "+pchar.GenQuest.contraTravel.price+" Gold. Bring das Geld rein "+FindRussianDaysString(nDay)+". Das Schiff wird auf Sie warten bei "+GetLocationNameByRef(&locations[FindLocation(Pchar.GenQuest.contraTravel.CurrentPlace)])+" genau 24 Stunden.";
		...
		case "Travel_pay":
			...
			{
				Dialog.Text = "Es sieht so aus, als hättest du nicht genug Geld.";
				Link.l1 ="Wie schade, ich komme später wieder.";
			}
			Link.l1.go = "Exit";
		break;  
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
		...
		case "TPZ_smugglers_1":
			dialog.text = "Ha, der Gauner hinter dem Tresen hat dich geschickt, was?";
			link.l1 = "Und wenn schon? Bekommst du die Ware oder muss ich jemand anderen suchen?";
			link.l1.go = "TPZ_smugglers_2";
			DelLandQuestMark(npchar);
		break;
		
		case "TPZ_smugglers_2":
			dialog.text = "Ich kann alles besorgen, kapiert? Solange die Bezahlung stimmt. Aber dieser verfluchte Gouverneur sitzt uns im Nacken. Mit Alkohol wollen wir nichts mehr zu tun haben - die Flaschen klirren, man hört sie noch aus einer guten Meile Entfernung, der Profit ist mickrig, und das Risiko, am Galgen zu enden, riesig. Also, nimm's mir nicht übel.";
			link.l1 = "Tja... Und dabei behauptest du, du kannst alles besorgen.";
			link.l1.go = "TPZ_smugglers_3";		
		break;
		
		case "TPZ_smugglers_3":
			dialog.text = "Ich kann es auch! Ich will nur nicht. Da gibt's einen Kerl, der wohnt direkt neben der Residenz, stell dir das vor! Ha-ha. Was für ein Dummkopf... Wo war ich? Ach ja. Der vertickt heimlich ein bisschen Schnaps unter der Hand. Direkt unter der Nase des Gouverneurs, ha-ha. Frag ihn mal, vielleicht weiß er was. Die Mengen sind zwar klein, aber besser als nichts.";
			link.l1 = "Viel hast du mir nicht geholfen, aber danke trotzdem.";
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "Ist das so? Fein. Mynheer Rosenkraft braucht eine Charge Eisenholz, aber nicht weniger als "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+". Haben Sie genug Ladung?";
