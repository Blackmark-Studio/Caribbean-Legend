...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "Ich muss eskortiert werden zu "+XI_ConvertString("Colony"+pchar.quest.destination+"Gen")+", das sich auf "+XI_ConvertString(GetIslandByCityName(pchar.quest.destination)+"Dat")+", in "+FindRussianDaysString(int(pchar.ConvoyQuest.iDay))+", und dafür werde ich dich bezahlen "+FindRussianMoneyString(int(pchar.ConvoyQuest.convoymoney))+". Also, deine Entscheidung?";
