...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("A férjem nem engedi, hogy idegenekkel beszélgessek!",
                         "Nincs semmi, amirôl beszélhetnénk.");
				Link.l1 = "Nem nagy ügy.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase ("Mondd csak, mik a népszerû pletykák a helyi kocsmában?",
									"Inkább elmondanád, mi történik errefelé?",
									"Hé, haver, elmondanád, milyen az élet a szárazföldön?");
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,
		                PCharRepPhrase(LinkRandPhrase("Ó, igen, örülök"+NPCharSexPhrase(NPChar, "", "")+" , hogy majdnem olyan vagy, mint egy csésze pia. Kérdezz, amit csak akarsz.", "Szeret beszélgetni, kapitány? Nos, én is... Fôleg egy csésze rum mellett.", "Igen, " + PChar.name + "?"),
                                        LinkRandPhrase("Mire van szüksége?", "Látom, kapitány "+PChar.name+", maga elég beszédes?", "Fárasztó kérdések kapitány, jobb lenne, ha rummal bánna velem.")),
		                PCharRepPhrase(LinkRandPhrase("Mit akart tudni "+ GetAddress_Form(NPChar) + "?", "Hallgatom magát, kapitány.", "Mennem kell, úgyhogy kérem, siessen, kapitány úr."),
                                        LinkRandPhrase("Mindig örülök a kellemes társaságnak, " + GetAddress_Form(NPChar) + " " + PChar.lastname + ". Beszélj a véleményedrôl.",
                                                        "Igen, " + GetAddress_Form(NPChar) + "?",
                                                        "Szeret beszélgetni, kapitány? Nos, én is..."))
                            );

            // homo 25/06/06
			link.l1 = LinkRandPhrase ("Milyen pletykák népszerûek a helyi kocsmában?",
                                    "Mi folyik ezeken a tájakon?",
                                    "Mi újság az itteni földeken?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("Nem, te hallasz dolgokat. Most már megyek.", "Nem, semmi. Csak sétálni mentem."),
                                        RandPhraseSimple("Nem, semmi. Sok szerencsét neked!", "Csak sétálok egyet. Viszontlátásra."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar, "Nézze, mint a város polgára, meg kell kérnem, hogy ne sétálgasson felemelt pengével.");
				link.l1 = LinkRandPhrase("Jól van.", "Rendben.", "Ahogy mondod...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar, "Vigyázz a lovakra, "+ GetSexPhrase("pajtás","lass") +", amikor fegyverrel a kezedben rohangálsz. Ez engem is idegessé tud tenni...", "Nem szeretem, amikor "+ GetSexPhrase("férfiak","emberek") +" fegyverrel a kézben járkálnak. Megijeszt...");
				link.l1 = RandPhraseSimple("Megvan.", "eltávolítom.");
			}
			link.l1.go = "exit";
