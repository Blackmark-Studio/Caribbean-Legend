...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "quests":
			...
			{
                Dialog.text = "Non sono autorizzato a trattare con te. Parla con il comandante del nostro squadrone. Il suo nome è  "+GetFullName(characterFromID(NPChar.MainCaptanId))+", lo puoi trovare sul "+XI_ConvertString(RealShips[int(characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Type)].BaseName)+" '"+characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Name+"'.'";
        ...
        case "Talk_board":
            ...
            else
            {
                ...
                else
                {
                    ...
                    Pchar.GenQuest.MoneyForCaptureShip = (70 + hrand(60, "&CapSur"+NPChar.id)) * iEncPow;
					if(HasShipTrait(pchar, "trait14")) Pchar.GenQuest.MoneyForCaptureShip = int(int(Pchar.GenQuest.MoneyForCaptureShip) * 1.35);
                    Dialog.text = RandSwear()+"Sì, ho sentito molto parlare delle tue oltraggi. Così sia, ma ricorda "+XI_ConvertString(NationShortName(int(NPChar.nation))+"cacciatore")+" non lo lascerò impunito!";
		...
		case "MerchantTrap_Abordage":
			...
			Dialog.Text = "Nel nome di "+NationKingsName(NPChar)+" arrenditi, "+GetSexPhrase("sporco pirata","sgualdrina ripugnante!")+"! Allora la tua vita sarà risparmiata fino a un tribunale equo, dove sarai giudicato per tutto "+NationNameSK(int(NPChar.nation))+"navi che hai depredato"+GetSexPhrase("","")+".";
		...
		case "Siegehelp":		
            ...
            {
                Dialog.text = "Non sono autorizzato a trattare con te. Parla con il comandante del nostro squadrone. Il suo nome è  "+GetFullName(rchar)+", puoi trovarlo su "+XI_ConvertString(RealShips[int(rchar.Ship.Type)].BaseName)+" ''"+rchar.Ship.Name+"''.";
                link.l1 = "Grazie. Farò come dici tu.";
                link.l1.go = "exit";
		...
		case "attack_fort":
                ...
                link.l1 = "Posso aiutarti a schiacciare il forte della colonia di "+GetCityName(aData.Colony)+" e conquistare la città, e un bottino, risultando nel caso del nostro successo, dividiamo tra di noi.";
		...
		case "GoldSquadron":
            ...
            {
                Dialog.text = "Non sono autorizzato a trattare con te. Parla con il comandante del nostro squadrone. Il suo nome è "+GetFullName(rch)+", puoi trovarlo su "+XI_ConvertString(RealShips[int(rch.Ship.Type)].BaseName)+" ''"+rch.Ship.Name+"''.";
		...
		case "WMShip_final":
			...
			dialog.text = "Voglio ringraziarti ancora una volta, capitano - mi hai aiutato moltissimo, a me e al mio equipaggio, a uscire da quella situazione difficile. Come promesso, compenserò i costi dei materiali necessari per riparare la mia nave. Ecco "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+", per favore, prendili. E questo è il mio regalo personale per te.";
		...
		case "Количество вбитого провианта":
			...
			{
				dialog.text = "Affare fatto! "+int(iQty*iMoney)+" pesos.";
		...
		case "Количество вбитых гарпунов":
			...
			if(iQty > 0 && iQty <= iTemp)
			{
				dialog.text = "Affare fatto! "+int(iQty*100)+" pesos.";
