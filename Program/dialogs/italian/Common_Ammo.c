   ...
   /*NPChar.FileDialog2 = "DIALOGS\" + LanguageGetLanguage() + "\Prison\" + NPChar.City + "_Prison.c";
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "GiveTaskGun_1":
			...
			dialog.Text = "Ho bisogno "+pchar.questTemp.PrisonGun.Text+", nella quantità di "+int(pchar.questTemp.PrisonGun.Qty)+"  unità esattamente. Pagherò in dobloni d'oro, "+int(pchar.questTemp.PrisonGun.Price)+" per un pezzo. Questo ammonta a "+FindRussianDublonString(int(pchar.questTemp.PrisonGun.Sum))+" in totale. Cosa dirai? Ah, e un'altra cosa - accetterò l'intero lotto, ma non prima di un mese - i soldi non sono ancora arrivati.";
		...
		case "TakeGun_1":
			...
			Log_Info("Avete ricevuto "+FindRussianDublonString(int(pchar.questTemp.WPU.Escort.Money))+"");
