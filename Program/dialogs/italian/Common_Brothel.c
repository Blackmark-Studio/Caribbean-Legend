	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "Hostess_NotChoice":
			...
			else
			{
				...
				// <-- legendary edition
				dialog.text = ""+GetSexPhrase("Bene, eccellente, mio stallone!","Sono tutte molto abili in quello che fanno, non avere dubbi.")+" Posso offrirti una ragazza molto carina, si chiama "+GetFullName(sld)+", e al momento è libera. Quel piacere ti costerà "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+". D'accordo?";
        ...
        case "Hostess_NotChoice_agree":
			...
			{
				dialog.text = "Oh, vedi, il problema è che "+sld.name+" non è una ragazza a buon mercato, il suo prezzo è "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+". E vedo che al momento semplicemente non puoi permetterti di averla. Torna quando sarai ricco"+GetSexPhrase(", caro","")+"...";
		...
		// belamour ночной приключенец
		case "NA_Girl":
            dialog.text = "Oh, quindi abbiamo un intenditore qui, hee hee ... Beh, beh, ho una ragazza così. Ma non viene a buon mercato: "+int(pchar.GenQuest.NightAdventure_money)+" pesos.";
			link.l1 = "Affare.";
