...
//			if(rand(1) == 0) PChar.GenQuest.ChurchQuest_1.CapWaitOnTavern = true;
		...
		case "ShipLetters_Portman1_1":
			...
			dialog.text = s1+"Dimmi, considereresti una somma di "+int(pchar.questTemp.different.GiveShipLetters.price1)+" sono pesos una ricompensa adatta?";
...
///Jason-----------------------новые генераторы накопительного типа для портмана----------------------------
		case "Work_check"://фейс-контроль - общая проверка профпригодности ГГ и выбор задания
			...
			if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") >= 2)//проверка повтора
				{
					...
					{
						dialog.text = RandPhraseSimple("Non c'è lavoro per coloro che navigano sotto la bandiera di "+NationNameGenitive(int(pchar.nation))+"! Uscite dal mio ufficio immediatamente!","Non collaborerò con "+NationNameAblative(int(pchar.nation))+". Vattene!");
					...
					link.l2 = "Voglio guadagnare soldi extra scortando mercanti.";
					Link.l2.go = "escort_begin";	//эскорт
					link.l3 = "Voglio guadagnare soldi extra consegnando posta.";
					Link.l3.go = "cureer_begin";	//доставка почты
					Link.l4 = "Ebbene, forse potresti offrire qualcosa?";
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Vorrebbe persino presentarsi a scortare convogli con una tartana? Per un lavoro del genere serve una nave con adeguata potenza di fuoco — un warrior o una nave da incursione.";
				link.l1 = "Ok, ho capito.";
				link.l1.go = "exit";
			}
		}
		else
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Mi dispiace, ma non ho nulla per te. Incarichi di questo tipo richiedono una nave da incursione agile. Con la tua è meglio occuparsi di noli.";
				link.l1 = "Ok, ho capito.";
				link.l1.go = "exit";
			}
		}
...
///--> ------фрахт со свободным выбором пункта назначения, оплаты и вида груза из предложенного списка---------
		...
		case "Fraht_choise":
			...
			{
				dialog.text = "Ecco le varianti disponibili:\n"+"carico "+GetGoodsNameAlt(iFrahtGoods1)+" in quantità di "+FindRussianQtyString(iFrahtGoodsQty1)+" alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+", in "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\nmerce "+GetGoodsNameAlt(iFrahtGoods2)+" in quantità di "+FindRussianQtyString(iFrahtGoodsQty2)+" alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+", in "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"\nmerce "+GetGoodsNameAlt(iFrahtGoods3)+" in quantità di "+FindRussianQtyString(iFrahtGoodsQty3)+" alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City3)+", in "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty3)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money3))+"\nQual è la tua scelta?";
			...
			{
				dialog.text = "Ecco le varianti disponibili:\n"+"carico "+GetGoodsNameAlt(iFrahtGoods1)+" nell'importo di "+FindRussianQtyString(iFrahtGoodsQty1)+" alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+", in "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\nmerce "+GetGoodsNameAlt(iFrahtGoods2)+" nella quantità di "+FindRussianQtyString(iFrahtGoodsQty2)+" alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+", in "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"\nQual è la tua scelta?";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (amount > 0)
			{
				dialog.text = "Vediamo allora. Dovevi consegnare un carico di "+GetGoodsNameAlt(iFrahtGoods)+" in quantità di "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+". Ma come posso vedere, non hai consegnato l'intera spedizione - tu sei "+FindRussianQtyString(int(amount))+" breve.";
		...
		case "Fraht_complete_1":
			if (pchar.questTemp.WPU.Fraht == "late")//опоздали
			{
				...
				dialog.text = "Mh... Ma tu eri "+FindRussianDaysString(iTime)+" in ritardo. Pertanto devo tagliare il tuo pagamento per il trasporto. Secondo il contratto, la tua multa è del cinque percento dell'importo totale per ogni giorno di ritardo. Ora per favore, prendi il tuo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+", e cerca di attenerti al contratto in futuro.";
				link.l1 = "Bene, è comunque qualcosa.";
				link.l1.go = "Fraht_complete_2";
				break;
			}
			dialog.text = "Grazie per il tuo lavoro. Per favore, prendi il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
			link.l1 = "Grazie!";
			link.l1.go = "Fraht_complete_2";
		break;
		...
		case "Fraht_complete_bad_1":
			...
			dialog.text = "Tre volte il costo delle merci. Questo ammonta a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+".";
		...
		case "Fraht_complete_fail":
			dialog.text = "Oh, capisco... Beh, in tal caso non dovresti mai mostrare di nuovo il tuo viso in qualsiasi autorità portuale di "+NationNameGenitive(int(pchar.questTemp.WPU.Fraht.Nation))+"! Vattene!";
...
/// -------------------------------работа курьером по доставке почты-------------------------------------
		...
		case "Postcureer_complete":
			if (pchar.questTemp.WPU.Postcureer == "late")//опоздали
			{
				...
				dialog.text = "Mh... Ma tu eri "+FindRussianDaysString(iTime)+" in ritardo. Pertanto devo ridurre il tuo pagamento per il lavoro. Secondo il contratto, la tua penalità è del cinque percento dell'importo totale per ogni giorno di ritardo. Ora per favore, prendi il tuo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+", e cerca di attenerti al contratto in futuro.";
				link.l1 = "Bene, è qualcosa almeno.";
				link.l1.go = "Postcureer_complete_1";
				break;
			}
			dialog.text = "Grazie per il tuo lavoro. Per favore, prendi il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
			link.l1 = "Grazie!";
			link.l1.go = "Postcureer_complete_1";
		break;
...
/// ---------------------------работа в качестве эскорта для торговых кораблей----------------------------
		...
		case "escort":
			...
			dialog.text = "Allora, dovrai scortare le navi '"+pchar.questTemp.WPU.Escort.ShipName1+"' e '"+pchar.questTemp.WPU.Escort.ShipName2+"' alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+", per "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+". Entrambe le navi devono raggiungere la loro destinazione sani e salvi. Consegnerai le navi all'ufficiale dell'autorità portuale locale. Sarà anche lui a pagarti per il lavoro.";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				...
				if (amount > 0)
				{
					dialog.text = "Bene, allora. Dovevi scortare le navi e consegnare il carico di "+GetGoodsNameAlt(iGoods)+" in quantità di "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+". Ma vedo che non hai consegnato l'intero carico - tu sei "+FindRussianQtyString(int(amount))+" breve.";
					link.l1 = "Mille squali! Devono essere quei maledetti ratti! Ma non preoccuparti, "+GetAddress_FormToNPC(NPChar)+", Acquisterò la quantità mancante e consegnerò l'intero lotto.";
					link.l1.go = "Fraht_complete_3";
					break;
				}
			...
			if (pchar.questTemp.WPU.Escort == "late")//опоздали
			{
				...
				dialog.text = "Mm... Ma tu eri "+FindRussianDaysString(iTime)+" in ritardo. Pertanto devo tagliare il tuo pagamento per il lavoro. Secondo il contratto, la tua multa è del cinque percento dell'importo totale per ogni giorno di ritardo. Ora per favore, prendi il tuo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+", e cerca di attenerti al contratto in futuro.";
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				dialog.text = "Bene, allora. Dovevi scortare le navi e consegnare il carico di "+GetGoodsNameAlt(iGoods)+" nell'importo di "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+". Grazie per il tuo lavoro. Per favore, prendi il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			else
			{
				dialog.text = "Grazie per il tuo lavoro. Per favore, prendi il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			link.l1 = "Grazie!";
			link.l1.go = "Escort_complete_1";
		}
		...
		case "escort_bonus":
			...
			dialog.text = "Allora, dovrai scortare le navi '"+pchar.questTemp.WPU.Escort.ShipName1+"' e '"+pchar.questTemp.WPU.Escort.ShipName2+"' alla città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+", per "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+". Pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"\nAnche sulla tua nave, colloco il carico di "+GetGoodsNameAlt(iGoods)+" nella quantità di "+FindRussianQtyString(iGoodsQty)+". Pagamento per la consegna - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+".";
			link.l1 = "Tutto sembra chiaro. Posso procedere?";
		...
		case "EscortBonus_complete_fail":
			...
			if (pchar.questTemp.WPU.Escort == "begin")
			{
				...
				if (amount > 0 && amount <= 100)
				{
					...
					dialog.text = "Incredibile! E a cosa servivi? Come hai potuto permettere che succedesse?! Spero tu capisca che qualsiasi ricompensa per la scorta è ora fuori discussione... Non solo, ma sei anche a corto di carico - "+FindRussianQtyString(int(amount))+"\nIn generale, ti pagherò solo per il trasporto di merci che sono state consegnate in condizioni accettabili, meno il valore degli articoli mancanti. Prendi il tuo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+" e puoi andare dove ti pare.";
			...
			if (pchar.questTemp.WPU.Escort == "late")
			{
				...
				dialog.text = "Incredibile! E a cosa servivi tu? Come hai potuto permettere che ciò accadesse?! Spero tu capisca che ora qualsiasi ricompensa per la scorta è fuori discussione... Inoltre, sei arrivato "+FindRussianDaysString(iTime)+" in ritardo.\nQuindi ho dovuto ridurre il pagamento per il trasporto. Secondo l'accordo, la multa è del 10 percento dell'importo originale per ogni giorno di ritardo. Ora, prendi il tuo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+", e puoi andare.";
		...
		case "EscortBonus_complete_fail_3"://потеряли корабль и груз, время норма или нет, война
			dialog.text = "Oh, capisco... Beh, in questo caso non dovresti mai mostrare di nuovo il tuo volto in qualsiasi autorità portuale di "+NationNameGenitive(int(pchar.questTemp.WPU.Escort.Nation))+"! Vattene!";
			link.l1 = "Certo che non mi sono perso molto...";
			link.l1.go = "exit";
		...
		case "Escort_fail_complete":
			...
			if (amount > 0)
			{
				dialog.text = "Vediamo, allora. Dovevi consegnare un carico di "+GetGoodsNameAlt(iFrahtGoods)+" nella quantità di "+FindRussianQtyString(pchar.questTemp.WPU.Escort.GoodsQty)+". Ma vedo che non hai consegnato l'intero carico - tu sei "+FindRussianQtyString(int(amount))+" breve.";
				link.l1 = "Mille squali! Devono essere quei maledetti ratti! Ma non preoccuparti, "+GetAddress_FormToNPC(NPChar)+", Acquisterò la quantità mancante e consegnerò l'intero lotto.";
				link.l1.go = "Fraht_complete_3";
				break;
			}
		...
		case "Escort_fail_complete_1":
			...
			if (pchar.questTemp.WPU.Escort == "late")//опоздали
			{
				...
				pchar.questTemp.WPU.Escort.PercentLate = iTime/0.1;
				pchar.questTemp.WPU.Escort.Money = int(int(pchar.questTemp.WPU.Escort.Money) - int(pchar.questTemp.WPU.Escort.Money)*int(pchar.questTemp.WPU.Escort.PercentLate)/100);//снижаем оплату согласно числа дней опоздания
				dialog.text = "Mmm... Non solo hai perso le navi che ti erano state affidate, ma sei anche in ritardo di "+FindRussianDaysString(iTime)+". Quindi ho dovuto tagliare il pagamento per il trasporto. Dal momento che non sei stato in grado di salvare le navi, ho dimezzato il pagamento per il trasporto, e questo è il massimo che posso offrirti.\nInoltre, secondo il contratto, la multa è del 10 percento dell'importo originale per ogni giorno di ritardo. Ora, prendi il tuo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+", e via vai.";
				link.l1 = "Bene, è qualcosa almeno.";
				link.l1.go = "Escort_fail_complete_2";
				break;
			}
			dialog.text = "Ebbene, anche se sono estremamente sconvolto dal fatto che non sei riuscito a proteggere le navi che ti erano state affidate, almeno hai consegnato la merce. Poiché non sei stato in grado di salvare le navi, ho ridotto del cinquanta per cento il pagamento per il trasporto, ed è il massimo che posso offrirti. Ora, prendi il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+" e puoi andare dove ti pare.";
			link.l1 = "Bene, è qualcosa almeno.";
			link.l1.go = "Escort_fail_complete_2";
		break;
...
//-------------------------------------задания 2 уровня для курьера---------------------------------------------
		case "Postcureer_LevelUp":
			...
			{
				case 0:
					dialog.text = "Dovrai consegnare un dispaccio ufficiale urgente al governatore della città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" non più tardi di "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Fai molta attenzione - i nostri nemici potrebbero essere alla ricerca di quella spedizione. Il pagamento per la consegna - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Sono d'accordo!";
				...
				case 1:
					dialog.text = "Dovrai consegnare questi documenti al proprietario del negozio di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" non più tardi di "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Questo è l'ultimo riepilogo dei prezzi nei negozi di tutto l'arcipelago - un documento commerciale di valore. Bisogna fare attenzione - i concorrenti potrebbero organizzare una caccia. Pagamento per la consegna - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Sono d'accordo!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "No, credo che passerò.";
					link.l2.go = "Postcureer_LevelDown";
...
//					pchar.questTemp.WPU.Current.Add = "сводку цен для хозяина магазина";
				...
				case 2:
					dialog.text = "Dovrai consegnare questo pacco di documenti all'usuraio nella città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" non più tardi di "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". È un'informazione molto importante sui tassi di cambio e sui prezzi delle azioni. Fai molta attenzione - alcuni personaggi loschi sono già interessati a ricevere questo pacchetto. Pagamento per la consegna - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Sono d'accordo!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "No, credo che passerò.";
					link.l2.go = "Postcureer_LevelDown";
				...
				case 3:
					dialog.text = "Dovresti consegnare questo pacco al capo del molo nella città di "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" non più tardi di "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Contiene i progetti per il nuovo "+RandPhraseSimple(LinkRandPhrase("brigantino","corvetta","fregata"),LinkRandPhrase("flauto","pinnace","galeone"))+", utilizzando disegni tecnologici unici. Spero che tu capisca che ci sono altre... parti interessate, quindi dovrai agire con discrezione. Il pagamento per la consegna - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Sono d'accordo!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "No, credo che passerò.";
					link.l2.go = "Postcureer_LevelDown";
...
//----------------------------------------задания 2 уровня фрахт------------------------------------------
	...
	case "Fraht_LevelUp":
		...
		switch (rand(4))//для тестов
		{
			case 0:
				...
				dialog.text = "Una richiesta per una certa merce è recentemente aumentata drasticamente nella nostra colonia. I negozi locali non l'hanno più in stock. Pertanto, vorrei che tu mi fornissi "+GetGoodsNameAlt(iGoods)+" in quantità di "+FindRussianQtyString(iGoodsQty)+", e sono disposto a pagare il doppio - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" per articolo. Questo ammonta a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, e cerca di rispettare la scadenza di un mese - non potrò aspettare la merce più a lungo.";
			...
			case 2:
				...
				// 08Mar17 "month" was "moths"
				dialog.text = "L'arsenale della nostra colonia è drasticamente carente "+GetGoodsNameAlt(iGoods)+". I negozi locali non lo hanno più in magazzino. Quindi, vorrei che tu mi fornissi "+GetGoodsNameAlt(iGoods)+" in quantità di "+FindRussianQtyString(iGoodsQty)+", e sono disposto a pagare il triplo - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" per articolo. Questo ammonta a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, e cerca di rispettare la scadenza di un mese - non potrò aspettare la merce più a lungo.";
				link.l1 = "Bene, accetterò questa missione.";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "Mi dispiace deluderti, ma non posso farlo in questo momento.";
				link.l2.go = "Fraht_LevelDown";
			...
			case 3:
				...
				pchar.questTemp.WPU.Current.Add = "triple";
				// 08Mar17 "month" was "moths"
				dialog.text = "Dopo la recente epidemia di febbre nella nostra colonia, ci mancano le forniture mediche - e non c'è più nulla nei negozi. Il governatore teme per la sicurezza dei cittadini. Quindi, vorrei che tu mi fornissi "+GetGoodsNameAlt(iGoods)+" in quantità di "+FindRussianQtyString(iGoodsQty)+", e sono disposto a pagare il triplo - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" al pacchetto. Questo ammonta a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, e cerca di rispettare la scadenza di un mese - non potrò aspettare oltre per la merce.";
				link.l1 = "Bene, accetterò questa missione.";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "Mi dispiace deluderti ma non posso farlo in questo momento.";
				link.l2.go = "Fraht_LevelDown";
			...
			case 4:
				...
				pchar.questTemp.WPU.Current.Add = "double";
				// 08Mar17 "month" was "moths"
				dialog.text = "Dopo un cattivo raccolto, la nostra colonia ha costantemente bisogno di cibo importato. Al momento le nostre scorte sono quasi vuote, e tutto è stato spazzato via dagli scaffali dei negozi già da un po' di tempo. Vorrei chiederti di consegnarmi "+GetGoodsNameAlt(iGoods)+" in quantità di "+FindRussianQtyString(iGoodsQty)+", e sono disposto a pagare il doppio - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" per pacchetto. Questo ammonta a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, e cerca di rispettare la scadenza di un mese - non sarò in grado di aspettare la merce più a lungo.";
				link.l1 = "Bene, accetterò questa missione.";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "Mi dispiace deluderti, ma non posso farlo in questo momento.";
				link.l2.go = "Fraht_LevelDown";
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		{
			dialog.text = "Bene, dovevi consegnare un carico di "+GetGoodsNameAlt(iGoods)+" in quantità di "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+". Eccellente, grazie mille. Ecco il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
...
//----------------------------------------задания 2 уровня эскорт ------------------------------------------
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		dialog.text = "Eccellente! Ancora una volta, hai dimostrato di essere affidabile. Grazie per l'ottimo lavoro. Per favore, accetta il tuo pagamento - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+". E non dimenticare di venire a trovarmi di tanto in tanto - c'è sempre un lavoro o due per un capitano come te.";
...
//-------------------------------- квест доставки судового журнала до рассеяного кэпа ---------------------
		...
		case "SeekShip_good_1":
			dialog.text = "Sono pronto a pagarti la tua ricompensa. Consiste in " + int(npchar.quest.money) + " in doubloons. Purtroppo, non posso pagarti di più di così.";
...
*/		
		case "ShipStock_2":
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName)+" '"+chref.Ship.Name+"', classe "+RealShips[int(chref.Ship.Type)].Class+", il costo dell'ormeggio è "+FindRussianMoneyString(int(NPChar.MoneyForShip))+" al mese, pagamento per un mese in anticipo.";
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName) + " '" + chref.Ship.Name + "', classe " + RealShips[int(chref.Ship.Type)].Class +
								 ", costo di ormeggio " + FindRussianMoneyString(int(NPChar.MoneyForShip)) + " al mese, pagamento anticipato.";
			dialog.Text = dialog.Text + " Se lo lasci con un ufficiale e un equipaggio, allora con loro " + FindRussianMoneyString(int(NPChar.MoneyForShip) + int(NPChar.MoneyForCrew));

			Link.l1 = "Sì. Va bene per me.";
			if (int(Pchar.Money) >= int(NPChar.MoneyForShip)) Link.l1.go = "ShipStock_3";
			else  Link.l2.go = "ShipStock_NoMoney";

			Link.l2 = "Sì. Va bene per me. Lo lasciamo con il capitano e l’equipaggio.";
		...
		case "ShipStock_3":
			...
			dialog.text = "Bene. Lo ritirerai quando ne avrai bisogno.";
			Link.l1 = "Grazie.";
			Link.l1.go = "exit";
		break;
        ...
        case "ShipStockManBack":
			...
			if (AttributeIsTrue(NPChar, "StoreWithOff") && FindFreeRandomOfficer() < 1 ) {  
				dialog.text = "Capitano, sembra che non ci sia posto per un altro ufficiale nel suo equipaggio.";  
				link.l1 = "Hmm... Allora tornerò più tardi.";  
				link.l1.go = "exit";  
				break;  
			}
			...
			{
			    dialog.Text = "Vuoi riprendere la tua nave? Per l'ormeggio, devi ancora "+FindRussianMoneyString(int(NPChar.MoneyForShip))+".";
			}
			else
			{
