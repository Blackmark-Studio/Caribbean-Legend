...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("Mio marito non mi permette di parlare con gli sconosciuti!","Non c'è niente di cui potremmo parlare.");
				Link.l1 = "Non è un grosso problema.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Dimmi, quali sono i pettegolezzi popolari nella taverna locale?","Preferiresti dirmi cosa sta succedendo qui intorno?","Ehi, compagno, potresti dirmi com'è la vita sulla terraferma?");
		...
		case "sovet":
			...
			{
                dialog.text = RandPhraseSimple("Mio marito non mi permette di parlare con gli sconosciuti!","Non c'è nulla di cui potremmo parlare.");
				Link.l1 = "Non è un grosso problema.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Dimmi, quali sono i pettegolezzi popolari alla taverna locale?","Preferiresti dirmi cosa sta succedendo qui?","Ehi, compagno, mi diresti com'è la vita sulla terra ferma?");
			link.l10.go = "rumours_citizen"; //повесил на горожанина, если нужны отедельные слухи, то пишем соответствующую реализацию
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("Oh sì, sono contento"+NPCharSexPhrase(NPChar,"","")+" vederti è quasi come un bicchiere di liquore. Chiedi quello che vuoi.","Ti piace parlare, capitano? Beh, anche a me... Soprattutto davanti a una tazza di rum.","Sì, "+PChar.name+"?"),LinkRandPhrase("Di cosa hai bisogno?","Capisco, capitano "+PChar.name+", sei proprio un chiacchierone?","Domande noiose Capitano, sarebbe meglio se mi trattassi con del rum.")),PCharRepPhrase(LinkRandPhrase("Cosa volevi sapere "+GetAddress_Form(NPChar)+"?","Ti sto ascoltando, capitano.","Devo andare, quindi per favore sbrigati, capitano."),LinkRandPhrase("Sono sempre felice di avere una buona compagnia, "+GetAddress_Form(NPChar)+" "+PChar.lastname+". Parla liberamente.","Sì, "+GetAddress_Form(NPChar)+"?","Ti piace chiacchierare, capitano? Beh, a me anche...")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("Quali tipi di pettegolezzi sono popolari nella taverna locale?","Cosa sta succedendo in queste terre?","Cosa c'è di nuovo sulla terra quaggiù?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("No, stai sentendo cose. Me ne vado ora.","No, niente. Sono solo uscito per una passeggiata."),RandPhraseSimple("No, niente. Buona fortuna a te!","Solo facendo una passeggiata. Addio."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Ascolta, come cittadino di questa città, devo chiederti di astenerti dal camminare con una spada sguainata.");
				link.l1 = LinkRandPhrase("Bene.","Bene.","Come dici tu...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"Frena i tuoi cavalli, "+GetSexPhrase("compagno","ragazza")+", quando corri in giro con un'arma in mano. Questo può rendermi nervoso...","Non mi piace quando "+GetSexPhrase("uomini","persone")+" gira in giro con le armi pronte. Mi spaventa...");
				link.l1 = RandPhraseSimple("L'ho capito.","Lo rimuoverò.");
			}
			link.l1.go = "exit";
