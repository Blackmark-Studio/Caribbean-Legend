void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Hold_GenQuest61":
			dialog.text = "Mi hai frainteso: sono un membro del consiglio cittadino. Vegliamo sul rispetto dello statuto.\n"+"Guardie!!! Guardie! Arrestatelo con l'accusa di tratta di schiavi e di saccheggio di navi di "+NationNameGenitive(int(pchar.GenQuest.Hold_GenQuest.Nation))+"!";
		...
		case "ShipWreck_2":
			...
			if(rand(1) == 0)
			{
				dialog.text = RandPhraseSimple("Ciao, sono "+pchar.GenQuest.ShipWreck.Name+", capitano del "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Acc"))+" '"+pchar.GenQuest.ShipWreck.ShipTypeName+", la mia nave si è schiantata sugli scogli non lontano dalla costa."+"Solo pochi marinai di tutto l'equipaggio sono riusciti a sopravvivere. La marea ci ha spinto su questa desolata spiaggia. Siamo stati affamati e assetati per "+(5+hrand(7))+" settimane ormai","Ciao, sono "+pchar.GenQuest.ShipWreck.Name+", capitano del "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Gen"))+" '"+pchar.GenQuest.ShipWreck.ShipTypeName+". La nostra nave si è schiantata non lontano da qui."+"Siamo riusciti a raggiungere questa riva sui relitti, ma questo posto sembra disabitato. Siamo stati costretti a sopravvivere con frutti di mare e palme per "+(5+hrand(7))+" settimane ormai.");
		...
		case "ShipWreck_3":
			dialog.text = RandPhraseSimple("Dall'intero equipaggio, solo "+ShipWreck_GetStringQty(int(pchar.GenQuest.ShipWreck.Qty))+" survived. Other's have perished; several of them died already here of wounds and fever",
		...
		case "ShipWreck_4":
			dialog.text = "Certo che no! Coloro che hanno rifiutato di unirsi ai ammutinati erano molti, ma questi malviventi hanno catturato l'arsenale, ed è bene che non abbiamo opposto resistenza, altrimenti saremmo stati uccisi sul posto. Molti qui sono già caduti a causa della febbre e del veleno degli insetti velenosi, solo "+ShipWreck_GetStringQty(int(pchar.GenQuest.ShipWreck.Qty))+" sopravvissuto. Grazie a Dio, le nostre prove sono finalmente terminate.";
		...
		case "ShipWreck_7":
			if(rand(1) == 0)
			{
				if(rand(100) < 90)
				{
					...
					dialog.text = "A qualsiasi insediamento di "+NationNameGenitive(int(pchar.GenQuest.ShipWreck.Nation))+".";
		...
		case "Convict_13_1":
			...
			link.l1 = "Considerando le circostanze, non ti concederò un passaggio più economico di "+FindRussianMoneyString(int(pchar.GenQuest.Convict.Sum))+" pesos a persona.";
		...
		case "Convict_13_2":
			dialog.text = "E abbiamo una scelta? Va bene, "+FindRussianMoneyString(int(pchar.GenQuest.Convict.Sum))+"...";
		...
		case "Convict_14":
			dialog.text = RandPhraseSimple(RandPhraseSimple("Eh, capitano, anche tu una volta eri povero, avevi le tasche vuote. E ora disprezzi la gente semplice come noi.", "Capitano, non è giusto trattare così la gente comune. Siamo tutti sotto lo stesso Dio."),
						RandPhraseSimple("Eh, la gente sta davvero diventando troppo egoista in questi giorni. Vivi come se avessi ancora due secoli davanti a te.","Oh, vedo che sei appena arrivato dall'Europa, non hai ancora fiutato la puzza del nostro discarico tropicale."));
		...
		case "Convict_15":
			...
			{
				link.l1 = "Sì, ecco la tua tinozza. E non dimenticare del pagamento. Ho pagato "+pchar.GenQuest.Convict.ShipPrice+" monete per questo, quindi mi devi "+int(pchar.GenQuest.Convict.ShipPrice)* 2+", proprio come abbiamo concordato.";
		...
		case "CaptainComission_22":		
			...
			{			
				...
				dialog.text = "Per favore, informa il capitano di una pattuglia "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Gen"))+" '"+pchar.GenQuest.CaptainComission.ShipTypeName+"'"+pchar.GenQuest.CaptainComission.Name+", che non dovrebbe mostrare il suo volto in "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City+"Voc")+"... "+"Le autorità sono a conoscenza delle vere circostanze della sua morte "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+" '"+pchar.GenQuest.CaptainComission.VictimShipName+"... il tempo è breve, sarà arrestato... navigando verso "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City+"Acc")+" da "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City1+"Gen")+", il viaggio richiederà quasi quindici giorni... ahh ... dobbiamo affrettarci..."; // belamour gen
		...
		case "CaptainComission_32_1":
			GetSlaveSpeciality();
			dialog.text = "Lo so, lo so... Ma c'è anche la tua colpa in questo. Come hai potuto affondare tre casse d'oro, che abbiamo dovuto letteralmente scuotere fuori dal tesoro, così che "+NationNameNominative(int(pchar.GenQuest.CaptainComission.Nation))+" potrebbe avere un tale specialista "+pchar.GenQuest.CaptainComission.SlaveSpeciality+"?";
			link.l1 = "Sappi, che non te lo darò senza una giusta ricompensa.";
			link.l1.go = "CaptainComission_32_2";
		break;
		
		case "CaptainComission_32_2":
			dialog.text = "Ebbene, sei un uomo intelligente, di sicuro capisci che è troppo tardi per trattare. Avremmo potuto arrestare la tua nave con il suo carico.\nMa la compensazione è stata fornita prima. Pertanto, il governo di "+NationNameGenitive(int(pchar.GenQuest.CaptainComission.Nation))+"ti congratula per un'operazione riuscita e ti presenta questo prezioso dono e una modesta ricompensa.";
		...
		case "CaptainComission_202_1":
			dialog.text = "Allora, i miei sospetti erano confermati. Sei stato costretto a farlo da "+NationNamePeople(int(pchar.GenQuest.CaptainComission.Nation))+"?";
		...
		case "CaptainComission_301":
			...
			link.l1 = "Ciao, sono il capitano "+GetFullName(pchar)+", agendo per conto di qualche capitano "+pchar.GenQuest.CaptainComission.CapName+". Ha chiesto di dirti che saresti stato arrestato in "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City+"Voc")+". Le autorità sanno di "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+"."; // ПРАВКА + belamour gen
...
//			bQuestDisableMapEnter = true;
		...
		case "CaptainComission_320":
			...
			dialog.text = "Dobbiamo affrettarci prima che arrivino i rinforzi. Il caricamento sta per finire, e la tua parte è "+pchar.GenQuest.CaptainComission.GoodsQty+" unità di "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+".";
		...
		case "CaptainComission_Canoneer4":
			...
			link.l1 = "Oh, non farti prendere dall'eccitazione. Sai... Era troppo tardi. Capitano "+pchar.GenQuest.CaptainComission.CapName+" mi ha chiesto di dire che "+pchar.GenQuest.CaptainComission.Name+"non dovrebbe mostrare il suo volto in quel insediamento, le autorità sanno delle circostanze della distruzione di "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Acc"))+" '"+pchar.GenQuest.CaptainComission.VictimShipName+", e sarà arrestato.";
		...
		case "CaptainComission_Canoneer7":
			dialog.text = "Disse loro che fui contuso durante l'imbarco e non sapevo niente su nessun carico. Ho anche finto di essere debole di mente, sai, mi sono appena alzato, giusto prima dell'arrivo di "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+". Non potevo muovermi correttamente, la mia coordinazione era a pezzi.";
		...
		case "CaptainComission_Canoneer11":
			...
			dialog.text = "Bene, d'accordo. Ci incontriamo stasera dopo la mezzanotte nei dintorni "+XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore+"Gen")+". E assicurati di avere spazio nella tua stiva per "+pchar.GenQuest.CaptainComission.GoodsQty+" pezzi. "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+". Sarebbe un peccato gettare via una parte di esso..."; // belamour gen
		...
		case "CaptainComission_372":
			...
			link.l1 = "Capitano "+pchar.GenQuest.CaptainComission.CapName+" mi ha chiesto di avvertirti che le circostanze della distruzione di "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Acc"))+" '"+pchar.GenQuest.CaptainComission.VictimShipName+" è diventato noto alle autorità, "+"ma non ho visto la tua nave nel viaggio da "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City1+"Gen")+".";
		...
		case "CaptainComission_374":
			dialog.text = "Capisco... Quindi è così, eh? Molte cose ora sono al loro posto. Quindi, qualcuno ha sparso la voce che abbiamo saccheggiato "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+". "+"Prima o poi, troveranno il carico e mi impiccheranno. Capitano "+pchar.GenQuest.CaptainComission.CapName+" è morto, significa che nessuno può aiutarmi\nTranne te\n "+GetFullName(pchar)+", tirami fuori da qui e ne varrà la pena per te.";
		...
		case "CaptainComission_381":
			pchar.GenQuest.CaptainComission.GoodsQty = int(int(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Capacity")) / int(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Weight));
			dialog.text = "Hai ragione, assicurati di avere abbastanza spazio per "+pchar.GenQuest.CaptainComission.GoodsQty+" pezzi di "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+". Sarebbe un peccato perdere un bottino così prezioso con tanto sangue su di esso. Non ci daranno una seconda possibilità.";
		...
		case "CaptainComission_387":
			...
			dialog.text = "Siamo qui. La cache si trova in una nicchia in quel dirupo. L'entrata è bloccata con delle rocce. Nella cache, c'è "+pchar.GenQuest.CaptainComission.GoodsQty+" unità di "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+". Ricorda che una metà è mia?";
