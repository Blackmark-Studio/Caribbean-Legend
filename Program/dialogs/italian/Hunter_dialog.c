void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "meeting":
			dialog.text = XI_ConvertString(Nations[int(NPChar.nation)].Name)+" ha messo una bella taglia sulla tua testa, dobbiamo consegnarti a una delle sue colonie e ci pagheranno, non importa se sei morto o vivo.";
		...
		case "ReasonToFast_HunterShore21":
			dialog.text = "Non secondo il suo rango. Hai rovinato la nostra operazione e abbiamo perso "+int(pchar.questTemp.ReasonToFast.p10)+" pesos a causa tua. Quindi ora hai un debito da pagare.";
			link.l1 = "Prendilo e saluta "+GetName(NAMETYPE_MAIN,pchar.questTemp.ReasonToFast.p3,NAME_DAT)+". Dìgli che ci incontreremo presto...";
