	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Avast. Navigi sotto la bandiera di "+NationNameGenitive(int(pchar.nation))+", mozzo. Non ho alcun desiderio di parlare con te e non voglio problemi... Levati dai piedi!";
			...
			else
			{
				...
				{
					dialog.text = "Dovrei ripetermi? Non voglio essere sospettato di essere un amico di "+NationNameAblative(int(pchar.nation))+"! Sparisci o chiamerò le guardie!";
		...
		//квестовая часть
			...
			case "march_1":
				...
				dialog.text = "La mia nave è "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.shiptype),"Name")))+" '"+pchar.GenQuest.MarchCap.shipname+". È in porto ora. Non fare tardi capitano, partirò tra sei ore!";
