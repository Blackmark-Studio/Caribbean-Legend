...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				link.l6 = "Cerco la zingara che cura gli ammalati. Sei tu?";
...
// --> продажа мышьяка
		...
		case "get_poison_2" :
			dialog.text = "Oh affascinante, un uomo così galante! (sussurrando) Pagami "+int(npchar.quest.poison_price)+" dobloni.";
...
// <-- продажа мышьяка
	...
	// --> Мангароса
		...
		case "dwh_ne_ta":
			...
			dialog.text = "No, " + GetSexPhrase("caro", "bella") + ", non sono io quella che cerchi, è " + sld.name + ". È qui in città in questo momento. L’ho vista di recente.";
			link.l1 = "Grazie.";
			link.l1.go = "exit";
		break;
		// <-- Тёмные воды исцеления
