    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Potresti aver notato, capitano, che la città è sotto assedio. Ci aspettiamo che gli spagnoli ci attacchino da un momento all'altro e stiamo mobilitando le nostre forze. Ma la regata continua\nIscriviamoci: capitano "+GetFullName(pchar)+", la nave è - "+pchar.Ship.Name+"... Il tempo della regata in ore è "+int(pchar.questTemp.Regata.FourthTransitionTime)+". Fatto, il tuo risultato è registrato, puoi continuare la tua strada.";
