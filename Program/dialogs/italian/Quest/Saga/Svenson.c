...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(CheckAttribute(pchar, "questTemp.Saga"))
			{
				...
				if(pchar.questTemp.Saga == "calendar") // квесты баронов завершены
				{
					...
					if (CheckAttribute(pchar, "questTemp.Saga.Late"))
					{
						dialog.text = "Ho sentito delle tue gesta eroiche! Dannie mi ha raccontato brevemente la storia. A proposito, ora siamo amici. E Nathan è cambiato del tutto! Tutta la sua malinconia è svanita quando ha rivisto il suo 'Centurion'.\nMa ho brutte notizie per te, "+pchar.name+"Abbiam perso talmente tanto tempo che i diritti sull’eredità di Sharp sono ormai svaniti e Isla Tesoro s’è fatta base militare inglese. Non posso più far nulla per aiutarti contro Levasseur.";
						link.l1 ="Non ho davvero parole... Abbiamo passato mille peripezie per un pugno di mosche. Davvero non c’è più nulla da fare?";
						link.l1.go = "saga_l2";
					}
					else
					{
		...
		case "saga_55":
			...
			link.l1 = "Ho trovato "+FindRussianQtyString(int(npchar.quest.bakaut))+" sulla 'Ostrica'.";
			link.l1.go = "saga_56";
		break;
		...
		case "saga_60":
			...
			link.l1 = "In tutto, ho tirato su duemila dobloni per Loxley e "+int(npchar.quest.bakaut_sum)*2+" dobloni in aggiunta a ciò.";
			link.l1.go = "saga_61";
		break;
		...
		case "saga_61":
			...
			if (PCharDublonsTotal() > 0)
			{
				...
				link.l1 = "Ecco qua. Ho preso "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
		...
		case "saga_61_1":
			...
			{
				npchar.quest.bakaut_pay = func_min(int(npchar.quest.bakaut_sum), PCharDublonsTotal()); // дублоны в кармане
				link.l1 = "Ecco qua. Ho "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
				link.l1.go = "bakaut_pay";
			}
			else
			{
		...
		case "bakaut_pay": // оплата
			...
			{
				dialog.text = "Perfetto. Porterai anche il resto "+FindRussianQtyString(int(npchar.quest.bakaut_sum))+" dobloni dopo?";
