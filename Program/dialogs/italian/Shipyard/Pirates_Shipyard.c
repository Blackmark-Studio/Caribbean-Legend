...
//*************************** Открывание двери ***************************
	...
	// <-- хронометр Алекса
		...
		case "FDM_cabin":
			dialog.text = "Perché no? Le mie mani sono ancora abili.";
			link.l1 = "...";
		...
		case "FDM_cabin_s":
			dialog.text = "Sette casse di dobloni e sarà fatto in cinque giorni.";
			link.l1 = "Affare fatto!";
		...
		case "FDM_cabin_pay":
			...
			if (amount <= 0) // всё принес
			{
				...
				dialog.text = "Allora, dovevi portarmi "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" casse, hai portato "+FindRussianQtyString(iTemp)+". Ho il pagamento. Ora è il mio turno. Vai e fai un giro. Non preoccuparti, farò del mio meglio.";
				link.l1 = "Bene...";
			...
			else
			{
				...
				dialog.text = "Allora, hai dovuto portarmi "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" casse, hai portato "+FindRussianQtyString(iTemp)+". "+FindRussianQtyString(amount)+" casse rimaste.";
		...
		case "FDM_hull_01":
			dialog.text = "Be’, facciamo due conti... Per una ricostruzione completa dello scafo mi serviranno 150 unità per ciascuno di legno ferro, cordame, seta navale e resina. E anche 10.000 dobloni d’oro. E non guardatemi così — la maggior parte del denaro andrà nei materiali. Questo lavoro equivale a smontare la nave pezzo per pezzo e ricostruirla da zero.";
			link.l1 = "No, direi di no. Non sono pronto a portare così tante risorse e a consegnare una montagna d'oro per la ricostruzione. Rimarrò con quello che ho.";
		...
		case "FDM_hull_waitmaterials":
			...
			dialog.text = "Non hai paura di girare per un insediamento pirata con una somma così cospicua, hehe? Va bene, sto scherzando, sono tutti persone oneste, a modo loro. Ora sto aspettando i materiali. Come promemoria, dovrai portare un totale di 150 legni di ferro, 150 bobine di corde, 150 rotoli di seta per navi e 150 barili di resina.";
		...
		case "FDM_hull_givemoney":
			...
			if(PCharDublonsTotal() >= 10000)
			{
    			link.l2 = "Sì, l'ho portato. 10.000 dobloni pieni, come concordato.";
