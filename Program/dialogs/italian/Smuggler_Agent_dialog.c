	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "Nessun affare oggi. Ci sono pattuglie in ogni baia, il nostro governatore sta cercando un carico che è stato confiscato da qualche capitano di"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" e lui lo ha nascosto da qualche parte.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "Perché no? Forse troverò un uso per il tuo 'carico'. Prendi "+int(pchar.questTemp.different.GiveShipLetters.price2)+" monete e dimenticatene.";
					link.l1 = "No, grazie.";
				...
				else
				{
					...
					{
						dialog.text = "Esattamente! Sono davvero curioso ora. Suppongo che se ti pago "+int(pchar.questTemp.different.GiveShipLetters.price3)+" monete e non ti importerà affatto di quei documenti.";
						link.l1 = "No, grazie.";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "Ecco, questa è un'occasione per prendere quel maledetto insolente! Sono davvero, davvero curioso. Guarda, ti pagherò "+int(pchar.questTemp.different.GiveShipLetters.price4)+" in oro e dimenticherai completamente i documenti sul tavolo?";
						link.l1 = "Non lo farò.";
						link.l1.go = "SM_ShipLetters_2";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "Nessun affare oggi. Ci sono pattuglie in ogni baia, il nostro governatore sta cercando un carico che è stato confiscato da qualche capitano di"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" e lui l'ha nascosto da qualche parte.";
					link.l1 = "E dove si trova questo capitano"+pchar.GenQuest.CaptainComission.Name+"?";
					link.l1.go = "CapComission_1";
					break;
				}
		...
		case "Meeting_2":
			...
			Link.l1 = "Forse, saresti interessato a qualche merce speciale? O forse, hai qualcosa da offrirmi in cambio?";
			Link.l1.go = "Meeting_3";
		break;
...
//navy <--
		...
		case "Travel_pay":
			...
			{
				Dialog.Text = "Sembra che tu non abbia abbastanza soldi.";
				Link.l1 ="Che peccato, tornerò più tardi.";
			}
			Link.l1.go = "Exit";
		break;  
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "È così? Bene. Il signor Rosenkraft ha bisogno di una partita di legno di ferro, ma non meno di "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+". Hai abbastanza carico?";
