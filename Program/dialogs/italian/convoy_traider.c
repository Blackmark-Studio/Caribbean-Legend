...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "Ho bisogno di essere scortato a "+XI_ConvertString("Colony"+pchar.quest.destination+"Gen")+", che si trova su "+XI_ConvertString(GetIslandByCityName(pchar.quest.destination)+"Dat")+", in "+FindRussianDaysString(int(pchar.ConvoyQuest.iDay))+", e per questo ti pagherò "+FindRussianMoneyString(int(pchar.ConvoyQuest.convoymoney))+". Allora, la tua decisione?";
			link.l1 = "Sono dentro.";
