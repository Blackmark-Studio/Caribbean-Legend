   ...
   /*NPChar.FileDialog2 = "DIALOGS\" + LanguageGetLanguage() + "\Prison\" + NPChar.City + "_Prison.c";
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "GiveTaskGun_1":
			...
			dialog.Text = "必要だ "+pchar.questTemp.PrisonGun.Text+"、の額で "+int(pchar.questTemp.PrisonGun.Qty)+"  「ちょうどその数の単位だ。支払いは金のドゥブロンでやるぜ。」 "+int(pchar.questTemp.PrisonGun.Price)+" 一つにつき。それで合計は "+FindRussianDublonString(int(pchar.questTemp.PrisonGun.Sum))+" 全部で、ということだ。どうだ？ああ、それともう一つ――全部まとめて買うが、金がまだ届いていないから、受け取るのは一か月後になるぞ。";
