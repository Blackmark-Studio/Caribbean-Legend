	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "Hostess_NotChoice":
			...
			else
			{
				...
				// <-- legendary edition
				dialog.text = ""+GetSexPhrase("よし、素晴らしいぞ、俺の暴れ馬！","奴らはみんな自分の仕事にとても長けている、疑う余地はねえぜ。")+" とても可愛い娘を紹介できるよ、名前は "+GetFullName(sld)+"、そして今なら彼女は利用可能だ。あの楽しみには料金がかかるぜ "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+"。同意するか？";
        ...
        case "Hostess_NotChoice_agree":
			...
			{
				dialog.text = "ああ、実は問題はね\n "+sld.name+" 安い女じゃねえ、あいつの値段は "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+"。しかも、今はその船を買う余裕がないようだな。裕福になったらまた来てくれ"+GetSexPhrase("、親愛なる","")+"...";
				Link.l1 = "「これが俺の運か……」";
				Link.l1.go = "exit";
			}
		break;
		...
		// belamour ночной приключенец
		case "NA_Girl":
            dialog.text = "おやおや、ここに目利きがいるとはな、へへ……まあまあ、ちょうどいい娘がいるぜ。ただし、安くはないぞ： "+int(pchar.GenQuest.NightAdventure_money)+" ペソ。";
			link.l1 = "取引成立だ。";
