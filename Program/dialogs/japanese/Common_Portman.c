...
//			if(rand(1) == 0) PChar.GenQuest.ChurchQuest_1.CapWaitOnTavern = true;
		...
		case "ShipLetters_Portman1_1":
			...
			dialog.text = s1+"「なあ、いくらかの金を考えてみないか "+int(pchar.questTemp.different.GiveShipLetters.price1)+" ペソだが、それで報酬として十分か？";
			link.l1 = "もちろん違う！";
			link.l1.go = "exit";
		...
		case "Work_check"://фейс-контроль - общая проверка профпригодности ГГ и выбор задания
			...
			if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") >= 2)//проверка повтора
				{
					...
					{
						dialog.text = RandPhraseSimple("の旗の下で航海する者には仕事はない "+NationNameGenitive(int(pchar.nation))+"「俺の執務室から今すぐ出ていけ！」","俺は協力しねえ "+NationNameAblative(int(pchar.nation))+"。失せろ！");
...
///--> ------фрахт со свободным выбором пункта назначения, оплаты и вида груза из предложенного списка---------
		...
		case "Fraht_choise":
			...
			{
				dialog.text = "以下の選択肢があります:\n"+"積み荷 "+GetGoodsNameAlt(iFrahtGoods1)+" の額で "+FindRussianQtyString(iFrahtGoodsQty1)+" の町へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+"、で "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\n積荷 "+GetGoodsNameAlt(iFrahtGoods2)+" 「～の額で」 "+FindRussianQtyString(iFrahtGoodsQty2)+" の町へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+"、の "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"\n積荷 "+GetGoodsNameAlt(iFrahtGoods3)+" の額で "+FindRussianQtyString(iFrahtGoodsQty3)+" の町へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City3)+"、で "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty3)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money3))+"\nどうなさいますか？";
			...
			{
				dialog.text = "以下の選択肢があります:\n"+"積荷 "+GetGoodsNameAlt(iFrahtGoods1)+" 「～の額で」 "+FindRussianQtyString(iFrahtGoodsQty1)+" の町へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+"、で "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\n積荷 "+GetGoodsNameAlt(iFrahtGoods2)+" 「～の額で」 "+FindRussianQtyString(iFrahtGoodsQty2)+" の町へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+"、で "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"\nお前の選択はどうする？";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (amount > 0)
			{
				dialog.text = "さて、見せてもらおうか。お前は本来、荷物を届けるはずだった "+GetGoodsNameAlt(iFrahtGoods)+" 「～の額で」 "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+"。しかし見たところ、全ての積荷を届けてはいないようだな――お前は "+FindRussianQtyString(int(amount))+" 短い。";
		...
		case "Fraht_complete_bad_1":
			...
			dialog.text = "商品の三倍の値段だ。つまり "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+".";
		...
		case "Fraht_complete_fail":
			dialog.text = "ああ、そうか……それなら、二度とどこの港の役所にも顔を出すんじゃねえぞ "+NationNameGenitive(int(pchar.questTemp.WPU.Fraht.Nation))+"「消え失せろ！」";
...
/// -------------------------------работа курьером по доставке почты-------------------------------------
		...
		case "Postcureer":
			...
			dialog.text = "この荷物は、都市の港湾当局に届けなければならない "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City)+"、のために "+FindRussianDaysString(pchar.questTemp.WPU.Postcureer.DaysQty)+"、できるだけ早く頼む。報酬は― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
			link.l1 = "同意するぜ！";
		...
		case "Postcureer_complete":
			if (pchar.questTemp.WPU.Postcureer == "late")//опоздали
			{
				...
				dialog.text = "ふむ……だが、お前は "+FindRussianDaysString(iTime)+" 遅れましたね。したがって、今回の仕事の報酬を減額せざるを得ません。契約によれば、 遅延一日ごとに総額の五パーセントがあなたの違約金となります。では、どうぞ、あなたの\n "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+"、そして今後は契約を守るようにしてくれ。";
				link.l1 = "まあ、少なくとも何かはあるな。";
				link.l1.go = "Postcureer_complete_1";
				break;
			}
			dialog.text = "お勤めご苦労だった。さあ、報酬を受け取ってくれ－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
			link.l1 = "ありがとう！";
			link.l1.go = "Postcureer_complete_1";
		break;
...
/// ---------------------------работа в качестве эскорта для торговых кораблей----------------------------
		...
		case "escort":
			...
			dialog.text = "つまり、船の護衛をしてもらうことになる。"+pchar.questTemp.WPU.Escort.ShipName1+"「」と「」"+pchar.questTemp.WPU.Escort.ShipName2+"「」の街へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+"、のために "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"。両方の船を無事に目的地まで届けなければならん。現地の港役人に船を引き渡すんだ。 報酬もその役人から受け取ることになるぞ。";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				...
				if (amount > 0)
				{
					dialog.text = "よし、そういうことだな。お前は船団を護衛して、積荷を届けるはずだった "+GetGoodsNameAlt(iGoods)+" 「～の額で」 "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+"。しかし、まだ全ての積荷を届けていないようだな――お前は "+FindRussianQtyString(int(amount))+" 短い。";
					link.l1 = "「千匹のサメだと！きっとあの忌々しいネズミどもに違いねえ！だが心配すんな、」 "+GetAddress_FormToNPC(NPChar)+"、足りない分は俺が買い取って、全部まとめて納品するぜ。";
					link.l1.go = "Fraht_complete_3";
					break;
				}
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				dialog.text = "よし、それじゃあ。お前は船団を護衛して、積み荷を届ける役目だったんだ "+GetGoodsNameAlt(iGoods)+" の額で "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+"。ご苦労だった。さあ、報酬を受け取ってくれ― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			else
			{
				dialog.text = "お勤めご苦労だった。さあ、報酬を受け取ってくれ－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			link.l1 = "ありがとう！";
			link.l1.go = "Escort_complete_1";
		}
		...
		case "escort_bonus":
			...
			dialog.text = "つまり、船団の護衛を頼まれることになる "+pchar.questTemp.WPU.Escort.ShipName1+"「と」"+pchar.questTemp.WPU.Escort.ShipName2+"「」の街へ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+"、のため "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+"。支払い－ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"\nそれから、積荷もお前の船に載せておくぜ。 "+GetGoodsNameAlt(iGoods)+" の額で "+FindRussianQtyString(iGoodsQty)+"。配達の報酬だ―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+".";
		...
		case "EscortBonus_complete_fail":
			...
			if (pchar.questTemp.WPU.Escort == "begin")
			{
				...
				if (amount > 0 && amount <= 100)
				{
					...
					dialog.text = "信じられん！一体お前に何を頼んだ意味があったんだ？どうしてこんな事態を許したんだ！？ これで護衛の報酬なんて論外だと分かっているだろうな……それだけじゃない、積荷まで足りてないじゃないか― "+FindRussianQtyString(int(amount))+"\n基本的に、俺が支払うのは、無事に届けられた貨物分の運賃だけだ。足りない分の価値は差し引くからな。自分の "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+" 好きなところへ行っていいぞ。";
					link.l1 = "まあ、少なくとも何かはあるな。";
				...
				if (amount > 100)
				{
					dialog.text = "信じられん！それでお前が必要だった意味は何だ？どうしてこんな事態を許したんだ！？ これで護衛の報酬なんて論外だと分かっているだろうな……それだけじゃない、積荷も大幅に不足しているじゃないか。\n足りない品を揃えるのを待つ時間なんてない。だから全額補償してもらうぞ― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+"。貨物については、お前が持っていていいぜ。";
					if (int(pchar.money) >= int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))
					{
				...
				dialog.text = "信じられん！それでお前に何の用があったんだ？どうしてこんな事態を許したんだ！？ これで護衛の報酬は一切なしだということ、分かっているだろうな。\nとにかく、お前が運んだ貨物の運賃だけは払う。さあ、 "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+" 好きなところへ行っていいぞ。";
				link.l1 = "まあ、少なくとも何かはあるな。";
				link.l1.go = "EscortBonus_complete_fail_1";
				break;
			}
			if (pchar.questTemp.WPU.Escort == "late")
			{
				...
				if (amount > 0)
				{
					dialog.text = "信じられん！それでお前が必要だったのか？どうしてこんな事を許したんだ！？ 護衛の報酬なんて論外だと分かってるだろうな……船を失い、お前は到着した\n "+FindRussianDaysString(iTime)+" 遅れてきた上に、しかも貨物も足りてねえじゃねえか── "+FindRussianQtyString(int(amount))+"\nお前が足りない品を手に入れるのを待つ暇はない。だから全額を補償してもらうぞ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+"。貨物については、お前が持っていていいぜ。";
					if (int(pchar.money) >= int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))
					{
						link.l1 = "わかった、そんなに熱くなるなよ。ほら、金だ。";
						link.l1.go = "EscortBonus_complete_fail_2";
				...
				dialog.text = "信じられん！で、何のためにお前を雇ったんだ？どうしてこんな事態を許したんだ！？ これで護衛の報酬なんて論外だということは分かっているだろうな……それに、お前は到着した\n "+FindRussianDaysString(iTime)+" 遅れたな。\nだから貨物運賃の支払いを減額せざるを得なかった。契約によれば、遅延一日ごとに元の金額の10パーセントが罰金となる。さあ、受け取れ "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+"、それでは行ってよい。";
				link.l1 = "まあ、少なくとも何かはあるな。";
				link.l1.go = "EscortBonus_complete_fail_1";
				break;
			}
		...
		case "EscortBonus_complete_fail_3"://потеряли корабль и груз, время норма или нет, война
			dialog.text = "ああ、そうか……なら、その場合は二度とどこの港の役所にも顔を出すんじゃねえぞ "+NationNameGenitive(int(pchar.questTemp.WPU.Escort.Nation))+"「うせろ！」";
			link.l1 = "「大して見逃してねえだろう……」";
			link.l1.go = "exit";
		...
		case "Escort_fail_complete":
			...
			if (amount > 0)
			{
				dialog.text = "さて、見せてもらおうか。お前は本来、積荷を届けるはずだったよな "+GetGoodsNameAlt(iFrahtGoods)+" 「の額で」 "+FindRussianQtyString(pchar.questTemp.WPU.Escort.GoodsQty)+"。しかし、まだ全ての荷を届けていないようだな――お前は "+FindRussianQtyString(int(amount))+" 短い。";
		...
		case "Escort_fail_complete_1":
			...
			if (pchar.questTemp.WPU.Escort == "late")//опоздали
			{
				...
				pchar.questTemp.WPU.Escort.PercentLate = iTime/0.1;
				pchar.questTemp.WPU.Escort.Money = int(int(pchar.questTemp.WPU.Escort.Money) - int(pchar.questTemp.WPU.Escort.Money)*int(pchar.questTemp.WPU.Escort.PercentLate)/100);//снижаем оплату согласно числа дней опоздания
				dialog.text = "ふむ……任された船を失っただけでなく、さらに遅れてきたのだな "+FindRussianDaysString(iTime)+"。だから運賃の支払いを減額せざるを得なかった。船を救えなかったので、運賃の支払いは半分にした。 これが私が出せる精一杯だ。\nさらに、契約によれば遅延一日ごとに元の金額の一割が罰金となる。さあ、受け取りなさい "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"、それじゃあ行ってこい。";
				link.l1 = "まあ、少なくとも何かはあるな。";
				link.l1.go = "Escort_fail_complete_2";
				break;
			}
			dialog.text = "まあ、君に託した船を守れなかったことには非常に腹立たしいが、少なくとも貨物は届けてくれたな。 船を救えなかったので、運賃の支払いは半額にした。これが君にできる最大限の支払いだ。さあ、報酬を受け取れ—— "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+" そして、好きなところへ行っていいぞ。";
			link.l1 = "まあ、少なくとも何かはあるな。";
			link.l1.go = "Escort_fail_complete_2";
		break;
...
//-------------------------------------задания 2 уровня для курьера---------------------------------------------
		case "Postcureer_LevelUp":
			...
			{
				case 0:
					dialog.text = "至急の公用書簡をその町の総督に届けてもらう必要がある "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" 遅くとも "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+"。十分に気をつけてくれ――敵もその書状を狙っているかもしれん。配達の報酬は―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "同意するぜ！";
				...
				case 1:
					dialog.text = "これらの書類を店主に届けてもらう必要がある "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" 遅くとも "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+"。これは諸島中の店の最新価格一覧だ――貴重な商業書類だぜ。気をつけろよ、ライバルが狙ってくるかもしれねえ。配達の報酬は―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "同意するぜ！";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "いや、今回は遠慮しておくぜ。";
					link.l2.go = "Postcureer_LevelDown";
...
//					pchar.questTemp.WPU.Current.Add = "сводку цен для хозяина магазина";
				...
				case 2:
					dialog.text = "この書類の束を町の高利貸しに届けてもらう必要がある "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" 遅くとも "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+"。これは通貨の為替レートや株価に関する非常に重要な情報だ。十分に注意しろ――この荷物を狙っている怪しい連中がすでに動き出している。配達の報酬は―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "同意するぜ！";
					link.l1.go = "Postcureer_LevelUp_Go";
				...
				case 3:
					dialog.text = "この包みをその町のドックマスターに届けるべきだ "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" 遅くとも "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+"。新しいものの設計図が含まれている "+RandPhraseSimple(LinkRandPhrase("ブリッグ","コルベット","フリゲート"),LinkRandPhrase("フルート","ピナス","ガレオン"))+"、独自の技術設計を用いているんだ。他にも…関心を持つ者たちがいることは理解してほしい、だから慎重に行動してくれ。納品の報酬は― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "同意するぜ！";
					link.l1.go = "Postcureer_LevelUp_Go";
...
//----------------------------------------задания 2 уровня фрахт------------------------------------------
	...
	case "Fraht_LevelUp":
		...
		switch (rand(4))//для тестов
		{
			case 0:
				...
				dialog.text = "我が植民地で最近、ある商品への需要が急激に高まっている。\n地元の店ではもう在庫がないのだ。\nそこで、君にその品を用意してもらいたい。 "+GetGoodsNameAlt(iGoods)+" 「～の額で」 "+FindRussianQtyString(iGoodsQty)+"、そして倍額を支払うつもりだ― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" 1つあたり。合計で "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+"。ああ、それと納期は一ヶ月以内に頼むぞ――これ以上は品物を待てないからな。";
			...
			case 1:
				...
				pchar.questTemp.WPU.Current.Add = "double";
				// 08Mar17 "month" was "moths"
				dialog.text = "最近、我が植民地である商品への需要が急激に高まっている。\n地元の店ではもう在庫がない。\nそこで、君にその品を調達してもらいたいのだ。 "+GetGoodsNameAlt(iGoods)+" の額で "+FindRussianQtyString(iGoodsQty)+"、そして倍額払うつもりだ―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" 1つあたり。合計で "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+"。ああ、それと納期は一か月以内に頼むぞ――これ以上品物を待てないんだ。";
			...
			case 2:
				...
				// 08Mar17 "month" was "moths"
				dialog.text = "我々の植民地の兵器庫は深刻に不足している "+GetGoodsNameAlt(iGoods)+"。地元の店にはもう在庫がありません。それゆえ、あなたに用意してもらいたいのです\n "+GetGoodsNameAlt(iGoods)+" 「…の額で」 "+FindRussianQtyString(iGoodsQty)+"、それに俺は三倍払うつもりだ―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" 1個あたり。合計で "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+"。ああ、それと納期は一か月以内に頼むぞ――これ以上品物を待てないんだ。";
				link.l1 = "よし、この任務を引き受けるぜ。";
				link.l1.go = "Fraht_LevelUp_Go";
			...
			case 3:
				...
				pchar.questTemp.WPU.Current.Add = "triple";
				// 08Mar17 "month" was "moths"
				dialog.text = "最近わが植民地で熱病が流行したため、医療用品が不足しており、倉庫にも何も残っていません。 総督は町民の安全を案じております。そこで、あなたに用意していただきたいのです\n "+GetGoodsNameAlt(iGoods)+" の額で "+FindRussianQtyString(iGoodsQty)+"、そして俺は三倍払うつもりだ― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" 1パックあたり。これで合計 "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+"。ああ、それと納期は一ヶ月以内に頼む――これ以上品物を待つことはできんからな。";
				link.l1 = "よし、この任務を引き受けるぜ。";
				link.l1.go = "Fraht_LevelUp_Go";
				link.l2 = "ご期待に添えず申し訳ありませんが、今はそれができません。";
				link.l2.go = "Fraht_LevelDown";
			...
			case 4:
				...
				pchar.questTemp.WPU.Current.Add = "double";
				// 08Mar17 "month" was "moths"
				dialog.text = "不作のせいで、我々の植民地は常に輸入食料を必要としている。今のところ、備蓄はほとんど空で、 店の棚からもずいぶん前に全てが消えてしまった。そこで、あなたに届けてほしいのだ\n "+GetGoodsNameAlt(iGoods)+" 「～の額で」 "+FindRussianQtyString(iGoodsQty)+"、そして倍額払うつもりだ―― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" 1パックあたり。これで合計は "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+"。ああ、それと納期は一ヶ月以内に頼むぞ――もうこれ以上、品物を待てんからな。";
				link.l1 = "よし、この任務を引き受けるぜ。";
				link.l1.go = "Fraht_LevelUp_Go";
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		{
			dialog.text = "よし、お前は本来なら積荷を届けるはずだった "+GetGoodsNameAlt(iGoods)+" の額で "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+"。素晴らしい、ありがとうございます。こちらがあなたへの報酬です― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
...
//----------------------------------------задания 2 уровня эскорт ------------------------------------------
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		dialog.text = "素晴らしい！今回も君が頼りになることを証明してくれたな。よくやってくれてありがとう。さあ、 報酬を受け取ってくれ― "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"。 それと、時々は私のところに顔を出すのを忘れないでくれ――あんたみたいな船長には、いつだって仕事があるからな。";
...
//-------------------------------- квест доставки судового журнала до рассеяного кэпа ---------------------
		...
		case "SeekShip_good_1":
			dialog.text = "報酬を支払う準備ができている。内容は\n " + int(npchar.quest.money) + " ドゥブロン金貨で支払う。残念だが、それ以上は払えない。";
      ...
      //       chref = GetCharacter(int(NPChar.ShipToStoreIdx));
			...
			// }
			// dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName)+" 「」"+chref.Ship.Name+"「」、クラス "+RealShips[int(chref.Ship.Type)].Class+"、係留費用は "+FindRussianMoneyString(int(NPChar.MoneyForShip))+" 月ごと、支払いは一ヶ月前払い。";
			// Link.l1 = "「ああ、それで構わねえぜ。」";
      ...
      //           Link.l1.go = "ShipStock_NoMoney";
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName) + " 「」" + chref.Ship.Name + "「」、クラス " + RealShips[int(chref.Ship.Type)].Class +
                     "、停泊費 " + FindRussianMoneyString(int(NPChar.MoneyForShip)) + " 月ごとに、前払いです。";
			dialog.Text = dialog.Text + " 士官と乗組員に任せれば、彼らと一緒に " + FindRussianMoneyString(int(NPChar.MoneyForShip) + int(NPChar.MoneyForCrew));
		...
		case "ShipStockManBack":
			...
			{
			    dialog.Text = "船を受け取りたいのか？係留料がまだ残っているぞ "+FindRussianMoneyString(int(NPChar.MoneyForShip))+".";
			}
			else
			{
				dialog.Text = "「受け取りか？」";
