	...
	//<-- Бремя гасконца
		...
		case "ship_tunning_start" :		
			...
			{				
				...
				if(!TuningAvailable())
				{
					dialog.Text = "まあまあ…さて、ここに何があるか見てみよう…よし― "+XI_ConvertString(RealShips[int(Pchar.Ship.Type)].BaseName)+"。あなたの船はすでに改良された特性の最大数に達しています。これ以上手を加えても意味がなく、 かえって害になる恐れがあります。";
					Link.l1 = "なるほど……";
					Link.l1.go = "ship_tunning_not_now_1";
					break;
				}
		...
		case "ship_tunning_Capacity_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_Capacity_again";
				dialog.Text = "俺にはまだ必要なものがある：アイアンウッド－ "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "いいだろう。";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_SpeedRate_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_SpeedRate_again";
				dialog.Text = "俺にはまだ必要なものがある：船用シルク－ "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "いいだろう。";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_TurnRate_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_TurnRate_again";
				dialog.Text = "俺にはまだ必要なものがある：ロープ― "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "「いいだろう。」";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_MaxCrew_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_MaxCrew_again";
				dialog.Text = "俺がまだ必要なのは：鉄木だ－ "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "いいだろう。";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: ironwood - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "ship_tunning_MinCrew_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_MinCrew_again";
				dialog.Text = "まだ必要なのは……ロープだ―― "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "いいだろう。";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: ropes - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "ship_tunning_HP_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_HP_again";
				dialog.Text = "俺にはまだ必要なものがある：樹脂－ "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "いいだろう。";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_rig_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_rig_again";
                dialog.Text = "俺はまだ必要なんだ：船用の絹－ "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "「いいだろう。」";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: ship silk - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "Findship_complete":
			...
			dialog.text = "ほら、金だ― "+FindRussianMoneyString(int(pchar.GenQuest.Findship.Shipyarder.Money))+"。仕事を引き受けてくれてありがとう。また必ず会いに来てくれよ。じゃあな！";
		...
		case "Device_complete_1":
			...
			dialog.text = "お前の働きに対して、報酬を払うぜ "+FindRussianMoneyString(int(pchar.GenQuest.Device.Shipyarder.Money))+"。それがふさわしい報酬になるといいんだが。";
			link.l1 = "ありがとう！";
		...
		case "SailsTypeChoose":
			dialog.text = "どの帆を取り付けたいんだ？今はお前が持っているのは "+GetSailsType(int(RealShips[int(Pchar.Ship.Type)].ship.upgrades.sails))+".";
		...
		case "SailsTypeChoose2":
			...
			dialog.text = "現在、あなたは持っている "+GetSailsType(int(RealShips[int(Pchar.Ship.Type)].ship.upgrades.sails))+"、あなたが選択した "+GetSailsType(int(NPChar.SailsTypeChooseIDX))+"。交換の費用は "+FindRussianMoneyString(int(NPChar.SailsTypeMoney))+"。進めるか？";
		...
		case "SailsColorDone":
			...
			dialog.text = "承知した。帆を塗り替えるぞ "+GetStrSmallRegister(XI_ConvertString(SailsColors[int(NPChar.SailsColorIdx)].name))+" 色。";
		    Link.l9 = "ありがとう。";
			Link.l9.go = "exit";
...
//<-- -----------------------------------------Бремя гасконца-----------------------------------------------------
		...
		case "IslaMona_7":
            dialog.text = "つまり、俺に借りができたな "+int(pchar.questTemp.IslaMona.Shipyarder.Money)+" ドゥブロン金貨。";
		...
		case "IslaMona_money":
            ...
            dialog.text = "俺に同情なんかするなよ、お前の船室のロッカーは金でいっぱいだろ。借りがあるんだぜ "+FindRussianDublonString(int(pchar.questTemp.IslaMona.Shipyarder.Money))+".";
			link.l1 = "";
			link.l1.go = "IslaMona_money_exit";
		break;
