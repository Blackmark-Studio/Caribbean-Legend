	...
	switch(Dialog.CurrentNode)
	{
	  ...
	  // to_do
		...
		case "SitySpy_1":
			...
			dialog.text = "この町の誰か！ "+NPCharSexPhrase(npchar,GetFullName(&characters[int(pchar.GenQuest.SeekSpy.Idx)])+" 例えば、今ならできる "+NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.Idx)],"彼","彼女")+" ちょうどそこに立っている。","「ああ、旦那、そうだな、例えば、」 "+NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.Idx)],GetAddress_FormToNPC(NPChar)+" "+characters[int(pchar.GenQuest.SeekSpy.Idx)].lastname,GetFullName(&characters[int(pchar.GenQuest.SeekSpy.Idx)]))+" 俺にできるぜ。");
		...
		case "SeekSpy_Checking":
			dialog.text = "「ああ」 "+NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.BaseIdx)]," 確かに地元の者だ。","彼女は確かに地元の人だ。");
		...
		case "CitizenFindItem_4":
			dialog.text = "ああ、ぜひ頼む。急いでくれ――覚えておけ、残された時間は限られているんだぞ "+int(NPChar.LifeDay)+" 日間。";
			link.l1 = "覚えている。";
			link.l1.go = "exit";
