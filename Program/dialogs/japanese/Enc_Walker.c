...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("「旦那が見知らぬ人と話すのを許してくれません！」","話すことは何もない。");
				Link.l1 = "大したことじゃねえ。";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("教えてくれ、地元の酒場で流行っている噂は何だ？","ここで何が起きているのか、俺に話してくれないか？","おい、相棒、陸の暮らしってどんなもんか教えてくれねえか？");
		...
		case "sovet":
			...
			{
                dialog.text = RandPhraseSimple("「夫が見知らぬ人と話すのを許してくれません！」","話すことは何もない。");
				Link.l1 = "大したことじゃねえ。";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("教えてくれ、地元の酒場で流行っている噂は何だ？","ここで何が起きているのか、俺に話してくれねえか？","おい、相棒、陸の暮らしってどんなもんか教えてくれよ？");
			link.l10.go = "rumours_citizen"; //повесил на горожанина, если нужны отедельные слухи, то пишем соответствующую реализацию
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("ああ、嬉しいよ"+NPCharSexPhrase(NPChar,"","")+" お前に会うのはまるで一杯の酒みたいだぜ。何でも聞いてくれ。","話すのが好きかい、船長？俺もさ……特にラム酒を一杯やりながらな。","「ああ」 "+PChar.name+"?"),LinkRandPhrase("何の用だ？","なるほど、船長 "+PChar.name+"、お前はずいぶんとおしゃべりじゃねえか？","面倒な質問だな、船長。ラムでも奢ってくれたほうがいいぜ。")),PCharRepPhrase(LinkRandPhrase("何を知りたかったんだ "+GetAddress_Form(NPChar)+"?","お話を伺います、船長。","俺は行かなくちゃならねえんだ、だから早くしてくれよ、船長。"),LinkRandPhrase("「気のいい連中と一緒にいられて、いつだって嬉しいぜ」 "+GetAddress_Form(NPChar)+" "+PChar.lastname+"。言いたいことを言え。","「ああ、」 "+GetAddress_Form(NPChar)+"?","おしゃべりが好きかい、船長？俺も同じさ……")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("地元の酒場で流行っている噂話にはどんなものがあるんだ？","この地で何が起きているんだ？","「この辺りの陸では何か新しいことがあったか？」");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("いや、空耳だぜ。もう行くぜ。","いや、何もない。ただ散歩に出ただけだ。"),RandPhraseSimple("いや、何もない。幸運を祈るぜ！","ちょっと散歩してるだけだ。じゃあな。"));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"聞いてくれ、この町の市民として、抜き身の刃を持って歩き回るのは控えてもらわねばならん。");
				link.l1 = LinkRandPhrase("「いいだろう。」","結構だ。","おっしゃる通りだ……");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"「落ち着け、」 "+GetSexPhrase("相棒","娘")+"、武器を手に持って走り回っているときはね。それがあると、俺はちょっと落ち着かなくなるんだ……","俺はこういうのが気に入らねえんだ "+GetSexPhrase("男たち","人々")+" 武器を構えたまま歩き回るなんて……怖いよ……");
				link.l1 = RandPhraseSimple("分かった。","俺がそれを取り除くぜ。");
			}
			link.l1.go = "exit";
