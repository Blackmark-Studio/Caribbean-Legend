void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Hold_GenQuest61":
			dialog.text = "俺の言いたいことを勘違いしてるぜ。俺は市議会の一員だ。俺たちは憲章の遵守を監督しているんだ。\n"+"衛兵！！衛兵！こいつを奴隷売買と船の略奪の容疑で逮捕しろ "+NationNameGenitive(int(pchar.GenQuest.Hold_GenQuest.Nation))+"!";
		...
		case "ShipWreck_2":
			...
			if(rand(1) == 0)
			{
				dialog.text = RandPhraseSimple("こんにちは、私は "+pchar.GenQuest.ShipWreck.Name+"の船長 "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Acc"))+" 「」"+pchar.GenQuest.ShipWreck.ShipTypeName+"、俺の船は岸からそう遠くない岩礁で座礁しちまったんだ。"+"乗組員全体のうち、ほんの数人の水夫だけが生き残ることができたんだ。潮に流されて、 この荒れ果てた岸に打ち上げられた。俺たちは飢えと渇きに苦しみ続けていたんだよ\n "+(5+hrand(7))+" もう何週間も前からだ","こんにちは、私は "+pchar.GenQuest.ShipWreck.Name+"の船長 "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Gen"))+" 「」"+pchar.GenQuest.ShipWreck.ShipTypeName+"。俺たちの船はここからそう遠くない場所で難破したんだ。"+"俺たちは難破のあとなんとかこの岸にたどり着いたが、ここはどうやら無人島らしい。 仕方なく貝やヤシの実で生き延びてきたんだが\n "+(5+hrand(7))+" もう何週間にもなるんだ。");
			...
			{						
				...
				dialog.text = "ああ、船長、我々の被害は本当にひどい……初めまして、私は "+pchar.GenQuest.ShipWreck.Name+"、船長兼船主で "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Gen"))+" 「」"+pchar.GenQuest.ShipWreck.ShipTypeName+"。いや、元船長兼元船主と言うべきか。 "+"この野郎 "+pchar.GenQuest.ShipWreck.BadName+" 俺が雇ったのは "+XI_ConvertString("Colony"+pchar.GenQuest.ShipWreck.City+"Voc")+" 俺の乗組員を反乱に駆り立てちまったんだ。結局、俺たちはこんな神にも見放された場所に置き去りにされたってわけさ。 そうなったのは "+(5+hrand(7))+" 数週間前だ。";
		...
		case "ShipWreck_3":
			dialog.text = RandPhraseSimple("乗組員全員の中で、ただ "+ShipWreck_GetStringQty(int(pchar.GenQuest.ShipWreck.Qty))+" 生き残った者もいる。他の連中は死んじまった；何人かはここで傷や熱病で既に死んだんだ\n",
				"Only " + ShipWreck_GetStringQty(int(pchar.GenQuest.ShipWreck.Qty)) + " 運が良かった方さ。ほかの連中は……深海か、この海岸の砂丘で最期を迎えたんだ。");
		...
		case "ShipWreck_7":
			if(rand(1) == 0)
			{
				if(rand(100) < 90)
				{
					...
					dialog.text = "どこの居留地へ "+NationNameGenitive(int(pchar.GenQuest.ShipWreck.Nation))+".";
		...
		case "Convict_13_1":
			...
			link.l1 = "状況を考えると、それ以下の料金では乗船を許可できねえ "+FindRussianMoneyString(int(pchar.GenQuest.Convict.Sum))+" 一人あたりペソだ。";
		...
		case "Convict_13_2":
			dialog.text = "「俺たちに選択肢があるのか？まあ、仕方ねえな、」 "+FindRussianMoneyString(int(pchar.GenQuest.Convict.Sum))+"...";
		...
		case "Convict_15":
			...
			{
				link.l1 = "ああ、これがお前の船だ。支払いのことは忘れるなよ。俺が払ったんだからな "+pchar.GenQuest.Convict.ShipPrice+" そのための金貨を出したんだ、だからお前は俺に借りがあるぜ "+int(pchar.GenQuest.Convict.ShipPrice)* 2+"、約束通りだ。";
		...
		case "CaptainComission_22":		
			...
			{			
				...
				dialog.text = "どうか、巡回隊の船長に伝えてくれ "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Gen"))+" 「」"+pchar.GenQuest.CaptainComission.ShipTypeName+"「」 "+pchar.GenQuest.CaptainComission.Name+"、二度と顔を出すんじゃねえぞ "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City+"Voc")+"... "+"当局は彼の死の本当の事情を把握している "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+" 「」"+pchar.GenQuest.CaptainComission.VictimShipName+"…時間がない、あいつは逮捕されちまう……航海に出る "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City+"Acc")+" から "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City1+"Gen")+"…航海にはほぼ十五日もかかる……ああ……急がないとな……"; // belamour gen
		...
		case "CaptainComission_32_1":
			GetSlaveSpeciality();
			dialog.text = "わかってる、わかってる……だが、それもお前のせいだぜ。 どうしてあんなに苦労して財務官からかっぱらった金貨の入った三つの箱を沈めちまうんだよ、まったく "+NationNameNominative(int(pchar.GenQuest.CaptainComission.Nation))+" そんな専門家がいるかもしれねえ "+pchar.GenQuest.CaptainComission.SlaveSpeciality+"?";
			link.l1 = "ちゃんとした見返りがなけりゃ、あいつを渡すつもりはねえからな。";
			link.l1.go = "CaptainComission_32_2";
		break;
		
		case "CaptainComission_32_2":
			dialog.text = "なるほど、お前は頭の切れる男だな。もう交渉の余地はねえってことくらい分かってるだろう。お前の船も積み荷も、 俺たちは奪うことができたんだぜ。\nだが、補償は事前に用意されていた。だからこそ、政府は "+NationNameGenitive(int(pchar.GenQuest.CaptainComission.Nation))+" 成功した作戦を祝福し、この貴重な贈り物とささやかな報酬を贈呈する。";
		...
		case "CaptainComission_202_1":
			dialog.text = "そうか、俺の疑いは当たってたんだな。お前は無理やりやらされたんだな "+NationNamePeople(int(pchar.GenQuest.CaptainComission.Nation))+"?";
		...
		case "CaptainComission_101":
			GetSlaveSpeciality();
			dialog.text = "実は、俺は科学者で、専門は "+pchar.GenQuest.CaptainComission.SlaveSpeciality+" そして "+NationNamePeople(int(pchar.GenQuest.CaptainComission.EnemyNation))+" 「しばらくの間、俺に協力を申し出ていたんだが、」"+"だが、俺は軍の技師であり、忠誠の誓いは守っている。連中は俺を奪おうとしたが、本当の問題は、 海賊どもが俺の正体と重要性を知ったときに始まったんだ\n"+"結局、奴らは俺を捕まえることに成功したが、欲しかったのは金だけで、俺の知識なんて必要なかったんだ。\n友も敵も身代金を払うのが間に合わず、俺はプランテーションに売られた。\nあとは知ってるだろう。";
		...
		case "CaptainComission_301":
			...
			link.l1 = "やあ、俺が船長だ "+GetFullName(pchar)+"「ある船長の代理として」 "+pchar.GenQuest.CaptainComission.CapName+"。あんたは で逮捕されるって伝えてくれと頼まれた "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City+"Voc")+"。役人たちは知っている "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+"."; // ПРАВКА + belamour gen
...
//			bQuestDisableMapEnter = true;
		...
		case "CaptainComission_320":
			...
			dialog.text = "増援が来る前に急がなきゃならねえ。積み込みはほとんど終わったし、お前の取り分は "+pchar.GenQuest.CaptainComission.GoodsQty+" の単位 "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+".";
		...
		case "CaptainComission_Canoneer4":
			...
			link.l1 = "おいおい、そんなに興奮すんなよ。\nあいつはな……遅すぎたんだ。船長 "+pchar.GenQuest.CaptainComission.CapName+" 「そう伝えてくれ」と頼まれた "+pchar.GenQuest.CaptainComission.Name+" あの居留地には顔を出さない方がいいぜ、当局はその破壊の経緯を知っているからな "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Acc"))+" 「」"+pchar.GenQuest.CaptainComission.VictimShipName+"、そして彼は逮捕される。";
		...
		case "CaptainComission_Canoneer7":
			dialog.text = "俺は乗り込みの時にケガしたって言って、積み荷のことなんか何も知らねえって言い張ったんだ。それに、 ちょっと頭が弱いふりもしたんだぜ。ほら、ついさっきまで寝込んでて、 あんたが来る直前にやっと立ち上がれたばかりってな。\n "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+"。うまく動けなかった、体の調子が全然ダメだったんだ。";
		...
		case "CaptainComission_Canoneer9":
			dialog.text = "知らねえな。普通の巡回だったし、普通の船がその旗の下で航海してたんだ。 "+NationNameGenitive(int(pchar.GenQuest.CaptainComission.Nation))+"。なぜ船長がそれを調べることにしたのか全く分からねえが、警告の一発を無視しやがって、 俺たちが追いついたときには“ジョリー・ロジャー”を掲げやがったんだ。 "+"砦の支援がなかったから、戦いは楽じゃなかったぜ。そんで、白兵戦になったんだ。どうやって奴らを倒せたのか、 今でもわからねえよ。\n"+"そうさ、あの貴重な積み荷は全部見たぜ。悪魔が俺たちに授けたに違いねえ。積み荷は陸に隠して、 奪った船は派手に吹き飛ばすしかなかった。あいつはもうボロボロだったからな。";
		...
		case "CaptainComission_Canoneer11":
			...
			dialog.text = "よし、わかった。今夜、真夜中過ぎにあたりで俺に会え。 "+XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore+"Gen")+"。あと、ちゃんと船倉に空きがあるか確認してくれよ "+pchar.GenQuest.CaptainComission.GoodsQty+" 個 "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+"……どれも捨てるのはもったいないな……"; // belamour gen
		...
		case "CaptainComission_372":
			...
			link.l1 = "Captain "+pchar.GenQuest.CaptainComission.CapName+" 「の破壊を取り巻く状況について警告するように頼まれた」 "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Acc"))+" 「」"+pchar.GenQuest.CaptainComission.VictimShipName+" 当局に知られるようになった、 "+"だが、航海中にお前の船は見かけなかったぜ、+\n "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City1+"Gen")+".";
		...
		case "CaptainComission_374":
			dialog.text = "なるほど……そういうことか。いろいろ納得がいったぜ。つまり、誰かが俺たちが略奪したってしゃべっちまったんだな\n "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+". "+"遅かれ早かれ、奴らは積み荷を見つけて俺を絞首刑にするだろう。船長 "+pchar.GenQuest.CaptainComission.CapName+" 死んじまったってことは、もう誰にも助けてもらえねえ\nお前以外にはな\n "+GetFullName(pchar)+"、ここから出してくれたら、きっとお前に見返りをやるぜ。";
		...
		case "CaptainComission_381":
			pchar.GenQuest.CaptainComission.GoodsQty = int(int(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Capacity")) / int(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Weight));
			dialog.text = "その通りだ、十分なスペースを確保しておけ "+pchar.GenQuest.CaptainComission.GoodsQty+" 「～のかけら」 "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+"。これほど血塗られた貴重な戦利品を失うのは惜しいぜ。奴らは二度目のチャンスなんてくれやしねえ。";
			link.l1 = "それは俺に任せとけ。";
		...
		case "CaptainComission_387":
			...
			dialog.text = "着いたぜ。隠し場所はあの岩の割れ目の中だ。入口は岩で塞がれてる。隠し場所の中には\n "+pchar.GenQuest.CaptainComission.GoodsQty+" の単位 "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+"。覚えてるか？その半分は俺のもんだぜ？";
...
//				link.l1.go = "ChurchGenQuest1_DialogShip_5_2_1"; // Сваливаем, поверили ему
		...
		case "PiratesOnUninhabited_4":
			...
			dialog.text = RandPhraseSimple("船長、どうか情けをかけて助けてください  "+PiratesOnUninhabited_GetStringNum(int(PChar.GenQuest.PiratesOnUninhabited.PiratesCount))+" 落ちぶれた男たちだ。俺たちの唯一の希望はあんただ、あとは慈悲深い主だけだ。","船長、あんたの船に空いてるロッカーはあるか "+PiratesOnUninhabited_GetStringNum(int(PChar.GenQuest.PiratesOnUninhabited.PiratesCount))+" 落ちぶれた男たちか？");
		...
		case "PiratesOnUninhabited_44":
			...
			dialog.text = "それは簡単に手配できるぜ。奴は航海中だ  "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(PChar.GenQuest.PiratesOnUninhabited.StartShipType)].Name+"Voc"))+" 「」"+sTitle+" 奴隷貿易に手を染めている……こうしようぜ：もしそいつを見つけたら、お前が積み荷をもらって、俺たちは船をもらう。ああ、それと、 そいつにこれについて知ってるかどうかも聞いてみてくれ\n "+ChangeNameCase(NAMETYPE_NICK,PChar.GenQuest.PiratesOnUninhabited.MainPirateName,NAME_ACC)+"…あいつがその名前を聞いてビビって漏らさなきゃいいがな、だってそれは俺の船だからな…"; // belamour gen
			link.l1 = "よし、取引成立だ。";
		...
		case "JusticeOnSale_6":
			...
			dialog.text = "ありがとうよ、船長。報酬に加えて保釈金の分もちゃんと払わせてもらうぜ。 俺たちの船長については司令官と話すべきだと思う。あんたなら説得できるかもしれねえ。無事に出せたら、 俺たちのところに来てくれ\n "+XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId+"Dat")+"、そこに俺たちの船が錨を下ろしているんだ― "+LowerFirst(XI_ConvertString(ShipsTypes[int(PChar.GenQuest.JusticeOnSale.ShipType)].name))+" 「」"+PChar.GenQuest.JusticeOnSale.ShipName+"』。岸で待ってるぜ。"; // belamour gen
			link.l1 = "...";
