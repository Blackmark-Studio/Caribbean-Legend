...
//--> -----------------------------Сага - Искушение Барбазона завершающие этапы---------------------------
		...
		case "pirate_threat":
            ...
            {
                dialog.text = NPCStringReactionRepeat("今日はこの話はもうしただろ。",
                                                      "はっきり言ったつもりだが？",
                                                      "しつこいにも程があるぞ。",
                                                      "もううんざりだ。失せろ！",
                                                      "repeat", 3, npchar, Dialog.CurrentNode);
                link.l1 = HeroStringReactionRepeat("じゃあ、また今度にしよう…",
                                                   "もちろんだよ、マーカス…",
                                                   "ごめん、マーカス…",
                                                   "あっ…", npchar, Dialog.CurrentNode);
                link.l1.go = "exit";
                break;
            }
            if (iGPThreat != 0)
            {
                ...
                dialog.text = "ハハ！うちの連中に痛い目見せられてるようだな、" + GetSexPhrase("相棒", "お嬢ちゃん") + "。まあ、少しの間は抑えてやることもできるが、その代わりにしっかり金を払ってもらうぞ。"+ FindRussianDublonString(iTerraxTotalTemp) + "払えば、話はついたってことだ。";
                if (PCharDublonsTotal() >= iTerraxTotalTemp)
                {
                    if (iGPThreat < 5) link.l0 = "もちろん、これが金だ。";
                    else link.l0 = "選択肢はなさそうだな。持ってけ。";
            ...
            else
            {
                if (NextDiag.TempNode != "I_know_you_good")
                    ...
                    dialog.text = "何の話だ、" + GetSexPhrase("相棒", "お嬢ちゃん") + "？お前の評判は最悪でな、犬っころですら逃げ出すレベルだ。誰も関わりたがっちゃいねえよ。";
                link.l1 = "そうか…";
                link.l1.go = "exit";
            }
		...
		case "mtraxx_110":
            dialog.text = "この作戦は全部俺が計画したし、「レッドドラゴン」が砦攻略という一番厄介な役目を引き受けたからな、 俺と俺の手下どもが戦利品の半分をもらうぜ。残りの半分は、他の四隻の船の船長たちに、 それぞれの乗組員の数に応じて分けてやる。お前には\n "+GetCrewQuantity(pchar)+" お前の手下、取り分は "+int(pchar.questTemp.Mtraxx.Cartahena.Gold)+" 金貨と "+int(pchar.questTemp.Mtraxx.Cartahena.Money)+" ペソ。";
