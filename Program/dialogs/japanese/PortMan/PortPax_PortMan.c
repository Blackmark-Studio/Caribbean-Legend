...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			//регата
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("どんな質問だ？","何の用だ、 "+GetAddress_Form(NPChar)+"?"),"もうすでに俺に質問しようとしたじゃねえか "+GetAddress_Form(NPChar)+"...","今日はこれで三度目の質問だな……","港のことについて何も話すことがないなら、俺に質問して邪魔しないでくれ。","ブロック",1,npchar,Dialog.CurrentNode);
			link.l1 = HeroStringReactionRepeat(RandPhraseSimple("考え直した。","話すことは何もねえ。"),"気にするな。","確かに、もう三度目だな……","悪いが、今は港の用事には興味がないんだ。",npchar,Dialog.CurrentNode);
			link.l1.go = "exit";
		break;
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			{
			dialog.text = "旦那！あなたはレガッタの規則を破りましたので、私としては失格にせざるを得ません。申し訳ありません。 次の航海には参加できません。すでにポートロイヤルへの報告書も用意しました。";
			...
			dialog.text = "登録を済ませましょう、船長 "+GetFullName(pchar)+"、船は "+pchar.Ship.Name+"…レガッタの時間（時単位）は "+int(pchar.questTemp.Regata.SecondTransitionTime)+"。完了しました、結果は登録されましたので、どうぞお進みください。";
			link.l1 = "俺の階級を教えてくれ。";
		...
		case "Regata_info":
			...
			if (n==1)
			{
			dialog.text = "君が先頭だ。おめでとう！この調子で頑張れ！";
			link.l1 = "ありがとう！";
			link.l1.go = "exit";
