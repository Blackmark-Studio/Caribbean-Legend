void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "WildRose_Alonso_12":
			...
			dialog.text = "キャプテン、あなたに会えて本当に嬉しい！ あの野郎にあなたが追っていると誰かが吹き込んだらしく、中途でグレイプショットの嵐を受けた。俺は数人の仲間と撤退に成功して、港へ急いで警告に来た。幸い、急いで街で買った小舟に乗っていた俺を見た見張りがすぐに状況を理解したんだ。彼は素早く人を集めてくれて――ここへ助けに来てくれたんだ。";
			link.l1 = "ありがとう、友よ… "+sld.name+" は俺のせいで死んだ… 俺は君たちを罠に導いてしまった。";
			link.l1.go = "WildRose_Alonso_13";
		break;

		case "WildRose_Alonso_13":
			dialog.text = "誰もそれが罠だとは知らなかった。君は正しい判断をした――俺たちがもっと注意深くあるべきだった。";
			link.l1 = "それでも… 俺はこれを予見すべきだった。あの酒場に入るべきではなかった。";
			link.l1.go = "WildRose_Alonso_14";
		break;

		case "WildRose_Alonso_14":
			dialog.text = "もう遅いよ、キャプテン。次はどう命令する？";
			link.l1 = "仲間たちの遺体は伝統に従って埋めなければならない。Northwood も、ジャングルの奥深くに埋めてくれ。この場所を誰にも、特にメアリーには知られてはいけない。だから信頼できる者だけを連れていけ。";
			link.l1.go = "WildRose_Alonso_15";
		break;

		case "WildRose_Alonso_15":
			dialog.text = "了解した。";
			link.l1 = "メアリーと俺は後で船に戻る。二人きりの時間が必要だ。";
			link.l1.go = "exit";
		...
		case "WildRose_Mary_261":
			...
			{
				dialog.text = "なんてこと、"+pchar.name+"... いまだに信じられない。私の父が… 化け物だったなんて… なぜ私にこんなことを？ "+sSname+"を殺したのよ！ 本当に正気を失ってしまったの？";
				link.l1 = "『コーンウォール』での反乱、大西洋を越えた逃亡、嵐、そして母親の死。あの偶像のポータルをコマンチの秘薬なしで通り抜けたなんて… それは確かに試練だった。だが、あの地獄から生きて戻らなかった方が良かったのかもしれない。";
			}
			else
			{	
				dialog.text = "なんてこと、"+pchar.name+"... いまだに信じられない。私の父が… 化け物だったなんて… なぜ私にこんなことを？ 本当に正気を失ってしまったの？";
				link.l1 = "彼は恐ろしい苦しみを経験したのよ。『コーンウォール』の反乱、大西洋の逃亡、嵐、そしてあなたの母の死。コマンチの秘薬なしで、あの偶像のポータルをくぐったなんて… それが彼の精神を壊してしまったの。";
			}
