	...
	switch(Dialog.CurrentNode)
	{
		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				case 0: //конвой у острова, установка параметров
					...
					dialog.text = "俺は、～で突き止めることができた "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" の旗の下にある小さな交易船団 "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+"、交易船二隻と護衛船一隻が、まもなく近くを航行するだろう "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+"、ここは～から遠くない "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+"。商人たちはたくさんの "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 船倉に隠しているんだ。この情報を利用しない手はないぜ、船長\nあんたと俺なら見張りなんざ簡単に片付けて、積み荷をいただけるってわけだ。どうだ、乗るか？";
					link.l1 = "魅力的だな。俺は賛成だぜ！";
					link.l1.go = "MarchCap_1_1";
					link.l2 = "Some"+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 「俺が関係を壊すには足りねえな」 "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+"「……いや、俺はこんなことやらねえ！じゃあな、旦那、止めようなんて思うなよ！」";
				...
				case 1: //энкаунтер на глобалке, установка параметров
					...
					dialog.text = "俺が突き止めたのは、\n "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" の旗の下にある小さな交易船団 "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+"、交易船二隻と護衛船一隻が、出航するのは "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Startcity+"Gen")+" へ "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Finishcity+"Acc")+"。商人たちはたくさん持っている "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 船倉にあるんだ。この情報を利用しない手はないぜ、船長。\nあんたと俺で衛兵どもを簡単に片付けて、積荷を自分たちのものにできる。一番難しいのは、 この二つの植民地の間の外洋で奴らを見つけることだ。どうだ、乗るか？";
					link.l1 = "魅力的だな。俺は賛成だぜ！";
				...
				case 2: //одиночный пиратский кулсейлор у бухты, установка параметров
					...
					dialog.text = "「という名の海賊について聞かされた」 "+GetName(NAMETYPE_ORIG,pchar.GenQuest.MarchCap.PirateName,NAME_NOM)+"。こいつは最近盗みを働いた "+RandPhraseSimple(RandPhraseSimple("English","French"),RandPhraseSimple("Spanish","Dutch"))+" そして船倉に積み込んだのは "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+"。しかし、残念だが、彼の船が "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.ShipType),"Name")+"Acc"))+" ひどく損傷したため、修理にしばらく時間がかかった。\n現在は "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+" の "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+"。だいたいかかると思う "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" 彼にマストや船体の穴を修理させるんだ。急げば間に合うし、俺たち全員が "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 俺たちのものになるぜ\nこの海賊は俺一人じゃ手に負えねえ、あいつは腕の立つ船乗りで戦いも強い。でも、 二人でかかれば経験があっても勝てるはずだ。どうする、乗るか？";
					link.l1 = "魅力的だな。俺は賛成だぜ！";
		...
		case "MarchCap_AfterBattle":
			...
			if (iTemp > 50 && iTemp < 500)
			{
				...
				link.l1 = "もしお前が空の鳥を数えてばかりいないで俺を手伝ってくれていたら、襲撃はうまくいってたんだぜ……ほら、取り分を持っていけ― "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" そして消え失せろ！";
				link.l1.go = "MarchCap_Talk_exit";
			...
			{
				dialog.text = "よくやったな、船長！言った通りだろう――あの商人どもの貨物倉を空っぽにするなんざ、楽なもんだったぜ、ははは……戦利品は "+FindRussianQtyString(iQty)+" 分け前だ。俺の取り分は "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "素晴らしいぜ、船長！言った通りだ――あの商人どもの貨物倉を空にするのは思ったより簡単だったな、はは……戦利品はでかい、予想以上だ。これは "+FindRussianQtyString(iQty)+" 分け前だ。俺の取り分は "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
		...
		case "MarchCap_AfterBattleGold":
			...
			if (iTemp > 10 && iTemp < 100)
			{
				...
				link.l1 = "もしお前が空の鳥を数えるんじゃなくて俺を手伝ってくれてたら、襲撃はうまくいってたんだぜ……ほら、お前の取り分だ― "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" そして消え失せろ！";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "poor";
				break;
			}
			...
			{
				dialog.text = "よくやったぞ、船長！戦利品は "+FindRussianQtyString(iQty)+" 分け前だ。俺の取り分は "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
				link.l1 = "正しいです。 "+npchar.name+"。異論はねえよ。今まさにロングボートに荷物を積み込んでるところだ。また会うこともあるかもな……";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "good";
				break;
			}
			dialog.text = "素晴らしいぜ、船長！言っただろう、楽勝だったって、ははは……賞金はでかいぜ、俺の予想をはるかに超えてる。そいつは "+FindRussianQtyString(iQty)+" 分け前だ。俺の取り分は "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
