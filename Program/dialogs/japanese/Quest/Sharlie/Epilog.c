void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SharlieEpilog_Cureer":
			dialog.text = "失礼します。あなたは " + GetTitle(NPChar, false) + " " + GetFullName(pchar) + " さんですか？";
			link.l1 = "私本人です。ご用件は？";
			link.l1.go = "SharlieEpilog_Cureer_1";
		break;

		case "SharlieEpilog_Cureer_1":
			dialog.text = "ああ、やっと…あなたを見つけるのは簡単ではありませんでした。ほぼ全ての群島を探し回らなければなりませんでしたよ。修道院長ブノワからの書簡があります。";
			link.l1 = "ふむ…修道院長から？意外だ。";
			link.l1.go = "SharlieEpilog_Cureer_2";
		break;

		case "SharlieEpilog_Cureer_2":
			dialog.text = "手渡すように言われています。支払いは心配しないでください — 修道院長が事前に全ての費用を負担しました。";
			link.l1 = "それでは、こちらをください。ありがとうございます。";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Dassen":
			dialog.text = "ご命令は何でしょうか、"+GetTitle(NPChar, true)+"？";
			link.l1 = ""+npchar.name+"、引き延ばすつもりはありません――私はこの群島を去ります。家族の事情により、父の屋敷に戻らねばなりません。";
			link.l1.go = "SharlieEpilog_Dassen_1";
		break;
		
		case "SharlieEpilog_Dassen_1":
			dialog.text = "私の理解が正しければ、ここでお別れということですね？";
			link.l1 = "残念ですが……他に選択肢はありません。";
			link.l1.go = "SharlieEpilog_Dassen_2";
		break;
		
		case "SharlieEpilog_Dassen_2":
			dialog.text = "さて、あなたと同じ旗の下で仕えられたことは、私にとって大きな名誉でした。運命が許すなら、また喜んでそうしたい。";
			link.l1 = "我々の友情が途切れることなく、むしろより強固なものになることを願っています。共に多くの冒険を分かち合いましたから。";
			link.l1.go = "SharlieEpilog_Dassen_nothing";
			link.l2 = "あなたの忠誠心と職務ぶりに感謝し、退任手当として一か月分の給与をお支払いしたい。";
			link.l2.go = "SharlieEpilog_Dassen_salary";
			link.l3 = "あなたの勤務は非の打ちどころがなく、相応の報いを与えるべきだと考えています。これを受け取ってください――三か月分の給与です。あなたはこの報酬に十分値します。";
			link.l3.go = "SharlieEpilog_Dassen_salaryX3";
		break;
		
		case "SharlieEpilog_Dassen_nothing":
			dialog.text = "私もその希望を心から共有しています、"+GetTitle(NPChar, true)+"。";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "シャルルと呼んでください。ちなみに、あなたにお渡ししたいものがあります。";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
				link.l1 = "それでは、またお会いする日まで、ムッシュー "+npchar.lastname+"。ごきげんよう。";
				link.l1.go = "SharlieEpilog_Dassen_Leave";
			}
		break;
		
		case "SharlieEpilog_Dassen_salary":
			dialog.text = "ありがとうございます、"+GetTitle(NPChar, true)+"。";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "シャルルと呼んでください。ちなみに、あなたにお渡ししたいものがもう一つあります。";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
		...
		case "SharlieEpilog_Dassen_salaryX3":
			dialog.text = "大変感謝しております、"+GetTitle(NPChar, true)+"。あまりにも寛大すぎます。";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "シャルルと呼んでください。ちなみに、あなたにお渡ししたいものがもう一つあります。";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			...
			{
				link.l1 = "堅苦しいのはやめましょう、シャルルと呼んでください。出航を記念して、酒場でささやかな祝宴を開くつもりです。ぜひご一緒いただけますか？";
		...
		case "SharlieEpilog_Dassen_Governor":
			dialog.text = "興味をそそられました、"+pchar.name+"。";
			link.l1 = "あなたをイスラ・モナの総督職に推薦しました。まもなく官庁から正式な書簡が届くはずですし、どうかそれを辞退なさらないことを心より願っています。この島は私にとって非常に大切な存在であり、あなたがその統治の先頭に立つ姿を見られれば、これ以上の喜びはありません。";
			link.l1.go = "SharlieEpilog_Dassen_Governor_1";
		break;
		
		case "SharlieEpilog_Dassen_Governor_1":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+"... "+pchar.name+"... それは実に予想外です……";
			link.l1 = "それでもなお。あなたの返答はいかがでしょうか？";
			link.l1.go = "SharlieEpilog_Dassen_Governor_2";
		break;
		
		case "SharlieEpilog_Dassen_Governor_2":
			dialog.text = "断ることのできない申し出です。喜んでお受けし、この島の繁栄と住民の幸福のために最善を尽くすことをお約束します。";
			link.l1 = "素晴らしい、友よ！ では、出航を祝う宴が開かれる酒場でお待ちしています。準備が整い次第、アロンソが知らせてくれるでしょう。";
			link.l1.go = "SharlieEpilog_Dassen_Governor_3";
		break;
		
		case "SharlieEpilog_Dassen_Governor_3":
			dialog.text = "それを逃すことなど決してありません、モンセニョール。";
			link.l1 = "";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		
		case "SharlieEpilog_Dassen_Tavern":
			dialog.text = "決して逃しません、"+pchar.name+"。";
			link.l1 = "素晴らしい、友よ。ではそこで会いましょう。始められる準備が整い次第、アロンソが知らせてくれます。";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		...
		case "SharlieEpilog_PikarOnUliss_D_1":
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarAlive") || CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarResurrected")){dialog.text = "「その通りだ。」";}
		...
		case "SharlieEpilog_InKitchenDeMonpe_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer") || CheckAttribute(pchar, "questTemp.LSC.Mary_officer"))
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "親しい友達たち";
				else sStr = "戦友";
				dialog.text = "";
			...
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "君の親しい友達たち";
				else sStr = "君の友達";
				dialog.text = "";
		...
		case "SharlieEpilog_KomendantFort_5":
			dialog.text = "見事な計画でございます、閣下！ しかし……海戦に鍛えられたお方であるとはいえ、そのご身分で自ら命を危険にさらすのは本意ではありません。艦隊は……が率いることもできましょう。";
			link.l1 = "静かなヨーロッパの暮らしに耐えられなかったように、戦いの行方が決する間、岸で手をこまねいていることもできません。運命からは逃れられません、"+GetAddress_Form(NPChar)+" "+npchar.lastname+"。";
			link.l1.go = "SharlieEpilog_KomendantFort_6";
		break;

		case "SharlieEpilog_KomendantFort_6":
			dialog.text = "では、そのお望みどおりに。準備を開始する命令を出してもよろしいでしょうか。";
			link.l1 = "進めてください。神のご加護がありますように。";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Mishelle_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer")) sStr = " と妻とともに";
			dialog.text = "まだ分からないのか？お前は死んだ。仲間たちとともに"+sStr+"。お前が襲おうとしたただの商人が、火薬庫を爆破した――お前のような連中に一文たりとも渡さぬためにな。\nSpectra sunt memoria peccatorum――亡霊とは罪人の記憶だ。お前こそが幽霊だ、 "+pchar.name+"…… 欲と盲目的な虚栄心に呑み込まれたカリブの伝説の男。金のために、何人殺してきた？";
