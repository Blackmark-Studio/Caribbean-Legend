	...
	// <-- хронометр Алекса
		...
		case "FDM_cabin_pay":
			...
			if (amount <= 0) // всё принес
			{
				...
				dialog.text = "それで、俺のところに連れて来なきゃならなかったんだな "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" 持ってきた宝箱だ "+FindRussianQtyString(iTemp)+"。支払いは用意できてる。今度は俺の番だ。ちょっと外を歩いてきな。心配すんな、全力を尽くすぜ。";
				link.l1 = "「よし……」";
			...
			else
			{
				...
				dialog.text = "それで、俺のところに連れてきたってわけか "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" 持ってきた箱だ "+FindRussianQtyString(iTemp)+". "+FindRussianQtyString(amount)+" 残りの箱だ。";
				link.l1 = "承知しました、旦那。";
				link.l1.go = "FDM_cabin_wait";
		...
		case "FDM_hull_01":
			dialog.text = "さて、計算してみましょう……船体を全面的に組み直すには、リグナムバイタ材、ロープ、船用シルク、樹脂がそれぞれ150単位ずつ必要です。それに加えて金のダブロン貨が10000枚。そんな目で見ないでくださいよ——金の大半は資材代に消えるんです。この仕事は、船をいったんばらばらに解体して、ゼロから組み直すのと同じようなものです。";
