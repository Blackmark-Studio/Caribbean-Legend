	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "今日は商売にならねえ。どの湾にも巡回がいてな、うちの総督が、とある船長に押収された積荷を探してるんだ。"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" そいつはどこかに隠したんだ。";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "なぜだ？お前の「積み荷」が役に立つかもしれねえ。受け取れ "+int(pchar.questTemp.different.GiveShipLetters.price2)+" コインのことは忘れちまえ。";
					link.l1 = "いや、結構だ。";
				...
				else
				{
					...
					{
						dialog.text = "その通りだ！ますます気になってきたぜ。もし俺が金を払えば\n "+int(pchar.questTemp.different.GiveShipLetters.price3)+" コインさえあれば、その書類なんて全く気にしねえだろう。";
						link.l1 = "いや、結構だ。";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "さて、あの生意気な奴を捕まえるチャンスだぜ！俺は本当に、本当に気になってるんだ。ほら、金は払うぜ "+int(pchar.questTemp.different.GiveShipLetters.price4)+" 金で、机の上の書類のことなんてすっかり忘れてしまうんじゃないか？";
						link.l1 = "俺はやらねえ。";
						link.l1.go = "SM_ShipLetters_2";
						link.l2 = "よし、取引成立だ！";
						link.l2.go = "SM_ShipLetters_3";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "今日は商売はなしだ。どの入り江にも巡回がいてな、我々の総督がある船長に押収された積荷を探しているんだよ"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" そして、どこかに隠したんだ。";
...
////////////////////////////////////////////////////////////////////////////////
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "そうかい？いいだろう。ローゼンクラフト旦那がアイアンウッドの一山を必要としてるが、量は最低でも "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+"。積み荷は十分あるか？";
