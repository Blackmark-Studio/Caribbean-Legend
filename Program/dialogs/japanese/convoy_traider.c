...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "私を護衛してほしいのは "+XI_ConvertString("Colony"+pchar.quest.destination+"Gen")+"「は、～に位置している」 "+XI_ConvertString(GetIslandByCityName(pchar.quest.destination)+"Dat")+"、で "+FindRussianDaysString(int(pchar.ConvoyQuest.iDay))+"、それについては報酬を払う "+FindRussianMoneyString(int(pchar.ConvoyQuest.convoymoney))+"。さて、どうするんだ？";
			link.l1 = "俺も乗ったぜ。";
