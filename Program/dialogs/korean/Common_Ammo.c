   ...
   /*NPChar.FileDialog2 = "DIALOGS\" + LanguageGetLanguage() + "\Prison\" + NPChar.City + "_Prison.c";
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "GiveTaskGun_1":
			...
			dialog.Text = "필요해 "+pchar.questTemp.PrisonGun.Text+", 수량은 "+int(pchar.questTemp.PrisonGun.Qty)+"  정확히 그 수만큼. 금화 더블룬으로 지불하지. "+int(pchar.questTemp.PrisonGun.Price)+" 한 조각당입니다. 그러면 총 "+FindRussianDublonString(int(pchar.questTemp.PrisonGun.Sum))+" 전부 합해서. 어때? 아, 그리고 한 가지 더 있는데 - 나는 한 달이 지나야 전부 가져가겠네. 돈이 아직 도착하지 않았거든.";
