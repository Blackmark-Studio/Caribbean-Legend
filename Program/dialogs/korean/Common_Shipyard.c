	...
	//<-- Бремя гасконца
		...
		case "ship_tunning_start" :		
			...
			{				
				...
				if(!TuningAvailable())
				{
					dialog.Text = "그저 그렇군... 어디 한번 볼까... 그래 - "+XI_ConvertString(RealShips[int(Pchar.Ship.Type)].BaseName)+". 당신 배는 이미 최대한으로 개조되어 있소. 더 이상 손을 대는 건 의미가 없고, 오히려 해가 될 수도 있겠구려.";
					Link.l1 = "그렇군...";
					Link.l1.go = "ship_tunning_not_now_1";
					break;
				}
		...
		case "ship_tunning_Capacity_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_Capacity_again";
				dialog.Text = "나는 아직도 필요해: 철목 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋아.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_SpeedRate_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_SpeedRate_again";
				dialog.Text = "나는 아직도 필요한 게 있어: 선박용 비단 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋아.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_TurnRate_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_TurnRate_again";
				dialog.Text = "아직 필요한 게 있어: 밧줄 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋소.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_MaxCrew_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_MaxCrew_again";
				dialog.Text = "아직도 필요한 게 있어: 철목 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋아.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: ironwood - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "ship_tunning_HP_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_HP_again";
				dialog.Text = "아직 필요한 것: 송진 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋아.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
		...
		case "ship_tunning_rig_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_rig_again";
                dialog.Text = "나는 아직도 필요해: 배의 비단 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋소.";
				link.l1.go = "Exit";
                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: ship silk - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "ship_c_quantity_again_2":		
			...
			{
				NextDiag.TempNode = "ship_c_quantity_again";
				dialog.Text = "나는 아직도 필요해: 송진 - "+int(NPChar.Tuning.Matherial)+".";
				link.l1 = "좋아.";
				link.l1.go = "Exit";
				AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "I need to bring: resin - "+ int(NPChar.Tuning.Matherial) + ".");
		...
		case "Findship_complete":
			...
			dialog.text = "여기 돈이오 - "+FindRussianMoneyString(int(pchar.GenQuest.Findship.Shipyarder.Money))+". 일 맡겨줘서 고맙소. 또 들르기를 잊지 마시오. 안녕히 가시오!";
		...
		case "Device_complete_1":
			...
			dialog.text = "수고에 대한 보수를 주겠소 "+FindRussianMoneyString(int(pchar.GenQuest.Device.Shipyarder.Money))+". 그게 적절한 보상이 되길 바라오.";
			link.l1 = "고마워!";
		...
		case "SailsTypeChoose":
			dialog.text = "어떤 돛을 설치하시겠습니까? 현재 가지고 계신 것은\n "+GetSailsType(int(RealShips[int(Pchar.Ship.Type)].ship.upgrades.sails))+".";
		...
		case "SailsTypeChoose2":
			...
			dialog.text = "현재 당신은 가지고 있소\n "+GetSailsType(int(RealShips[int(Pchar.Ship.Type)].ship.upgrades.sails))+", 선택하셨습니다 "+GetSailsType(int(NPChar.SailsTypeChooseIDX))+". 교체 비용은 "+FindRussianMoneyString(int(NPChar.SailsTypeMoney))+". 진행하시겠습니까?";
		...
		case "SailsColorDone":
			...
			dialog.text = "알겠소. 우리가 돛을 칠하겠소\n "+GetStrSmallRegister(XI_ConvertString(SailsColors[int(NPChar.SailsColorIdx)].name))+" 색.";
		    Link.l9 = "고맙소.";
			Link.l9.go = "exit";
...
//<-- -----------------------------------------Бремя гасконца-----------------------------------------------------
		...
		case "IslaMona_7":
            dialog.text = "그러니까, 너 나한테 빚진 거야 "+int(pchar.questTemp.IslaMona.Shipyarder.Money)+" 두블룬.";
		...
		case "IslaMona_money":
            ...
            dialog.text = "나한테 미안해할 필요 없어, 네 선실 사물함엔 금이 가득하잖아. 나한테 빚졌어. "+FindRussianDublonString(int(pchar.questTemp.IslaMona.Shipyarder.Money))+".";
			link.l1 = "";
			link.l1.go = "IslaMona_money_exit";
		break;
