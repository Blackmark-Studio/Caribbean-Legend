...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("남편이 낯선 사람과 말하지 말라고 했어요!","우리 사이에 더 이상 할 말이 없어.");
				Link.l1 = "별일 아니야.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("말해 보시오, 이 근처 선술집에서 도는 인기 있는 소문은 무엇이오?","여기서 무슨 일이 벌어지고 있는지 차라리 말해 주겠나?","이봐, 친구, 육지에서의 삶이 어떤지 말해줄래?");
		...
		case "sovet":
			...
			{
                dialog.text = RandPhraseSimple("남편이 낯선 사람과 말하지 말라고 했어요!","우리가 이야기할 만한 것은 아무것도 없어.");
				Link.l1 = "별일 아니야.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("말해 봐, 이 동네 선술집에서 도는 인기 있는 소문이 뭐지?","여기서 무슨 일이 벌어지고 있는지 말해 주겠나?","이봐, 친구, 육지에서의 삶이 어떤지 말해줄래?");
			link.l10.go = "rumours_citizen"; //повесил на горожанина, если нужны отедельные слухи, то пишем соответствующую реализацию
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("아, 그래, 기쁘다"+NPCharSexPhrase(NPChar,"","")+" 너를 보니 거의 술 한 잔 마신 기분이군. 뭐든 물어봐.","말하는 거 좋아하나, 선장? 나도 그래... 특히 럼 한 잔 하면서 말이지.","그래, "+PChar.name+"?"),LinkRandPhrase("무슨 일이야?","알겠습니다, 선장님 "+PChar.name+", 너 말이 참 많구나?","성가신 질문이군요, 선장님. 럼 한 잔 사주시면 더 좋겠는데요.")),PCharRepPhrase(LinkRandPhrase("무엇을 알고 싶으신가요 "+GetAddress_Form(NPChar)+"?","듣고 있소, 선장.","가봐야 하니, 빨리 해주시오, 선장."),LinkRandPhrase("유쾌한 동행은 언제나 반갑지, "+GetAddress_Form(NPChar)+" "+PChar.lastname+". 할 말 있으면 해.","그래, "+GetAddress_Form(NPChar)+"?","수다 떠는 거 좋아하십니까, 선장님? 나도 그렇소...")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("이 동네 선술집에서는 어떤 소문이 인기인가?","이 땅에서는 무슨 일이 벌어지고 있지?","여기 육지에는 무슨 새 소식이 있지?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("아니야, 네가 헛것을 듣고 있는 거야. 난 이제 간다.","아니, 아무것도 아니야. 그냥 산책 좀 나갔어."),RandPhraseSimple("아니, 아무것도 아니야. 행운을 빌어!","그냥 산책 중이야. 잘 가."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"이봐, 이 마을 시민으로서 부탁하는데, 칼집에서 칼을 빼 들고 다니는 건 삼가 주게.");
				link.l1 = LinkRandPhrase("좋아.","좋아.","말씀대로...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"진정해, "+GetSexPhrase("동료","아가씨")+", 네가 손에 무기를 들고 돌아다니면 말이야. 그거 좀 신경 쓰이거든...","나는 ~할 때가 싫어 "+GetSexPhrase("남자들","사람들")+" 무기를 꺼내 들고 돌아다니잖아. 나 무서워...");
				link.l1 = RandPhraseSimple("알겠어.","내가 그것을 없애겠다.");
			}
			link.l1.go = "exit";
