	...
	//-----------------------------------Поручение капитана - Выкуп-------------------------------------------------
		...
		case "CapComission2":
			...
			else
			{
				...
				{
					link.l2 = "그놈을 아직 안 팔아서 다행이군. 자, 네 돈이다 – 15만 페소. 그놈은 어디에 있지?";
		...
		break;
			...
			if(int(pchar.money) > 150000)
			{
				link.l2 = "그놈을 아직 팔지 않았다니 다행이군. 여기 네 돈이다 - 15만 페소. 그놈은 어디에 있지?";
                link.l2.go ="CapComission2_3";
			}			
		break;*/
...
//--------------------------------------------Похититель------------------------------------------------------
		...
		case "pirate_threat":
			...
			if (iGPThreat != 0)
			{
				...
				dialog.text = "하! 우리 형제단이 널 괴롭히고 있나 보군, " + GetSexPhrase("친구", "아가씨") + "? 뭐, 잠시 동안은 그들을 진정시켜 줄 수 있지. 하지만 대가가 필요해. " + FindRussianDublonString(iMarlowTotalTemp) + "만 내면, 그렇게 하지.";
