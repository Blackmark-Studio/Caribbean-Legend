...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			link.l1 = HeroStringReactionRepeat(RandPhraseSimple("생각이 바뀌었소, 미안하오.","지금은 아니오. 때와 장소가 맞지 않소."),"사실 지금 드릴 말씀이 없소, 미안하오.","나중에 여쭙겠소.","죄송합니다, "+GetAddress_FormToNPC(NPChar)+"...",npchar,Dialog.CurrentNode);
			link.l1.go = "exit";
		break;

		case "Cupture_after":
            dialog.text = RandPhraseSimple("이미 모든 것을 가져갔소. 더 무엇을 원하시오?","아직 가져가지 않은 것이 남아 있소?");
            link.l1 = RandPhraseSimple("그저 둘러보고 있을 뿐이오...","그냥 확인하는 중이오, 뭔가 놓고 온 게 있을까 해서...");
            link.l1.go = "exit";
