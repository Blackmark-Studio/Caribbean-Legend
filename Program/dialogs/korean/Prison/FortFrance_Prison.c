...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
 	switch(Dialog.CurrentNode)
	{
        case "quests":
			...
			{
				link.l1 = "장교 나리, 당신 감옥에 폴케 들루크라는 자가 있다고 들었소. 그를 풀어줄 수 있도록 내가 할 수 있는 일이 있소?";
                link.l1.go = "Sharlie";
			}
			if (CheckAttribute(pchar, "questTemp.Sharlie") && pchar.questTemp.Sharlie == "freeskiper")
			{
				link.l1 = "장교 나리, 또 왔소. 이번에는 죄수 폴크 들루크에 관해서요. 내가 그의 빚 권리를 샀으니, 이 자를 석방해서 내게 넘기시오. 여기 그의 채무 서류가 있으니, 확인해 보시오.";
		...
		case "Sharlie":
			...
			dialog.text = "아, 그 선원 말인가? 우리 은행원에게 꽤 큰 돈을 빌려 갚지 않았고, 도망치려 하기도 했지. 하지만 금방 잡혔다네. 그때부터 감금되어 있고, 빚을 갚을 때까지 여기서 지낼 거다.";
			link.l1 = "흠. 감옥에 있으면서 그걸 어떻게 하겠다는 거지?";
			link.l1.go = "Sharlie_1";
		break;
		
		case "Sharlie_1":
			dialog.text = "나는 신경 안 써, 나리.";
		...
		case "Sharlie_3":
			...
			DelLandQuestMark(npchar);
			dialog.text = "흠. 모두 맞군. 정말로 그 죄수를 풀어주길 원하는가?";
			link.l1 = "그래. 나는 그놈이 빚을 갚아야 하니 필요하고, 그가 갚을 방법도 알고 있지.";
			link.l1.go = "Sharlie_4";
		break;
		
		case "Sharlie_4":
			dialog.text = "좋소, 거절할 이유는 없지. 델루크를 곧 이리로 데려오겠다. 하지만 경고 하나 하지. 저 여우 같은 놈은 기회만 생기면 도망칠 테니, 그 자에 대해 헛된 기대는 하지 마라.";
			link.l1 = "알겠소, 경관. 이제 내 채무자를 데려가고 싶소.";
			link.l1.go = "Sharlie_5";
		break;
		
		case "Sharlie_5":
			dialog.text = "그래, 물론이지. 하사! 죄수를 데려와라! 폴케 들루크, 이제 새 주인이 생긴 것 같군!";
			link.l1 = "...";
		...
		case "Del_Deluck":
			DelLandQuestMark(npchar);
			dialog.text = "하! 이봐, 자네가 처한 곤경에 대한 소문이 생피에르에서 오래도록 회자될 거라네. 하지만 너무 신경 쓰지 말게, 선장. 자네는 분명 불운의 희생자일 뿐이야. 들어오게, 그리고 걱정하지 마. 자네의 항해사는 이 대화에서 절대 빠져나가지 못할 테니, 하하!";
			link.l1 = "정말 우습군. 고맙소, 간수.";
            link.l1.go = "exit";
