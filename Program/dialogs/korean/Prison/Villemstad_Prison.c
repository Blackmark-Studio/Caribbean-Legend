...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
 	switch(Dialog.CurrentNode)
	{
        case "quests":
            ...
            {
                link.l1 = "장교, 여기 바르톨로메우 포르투갈인을 수감하고 있지...";
                link.l1.go = "Portugal";
            }
		break;
		...
		//Португалец
		case "Portugal":
			dialog.text = "우리가 관리하고 있다. 그런데 왜 신경 쓰는 거지? 네덜란드 서인도 회사의 사절만이 그를 만날 수 있고, 그것도 총독의 서면 명령이 있어야만 가능하다. 지금 회사 소속 장교 한 명이 이 해적을 심문하고 있다. 단순히 그를 보러 온 거라면, 당장 꺼져라. 교수대에 매달릴 때나 보게 될 거다. 이제 가라.";
