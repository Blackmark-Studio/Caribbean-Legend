void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "WildRose_Chimiset_64_2":
			...
			link.l1 = "있잖아, 오렐리가 우리를 기다리고 있었던 것 같다는 생각이 들었어, 마치 우리가 그녀에게 올 걸 알고 있었던 것처럼 말이야. 그 노파는 하루 일찍이나 늦게 죽을 수도 있었는데, 하필이면 메리 아버지에 대해 중요한 말을 해준 직후에 바로 세상을 떠났지.";
			link.l1.go ="WildRose_Chimiset_65";
			AddCharacterExpToSkill(pchar, "sneak", 100);
		break;
		...
		case "WildRose_Alonso_12":
			...
			dialog.text = "캡틴, 당신을 보게 되어 정말 기쁘네! 누군가 그 개자식에게 네가 그를 찾고 있다고 귀띔한 모양이야 — 중간쯤에서 샷탄 세례를 받은 거지. 나는 몇 명의 동료와 후퇴하는 데 성공했고, 경고하기 위해 항구로 달려왔네. 다행히도 내가 급히 사온 작은 배를 보고 보초가 상황을 재빨리 파악했지. 그는 사람들을 빨리 모았고 — 우리를 도우러 여기까지 온 거야.";
			link.l1 = "고맙네, 친구... "+sld.name+" 은 내 탓에 죽었어... 내가 너희를 함정으로 이끌었어.";
			link.l1.go = "WildRose_Alonso_13";
		break;

		case "WildRose_Alonso_13":
			dialog.text = "그 누구도 그게 함정인지 몰랐어. 넌 옳은 결정을 했어 — 우리가 더 조심했어야 했지.";
			link.l1 = "그럼에도... 내가 미리 알았어야 했어. 그 술집에 들어가면 안 됐어.";
			link.l1.go = "WildRose_Alonso_14";
		break;

		case "WildRose_Alonso_14":
			dialog.text = "이제 늦었네, 캡틴. 다음 명령은 무엇인가?";
			link.l1 = "우리 사람들의 시신은 예절에 따라 묻어야 해. Northwood 도 정글 깊은 곳에 묻어라. 이 장소는 아무도, 특히 메리에게도 알려져선 안 돼. 그러니까 가장 신뢰할 수 있는 사람들만 데려가게.";
			link.l1.go = "WildRose_Alonso_15";
		break;

		case "WildRose_Alonso_15":
			dialog.text = "알겠네.";
			link.l1 = "메리와 나는 나중에 배로 돌아갈 거야. 우리 둘만의 시간이 필요해.";
			link.l1.go = "exit";
		...
		case "WildRose_Mary_261":
			...
			{
				dialog.text = "맙소사, "+pchar.name+"... 아직도 믿기지 않아. 내 아버지가... 괴물이었다니... 왜 나한테 이런 짓을 한 거야? 그는 "+sSname+"을 죽였어! 정말로 미쳐버린 걸까?";
				link.l1 = "'콘월'호에서의 반란, 대서양을 건넌 도주, 폭풍우, 그의 어머니의 죽음, 그리고 그 우상들의 포탈을 코만치의 약 없이 통과하다니! 그건 충분히 끔찍한 시련이었어. 하지만 그는 차라리 그 지옥에서 살아남지 말았어야 했어.";
			}
			else
			{	
				dialog.text = "맙소사, "+pchar.name+"... 아직도 믿기지 않아. 내 아버지가... 괴물이었다니... 왜 나한테 이런 짓을 한 거야? 정말로 미쳐버린 걸까?";
				link.l1 = "그는 끔찍한 고통을 겪었어. '콘월'호의 반란, 대서양을 넘는 탈출, 폭풍우, 네 어머니의 죽음... 코만치의 약 없이 그 우상들의 포탈을 통과했다는 게 믿기지 않아 – 하지만 바로 그게 그의 정신을 무너뜨린 거야.";
			}
		...
		case "WildRose_Mary_262":
			dialog.text = "만약 그 사람이 원래부터 그런 사람이었다면 어쩌지? 아니면, 똑같이 악당이었지만 조금 더 부드럽고, 정말로 우리 어머니를 사랑했을 수도 있잖아? 어쩌면 이 모든 일이 그 안에 있던 걸 그냥 풀어준 것뿐일 수도 있고. 그리고 만약 그렇다면... 나한테도 그 사람의 무언가가 있는 건 아닐까? 그럴 수도 있지," + pchar.name + ", 알지?";
