...
void ProcessDialogEvent()
{
	...
	// --------------------------------------вернулся на Остров---------------------------------------------
		...
		case "return_2":
			...
			{
				dialog.text = "아니, 안 추워. 그냥... 여기서는 얘기하고 싶지 않아. 나중에 내 선실로 와, 그치? 너랑 단둘이 얘기하고 싶어. 지금 네이선을 만나러 가는 거야?";
				link.l1 = "그래. 다니엘을 그에게 데려가야 해. 오래 걸리진 않을 거야. 곧 보자, Mary.";
			}
			link.l1.go = "return_3";
		break;
	...
	// --> Мэри вне LSC
		...
		case "LongHappy_9c":
			...
			dialog.text = "내 생각엔, "+int(pchar.questTemp.LongHappy.MarryMoney)+" 페소면 충분하겠지, 그치? "+int(pchar.questTemp.LongHappy.MarryRum)+" 럼 통이라니 - 술이 떨어지는 것만큼 끔찍한 건 없지! 다 모으면 선술집으로 와. 나머지는 내가 알아서 할게.";
