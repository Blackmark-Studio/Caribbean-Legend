	...
	switch(Dialog.CurrentNode)
	{
		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				case 0: //конвой у острова, установка параметров
					...
					dialog.text = "나는 ~에서 알아내는 데 성공했어 "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" 깃발 아래의 작은 무역 호송대 "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", 상선 두 척과 호위선 한 척이 곧 근처를 항해할 거야 "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+", 여기서 멀지 않아 "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+". 상인들이 많은 "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 창고 안에 있소. 이 정보를 안 쓸 이유가 없지, 선장님\n우리 둘이 경비병쯤은 쉽게 처리하고 화물을 가질 수 있소. 자, 할 텐가?";
					link.l1 = "솔깃한데. 난 찬성이야!";
					link.l1.go = "MarchCap_1_1";
					link.l2 = "Some"+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 내가 를 와(과) 관계를 망칠 만큼 충분하지 않아 "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+"아니, 난 이 짓 안 해! 잘 있어라, 나리, 그리고 날 막으려 들지도 마!";
				...
				case 1: //энкаунтер на глобалке, установка параметров
					...
					dialog.text = "나는 ~에서 알아내는 데 성공했어 "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" 작은 무역 호송대가 깃발 아래에 있는 "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", 상선 두 척과 호위선 한 척이 출항할 거야, 출발지는 "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Startcity+"Gen")+" 에게 "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Finishcity+"Acc")+". 상인들에게는 충분한 "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 그 배들의 창고에 있소. 이 정보를 이용하지 않는 건 어리석은 일이오, 선장님.\n경비병들은 당신과 내가 쉽게 처리할 수 있고, 화물도 우리 몫으로 챙길 수 있소. 가장 어려운 건 이 두 식민지 사이의 망망대해에서 그들을 찾는 일이오. 자, 할 생각이오?";
					link.l1 = "솔깃한데. 난 찬성이야!";
					link.l1.go = "MarchCap_2_1";
					link.l2 = "Some "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 내가 관계를 망칠 만큼 충분하지 않아 "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+". 아니, 난 이 짓 안 해! 잘 있어라, 나리, 그리고 날 막으려고도 하지 마!";
					link.l2.go = "MarchCap_exit";
				break;
				...
				case 2: //одиночный пиратский кулсейлор у бухты, установка параметров
					...
					dialog.text = "이라는 이름의 해적에 대해 들었소 "+GetName(NAMETYPE_ORIG,pchar.GenQuest.MarchCap.PirateName,NAME_NOM)+". 이 악당이 최근에 훔쳤어 "+RandPhraseSimple(RandPhraseSimple("English","French"),RandPhraseSimple("Spanish","Dutch"))+" 그리고 그의 화물창에 \n 실었다 "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+". 하지만 안타깝게도, 그의 배 "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.ShipType),"Name")+"Acc"))+" 심하게 손상되어서 수리를 하느라 시간이 좀 걸렸어.\n지금은 "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+" ~의 "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+". 내 생각에 대략 "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" 그가 돛대와 선체의 구멍을 고치게 하려면 말이야. 우리가 서두르면 제때 도착할 수 있어, 그리고 우리 모두가\n "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" 우리 것이 될 거야\n나 혼자서는 이 해적을 상대할 수 없어. 그는 아주 노련한 선원이고 싸움꾼이거든. 하지만 우리 둘이라면 그의 경험에도 불구하고 충분히 해낼 수 있어. 자, 같이 할 거야, 말 거야?";
					link.l1 = "혹하는데. 난 찬성이야!";
		...
		case "MarchCap_AfterBattle":
			...
			if (iTemp > 50 && iTemp < 500)
			{
				...
				link.l1 = "네가 하늘의 새나 세고 있지 말고 나를 도왔더라면, 습격이 성공했을 거야... 네 몫을 가져가라 - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" 그리고 꺼져!";
				link.l1.go = "MarchCap_Talk_exit";
			...
			{
				dialog.text = "잘했네, 선장! 내가 말했잖아 - 저 상인들 화물창 비우는 건 정말 쉬웠다고, 하하... 전리품은 "+FindRussianQtyString(iQty)+" 조각이지. 내 몫은 "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "훌륭해요, 선장님! 내가 말했듯이, 저 상인들의 화물창을 비우는 건 생각보다 쉬웠소, 하하... 전리품이 엄청나군요, 내가 기대했던 것보다 훨씬 많소. 이건\n "+FindRussianQtyString(iQty)+" 조각이다. 내 몫은 "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
		...
		case "MarchCap_AfterBattleGold":
			...
			if (iTemp > 10 && iTemp < 100)
			{
				...
				link.l1 = "네가 하늘에 나는 새나 세지 말고 나를 도왔더라면, 이번 습격은 성공했을 거야... 네 몫을 가져가라 - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+" 꺼져 버려!";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "poor";
				break;
			}
			...
			{
				dialog.text = "잘했소, 선장님! 전리품은 "+FindRussianQtyString(iQty)+" 조각이다. 내 몫은 "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
				link.l1 = "맞아. "+npchar.name+". 이의는 없소. 지금 장주가 실리고 있네. 아마 다시 만나게 되겠지...";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "good";
				break;
			}
			dialog.text = "훌륭합니다, 선장님! 내가 말했듯이, 꽤 쉬웠지, 하하... 전리품이 엄청나군요, 내가 기대했던 것보다 훨씬 많아. 그건\n "+FindRussianQtyString(iQty)+" 조각들이지. 내 몫은 "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
