void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SharlieEpilog_Cureer":
			dialog.text = "실례합니다. 당신은 " + GetTitle(NPChar, false) + " " + GetFullName(pchar) + " 입니까?";
			link.l1 = "직접 왔습니다. 무슨 일입니까?";
			link.l1.go = "SharlieEpilog_Cureer_1";
		break;

		case "SharlieEpilog_Cureer_1":
			dialog.text = "아, 드디어… 당신을 찾기가 정말 쉽지 않았습니다. 군도 거의 전역을 뒤져야 했죠. 여기 절대 베누아의 서신이 있습니다.";
			link.l1 = "흠… 절대에게서? 뜻밖이군요.";
			link.l1.go = "SharlieEpilog_Cureer_2";
		break;

		case "SharlieEpilog_Cureer_2":
			dialog.text = "직접 전달하라는 지시를 받았습니다. 지불은 걱정하지 마십시오 — 절대가 사전에 제 모든 비용을 처리했습니다.";
			link.l1 = "그렇다면 여기로 주세요. 감사합니다.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Mary_6":
			...
			link.l1 = "아니요, 내 사랑. 우리는 피나스선 을(를) 타고 항해할 거예요 '" + GetShipName("Ulysse") + "'. 나는 이 배를 타고 카리브에 왔으니, 이 배를 타고 떠날 것이오.";
		...
		case "SharlieEpilog_Dassen":
			dialog.text = "어떤 명령을 내리시겠습니까, "+GetTitle(NPChar, true)+"?";
			link.l1 = ""+npchar.name+", 더 이상 미루지 않겠습니다 - 저는 이 군도를 떠납니다. 가문의 일로 인해 아버지의 영지에 직접 가야 합니다.";
			link.l1.go = "SharlieEpilog_Dassen_1";
		break;
		
		case "SharlieEpilog_Dassen_1":
			dialog.text = "제 이해가 맞다면, 이제 서로 작별하는 것이군요?";
			link.l1 = "유감입니다만... 다른 선택지가 없습니다.";
			link.l1.go = "SharlieEpilog_Dassen_2";
		break;
		
		case "SharlieEpilog_Dassen_2":
			dialog.text = "그렇다면, 같은 깃발 아래에서 당신과 함께 복무할 수 있었던 것은 제게 큰 영광이었습니다. 운명이 허락한다면, 기꺼이 다시 그렇게 하겠습니다.";
			link.l1 = "우리의 우정이 여기서 끊어지는 것이 아니라, 오히려 더 굳건해지기를 바랍니다. 우리는 함께 수많은 모험을 나눴으니까요.";
			link.l1.go = "SharlieEpilog_Dassen_nothing";
			link.l2 = "당신의 충성심과 전문성에 대한 보답으로, 전역 수당으로 한 달치 급여를 지급하고자 합니다.";
			link.l2.go = "SharlieEpilog_Dassen_salary";
			link.l3 = "당신은 흠잡을 데 없이 복무했고, 저는 마땅한 보상을 드려야 한다고 생각합니다. 이것을 받아 주십시오 - 여기 세 달치 급여가 있습니다. 이 보상은 충분히 받을 자격이 있습니다.";
			link.l3.go = "SharlieEpilog_Dassen_salaryX3";
		break;
		
		case "SharlieEpilog_Dassen_nothing":
			dialog.text = "저 역시 그 희망을 진심으로 공유합니다, "+GetTitle(NPChar, true)+".";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "그냥 샤를이라고 불러 주십시오. 그런데, 당신께 드릴 것이 하나 있습니다.";
		...
		case "SharlieEpilog_Dassen_salary":
			dialog.text = "감사드립니다, "+GetTitle(NPChar, true)+".";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "그냥 샤를이라고 불러 주십시오. 그런데, 당신께 드릴 것이 또 하나 있습니다.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
		...
		case "SharlieEpilog_Dassen_salaryX3":
			dialog.text = "대단히 감사드립니다, "+GetTitle(NPChar, true)+"님. 제게 너무나도 후하십니다.";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "그냥 샤를이라고 불러 주십시오. 그런데, 당신께 드릴 것이 또 하나 있습니다.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
		...
		case "SharlieEpilog_Dassen_Governor":
			dialog.text = "저의 흥미를 끄시는군요, "+pchar.name+".";
			link.l1 = "당신을 이슬라 모나의 총독직에 추천했습니다. 머지않아 관청에서 공식 서한이 도착할 것이며, 부디 이를 거절하지 않으시길 바랍니다. 이 섬은 제게 매우 큰 의미가 있으며, 당신이 그 통치의 수장으로 서는 모습을 보게 된다면 더없이 기쁠 것입니다.";
			link.l1.go = "SharlieEpilog_Dassen_Governor_1";
		break;
		
		case "SharlieEpilog_Dassen_Governor_1":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+"... "+pchar.name+"... 이는 실로 뜻밖이군요...";
			link.l1 = "그럼에도 불구하고. 당신의 대답은 무엇입니까?";
			link.l1.go = "SharlieEpilog_Dassen_Governor_2";
		break;
		
		case "SharlieEpilog_Dassen_Governor_2":
			dialog.text = "거절할 수 없는 제안입니다. 기꺼이 받아들이겠으며, 이 섬의 번영과 주민들의 안녕을 위해 최선을 다할 것을 약속드립니다.";
			link.l1 = "훌륭합니다, 나의 친구여! 그렇다면 출항을 기념하는 연회가 열릴 술집에서 기다리겠습니다. 모든 준비가 끝나면 알론소가 알려줄 것입니다.";
			link.l1.go = "SharlieEpilog_Dassen_Governor_3";
		break;
		
		case "SharlieEpilog_Dassen_Governor_3":
			dialog.text = "결코 놓치지 않겠습니다, 몽세뇨르.";
			link.l1 = "";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		
		case "SharlieEpilog_Dassen_Tavern":
			dialog.text = "결코 놓치지 않겠습니다, "+pchar.name+".";
			link.l1 = "훌륭합니다, 나의 친구여. 그럼 그곳에서 뵙지요. 시작할 준비가 되면 알론소가 알려줄 것입니다.";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		...
		case "SharlieEpilog_PikarOnUliss_D_1":
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarAlive") || CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarResurrected")){dialog.text = "맞아.";}
			else {dialog.text = "맞아.";}
			link.l1 = "...";
		...
		case "SharlieEpilog_HenriDeMonper_Mary_4":
			if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "너는 나를 다른 사람들에게 소개할 거야";
			else sStr = "너는 나를 손님에게 소개할 거야";
		...
		case "SharlieEpilog_InKitchenDeMonpe_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer") || CheckAttribute(pchar, "questTemp.LSC.Mary_officer"))
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "좋은 친구들";
				else sStr = "전우";
				dialog.text = "";
			...
			{
				if (GetAttributeInt(pchar, "questTemp.SharlieEpilog_Friends") >= 2) sStr = "네 좋은 친구들";
				else sStr = "네 친구";
				dialog.text = "";
		...
		case "SharlieEpilog_KomendantFort_5":
			dialog.text = "훌륭한 계획입니다, 각하! 하지만… 해전에 단련된 분이시라 해도, 그와 같은 지위의 분께서 직접 생명을 걸 필요는 없습니다. 함대는 다른 이가 지휘해도 충분합니다.";
			link.l1 = "유럽의 평온한 삶을 견디지 못했던 것처럼, 전투의 향방이 결정되는 동안 해안에서 한가롭게 기다릴 수도 없습니다. 운명은 피할 수 없는 법입니다, "+GetAddress_Form(NPChar)+" "+npchar.lastname+". ";
			link.l1.go = "SharlieEpilog_KomendantFort_6";
		break;

		case "SharlieEpilog_KomendantFort_6":
			dialog.text = "그렇다면 뜻대로 하시지요. 준비를 시작하라는 명령을 내려도 되겠습니까?";
			link.l1 = "시작하시오. 그리고 신께서 우리와 함께하시길.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Mishelle_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer")) sStr = " 그리고 아내와 함께";
			dialog.text = "아직도 이해하지 못하겠나? 자네는 죽었다. 선원들과 함께"+sStr+". 자네가 약탈하려 했던 어떤 상인이 화약고를 폭파했다 — 자네 같은 자들에게 단 한 푼도 넘어가지 않게 하려고 말이지.\nSpectra sunt memoria peccatorum. 그 유령이 바로 자네다, "+pchar.name+"... 탐욕과 눈먼 허영에 사로잡힌 카리브의 전설적인 사내. 금을 위해 몇이나 죽였지?";
