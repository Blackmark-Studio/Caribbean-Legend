...
//Jason ------------------------------------почтовый генератор 2 уровня-------------------------------------------
		case "Postcureer_LevelUp_ForAll":
			...
			dialog.text = "기다리고 있었소. 딱 맞춰 왔군, 자네가 가져온 정보가 큰 도움이 될 거야. 자, 수고비로 이 돈을 받아주시오 - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+". 때맞춘 방문에 감사하오.";
        	link.l1 = "천만에. 잘 가!";
        	link.l1.go = "Postcureer_LevelUp_complete";
