	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "오늘은 거래 없어. 모든 만마다 순찰대가 돌아다니고 있거든, 우리 총독이 어떤 선장이 압수한 화물을 찾고 있대."+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" 그리고 그는 그것을 어딘가에 숨겼어.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "왜 안 되겠어? 네 '화물'을 쓸 데가 있을지도 모르지. 가져가. "+int(pchar.questTemp.different.GiveShipLetters.price2)+" 동전이나 챙기고 잊어버려.";
					link.l1 = "아니, 괜찮아.";
				...
				else
				{
					...
					{
						dialog.text = "정확해! 이제 정말 궁금해졌어. 내가 너한테 돈을 주면 "+int(pchar.questTemp.different.GiveShipLetters.price3)+" 동전만 있으면, 그 서류 따위는 전혀 신경 쓰지 않겠지.";
						link.l1 = "아니, 괜찮아.";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "이제 저 건방진 녀석을 잡을 기회야! 진짜, 진짜 궁금하거든. 있지, 내가 돈 줄게\n "+int(pchar.questTemp.different.GiveShipLetters.price4)+" 금으로 주면, 탁자 위의 서류는 완전히 잊어버릴 건가?";
						link.l1 = "안 해.";
						link.l1.go = "SM_ShipLetters_2";
						link.l2 = "좋아, 거래하자!";
						link.l2.go = "SM_ShipLetters_3";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "오늘은 거래 없어. 만마다 순찰대가 깔렸거든, 우리 총독이 어떤 선장이 압수한 화물을 찾고 있대."+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" 그리고 그는 그것을 어딘가에 숨겼어.";
					link.l1 = "그 선장은 어디 있지"+pchar.GenQuest.CaptainComission.Name+"?";
					link.l1.go = "CapComission_1";
					break;
				}
...
////////////////////////////////////////////////////////////////////////////////
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "그런가? 좋아. 로젠크라프트 나리가 철목 한 묶음이 필요하다네, 하지만 최소한 "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+". 화물이 충분하오?";
