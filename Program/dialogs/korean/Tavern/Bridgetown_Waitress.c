...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("질문, "+GetAddress_Form(NPChar)+"?","무엇을 도와드릴까요, "+GetAddress_Form(NPChar)+"?"),""+GetSexPhrase("흠, 이게 무슨 수작이지, "+GetAddress_Form(NPChar)+"? ","또 이상한 질문이야? 이봐, 가서 럼주나 한잔 해.")+"","오늘 하루 동안, 그 질문을 네가 꺼낸 게 벌써 세 번째야..."+GetSexPhrase(" 이게 애정의 표시인가?","")+"","또 질문이 있나 보군. "+GetAddress_Form(NPChar)+"?","block",1,npchar,Dialog.CurrentNode);
