...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("질문, "+GetAddress_Form(NPChar)+"?","무엇을 도와드릴까요, "+GetAddress_Form(NPChar)+"?"),""+GetSexPhrase("흠, 이게 다 무슨 짓이야, "+GetAddress_Form(NPChar)+"? ","또 이상한 질문이야? 자기야, 가서 럼주라도 한잔해...")+"","오늘 하루 동안, 그 질문을 네가 벌써 세 번째 꺼내는군..."+GetSexPhrase(" 이게 애정의 표시인가?","")+"","질문이 더 있겠지, "+GetAddress_Form(NPChar)+"?","block",1,npchar,Dialog.CurrentNode);
