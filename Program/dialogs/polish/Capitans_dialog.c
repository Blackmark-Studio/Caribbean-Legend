...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (!bDisableMapEnter)
			{
				...
				if (CheckAttribute(PChar, "questTemp.WPU.Escort.LevelUp_1WM") && pchar.questTemp.WPU.Escort == "begin" && NPChar.id == "WMCaptain")
				{
					...
					link.l1 = "Cześć. Pozwól, że się przedstawię - "+GetFullName(PChar)+". Cieszę się, że też cię widzę całego i zdrowego, specjalnie tu przybyłem, żeby cię szukać - i twojego statku.";
				...
				if (CheckAttribute(PChar, "questTemp.WPU.Escort.LevelUp_1WM") && pchar.questTemp.WPU.Escort == "wait" && NPChar.id == "WMCaptain")
				{
					...
					{
						link.l1 = "Tak. Wszystko, czego potrzebujesz do naprawy swojego statku, jest w mojej ładowni.";
				...
				{
					dialog.text = "Ahoy, "+GetAddress_Form(NPChar)+". Nazywam się "+GetFullName(NPChar)+", jestem kapitanem tego statku. Pozwól mi wyrazić najszczerszą wdzięczność za twoją pomoc w walce z tą przeklętą piracką szumowiną - gdybyś nie interweniował, mogło się dla nas skończyć dość ponuro. Naprawdę nas uratowałeś, cholera!";
					link.l1 = "To nic,  "+GetAddress_FormToNPC(NPChar)+". Pozwól, że się przedstawię - "+GetFullName(PChar)+", przyszedłem do ciebie na prośbę mistrza portu "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.StartCity+"Gen")+". Poinformował mnie o nieszczęściach twojej karawany i wysłał mnie, aby znaleźć twój statek.";
        ...
        case "quests":
			...
			{
                Dialog.text = "Nie jestem upoważniony do rozmowy z tobą. Porozmawiaj z dowódcą naszego szwadronu. Jego imię to "+GetFullName(characterFromID(NPChar.MainCaptanId))+", możesz go znaleźć na "+XI_ConvertString(RealShips[int(characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Type)].BaseName)+" '"+characters[GetCharacterIndex(NPChar.MainCaptanId)].Ship.Name+"'.'";
        ...
        case "Talk_board":
            ...
            else
            {
                ...
                else
                {
                    ...
                    Pchar.GenQuest.MoneyForCaptureShip = (70 + hrand(60, "&CapSur"+NPChar.id)) * iEncPow;
					if(HasShipTrait(pchar, "trait14")) Pchar.GenQuest.MoneyForCaptureShip = int(int(Pchar.GenQuest.MoneyForCaptureShip) * 1.35);
                    Dialog.text = RandSwear()+"Tak, słyszałem dużo o twoich skandalach. Niech będzie, ale pamiętaj "+XI_ConvertString(NationShortName(int(NPChar.nation))+"łowca")+" nie pozwolę na to bezkarnie!";
		...
		case "MerchantTrap_Abordage":
			...
			Dialog.Text = "W imię "+NationKingsName(NPChar)+" poddaj się, "+GetSexPhrase("brudny pirat","plugawa wench!")+"! Wtedy twoje życie zostanie oszczędzone do czasu sprawiedliwego trybunału, gdzie zostaniesz osądzony za wszystko "+NationNameSK(int(NPChar.nation))+"statki, które splądrowałeś"+GetSexPhrase("","")+".";
		...
		case "Siegehelp":		
            ...
            {
                Dialog.text = "Nie jestem upoważniony do rozmowy z tobą. Porozmawiaj z dowódcą naszego szwadronu. Nazywa się  "+GetFullName(rchar)+", możesz go znaleźć na "+XI_ConvertString(RealShips[int(rchar.Ship.Type)].BaseName)+"''"+rchar.Ship.Name+"''.";
                link.l1 = "Dzięki. Zrobię jak mówisz.";
                link.l1.go = "exit";
		...
		case "Talk_Capture_City":
            ...
            dialog.text = "Witaj, kapitanie! To na pewno była ciężka bitwa, ale udało nam się złamać zaciekły opór obrońców miasta. Splądrowaliśmy "+int(aData.loot)+" pesos. Zgodnie z naszą umową, twoja część to "+ilt+". Please accept this money; you certainly deserve every last coin of it. And now please excuse me, but I must return to my business.";
		...
		case "GoldSquadron":
            ...
            {
                Dialog.text = "Nie jestem upoważniony do rozmowy z tobą. Porozmawiaj z dowódcą naszego dywizjonu. Nazywa się "+GetFullName(rch)+", możesz go znaleźć na "+XI_ConvertString(RealShips[int(rch.Ship.Type)].BaseName)+" ''"+rch.Ship.Name+"''.";
                link.l1 = "Dzięki. Zrobię jak mówisz.";
                link.l1.go = "exit";
		...
		case "WM_Captain_3":
			...
			dialog.text = "Już wszystko to rozgryzłem. Biorąc pod uwagę to, co już mamy, potrzebuję "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Planks))+" deski, "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Sailcloth))+" płótno żaglowe i "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.Linen))+" bawełna. Z tymi materiałami, udałoby nam się naprawić statek w ciągu tygodnia, a potem moglibyśmy wypłynąć.";
		...
		case "WMShip_final":
			...
			dialog.text = "Chciałbym jeszcze raz podziękować, kapitanie - bardzo mi i mojej załodze pomogłeś wyjść z tego kłopotu. Tak jak obiecałem, pokryję koszty materiałów potrzebnych do naprawy mojego statku. Oto "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+", proszę, weź to. A to jest mój osobisty prezent dla ciebie.";
		...
		case "VSPShip_final":
			...
			dialog.text = "Chcę jeszcze raz podziękować, kapitanie - bardzo mi i mojej załodze pomogłeś wyjść z tego tarapatów. Oto są"+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+". Cała załoga dorzuciła się, więc nawet nie myśl o rezygnacji. A to jest mój osobisty prezent dla ciebie.";
			link.l1 = "Cieszę się, że mogłem ci pomóc!";
			link.l1.go = "WMShip_final_1";
		break;
		...
		case "SShip_final":
			...
			dialog.text = "Chcę jeszcze raz podziękować, kapitanie, za uratowanie naszych tyłków z tego bałaganu. Proszę, przyjmij te monety - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.LevelUp_1Money))+"  a tu, przyjmij ten podarunek. To nie wiele, ale to wszystko, co mogę zrobić, aby podziękować ci za to, co zrobiłeś. Proszę, nie odrzucaj oferty\nMoi byli towarzysze są pełni podziwu dla ciebie i twojego walki z Indianami i piratami. Wszyscy chcą dołączyć do ciebie. Mam nadzieję, że nie odrzucisz prośby o lojalne usługi od marynarzy, którzy właśnie stracili swój statek?";
		...
		case "Количество вбитого провианта":
			...
			{
				dialog.text = "Zgoda! "+int(iQty*iMoney)+" pesos.";
		...
		case "Количество вбитого провианта повтор":
			...
			if(iQty > 0 && iQty <= iTemp)
			{
				dialog.text = "Umowa!"+int(iQty*iMoney)+" pesos.";
				if(int(pchar.money) < iQty*iMoney)
				{
					link.l1 = "Hmm, nie mam tyle pieniędzy. Innym razem.";
					link.l1.go = "exit";
		...
		case "Количество вбитых гарпунов":
			...
			if(iQty > 0 && iQty <= iTemp)
			{
				dialog.text = "Zgoda! "+int(iQty*100)+" pesos.";
