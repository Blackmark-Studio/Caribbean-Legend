   ...
   /*NPChar.FileDialog2 = "DIALOGS\" + LanguageGetLanguage() + "\Prison\" + NPChar.City + "_Prison.c";
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "GiveTaskGun_1":
			...
			dialog.Text = "Potrzebuję "+pchar.questTemp.PrisonGun.Text+", w ilości "+int(pchar.questTemp.PrisonGun.Qty)+"  dokładnie jednostek. Będę płacił w złotych dublonach, "+int(pchar.questTemp.PrisonGun.Price)+" za sztukę. To wyniesie "+FindRussianDublonString(int(pchar.questTemp.PrisonGun.Sum))+" w sumie. Co odpowiesz? Ach, i jeszcze jedno - przyjmę całą partię, ale nie wcześniej niż za miesiąc - pieniądze jeszcze nie dotarły.";
		...
		case "TakeGun_1":
			...
			Log_Info("Otrzymałeś "+FindRussianDublonString(int(pchar.questTemp.WPU.Escort.Money))+"");
