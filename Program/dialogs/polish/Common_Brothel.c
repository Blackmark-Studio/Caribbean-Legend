void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
       			dialog.text = NPCharRepPhrase(pchar,LinkRandPhrase(""+GetSexPhrase("Jakie dziewczyny, drogi?! Połowa garnizonu jest za tobą, a ty idziesz prosto do domu publicznego!","Zjeżdżaj stąd, dobrze? Połowa garnizonu jest za tobą!")+"","Każdy strażnik miasta szuka cię. Nie jestem głupcem, żeby świadczyć ci usługi w tym momencie...","Nie masz tu nic do roboty!"),LinkRandPhrase(""+GetSexPhrase("Tylko spróbuj dotknąć moje dziewczyny, a oskubię cię ze skóry na żywca!","Spływaj, ty szumowino!")+"","Ty "+GetSexPhrase("brudasie","cwaniaro")+", opuść mój lokal, teraz! Strażnicy!!!","Nie boję się ciebie, "+GetSexPhrase("łotrze","gówniaro")+"! Wkrótce cie powieszą w naszym forcie, nie uciekniesz daleko..."));
			...
			{
				dialog.text = "Oh, "+GetSexPhrase("młody człowieku","drogie dziecko")+", dziś jesteśmy zamknięci.";
				link.l1 = "Zamknięci?";
			...
			if (CheckAttribute(pchar, "questTemp.ChickenGod.HostessDialog")) {
				...
				link.l1 = "Dobrze cię widzieć, "+npchar.name+". Byłem na liście gości. Jednakże, w takich lokalach, nawet goście muszą płacić, więc... Proszę, nie utrudniaj mi tego,  chcę zapłacić za pokój.";
			...
			if (CheckAttribute(pchar, "GenQuest.Badboy.Complete") || CheckAttribute(pchar, "GenQuest.Badboy.Continue"))
			{
				...
				{
					dialog.text = "Oh, wróciłeś! Czy załatwiłeś to z "+pchar.GenQuest.Badboy.Brothel.Name+", aby zostawił moje dziewczyny w spokoju?";
			...
			if (CheckAttribute(pchar, "questTemp.HWIC.Detector") || SandBoxMode)
			{
				...
				if (!CheckAttribute(pchar, "questTemp.Portugal") && bOk && !CheckAttribute(npchar, "quest.Portugal") && npchar.location == "Marigo_SecBrRoom" && makeint(environment.time) > 6.0 && makeint(environment.time) < 22.0)
				{
					...
					link.l5 = "Dla ciebie, madame, wszystko co zechcesz! Jestem do usług!";
					link.l5.go = "Portugal";
					link.l8 = "Przykro mi, ale teraz jestem zajęty.";
					link.l8.go = "exit";
			...
			{
				dialog.text = RandPhraseSimple(TimeGreeting()+". Witaj w moim domu miłości. Nazywam się "+npchar.name+", jestem właścicielką tego przybytku. "+GetSexPhrase("Co mogę dla ciebie zrobić, "+GetAddress_Form(NPChar)+"?","Szczerze mówiąc, jestem nieco zaskoczona, widząc cię tutaj, "+GetAddress_Form(NPChar)+", ale zapewniam cię, że świadczymy usługi nie tylko dla mężczyzn.")+"",TimeGreeting()+". Witam cię, "+GetSexPhrase("nieznajomy","młoda damo")+", w moim skromnym lokalu. Pozwól, że się przedstawię, jestem "+NPChar.Name+" - właścicielka tego azylu dla mężczyzn spragnionych kobiecego dotyku. "+GetSexPhrase("Co mogę dla ciebie zrobić?","Mimo że rozmawiam z tą samą płcią, co ja, to zapewniam cie, iż mamy też coś dla panien...")+"");
			...
			if (!CheckAttribute(pchar, "questTemp.Sharlie.Gigolo"))//не идёт квест девочки
			{
				...
				{
					link.l2 = npchar.name+", Chcę spędzić trochę czasu na osobności z jedną z twoich dziewczyn.";
			...
			{
				link.l8 = "Widzisz, chciałbym wziąć najlepszą dziewczynę, jaką masz.";
				link.l8.go = "NA_Girl";
			}
			// <-- приключенец
        ...
        case "Hostess_1":
			if (!CheckAttribute(npchar, "quest.selected"))
			{
				...
				{
					dialog.text = ""+GetSexPhrase("Zawsze jesteśmy zadowoleni, że możemy obsłużyć klienta. Powiedz mi teraz, przystojniaku - czy już kogoś wybrałeś, czy to ci niezbyt przeszkadza?","Cóż, moje dziewczyny na pewno są w stanie... ci pomóc. Czy wybrałaś już kogoś, czy to ci niezbyt przeszkadza?")+"";
					Link.l1 = ""+GetSexPhrase("Heh, potrzebuję tylko kurtyzany i potrzebuję jej natychmiast. Nie obchodzi mnie, która, wszystkie twoje dziewczyny wyglądają naprawde dobrze...","Każda będzie w porządku, jeśli zna swoje obowiązki, oczywiście...")+"";
					Link.l1.go = "Hostess_NotChoice";	
					Link.l2 = "Tak, jest jedna, "+GetSexPhrase("która skradła moje serce...","która się dla mnie nada...")+"";
        ...
        case "Hostess_Choice":
			dialog.text = "Zawsze się cieszę, kiedy dziewczyny i klienci zaczynają do siebie odczuwać ciepło... Powiedz mi jej imię.";
			Link.l1.edit = 9;
			Link.l1 = "";
        ...
        case "Hostess_Choice_1":
			...
			if (sld.id == "none")
			{
				...
				Link.l2 = "Być może. Lepiej jeśli ponownie zapytam ją o imię, tak na wszelki wypadek. Porozmawiamy o tym później.";
				Link.l2.go = "exit";	
			}
			else
			{
		...
		case "Woman_FackYou":
			dialog.text = "Skarbie, co ty właśnie robisz?! A wyglądałeś na przyzwoitego kapitana... Tym razem nie 'odlecisz', przystojniaku. Strażnicy obetną ci skrzydła...";
		...
		case "TakeMayorsRing_H2":
			dialog.text = "One też nie. Jeśli klient zapomni lub zgubi coś, moje dziewczyny przynoszą mi to. Ale nikt nie przyniósł mi pierścienia gubernatora.";
		...
		case "TakeMayorsRing_H3":
			...
			link.l1 = "Rozumiem. Dzięki, "+npchar.name+".";
			link.l1.go = "exit";
		break;
        ...
        case "Horse_talk":
			if (LAi_grp_playeralarm > 0)
			{
       			dialog.text = NPCharRepPhrase(pchar,LinkRandPhrase("Lepiej opuść ten lokal!","Wszyscy strażnicy w mieście szukają cię! Lepiej dla ciebie, jeśli teraz odejdziesz...","Narobiłeś bałaganu i teraz się tu pokazujesz?! Nie, nie tym razem..."),LinkRandPhrase("Znikaj!","Brudny morderco, wynoś się stąd! Strażnicy!","Nie boję się ciebie,"+GetSexPhrase("łotrze","cwaniaro")+"! Wkrótce zostaniesz powieszony w naszym forcie, nie uciekniesz daleko..."));
				link.l1 = NPCharRepPhrase(pchar,RandPhraseSimple("Heh, alarm nigdy dla mnie nie jest problemem...","Oni nigdy mnie nie złapią."),RandPhraseSimple("Heh, jaką głupią suką jesteś...","Zamknij swoje usta, kurwo, albo zamknę je czymś innym..."));
				link.l1.go = "exit";
				break;
			}
			dialog.text = NPCStringReactionRepeat("Witaj, "+GetAddress_Form(NPChar)+""+GetSexPhrase("",", hi-hi..")+". Musisz zobaczyć panią i wypełnić zamówienie.","Znowu ty? Proszę, porozmawiaj z karczmarką. Jest w swoim biurze.","Hmm, "+GetAddress_Form(NPChar)+""+GetSexPhrase(", Mam nadzieję, że jesteś tak samo uparty w innych sprawach, jak w rozmawaniu... ","ponownie ")+"proszę cię, abyś odwiedził właściciela tego lokalu.","Oh, "+GetAddress_Form(NPChar)+""+GetSexPhrase(", czyż nie jesteś uparty?",", Hee-hee... Jesteś uparta, nieprawdaż?")+". Musisz zobaczyć panią i wypełnić zamówienie.","block",1,npchar,Dialog.CurrentNode);
			link.l1 = HeroStringReactionRepeat("Dobrze, "+GetSexPhrase("skarbie, ","kochana")+". Zrobi się."+GetSexPhrase("","")+".","Tak, jasne...",""+GetSexPhrase("Nie miej wątpliwości, moja piękna, jestem uparty i silny jak byk!","Tak, tak...")+"",""+GetSexPhrase("O cholera, musiałem coś przegapić... Przykro mi, droga.","Dobrze, w porządku.")+"",npchar,Dialog.CurrentNode);
			link.l1.go = "exit";
			Link.l2 = "A gdzie ona jest?";
			Link.l2.go = "Horse_1";
			Link.l3 = LinkRandPhrase(""+GetSexPhrase("Och, piękności, założę się, że mógłbym utonąć w tych twoich oczach...","Wyglądasz pięknie, kochanie!")+"",""+GetSexPhrase("Wiesz, nigdy wcześniej nie spotkałem tak pięknej kobiety!","Wiesz, nigdy wcześniej nie spotkałem tak miłej laski!")+"",""+GetSexPhrase("Kochanie, wyglądasz pięknie.","Cholera, tak mi się nudziło z tymi wszystkimi wieśniakami... A ty jesteś taka urocza!")+"");
			...
			if (pchar.questTemp.different == "TakeMayorsRing" && pchar.questTemp.different.TakeMayorsRing.city == npchar.city && GetNpcQuestPastDayWOInit(npchar, "TakeMayorsRing") > 7)
			{
				link.l5 = LinkRandPhrase("Słuchaj, "+GetSexPhrase("piękna","kochanie")+", nie znalazłaś przypadkiem obrączki ślubnej? Wygląda na to, że jeden mężczyzna ją zgubił...","Kochanie, widziałaś może obrączkę ślubną czy coś w tym stylu?","Słuchaj, "+GetSexPhrase("mój kotku","kochanie")+", nie widziałaś tu gdzieś obrączki ślubnej?");
        ...
        case "Horse_1":
			dialog.text = "Jest w swoim biurze. Możesz tam dotrzeć stąd przez drzwi naprzeciwko wyjścia na ulicę, lub z ulicy po drugiej stronie domu. Nazywa się "+characters[GetCharacterIndex(npchar.city+"_Hostess")].name+".";
        ...
        case "Horse_2":
			if (rand(1))
			{
				dialog.text = LinkRandPhrase("O mój Boże, tak miło słyszeć takie rzeczy! Hej, jestem teraz wolna, więc jeśli mnie wybierzesz, nie pożałujesz...","Naprawdę tak myślisz? Wiesz, to takie urocze... Słuchaj, jestem teraz wolna, więc możesz mnie wybrać. "+GetSexPhrase("Obiecuję ci morze miłości i ocean pieszczot...","Wyglądasz mi na zacną partnerke do łóżka.")+"",""+GetSexPhrase("Naprawde tak myślisz? Och, naprawdę jestem zaszczycona. Wiesz, rzadko słyszę takie komplementy... ","Och, dziewczyno... gdybyś tylko wiedziała, jak mi doskwiera ciągłe obcowanie z wieśniakami... ")+"Hej, posłuchaj, teraz jestem wolna, więc możesz wybrać mnie, aby się uszczęśliwić. Nie zawiodę cię, obiecuję...");
				if (CheckAttribute(pchar, "questTemp.RomNaTroih") && !CheckAttribute(pchar, "questTemp.RomNaTroih_Pogovoril"))
				{
					link.l1 = "Moja droga, właśnie taki miałem zamiar! Jak szczęśliwie, że twoja gospodyni już zorganizowała prywatne spotkanie ze mną na górze. Chcesz dołączyć?";
        ...
        case "Horse_4": 
			dialog.text = NPCStringReactionRepeat("Już zapłaciłeś.","Powiedziałam ci 'idź na górę'.","Do góry, młody człowieku.","Niektórzy ludzie myślą tak wolno...","block",1,npchar,Dialog.CurrentNode);
        ...
        case "HorseChoice_0": 
			if (!CheckAttribute(npchar, "quest.sexHappend"))
			{
				...
				link.l1 = HeroStringReactionRepeat("Tak już ze mną jest czasami...","W porządku, zrobię to.","Tak, mam to.","Przepraszam, moja miłości.",npchar,Dialog.CurrentNode);
				link.l1.go = "exit";
			}
			else
			{
        ...
        case "HorseChoice_1": 
			if (!CheckAttribute(npchar, "quest.sexHappend"))
			{
				...
				{
					dialog.text = NPCStringReactionRepeat(""+GetSexPhrase("Skarbie, już wszystko obgadaliśmy. ","")+"Nie kaź mi czekać zbyt długo...",""+GetSexPhrase("Hmm... Słuchaj, drogi,","Słuchaj,")+" uważam twoje słowa za bardzo miłe i wszystko, ale czy mógłbyś przejść do rzeczy...","Być może, porozmawiasz o tym z madame, prawda?","Hm... Nawet nie wiem, co powiedzieć...","block",1,npchar,Dialog.CurrentNode);
			...
			{
				dialog.text = "O, to znowu ty, "+GetSexPhrase("mój chwalebny korsarzu","moja ładna dziewczyno")+"! Jeśli znowu mnie chcesz, przyjdź do pani, nie będziesz zawiedziony..."+GetSexPhrase(" Mimochodem, rozpoznałam cię, drogi, ale mówią nam, że musimy rozmawiać ze wszystkimi klientami na równi, nie powinno być ulubieńca, przepraszam za to...","")+"";
        ...
        case "HorseChoice_2": 
			if (!CheckAttribute(npchar, "quest.sexHappend"))
			{
				dialog.text = NPCStringReactionRepeat("Więc,"+GetSexPhrase(" drogi przyjacielu,","")+"lepiej zajmij się biznesem. Będzie to bardziej użyteczne niż nic nie robienie.",""+GetSexPhrase("Hmm, dlaczego tu jesteś ","Dlaczego tu jesteś")+" mówiąc ciągle to samo?","Hej, czy to nie wystarczy?!","Hm, co za niespodzianka, nic nowego"+GetSexPhrase(", znowu te głupie próby uwodzenia! Jeśli chcesz ze mną uprawiać seks, idź i zobacz madame, ty ptaszkopysku!","...")+"","block",1,npchar,Dialog.CurrentNode);
				link.l1 = HeroStringReactionRepeat("Tak, słyszałem"+GetSexPhrase("","")+" ty...","Cóż, tak to się okazuje...","Hmm, może to wystarczy, a może nie...","Uważaj na swoje słowa"+GetSexPhrase("","")+"...",npchar,Dialog.CurrentNode);
				link.l1.go = "exit";
			}
			else
			{
				dialog.text = "Ohh, to znowu ty! "+GetSexPhrase("I znowu te same głupie komplementy. Nic się nigdy nie zmienia w tym świecie... Jeśli ","Tylko przypomnienie: jeśli ")+"chcesz"+GetSexPhrase(" się ze mną zabawić"," się ze mną zabawić")+", zapłać właścicielowi miejsca. Twoje słowa nic mnie nie obchodzą.";
        ...
        case "Horse_ReadyFack":
			...
			{
				case "0":
					dialog.text = RandPhraseSimple("Bardzo się cieszę, że cię widzę. "+GetSexPhrase("No więc, co zamierzamy robić dalej?","Co powiesz na to, żebyśmy się dobrze zabawiły?")+"","Nie bądź nieśmiały"+GetSexPhrase(", poczuj się jak w domu przynajmniej przez dwie godziny.",". Będę w stanie cię zabawić, nie miej wątpliwości.")+"");
				...
				break;
				case "1":
					dialog.text = "O, to znowu ty, "+GetSexPhrase("mój chwalebny korsarzu! Obiecałam ci coś i jestem gotowa dotrzymać słowa","moja urocza dziewczyno! Nigdy nie zapomnisz następnych dwóch godzin")+"...";
					Link.l1 = "Teraz to brzmi kusząco...";	
				break;
				case "2":
					dialog.text = "Ohh, oto jesteś, w końcu. Nie marnujmy czasu!";
					Link.l1 = ""+GetSexPhrase("Do roboty, laluniu...","Teraz pokaż mi co potrafisz...")+"";
				break;
			}
			if (pchar.sex == "woman")
			{
        ...
        case "Horse_AfterSex":
			if (CheckAttribute(pchar, "questTemp.ReasonToFast") && pchar.questTemp.ReasonToFast == "Begin")
			{
				...
				link.l1 = LinkRandPhrase("Cóż, taki już jestem.","Porozmawiamy następnym razem.",RandPhraseSimple("Nie płacę ci za rozmowę.","Ale ty, "+GetSexPhrase("skarbie","moja droga")+", nie byłaś tak cicho cały ten czas."));
				link.l1.go = "exit";
				link.l2 = ""+GetSexPhrase("Byłaś po prostu wspaniała, prawie straciłem zmysły! To bardzo rzadka okazja, kiedy kobieta jest zarówno piękna, jak i gorąca","Ohh, na pewno wiesz, jak zadowolić kobietę... Jestem taka podekscytowana.")+".";
			...
			else
			{
				...
				{
					case "0":
						dialog.text = LinkRandPhrase("Czy ci się podobało?","No więc, co możesz powiedzieć? Czy wszystko było w porządku?","Więc,"+GetSexPhrase(" korsarzu, czy wszystko","wszystko")+" w porządku?");
						Link.l1 = RandPhraseSimple(""+GetSexPhrase("Oczywiście, wszystko jest w porządku","Na pewno znasz swoje obowiązki")+".",""+GetSexPhrase("Wszystko w porządku, kochanie","Wiesz, byłam całkiem zadowolona")+".");
						Link.l1.go = "exit";
					break;
					case "1":
						dialog.text = LinkRandPhrase("Czy dotrzymałam mojej obietnicy?","No,"+GetSexPhrase(" czy ci się podobałam",", czy ci się podobało")+"?","Mam nadzieję, "+GetSexPhrase("byłeś zadowolony, bo ja jestem ba-a-ardzo zadowolona","byłaś zadowolona, bo dałam z siebie wszystko")+"...");
						Link.l1 = RandPhraseSimple("Tak, bardzo mi się podobało.",""+GetSexPhrase("Spędziliśmy wspaniały czas, byłaś przepiękna!","Wszystko było po prostu wspaniałe!")+"");	
						Link.l1.go = "exit";
					break;
					
					case "2":
						dialog.text = RandPhraseSimple("No cóż, to wszystko, musisz iść.","Twój czas się skończył,"+GetSexPhrase(" Kapitanie,","")+".");
        ...
        case "Horse_AfterSex_2":
			if(CheckAttribute(pchar,"GenQuest.EncGirl") && pchar.GenQuest.EncGirl == "Bag_BrothelRoom" && !CheckCharacterItem(pchar, "leather_bag"))
			{
				...
				link.l1 = "Było wyśmienicie. A teraz powiedz mi piękności, czy nie widziałaś nigdzie walizki?";
		...
		case "Horse_ReasonToFast_2":
			...
			dialog.text = "Tuż przed tobą był sierżant sztabowy. Zazwyczaj nie odwiedza nas, ale tym razem jakiś diabeł go tu przyprowadził. Co więcej, wybrał mnie... Jęczał, będąc nade mną przez kilka minut, a potem nazwał mnie niewykwalifikowaną i pognał do jakiejś zatoki. Zmykał tak szybko, że prawie zostawił u nas spodnie, ha-ha-ha...";
			link.l1 = "Czy ty nie wiesz, "+GetSexPhrase("piękności","kochanie")+", kim naprawdę są mężczyźni? Powiedział swojej żonie, że pójdzie do domu publicznego na 'inspekcję', potem powiedział swojej kochance, że spieszy się do swojej żony, ale jedyną rzeczą, której zawsze chciał, było patrolowanie tej zatoki. "+GetSexPhrase("He-he-ah!..","Ha-ha-ha!")+"";
		...
		case "TakeMayorsRing_S1":
			...
			{
				dialog.text = LinkRandPhrase("Nie, kochanie, przepraszam, ale nie. Chętnie bym ci pomogła, ale nie mogę.","Nie, "+GetSexPhrase("przystojniaku","młoda damo")+", nie widziałem żadnego pierścionka...","Przykro mi, ale nie. Nie widziałem żadnego pierścionka.");
			...
			else
			{
				...
				{
					dialog.text = LinkRandPhrase("Nie, kochanie, przepraszam, ale nie. Chciałabym ci pomóc - ale nie mogę.","Nie, "+GetSexPhrase("przystojniaku","młoda damo")+", nie widziałem żadnego pierścienia...","Przykro mi, ale nie. Nie widziałem żadnego pierścienia.");
		...
		case "TakeMayorsRing_S2":
			dialog.text = "Przykro mi, ale pierścień został mi podarowany jako prezent, więc nie jestem zobowiązana go zwrócić.";
		...
		case "TakeMayorsRing_S4":
			dialog.text = "I co ja mam z tym wspólnego? Jeśli był pijany, to jest to jego problem, nie mój!";
		...
		case "Portugal_3":
			...
			dialog.text = "O nie! Nie mów mi, że zrobiłeś mu coś strasznego... Nie miał pieniędzy, a ja o tym wiedziałem! Wyrastaliśmy w tym samym małym miasteczku, nigdy bym nie zwróciła się do straży! Chciałam go tylko trochę przestraszyć... aby go wstrząsnąć zanim utonie w butelce!";
		...
		case "mtraxx_2":
            ...
            dialog.text = "Nie, nie ja. Opowiem ci całą historię, tylko mi nie przerywaj.";
		...
		case "mtraxx_4":
            dialog.text = "Do następnego przyjazdu kapitana wszystko zaaranżowałam tak, że jego ulubiona dziewczyna była zajęta innym klientem, a 'pracę' dostałam dla siebie. Próbowałam wydobyć od niego koordynaty, niestety, na próżno. On tylko chwalił się, jak stanie się bogaty jak Krassus i wróci do Starego Świata\nAni słowa o lokalizacji. Jedyne, co udało mi się dowiedzieć, to że jego statek został naprawiony i miał płynąć do Hawany, gdzie spróbuje zorganizować misję w odpowiednim kierunku. W końcu to nie on jest właścicielem statku.";
		...
		case "mtraxx_7":
			...
			link.l2 = "Innym razem, moja droga. Pomyśl o przerwie na lunch. Idź do kościoła, czy coś w tym stylu.";
		...
		case "chicken_god_pay_1":
			dialog.text = "";
			link.l1 = "Pani "+npchar.name+", czy przynajmniej możesz wyjaśnić, co tu się dzieje? Czy gubernator jest nawet świadomy, że przez miesiąc byłaś pod opieką tubylca? Żadna osada nie przetrwa zamknięcia domu publicznego na dzień, mogę ci to powiedzieć!";
		...
		case "RomNaTroih_2":
			dialog.text = "Jestem na ciebie zła, Kapitanie... Ale okażę ci łaskę, jeśli na nią ciężko zapracujesz.";
		...
		case "SantaMisericordia_2":
			dialog.text = "Żadnych 'ale'. Co jeśli za chwile tu wejdzie? Na zewnątrz, już!";
			link.l1 = "...";
			link.l1.go = "exit";
