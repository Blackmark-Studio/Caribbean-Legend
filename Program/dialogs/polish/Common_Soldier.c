...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":			
			...
			if (false)
			{
				...
				else
				{
					if (DeceptionChance > (10+rand(150)+rand(150)))
					{
						...
						link.l2 = "Zrzuciłem kotwicę niedaleko "+XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen")+" pod flagą "+NationNameGenitive(int(npchar.nation))+". Jakiś problem? (Prawdopodobieństwo oszustwa "+DeceptionChance+"%%)";
						link.l1.go = "NotPegYou";
					}
					else
					{
						dialog.text = RandPhraseSimple("Szpieg! Złóż broń i chodź za mną!","Wrogi agent! Łapać "+GetSexPhrase("go","ją")+"!");
						link.l1 = RandPhraseSimple("A żeby cie jakieś choróbsko zabrało na tamten świat! Albo nie, sam cię tam wyśle!!","Kurwa mać! Musiałeś się akurat spojrzeć w moją strone?! Wiesz co to dla ciebie oznacza?!","Mogłeś mnie zignorować i przeżyłbyś. Teraz zginiesz, a za twój nędzny żołd nie będzie jak pogrzebu urządzić!","Eh, i tak już mi się znudziło skradanie. Czas się przejść przez legowisko w należyty sposób!");
						link.l1.go = "fight"; 
					}
				}
			}
			else
			{
				...
				if (false)
				{
					...
					{
						dialog.text = RandPhraseSimple("Kim jesteś i co tutaj robisz?","Stój! Kim jesteś i jakim prawem próbujesz wejść do miasta?");
					...
					if (CheckNationLicence(HOLLAND))
					{
						...
						{
							link.l2 = "Nie widzisz flagi "+NationNameGenitive(int(pchar.nation))+"na maszcie mojego statku?! (Prawdopodobieństwo oszustwa "+DeceptionChance+"%%)";
						}
						else //если причалил не в городе
						{
							link.l2 = "Zrzuciłem kotwicę niedaleko "+XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen")+" pod flagą "+NationNameGenitive(int(pchar.nation))+". Jakiś problem? (Prawdopodobieństwo oszustwa "+DeceptionChance+"%%)";
						}
					...
					else
					{
						...
						if (findsubstr(pchar.location.from_sea, "_town" , 0) != -1) //если причалил в городе
						{
							link.l1 = "Czy nie widzisz flagi "+NationNameGenitive(int(pchar.nation))+" na moim statku?! (Prawdopodobieństwo oszustwa "+DeceptionChance+"%%)";
						}
						else //если причалил не в городе
						{
							link.l1 = "Zrzuciłem kotwicę niedaleko "+XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen")+" pod flagą "+NationNameGenitive(int(pchar.nation))+". Jakiś problem? (Prawdopodobieństwo oszustwa "+DeceptionChance+"%%)";
						}
						if (DeceptionChance < (10+rand(50)+rand(50)))
						{
							link.l1.go = "PegYou";
				...
				else
				{
					if (int(NPChar.nation) == PIRATE)
					{
						...
						{ // пираты, не наши
							...
							{
							    /* dialog.text = LinkRandPhrase(RandPhraseSimple("Masz do mnie sprawę, kapitanie?","Ja tu załatwiam sprawy, jeśli czegoś potrzebujesz - pytaj, nie zwlekaj."),RandPhraseSimple("Kapitanie, jestem bardzo zajęty, więc szukaj innych rozmówców.","Czy coś jest nie tak, kapitanie? Jeśli nie ma pytań, pozwól mi przejść."),"Przejdź, kapitanie, śpieszę się.");
							link.l1 = LinkRandPhrase("Przepraszam, przyjacielu, "+GetSexPhrase("pomyliłem się","pomyliłam się")+" ","Dobrze, zajmij się swoimi sprawami.","Nie, nic.");
							link.l1.go = "exit";
							link.l2 = LinkRandPhrase(RandPhraseSimple("Mam do ciebie kilka pytań.","Mam do ciebie sprawę."),RandPhraseSimple("Widzę, że się spieszyłeś. Jedno pytanie i będziesz wolny.","Ja "+GetSexPhrase("chciał","chciała")+"zapytać o coś."),"Twoje sprawy poczekają. Chciałem zapytać o coś "+GetSexPhrase("chciał","chciała")+".");
					            ...
					            link.l1 = RandPhraseSimple("Wiesz co? Nie szczekaj!","Ogranicz swój zapał. Nie ochłone tak szybko!");
					            link.l1.go = "exit";
						    	/* link.l2 = LinkRandPhrase(RandPhraseSimple("Mam do ciebie kilka pytań.","Mam do ciebie sprawę."),RandPhraseSimple(" Widzę, że się spieszysz. Jedno pytanie i będziesz wolny.","Ja "+GetSexPhrase("chciałem","chciałam")+" zapytać o coś."),"Twoje sprawy poczekają. Chciałem zapytać o coś.");
							link.l2.go = "quests";//(перессылка в файл города) */
							}
							break;
						}
					...
					if(isMainCharacterPatented() && int(Items[int(pchar.EquipedPatentId)].TitulCur) > 4 && npchar.nation == int(Items[int(pchar.EquipedPatentId)].Nation))
					{
						switch(rand(2))
						{
							...
							case 2: 
								...
								link.l1 = "Służyć "+NationNameGenitive(int(npchar.nation))+", żołnierzu! To najlepsze, co możesz zrobić.";
								link.l1.go = "exit";
							break;
						}
						break;
					...
					if(IsOfficerFullEquip() && npchar.nation == int(Items[int(pchar.EquipedPatentId)].Nation))
					{
						...
						{
							case 0: 
							dialog.text = "Pozdrowienia, kapitanie! Jeśli masz dla mnie jakieś instrukcje, to przepraszam: chociaż jestem w służbie "+NationNameGenitive(int(npchar.nation))+", Odpowiadam tylko komendantowi i gubernatorowi.";
							link.l1 = "Mam swoich oficerów i załogę, którzy słuchają moich wskazówek. Wykonaj swoje obowiązki, żołnierzu.";
							link.l1.go = "exit";
							break;

							case 1:
								dialog.text = "Kapitanie, czy możesz przyjąć mnie do służby na swoim statku? Tak samo służę za "+NationNameGenitive(int(npchar.nation))+", ale wolę morze.";
							...
							case 2: 
								dialog.text = "Oh, masz szczęście: jesteś kapitanem statku na służbie "+NationNameGenitive(int(npchar.nation))+"... A ja tkwię tu cały dzień.";
			...
			{
				dialog.text = "Witam w Bridgetown, "+GetAddress_Form(NPChar)+"...";
				link.l1 = "Myślałem, że moja twarz jest znana nie tylko w koloniach francuskich... Nazywam się "+GetFullName(pchar)+".";
		...
		case "WildRose_Soldier_1":
			...
			{
				dialog.text = "Ah, to Pan, kapitanie... Trochę mnie słońce rozłożyło... Przeklęte upały!";
			}
			else
			{
				dialog.text = "Nie da się z tym kłócić. Jednak nocą wszyscy ludzie wyglądają tak samo, wie Pan.";
			}
			link.l1 = "Cóż ci jestem winien, żołnierzu? Zwykle wasza brać mnie nie niepokoi rozmowami.";
			link.l1.go = "WildRose_Soldier_2";
		break;
		
		case "WildRose_Soldier_2":
			dialog.text = "Khm, tak, "+GetAddress_Form(NPChar)+". Mam jednak rozkaz od lorda Willoughby’ego: muszę ostrzec wszystkich nowo przybyłych na wyspę, że kilka dni temu z plantacji Bishopa uciekła grupa niewolników. Przeklęci nadzorcy pewnie smacznie spali, podczas gdy oni łamali kajdany. A dziś powinienem być w objęciach mojej kochanej Angeliki...";
			link.l1 = "Zgaduję: wysłano żołnierzy garnizonu na poszukiwanie mienia plantatora?";
			link.l1.go = "WildRose_Soldier_3";
		break;
		
		case "WildRose_Soldier_3":
			dialog.text = "Oczywiście... Bishop również wysłał w dżunglę kilka oddziałów swoich najemników, ale to mu nie wystarczyło. Obiecał sowitą nagrodę tym, którzy przyprowadzą niewolników do niego. Żywych. Zwykle jego czarnoskórzy pracownicy wyginali plecy, ale niedawno dostarczono partię dezerterów floty — w kamieniołomie niewiele się przydali, ale uciec z plantacji wystarczyło im sprytu...";
			link.l1 = "Czyli to byli biali?";
			link.l1.go = "WildRose_Soldier_4";
		break;
		
		case "WildRose_Soldier_4":
			dialog.text = "Bielszych nie ma, piorun mnie strzeli! Czarni po prostu by uciekli, a ci wzięli kilka muszkietów i szabli, które zabrali od zabitych najemników — i już zdążyli ich użyć. Jednego z naszych przybili, parę drabów Bishopa przy okazji — reszta oszalała. Prawie przez pomyłkę nie zastrzeliliśmy naszych własnych, myląc ich z uciekinierami. I jeszcze kilku cywilów zniknęło bez śladu w dżungli.";
			link.l1 = "Czy bramy miasta są zamknięte?";
			link.l1.go = "WildRose_Soldier_5";
		break;
		
		case "WildRose_Soldier_5":
			dialog.text = "Nie, oczywiście — miasto nie jest w stanie oblężenia. Możesz iść, dokąd zechcesz. Ale jeśli zapragniesz polować na uciekinierów i trafisz pod ogień krzyżowy, winę ponosisz wyłącznie Ty. Jeśli przeżyjesz, nie warto później obijać drzwi rezydencji, by narzekać Jego Wysokości — w końcu okazał wystarczająco dobrej woli, by zadbać nie tylko o mieszkańców Bridgetown, ale i o jego gości.";
			link.l1 = "Cóż za... uprzejmość z jego strony. Niech Bóg strzeże Francisa Willoughby’ego!";
			link.l1.go = "WildRose_Soldier_6";
			link.l2 = "Ach, gdyby wszyscy gubernatorzy troszczyli się o nowo przybyłych tak jak sir Willoughby!..";
			link.l2.go = "WildRose_Soldier_7";
		break;
		
		case "WildRose_Soldier_6":
			dialog.text = "Bardzo zabawne. Możesz iść, kapitanie.";
			link.l1 = "Do widzenia, żołnierzu.";
			link.l1.go = "WildRose_Soldier_8";
			AddCharacterExpToSkill(pchar, "Leadership", 100);
		break;
		
		case "WildRose_Soldier_7":
			dialog.text = "No tak, nasz brat miałby znacznie mniej pracy. Nie będę Cię już dłużej zatrzymywać, kapitanie.";
			link.l1 = "Powodzenia, żołnierzu.";
			link.l1.go = "WildRose_Soldier_8";
