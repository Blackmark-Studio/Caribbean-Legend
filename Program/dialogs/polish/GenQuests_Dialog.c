void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(NPChar.id == "ContraMeetManQuest_QuestMan" && PChar.location == PChar.GenQuest.ContraMeetMan.ShoreID) // Квестовый чел по генеру "meeting in the cove" 
			{
				dialog.text = "Kapitanie, twoje imię to "+GetFullName(PChar)+", czyż nie?";
					...
					link.l2 = "Nie. Pomyliłeś mnie z kimś innym.";
					link.l2.go = "exit";
					break;
			}
			...
			if(NPChar.id == "ContraMeetManQuest_Contrick_0") // Контрик в комнате по квесту "встреча в бухте"
			{
				...
				{
					dialog.text = "...Hmm, to rzeczywiście on... Dziękuję, "+GetSexPhrase("kamracie","dziewko")+". Bez twojej pomocy mielibyśmy trudności, nie ma mowy, żebyśmy wydostali się z miasta. Proszę, weź "+FindRussianMoneyString(iMoney)+" i możesz spływać. A my sobie 'pogadamy' z naszym przyjacielem...";
						link.l1 = "...";
						link.l1.go = "ContraMeetManQuest_3";
				}
				else
				{
					dialog.text = "...Hmm, to rzeczywiście on... Dziękuję, "+GetSexPhrase("kamracie","dziewko")+". Bez twojej pomocy mielibyśmy trudności, nie ma mowy, żebyśmy wydostali się z miasta. Proszę, weź "+FindRussianMoneyString(iMoney)+" i możesz spływać. Teraz wypatroszymy tego parszywego zdrajcę!";
			...
			{
				dialog.text = "Stójże! Hej, "+GetSexPhrase("kamracie","dziewko")+" pokaż nam, jak ciężka jest twoja sakiewka!";
					link.l1 = "Już zmarnowałeś wszystkie kościelne pieniądze?";
			...
			{
				dialog.text = RandPhraseSimple("Witaj! Wreszcie Pan wysłuchał naszych modlitw i zesłał dzielnego kapitana na ten opuszczony brzeg!","Dzięki Opatrzności! Teraz miejscowe sępy nie będą pożywiać się naszymi zgniłymi kośćmi!");
			...
			{
				dialog.text = LinkRandPhrase("Ejże, "+GetAddress_Form(pchar)+". Zechcesz poświęcić nam chwilę swego cennego czasu?","Czyżby szlachetny by się zgubił?"+GetSexPhrase("","")+" Wysłuchasz pokornego żądania tych niegodnych ludzi?","Szlachetna pa"+GetSexPhrase("nie","nienko")+", proszę, poświęć chwilę ze swego cennego czasu, by porozmawiać z nieszczęśnikiem.");
				link.l1 = LinkRandPhrase("Nie mam na to czasu.","Spadaj, mam dużo do zrobienia.","Zostaw mnie w spokoju, nie mam dla ciebie czasu.");
				link.l1.go = "Convict_0End";
				link.l2 = LinkRandPhrase("Czego chcesz?","Czego potrzebujesz?","Jaki masz problem?");
			...
			{
				dialog.text = "Hej, kapitanie! Czy interesują cię indiańskie błyskotki i towary, których nie można kupić w zwykłych sklepach? A może srebro?";
					link.l1 = "Lumpy jak ty zazwyczaj żądają - nie oferują niczego. Dlaczego jesteś tak dziwnie uprzejmy?";
					link.l1.go = "JusticeOnSale_1";
					link.l2 = "Mój pistolet zwykle rozmawia z takimi łajdakami jak ty!";
		...
		case "Hold_GenQuest3":
			...
			switch(hrand(3))
			{
				case 0:
					...
					link.l1 = "Poczekaj... Jesteś "+pchar.GenQuest.Hold_GenQuest.Name+", a ty nie wiesz o "+pchar.GenQuest.Hold_GenQuest.CapName+"?";
		...
		case "Hold_GenQuest61":
			dialog.text = "Źle mnie zrozumiałeś: jestem członkiem rady miejskiej. Pilnujemy przestrzegania karty."+"Straże!!! Straże! Aresztujcie go pod zarzutem handlu niewolnikami i plądrowania statków "+NationNameGenitive(int(pchar.GenQuest.Hold_GenQuest.Nation))+"!";
		...
		case "ShipWreck_2":
			...
			if(rand(1) == 0)
			{
				dialog.text = RandPhraseSimple(" Witaj, jestem "+pchar.GenQuest.ShipWreck.Name+", kapitanie z "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Acc"))+" '"+pchar.GenQuest.ShipWreck.ShipTypeName+", mój statek rozbił się na rafach niedaleko wybrzeża."+"Z całej załogi przeżyło tylko kilku marynarzy. Przypływ wyrzucił nas na ten opustoszały brzeg. Głodowaliśmy i pragnęliśmy "+(5+hRand(7))+"tygodnie do teraz","Cześć, jestem "+pchar.GenQuest.ShipWreck.Name+", kapitanie z "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Gen"))+" '"+pchar.GenQuest.ShipWreck.ShipTypeName+". Nasz statek rozbił się niedaleko stąd."+"Udało nam się dotrzeć do tego brzegu na wrakach statków, ale to miejsce wydaje się niezamieszkałe. Byliśmy zmuszeni przetrwać na skorupiakach i owocach palm przez "+(5+hRand(7))+" tygodni do tej pory.");
			...
			{						
				...
				dialog.text = "O, kapitanie, nasze straty są naprawdę straszne... Witaj, jestem "+pchar.GenQuest.ShipWreck.Name+", kapitan i właściciel "+GetStrSmallRegister(XI_ConvertString(ShipsTypes[int(pchar.GenQuest.ShipWreck.StartShipType)].Name+"Gen"))+" '"+pchar.GenQuest.ShipWreck.ShipTypeName+"Albo, powinienem rzec, były kapitanie i właścicielu."+"Ten drań "+pchar.GenQuest.ShipWreck.BadName+" którego wynająłem w "+XI_ConvertString("Colony"+pchar.GenQuest.ShipWreck.City+"Voc")+" sprowokował moją załogę do buntu. W końcu wylądowaliśmy w tym zagubionym miejscu. To się stało "+(5+hRand(7))+" tygodnie temu.";
		...
		case "ShipWreck_3":
			dialog.text = RandPhraseSimple("Z całej załogi tylko "+ShipWreck_GetStringQty(int(pchar.GenQuest.ShipWreck.Qty))+" przeżyło. Inni zginęli; kilku z nich zmarło już tutaj z powodu ran i gorączki",
				"Tylko " + ShipWreck_GetStringQty(int(pchar.GenQuest.ShipWreck.Qty)) + " mieło szczęście. Inni spotkali swój koniec... albo w głębinach, albo na wydmach tego wybrzeża.");
		...
		case "ShipWreck_7":
			if(rand(1) == 0)
			{
				if(rand(100) < 90)
				{
					...
					dialog.text = "Do dowolnej osady "+NationNameGenitive(int(pchar.GenQuest.ShipWreck.Nation))+".";
		...
		case "ShipWreck_8_3":
			...
			Log_SetStringToLog("Oddałeś " + pchar.GenQuest.ShipWreck.FoodQty + " jednostek pożywienia.");
			Log_SetStringToLog("Oddałeś " + pchar.GenQuest.ShipWreck.MedicamentQty + "jednostek lekarstw.");
			DialogExit();
		...
		case "ShipWreck_8_5":		
			...
			Log_SetStringToLog("Oddałeś " + pchar.GenQuest.ShipWreck.FoodQty + " jednostek pożywienia.");
			Log_SetStringToLog("Oddałeś " + pchar.GenQuest.ShipWreck.MedicamentQty + "jednostek lekarstw.");		
			Log_SetStringToLog("Oddałeś " + pchar.GenQuest.ShipWreck.AmunitionQty + " jednostek broni.");
			Log_SetStringToLog("Oddałeś " + pchar.GenQuest.ShipWreck.RumQty + " jednostek rumu");	
		...
		case "ShipWreck_17": // принимаем в команду
			...
			if(int(pchar.GenQuest.ShipWreck.Qty) >= 5) sText = "";
			else sText = "";
		...
		case "Convict_11":
			dialog.text = "Jesteśmy zbiegami... z "+GetStrSmallRegister(XI_ConvertString("MineType"+pchar.GenQuest.Convict.MineType+"Gen"))+". Wkrótce były pan zauważy... a wtedy na pewno zginiemy.";
		...
		case "Convict_12_1_1":
			...
			dialog.text = "Prosimy, kapitanie, zatrudnij nas, z pewnością nie pożałujesz! Pływaliśmy z "+GetName(NAMETYPE_NICK,pchar.GenQuest.Convict.PirateName,NAME_ABL)+" na Adriatyku, aż w końcu został powieszony, a naszych, "+pchar.GenQuest.Convict.ConvictQty+" ludzi, zostało wygnanych do tego miejsca.";
		...
		case "Convict_13_1":
			...
			link.l1 = "Z uwagi na okoliczności, nie dam ci przejazdu taniej niż "+FindRussianMoneyString(int(pchar.GenQuest.Convict.Sum))+"pesos za osobę.";
		...
		case "Convict_13_2":
			dialog.text = "Czy mamy wybór? Dobrze, "+FindRussianMoneyString(int(pchar.GenQuest.Convict.Sum))+"... ";
		...
		case "Convict_14":
			dialog.text = RandPhraseSimple(RandPhraseSimple("Eh, kapitanie, też byłeś kiedyś biedny, miałeś puste kieszenie. A teraz gardzisz prostymi ludźmi, takimi jak my.", "Kapitanie, nie wypada tak traktować zwykłych ludzi. Wszyscy żyjemy, wyznając tego samego Boga."),
		...
		case "Convict_15":
			...
			{
				link.l1 = "Tak, oto twoja łajba. I nie zapomnij o zapłacie. Zapłaciłem "+pchar.GenQuest.Convict.ShipPrice+"monet za to, więc jesteś mi winien"+int(pchar.GenQuest.Convict.ShipPrice)* 2+" , tak jak się umówiliśmy.";
		...
		case "ShipSituation12":
			...
			link.l1 = RandPhraseSimple("Do diabła! Odcinajcie odbijacze! Ruszajcie!","O Boże, jak mogliśmy wpaść w takie tarapaty! Otwórzcie cieśniny! Odpływajcie!");
		...
		case "ShipSituation32":
			dialog.text = RandPhraseSimple("Śledziłem cię przez długi czas. Szkoda, że musiałem poświęcić swój statek, ale twoja głowa jest warta dwóch nowych. Miłego łowienia ryb!","Zobacz, jak zamierzasz odejść? Z fajerwerkami, jak indyjski książę"+GetSexPhrase("","")+". Ale nie dam się wydymać, twoja głowa jest warta o wiele więcej niż mój statek. Żegnaj teraz, moja łajba nie będzie czekać zbyt długo...");
		...
		case "CaptainComission_32_1":
			GetSlaveSpeciality();
			dialog.text = "Ja wiem, ja wiem... Ale to i twoja wina. Jak mogłeś zatopić trzy skrzynie złota, które musieliśmy dosłownie wytrząsnąć ze skarbca, żeby "+NationNameNominative(int(pchar.GenQuest.CaptainComission.Nation))+" mógłby mieć takiego specjalistę "+pchar.GenQuest.CaptainComission.SlaveSpeciality+"?";
		...
		case "CaptainComission_32_2":
			dialog.text = "„Cóż, jesteś mądrym człowiekiem, z pewnością rozumiesz, że na targowanie się jest już za późno. Mogliśmy zatrzymać twój statek z ładunkiem.\nAle rekompensata została wcześniej zapewniona. Dlatego rząd "+NationNameGenitive(int(pchar.GenQuest.CaptainComission.Nation))+" gratuluje ci udanej operacji i wręcza ci ten cenny podarunek oraz skromną nagrodę.";
		...
		case "CaptainComission_202_1":
			dialog.text = "Zatem moje podejrzenia się potwierdziły. Zostałeś zmuszony do tego przez "+NationNamePeople(int(pchar.GenQuest.CaptainComission.Nation))+"?";
		...
		case "CaptainComission_93":
			dialog.text = "Więc, moje podejrzenia się potwierdziły. Zostałeś zmuszony do tego przez "+NationNamePeople(int(pchar.GenQuest.CaptainComission.Nation))+"?";
		...
		case "CaptainComission_305":
			sTemp = GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name") + "Gen"));
			dialog.text = "Dobrze, dam ci szczegóły\n"+"Podczas jednego z regularnych patroli spotkałem statek pod banderą "+NationNameGenitive(int(pchar.GenQuest.CaptainComission.Nation))+"Zignorowali moje rozkazy, by się poddać."+"Kiedy po krótkim pościgu dogoniliśmy "+sTemp+"' "+pchar.GenQuest.CaptainComission.VictimShipName+" i tak podnieśli czarną flagę. Fort był dość daleko, więc musieliśmy walczyć sami. Zniszczyli nasz bukszpryt, dlatego nie mogliśmy uniknąć abordażu.\n"+"Było blisko, ale udało nam się zabić ich wszystkich dzięki Świętej Marii i oddziałowi muszkieterów. Wyobraź sobie naszą radość, gdy dowiedzieliśmy się, że "+sTemp+"było wypełnione."+"Zdecydowaliśmy, że to jest godziwa nagroda za nasze rany i śmierć naszych towarzyszy\nByło już ciemno, wzięliśmy "+sTemp+" do najbliższej zatoki i ukryj ładunek na brzegu. Potem wysadziliśmy "+sTemp+" i zgłosiłem o zatopionym statku pirackim. Oczywiście, nie wspomniałem o ładunku.";
...
//			bQuestDisableMapEnter = true;
		...
		case "CaptainComission_320":
			...
			dialog.text = "Musimy się spieszyć, zanim przybędą posiłki. Załadunek zaraz się skończy, a twój udział to "+pchar.GenQuest.CaptainComission.GoodsQty+" jednostki "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Generał"))+".";
		...
		case "CaptainComission_Canoneer7":
			dialog.text = "Powiedziałem im, że zostałem poturbowany podczas abordażu i nic nie wiem o żadnym ładunku. Udawałem też, że jestem słabego umysłu, wiesz, dopiero co stanąłem na nogi, tuż przed przybyciem "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Imię")+"Acc"))+"Nie mogłem się poruszać prawidłowo, moja koordynacja była do kitu.";
		...
		case "CaptainComission_372":
			...
			link.l1 = "Kapitanie "+pchar.GenQuest.CaptainComission.CapName+" poprosił mnie, bym cię ostrzegł, że okoliczności zniszczenia "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Acc"))+" '"+pchar.GenQuest.CaptainComission.VictimShipName+" stało się znane władzom, "+"ale nie złapałem twojego statku podczas podróży z "+XI_ConvertString("Colony"+pchar.GenQuest.CaptainComission.City1+"Gen")+".";
		...
		case "CaptainComission_374":
			dialog.text = "Widzę... Więc tak to jest, co? Wiele rzeczy nabiera teraz sensu. Ktoś wygadał się, że złupiliśmy "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Name")+"Gen"))+". "+"Prędzej czy później znajdą ładunek i powieszą mnie. Kapitanie "+pchar.GenQuest.CaptainComission.CapName+" nie żyje, to znaczy, że nikt nie może mi pomóc\nZ wyjątkiem ciebie\n "+GetFullName(pchar)+", wyciągnij mnie stąd, a wynagrodzę ci to.";
		...
		case "CaptainComission_381":
			pchar.GenQuest.CaptainComission.GoodsQty = int(int(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipTypeVictim),"Capacity")) / int(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Weight));
			dialog.text = "Masz rację, upewnij się, że masz wystarczająco dużo miejsca na "+pchar.GenQuest.CaptainComission.GoodsQty+" kawałki "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Gen"))+"Byłoby szkoda stracić taki cenny łup, oblany taką ilością krwi. Nie dadzą nam drugiej szansy.";
		...
		case "CaptainComission_387":
			...
			dialog.text = "Jesteśmy tutaj. Skrytka jest w niszy w tamtej skale. Wejście jest zablokowane kamieniami. W skrytce jest "+pchar.GenQuest.CaptainComission.GoodsQty+" jednostki "+GetStrSmallRegister(XI_ConvertString(Goods[int(pchar.GenQuest.CaptainComission.Goods)].Name+"Generał"))+" Pamiętaj, że połowa należy do mnie?";
		...
		case "ChurchGenQuest1_DialogInTavernWithCap_5":
			dialog.text = "Nie, nie! Ten stary kapitan może być pijany, ale wciąż ma swoje rozumy przy sobie. Księgi i zwoje zostały mi przekazane przez Ojca "+PChar.GenQuest.ChurchQuest_1.ToName+", zabierałem ich do " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.QuestTown + "Acc") + ". A rano poszedłem do lokalnego kościoła i wspomniałem o tych książkach miejscowemu proboszczowi. Oczywiście, chciał je zobaczyć, wiadomo, jak to z nimi bywa, mają fioła na punkcie żywotów świętych i tych wszystkich rzeczy. Więc wysłałem chłopca z kabiny, aby przyniósł skrzynię z nimi. I podczas gdy czekałem, postanowiłem trochę pograć... i jakoś przegrałem wszystkie moje pieniądze. Wszystko! Nie miałem za co zapłacić za trunki! Więc właściciel tawerny wziął książki w zastaw, a potem dałem mu także zwoje...";
		...
		case "ChurchGenQuest1_DialogInTavernWithCap_6":
			dialog.text = "Wiem, wiem... Będę płonął w piekle na wieki wieków. Teraz nie mogę pokazać się ani Ojcu, który powierzył mi tę misję, ani temu, kto dostarczał papiery, ani miejscowemu... Kto teraz pomodli się za moją duszę? A co jeśli mnie wykluczą? O, biada mi... Potrzebuję drinka... ";
				link.l1 = "Ej, spokojnie, nie jest tak źle. Twoje działania były kiepskie i bezbożne, a jednak jestem gotów ci pomóc. Spłacę twój dług i kupię te manuskrypty. Poza tym, skoro znam obu wymienionych księży, popłynę bezpośrednio do " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.QuestTown + "Acc") + "aby dostarczyć tam dokumenty. Zgoda?";
		...
		case "ChurchGenQuest1_DialogShip_1":
			dialog.text = "...i prześcieradła! Musimy także wymienić prześcieradła!.. Och, cześć. Proszę się nie obrażać, panie"+GetSexPhrase("piwnica","s")+" cokolwiek-jak-się-nazywasz, ale teraz, jak widzisz, jestem dość zajęty, więc jeśli masz ze mną interes, proszę, załatw to szybko.";
				link.l1 = "Z tego, co wiem, ojcze "+PChar.GenQuest.ChurchQuest_1.ToName+"dał ci jakieś święte papiery w "+XI_ConvertString("Colony"+PChar.GenQuest.ChurchQuest_1.ToColony+"Voc")+" Obiecałeś je dostarczyć do " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.QuestTown + "Acc") + " bo i tak płynąłeś w tamtym kierunku."; // belamour gen
		...
		{
			dialog.text = "Nie martw się tym, pan"+GetSexPhrase("ie","ienko")+", i podróżuj do " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.QuestTown + "Acc") + "  łagodnie, bo to ja podjąłem się dostarczenia tych dokumentów, i to ja zrobię to za wszelką cenę, by nikt nie krzyczał na każdym rogu, że kapitan "+NPChar.name+"nie dotrzymał słowa!";
		...
		case "ChurchGenQuest1_DialogShip_5_2":
			dialog.text = "Właściwie, chodzi o to, że nie mam z tobą żadnych interesów. Święty Ojciec dał mi zadanie, dostarczyć książki do " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.QuestTown + "Acc") + " i oddaj je miejscowemu księdzu. A ciebie tam nie widzę. Wcale! Powodzenia.";
...
//				link.l1.go = "ChurchGenQuest1_DialogShip_5_2_1"; // Сваливаем, поверили ему
		...
		case "PiratesOnUninhabited_4":
			...
			dialog.text = RandPhraseSimple("Kapitanie, bądź tak dobry i pomóż  "+PiratesOnUninhabited_GetStringNum(int(PChar.GenQuest.PiratesOnUninhabited.PiratesCount))+" nędzarze. Jesteś naszą jedyną nadzieją, poza naszym najłaskawszym Panem.","Kapitanie, czy masz wolne schowki na swoim statku dla "+PiratesOnUninhabited_GetStringNum(int(PChar.GenQuest.PiratesOnUninhabited.PiratesCount))+"biedni ludzie?");
		...
		case "PiratesOnUninhabited_26":
			...
			dialog.text = "Dziękuję, kapitanie "+PChar.name+" . Wypełnił"+GetSexPhrase("eś","aś")+" swoją obietnicę, czas nadszedł "+ChangeNameCase(NAMETYPE_NICK,PChar.GenQuest.PiratesOnUninhabited.MainPirateName,NAME_DAT)+" by spełnić moje. Nie mam przy sobie mapy, ale powiem ci, gdzie jest skarb. Idź do zatoki...";
		...
		case "JusticeOnSale_6":
			...
			dialog.text = "Dziękujemy, kapitanie. Zrekompensujemy ci sumę kaucji oprócz twojej nagrody. Myślę, że powinieneś porozmawiać z komendantem o naszym kapitanie. Być może uda ci się go przekonać. Jak tylko go wyciągniesz, przyjdź do "+XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId+"Dat")+", tam nasz statek zarzucił kotwicę - "+LowerFirst(XI_ConvertString(ShipsTypes[int(PChar.GenQuest.JusticeOnSale.ShipType)].name))+" '"+PChar.GenQuest.JusticeOnSale.ShipName+". Będziemy czekać na ciebie na brzegu."; // belamour gen
			link.l1 = "... -> ";
		...
		case "JusticeOnSale_7":
			...
			dialog.text = "Dziękuję, kapitanie. Wynagrodzimy ci sumę kaucji oprócz twojej nagrody. Myślę, że powinieneś porozmawiać z komendantem o naszym kapitanie. Być może uda ci się go przekonać. Kiedy go wyciągniesz, przyjdź do "+XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId+"Tam")+", tam nasz statek zarzucił kotwicę - "+LowerFirst(XI_ConvertString(ShipsTypes[int(PChar.GenQuest.JusticeOnSale.ShipType)].name))+" '"+PChar.GenQuest.JusticeOnSale.ShipName+"  Będziemy czekać na ciebie na brzegu."; // belamour gen
			link.l1 = "... ";
			link.l1.go = "JusticeOnSale_8";
		break;
		...
		case "Unwantedpostor_room":
			dialog.text = ""+GetSexPhrase("Kim jesteś? I dlaczego wdzierasz się do mego pokoju?","Kim jesteś? To oczywiście mój pokój, ale nie mam nic przeciwko takiemu towarzystwu. Czy to miłość z dostawą do pokoju?")+"";
			link.l1 = ""+GetSexPhrase("Już nie twojego. Karczmarz wynajął mi pokój, a skoro za niego nie płacisz, zaraz stąd wylecisz. Możemy to załatwić po dobroci albo siłą. Wybór należy do ciebie.","Śnisz na jawie. W naszym wypadku to czyszczenie. Więc oczyść to miejsce ze swojej obecności. To teraz mój pokój, bo w przeciwieństwie do ciebie, zawsze płacę karczmarzom.")+"";
		...
		case "Unwantedpostor_room_1":
			...
			{
				dialog.text = " "+GetSexPhrase("Zdaje się, że o tobie słyszałem. Dobra, wygrałeś, odchodzę, chce jeszcze pożyć...","Hmm, wiele o tobie słyszałem. Przypuszczam, że nie tylko twój język jest ostry. Dobrze, nie gniewaj się, pokój jest twój.")+"";
				link.l1 = ""+GetSexPhrase("Podjąłeś dobrą decyzję.","Dobry chłopiec.")+"";
				link.l1.go = "Unwantedpostor_peace";
				break;
			}
			dialog.text = ""+GetSexPhrase("Czyli tak to wygląda, co? Cóż, wybieram trudną drogę.","Ty suko! Chciałem z tobą postąpić po ludzku...")+" ";
			link.l1 = ""+GetSexPhrase("Jak sobie życzysz...","Żal mi ciebie. Chociaż, może nie.")+"";
		...
		case "Unwantedpostor_Win":
			dialog.text = ""+GetSexPhrase("Dobrze, dobrze, przestań! Nie zabijaj mnie. Odchodzę.","Stój! Co mamy teraz zrobić, zabić się nawzajem o ten pokój?! Udław się nim, cały twój. Cwaniaro...")+"";
			link.l1 = ""+GetSexPhrase("Rozsądna decyzja. Następnym razem pomyśl głową: nie każdy jest tak łaskawy jak ja, mogą cię niechcący zabić.","Co za ludzie...")+"";
		...
		case "Unwantedpostor_Lose":
			dialog.text = ""+GetSexPhrase("I co, wyrzuciłeś mnie? Wynoś się stąd, zanim cię posiekam.","Jedyne, co potrafisz, to paplać. Ciesz się, że wciąż jestem w dobrym humorze, bo inaczej zrobiłbym ci coś jeszcze gorszego. Wynocha stąd!")+"";
			link.l1 = ""+GetSexPhrase("Cholera...","Cholera, niedoceniłam cię... Dobra, dobra, już wychodze.")+" ";
		...
		case "ExpertSailor_1_1":
			...
			{
				link.l2 = "Umowa stoi. Nie mam nic przeciwko tysiącu pesos za dobrego specjalistę. Poczekaj... prosze. Będe czekał na ciebie na statku.";
		...
		case "NightAdventure_Sailor_2_2":
			dialog.text = "Tam... w karczmie... hic!.. Szuler!";
			link.l1 = "Cóż, co w tym złego? W każdej karczmie każdej kolonii są szulerzy, jak błoto.";
		...
		case "NA_SailorTavern_1":
			...
			if(GetDataDay() >= 7  && GetDataDay() < 13) {GiveItem2Character(pchar, "slave_01");  Log_Info("Otrzymałeś harpun");}
			if(GetDataDay() >= 13 && GetDataDay() < 19) {GiveItem2Character(pchar, "potion5");   Log_Info("Otrzymałeś korzeń imbiru");}
			if(GetDataDay() >= 19 && GetDataDay() < 25) {GiveItem2Character(pchar, "cartridge"); Log_Info("Otrzymałeś nabój papierowy");}
			if(GetDataDay() >= 25) 
			{
				if(GetDataDay() == 25) {GiveItem2Character(pchar, "amulet_8");  Log_Info("Otrzymałeś amulet 'Kotwica'");}
				if(GetDataDay() == 26) {GiveItem2Character(pchar, "amulet_9");  Log_Info("Otrzymałeś amulet 'Encolpion'");}
				if(GetDataDay() == 27) {GiveItem2Character(pchar, "obereg_7");  Log_Info("Otrzymałeś amulet 'Rybak'");}
				if(GetDataDay() >  27) {GiveItem2Character(pchar, "obereg_11"); Log_Info("Otrzymałeś amulet 'Pielgrzym'");}
			}
		...
		case "NA_SailorGamblerBelka_1":
			...
			link.l1 = "Cóż, teraz już nic nie możesz zrobić. Na pewno nie będę go szukał w nocy po całej kolonii. Nie pij już w ten sposób. Albo przynajmniej nie graj będąc pijanym.";
		...
		case "NA_SailorShip_3":
			...
			link.l2 = "Żebym to ja płacił, a ty się upijasz i potem szukam cię po wszystkich karczmach i zaułkach kolonii? Nie, nie potrzebuję takiego szczęścia.";
		...
		case "NightAdventure_Bull":
			switch (rand(5))
			{
				...
				link.l1 = "Odrzut cie zabije. Wynoś się! Wybij mi zęby, jeśli śmiesz...";
				link.l1.go = "NightAdventure_End";
				break;
		...
		case "NightAdventure_Citizen_1_2":
			dialog.text = "Mój przyjaciel... on, ogólnie... upił się. Po prostu ogólnie... hic! ... upił się...";
			link.l1 = "Oczywiście, wytrzeźwiałem marynarzy, którzy pili na służbie, ale dość radykalnymi metodami. Nie sądzę, żeby twój przyjaciel tego potrzebował.";
		...
		case "CemeteryMan_1":
			...
			link.l1 = "Kwiaty z grobów? Cóż, jak się ma zielnik?";
			link.l1.go = "CemeteryMan_2";
		break;
		
		case "CemeteryMan_2":
			dialog.text = "Ojej... Czy to ma związek z ziołami?";
			link.l1 = "Hmm... Chyba rozumiem, co się tu dzieje.";
		...
		case "CemeteryMan_4":
			dialog.text = "Cóż, my... Mój ojciec zabrania nam się spotykać! I jej ojciec także! Więc ... szukaliśmy miejsca bez świadków. Być samemu. Zamek w drzwiach i tak jest zepsuty, więc łatwo tu wejść...";
			link.l1 = "Teraz rozumiem, czym były te przerażające jęki niespokojnej duszy...";
		...
		case "CemeteryMan_6":
			...
			link.l1 = "Nie boisz się umawiać na randki w krypcie? To niezbyt najlepsze miejsce...";
		...
		case "CemeteryMan_8":
			dialog.text = "Cóż... Straszenie strażnika nie było częścią naszego planu. Ale gdzież indziej możemy się spotkać? Czy sugerujesz, że mamy nasze randki pod otwartym niebem?";
		...
		case "CemeteryMan_9":
			dialog.text = "Dobra, coś wymyślimy... Nie będziemy już straszyć strażnika; daję ci moje słowo.";
