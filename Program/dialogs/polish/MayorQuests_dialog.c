void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "DestroyGang_begin":
			...
			dialog.text = LinkRandPhrase("Płacisz myto "+GetSexPhrase("przyjacielu","dziewczyno")+"! Nazywam się "+GetFullName(npchar)+", i nie jestem przyzwyczajony do sprzeciwów...","Teraz pokaż mi swoją sakiewkę, "+GetSexPhrase("przyjacielu","dziewczyno")+", i to prędko! Moje imię to  "+GetFullName(npchar)+", i mam nadzieję, że o mnie słyszałeś...","Daj mi wszystko co masz cennego, również zawartość twojej sakiewki. I pospiesz się, cierpliwość nie jest moją mocną stroną. Nie mogę powiedzieć tego samego o mojej żądzy krwi!");
			Link.l1 = LinkRandPhrase("Heh, więc ty jesteś tym słynnym bandytą "+GetFullName(npchar)+", o którym miejscowy gubernator "+sTemp+" nigdy nie przestaje mówić?","Ach, więc ty jesteś tym bandytą, którego ściga miejscowy gubernator "+sTemp+"?!","Cieszę się"+GetSexPhrase("","")+", że cie widzę, "+GetFullName(npchar)+". Lokalny gubernator, "+sTemp+" mówi tylko o tobie.");
			Link.l1.go = "DestroyGang_1";
		break;		
		case "DestroyGang_1":
			dialog.text = LinkRandPhrase("Tak, jestem dobrze znany w tych stronach, he-he... Czekaj, czy ty nie jesteś aby kolejnym "+GetSexPhrase("brudnym łajdakiem","brudną suką")+" gubernatora, którego wysłał, aby mnie wytropić?","Gubernator to mój dobry przyjaciel, to prawda. A czy ty nie przypadkiem jesteś, "+GetSexPhrase("kolejnym bohaterem, wysłanym","kolejną bohaterką, wysłaną")+" za moją głowę?","Gubernator to mój najlepszy kumpel, to nie jest tajemnicą, he-he. Ale skąd ty to wiesz? Może on cię po mnie wysłał?");
		...
		break;
		case "DestroyGang_2":
			dialog.text = LinkRandPhrase("Tak lepiej, "+GetSexPhrase("kumplu","dziewczyno")+"...A teraz znikaj!","I to jest słuszna decyzja. Chciałbyś wiedzieć, ilu bohaterów wysłałem do innego świata... Dobrze. Spadaj, "+GetSexPhrase("ty spierdolino","suko")+"!",""+GetSexPhrase("Grzeczny chłopiec","Grzeczna dziewczyna")+"! Bardzo mądra decyzja - nie wtrącać się w sprawy innych... No dobrze, znikaj już, "+GetSexPhrase("kolego","dziewczyno")+".");
			Link.l1 = "Żegnaj i powodzenia...";
		...
		case "Tempsailor":
			...
			link.l1 = "Świetnie! Gdzie on teraz jest?";
			link.l1.go = "Tempsailor_1";
		break;
		
		case "Tempsailor_1":
			dialog.text = "W naszej ładowni, tak jak nam kazałeś.";
		...
		case "Fugitive_city": // ходит по городу
			...
			link.l1 = "Prosze prosze... Więc to ty jesteś "+pchar.GenQuest.FindFugitive.Name+", czyż nie? Bardzo się cieszę, że cię widzę...";
			link.l1.go = "Fugitive_city_1";
		break;
		
		case "Fugitive_city_1":
			dialog.text = "Cóż, to ja, rzeczywiście, choć nie wiem, dlaczego cieszy cię mój widok? Wydaje mi się to dziwne, ponieważ nigdy wcześniej cię nie widziałem... Mógłbyś się tłumaczyć?";
			link.l1 = "Ależ oczywiście. Przybyłem z "+XI_ConvertString("Colony"+pchar.GenQuest.FindFugitive.Startcity+"Gen")+", na rozkaz miejscowego gubernatora. Jesteś aresztowany i muszę cię dostarczyć do tego samego miasta... O, i ręce z dala od szabli! Nie próbuj niczego głupiego dobry człowieku, bo skończy się źle dla ciebie!";
		...
		case "Fugitive_city_gift":
			...
			Log_Info("Otrzymałeś bursztyn");
			dialog.text = "Prosze... I mam nadzieję, że nigdy więcej nie zobaczę ani ciebie, ani innych 'wysłanników'.";
		...
		case "Fugitive_shore_1":
			...
			link.l1 = "Muszę cię przyprowadzić do "+XI_ConvertString("Colony"+pchar.GenQuest.FindFugitive.Startcity)+", miejscowy gubernator nie może się doczekać, aby cię zobaczyć. Bez głupich sztuczek, proszę! Oddaj swoją broń i chodź za mną!";
		...
		case "Fugitive_shore_gift":
			...
			PlaySound("interface\important_item.wav");
			Log_Info("Otrzymałeś perły");
			dialog.text = "Prosze... I mam nadzieję, że nigdy więcej nie zobaczę ani ciebie, ani innych 'posłańców'.";
			link.l1 = "Zapewniam cię, że nie. Do widzenia, panie!";
			link.l1.go = "Fugitive_gift_exit";
		break;
		...
		case "Fugitive_tavern": // в таверне
			pchar.quest.AllMayorsQuests_Late.over = "yes"; //снимаем общий таймер
			dialog.text = "Ehhh... hic! Panie, nie szukam towarzystwa - szczególnie twojego. Spadaj!";
			link.l1 = "Ale JA szukam twojego towarzystwa, "+pchar.GenQuest.FindFugitive.Name+"! I będziesz tolerować moje towarzystwo w ładowni mojego statku. Udajemy się do  "+XI_ConvertString("Colony"+pchar.GenQuest.FindFugitive.Startcity)+", do gubernatora miasta. Gubernator wręcz umiera z chęci aby cię zobaczyć.";
			link.l1.go = "Fugitive_tavern_1";
		break;
		
		case "Fugitive_tavern_1":
			dialog.text = "H-Hic!"+RandSwear()+" Więc jednak mnie dogonił! Słuchaj, kolego, nie wiesz co się stało, nie byłeś tam! Nie mogłem zostać na służbie, po prostu nie mogłem! Nadal co wieczór upijam się, żeby zapomnieć o tym\nPosłuchaj, zawrzyjmy umowę. Powiedz mu, że nie udało ci się mnie znaleźć lub że uciekłem z piratami na otwarte morze. W zamian oddam ci wszystkie samorodki, które znalazłem w lokalnej jaskini. To wszystko, co mam, widzisz, oddaję ci wszystko, tylko żeby nigdy więcej nie zobaczyć "+XI_ConvertString("Colony"+pchar.GenQuest.FindFugitive.Startcity)+"...";
