	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				link.l2 = RandPhraseSimple("Mam do ciebie pytanie.","Potrzebuję informacji.");
				link.l2.go = "quests";//(перессылка в файл города)
				link.l3 = "To nic, jestem w drodze do karczmy...";
				link.l3.go = "exit";
				npchar.quest.meeting = "1";
			}
...
//----------------------------------------захват пассажиров для выкупа------------------------------------------
			...
			case "passenger_1":
				dialog.text = "Są, kapitanie, są. To kosztuje znacznie więcej, niż proszę. Daj mi złoto, a powiem ci, kiedy i na którym statku popłynie pewna bardzo ważna osoba. Będziesz mógł ją wykupić za bardzo dobrą cenę... Nawet powiem ci imię kupca. Umowa stoi?";
			...
			case "passenger_2":
				...
				dialog.text = "Byłoby bardzo nierozsądnie oszukiwać cię, wilku morski. Mieszkam w tym mieście i nie potrzebuję kłopotów. Za jedyne "+int(pchar.GenQuest.Marginpassenger.Dublon)+" doubloonów dostarczę pełne informacje. Zarobisz znacznie więcej.";
			...
			case "passenger_4"://установка параметров
				...
				dialog.text = "Trzymaj proch suchy, wkrótce będzie ci potrzebny, kapitanie. Teraz słuchaj, w "+FindRussianDaysString(int(pchar.GenQuest.Marginpassenger.Days))+"  statek odpłynie z naszej kolonii, statek jest "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Marginpassenger.ShipType),"Name")))+" nazwany '"+pchar.GenQuest.Marginpassenger.ShipName+" i będzie zmierzać do "+XI_ConvertString("Colony"+pchar.GenQuest.Marginpassenger.Targetcity+"Gen")+". Będzie pasażer - "+pchar.GenQuest.Marginpassenger.Text+", nazwany "+pchar.GenQuest.Marginpassenger.q1Name+"\nKapitan "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Marginpassenger.ShipType),"Name")+"Gen"))+" to żółtodzioby tchórz jak wszyscy handlarze, ha-ha-ha! Może nawet unikniesz walki. Wystarczy, że napełnisz go strachem przed diabłem, a sam odda cel, he-he\nJak rozumiesz, "+pchar.GenQuest.Marginpassenger.Text1+", prawdopodobnie dostaniesz swoje monety, tak, ale lokalne władze będą cię ścigać na śmierć po tym. Lepiej przynieś swoją zdobycz do "+XI_ConvertString("Colony"+pchar.GenQuest.Marginpassenger.Targetcity)+"Znajdziesz tam człowieka o imieniu "+pchar.GenQuest.Marginpassenger.q2Name+"\n"+pchar.GenQuest.Marginpassenger.Text2+" i nawet nie będziesz musiał podawać swojego imienia. To twoja decyzja, gdzie wykupić swojego człowieka, ale nie bierz na siebie zbyt wiele... Ale "+int(pchar.GenQuest.Marginpassenger.Dublon)*4+"  dublonów możesz zażądać na pewno. Zrozumiano?";
				link.l1 = "Tak. Wrócę na mój statek i zapiszę to w dzienniku.";
				link.l1.go = "passenger_5";
			break;
		
			case "passenger_5":
				dialog.text = "Dokładnie! Słuszny wybór. Dobrze, kapitanie, powodzenia w tym łatwym zadaniu. Ach tak, prawie zapomniałem: "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.Marginpassenger.ShipType),"Name")))+"popłynie nie z portu, lecz z jednej z pobliskich zatok. Uważaj na to. Powodzenia i dzięki za złoto!";
...
void SelectMarginpassengerParameter()
{
	if (hrand(1) == 0)
	{
		...
		pchar.GenQuest.Marginpassenger.Text2 = "Chce się z nią ożenić i zapłaci za dziewczynę znaczną sumę.";
		switch (hrand(4))
		{
			case 0: 
				pchar.GenQuest.Marginpassenger.Text = "córka sklepikarza";
				pchar.GenQuest.Marginpassenger.Text1 = "od jej ojca, właściciela sklepu";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_store";
			break;
			case 1: 
				pchar.GenQuest.Marginpassenger.Text = "córka szkutnika";
				pchar.GenQuest.Marginpassenger.Text1 = "od jej ojca, szkutnika";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_shipyard";
			break;
			case 2: 
				pchar.GenQuest.Marginpassenger.Text = "córka karczmarza";
				pchar.GenQuest.Marginpassenger.Text1 = "od jej ojca, karczmarza";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_tavern";
			break;
			case 3: 
				pchar.GenQuest.Marginpassenger.Text = "córka bankiera";
				pchar.GenQuest.Marginpassenger.Text1 = "od jej ojca, bankiera";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_bank";
			break;
			case 4: 
				pchar.GenQuest.Marginpassenger.Text = "córka nadzorcy portu";
				pchar.GenQuest.Marginpassenger.Text1 = "od jej ojca, nadzorcy portu";
	...
	else
	{
		...
		pchar.GenQuest.Marginpassenger.Text2 = "Zaplanował, jak przypodobać się tej rodzinie i zapłaci za więźnia wysoką cenę ";
		switch (hrand(4))
		{
			case 0: 
				pchar.GenQuest.Marginpassenger.Text = "krewny kupca, szkutnika";
				pchar.GenQuest.Marginpassenger.Text1 = "od jego krewnego, szkutnika";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_store";
			break;
			case 1: 
				pchar.GenQuest.Marginpassenger.Text = "krewny szkutnika";
				pchar.GenQuest.Marginpassenger.Text1 = "od jego krewnego, szkutnika";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_shipyard";
			break;
			case 2: 
				pchar.GenQuest.Marginpassenger.Text = "krewny karczmarza";
				pchar.GenQuest.Marginpassenger.Text1 = "od jego krewnego, karczmarza";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_tavern";
			break;
			case 3: 
				pchar.GenQuest.Marginpassenger.Text = "krewny bankiera";
				pchar.GenQuest.Marginpassenger.Text1 = "od jego krewnego, bankiera";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_bank";
			break;
			case 4: 
				pchar.GenQuest.Marginpassenger.Text = "krewny nadzorcy portu";
				pchar.GenQuest.Marginpassenger.Text1 = "od jego krewnego, nadzorcy portu";
				pchar.GenQuest.Marginpassenger.loc = pchar.GenQuest.Marginpassenger.City+"_portoffice";
			break;
		}
	}
