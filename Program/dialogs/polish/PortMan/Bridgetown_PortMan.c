    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Może zauważyłeś, kapitanie, że miasto jest oblężone. Spodziewamy się ataku ze strony Hiszpanów w każdej chwili i mobilizujemy nasze siły. Ale regaty nadal trwają\nZarejestrujmy się: kapitanie "+GetFullName(pchar)+", statek jest - "+pchar.Ship.Name+"... Czas regat w godzinach to "+int(pchar.questTemp.Regata.FourthTransitionTime)+"Zrobione, twój wynik jest zarejestrowany, możesz kontynuować swoją drogę.";
		...
		case "caleuche":
			...
			link.l5 = "Kłopot w tym, że nie znam nazwy jego starej łajby. To dziwne, jakaś ptasia kobieta, urodzona...";
		...
		case "caleuche_2":
			...
			link.l4 = "Furia."; 
			link.l4.go = "caleuche_wrong_name";
			link.l5 = "Muszę się jeszcze zastanowić.";
			link.l5.go = "exit";
		break;
