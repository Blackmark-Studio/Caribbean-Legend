...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			dialog.text = "Zarejestrujmy cię: Kapitanie "+GetFullName(pchar)+", statek jest "+pchar.Ship.Name+"... Czas regat w godzinach to "+int(pchar.questTemp.Regata.SecondTransitionTime)+"Zrobione, twój wynik jest zarejestrowany, możesz kontynuować swoją drogę.";
