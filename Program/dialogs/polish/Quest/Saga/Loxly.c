...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
				...
				if (CheckAttribute(pchar, "questTemp.Saga"))
				{
					...
					if (pchar.questTemp.Saga == "removebakaut") // с бакаутом разобрались
					{
						...
						if (CheckAttribute(pchar, "questTemp.Saga.Late"))
						{
							...
							dialog.text = "Panie, dlaczego pracowałeś tak wolno?! Dla twego mocodawcy wszystko przepadło. Okres ważności testamentu minął i Isla Tesoro teraz należy do Anglii. Wkrótce stanie się bazą wojskową.";
							link.l1 ="O rany! Wygląda na to, że wszystkie moje wysiłki poszły na marne...";
							link.l1.go = "saga_l3";
						}
						else
						{
					...
					if (pchar.questTemp.Saga == "shadow")
					{
						...
						if (CheckAttribute(pchar, "questTemp.Saga.Late"))
						{
							dialog.text = "Panie, dlaczego pracowałeś tak wolno?! Wszystko już jest skończone dla twego zleceniodawcy. Okres ważności testamentu wygasł i Isla Tesoro teraz należy do Anglii. Wkrótce stanie się bazą wojskową.";
							link.l1 ="Wow!  Wydaje się, że wszystkie moje wysiłki poszły na marne...";
							link.l1.go = "saga_l3";
						}
						else
						{
		...
		case "saga":
			...
			{
				RemoveItems(pchar, "map_sharp_full", 1);
				dialog.text = "Sir, musiałeś za długo być na morzu. Isla Tesoro to teraz baza wojskowa angielskiej floty! Tak, testament Sharpa istnieje, ale wygasł i Isla Tesoro stało się własnością angielskiej korony. Niestety!";
				link.l1 ="Wow!  Wydaje się, że wszystkie moje wysiłki poszły na marne...";
				link.l1.go = "saga_l3";
			}
			else
		...
		case "saga_23":
			...
			if (CheckAttribute(pchar, "questTemp.Saga.Late"))
			{
				dialog.text = "Panie, zachwiałeś mną! Zrobiłeś tak wiele, wyspa była prawie w naszych rękach, ale teraz wszystko przepadło przez twoje opóźnienie z moją opłatą! Testament wygasł i Isla Tesoro należy teraz do angielskiej korony. To będzie baza wojskowa.";
				link.l1 ="O rany! Wygląda na to, że wszystkie moje wysiłki poszły na marne...";
				link.l1.go = "saga_l3";
			}
			else
	...
	//-------------------------------------- адвокатские услуги ------------------------------------------------
		...
		case "contraband":
			...
			dialog.Text = " I dlaczego to zrobiłeś? Przemytnicy to dobrzy chłopcy, są uczciwi na swój sposób. Wszyscy musimy żyć i jeść... Dobrze, to nie jest wielka sprawa i będzie kosztować cię tylko "+FindRussianMoneyString(iLoxlyPseudoGlobal)+".";
			if(sti(Pchar.money) >= iLoxlyPseudoGlobal)
			{
				Link.l1 = "Zgoda, panie Loxley, zgadzam się. Oto twoje pieniądze i postaraj się załatwić sprawy jak najszybciej.";
