...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(CheckAttribute(pchar, "questTemp.Saga"))
			{
				...
				if(pchar.questTemp.Saga == "calendar") // квесты баронов завершены
				{
					...
					if (CheckAttribute(pchar, "questTemp.Saga.Late"))
					{
						dialog.text = "Słyszałem o twoich bohaterskich czynach! Dannie opowiedział mi o nich pokrótce. A przy okazji, teraz jesteśmy przyjaciółmi. I Nathan zupełnie się zmienił! Cała jego depresja zniknęła, gdy ujrzał swojego 'Centuriona'.\nAle mam dla ciebie złe wieści, "+pchar.name+"Zmarnowaliśmy tyle czasu, że prawa dziedziczenia testamentu Sharpa dawno przepadły, a Isla Tesoro stała się angielską bazą wojskową. Nie mogę już nic zrobić, aby pomóc ci z Levasseurem.";
						link.l1 ="Jestem absolutnie oniemiały... Przeszliśmy przez tyle kłopotów na nic. Czy naprawdę nie ma nic, co możemy zrobić?";
						link.l1.go = "saga_l2";
					}
					else
					{
		...
		case "saga_55":
			...
			link.l1 = "Znalazłem "+FindRussianQtyString(int(npchar.quest.bakaut))+" na 'Ostrydze'.";
			link.l1.go = "saga_56";
		break;
		...
		case "saga_60":
			...
			link.l1 = "Ostatecznie, mam dwa tysiące złota dla Loxley'a i "+int(npchar.quest.bakaut_sum)*2+"   doublony na dodatek.";
			link.l1.go = "saga_61";
		break;
		...
		case "saga_61":
			...
			if (PCharDublonsTotal() > 0)
			{
				...
				link.l1 = "Oto i masz. Mam "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
		...
		case "saga_61_1":
			...
			{
				npchar.quest.bakaut_pay = func_min(int(npchar.quest.bakaut_sum), PCharDublonsTotal()); // дублоны в кармане
				link.l1 = "Proszę bardzo. Mam "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
				link.l1.go = "bakaut_pay";
			}
			else
			{
		...
		case "bakaut_pay": // оплата
			...
			{
				dialog.text = "Świetnie. Czy przyniesiesz pozostałe "+FindRussianQtyString(int(npchar.quest.bakaut_sum))+"  dublonów później?";
