...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "LH_abbat_35":
			string sTemp;
			if (int(pchar.questTemp.LongHappy.Mistake) > 1) sTemp = "(Szeptem) Charles, mój synu, proszę cię, tylko ruszaj ustami, nie próbuj wydawać żadnego dźwięku...";
			else sTemp = "";
		...
		case "SharlieEpilog_Benua_Delivery":
			...
			if (!CheckAttribute(pchar, "questTemp.SharlieEpilog_Patent") && CheckCharacterItem(pchar, "patent_fra") || CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && !CheckAttribute(pchar, "questTemp.SharlieEpilog_Governor"))
			{
				if (CheckAttribute(pchar, "questTemp.Patria.GenGovernor"))
				{
					link.l3 = "Chcę powiadomić barona Forgera oraz kancelarię Capsterville, że jestem zmuszony zrezygnować ze stanowiska gubernatora generalnego.";
					link.l3.go = "SharlieEpilog_Benua_Governor";
				}
				else
				{
					link.l3 = "Posiadam ważny francuski patent — chcę poinformować kancelarię Capsterville o swojej decyzji.";
			...
			{
				link.l6 = "Muszę wysłać wiadomość do Fadeja Moskiewity z Bas-Ter.";
			...
			{
				link.l7 = "Chcę poinformować rodzinę Houków z Maroon Town o mojej podróży.";
				link.l7.go = "SharlieEpilog_Benua_Houk";
			}
			if (!CheckAttribute(pchar, "questTemp.SharlieEpilog_Jino") && CharacterIsAlive("Jino"))
			{
				link.l8 = "Chcę powiadomić Gino Guinelego z Saint John's o wyjeździe do Starego Świata.";
		...
		case "SharlieEpilog_Benua_Money_4":
			dialog.text = "W takim razie darowizna w wysokości stu tysięcy peso będzie wystarczająca. Dzięki tym środkom będziemy mogli organizować posiłki dla potrzebujących jeszcze przez wiele miesięcy. Jeśli masz przy sobie wymaganą kwotę, wraz z datkiem dla Kościoła, możemy rozpocząć od razu.";
		...
		case "SharlieEpilog_Benua_Money_5":
			dialog.text = "W takim razie twoja darowizna powinna wynieść dwieście pięćdziesiąt tysięcy peso. Środki te pozwolą nam zbudować sierociniec, który będzie rozsławiał twoje imię i przynajmniej przez pewien czas zapewni jego utrzymanie. Jeśli masz przy sobie wymaganą kwotę, wraz z datkiem dla Kościoła, możemy rozpocząć od razu.";
		...
		case "SharlieEpilog_Benua_Money_6":
			dialog.text = "Cóż, wygląda na to, że nie traciłeś czasu, "+pchar.name+". Myślę, że darowizna w wysokości pięciuset tysięcy peso będzie w pełni wystarczająca. Dzięki takiemu wkładowi będziemy mogli zbudować szpital i zapewnić mu wszystko, co potrzebne, na długie lata. Jeśli masz przy sobie wymaganą kwotę, wraz z datkiem dla Kościoła, możemy rozpocząć od razu.";
		...
		case "SharlieEpilog_Benua_Governor":
			dialog.text = "Szczerze mi żal, "+pchar.name+", że dla spotkania z ojcem musisz porzucić tak wysokie stanowisko. Mimo to pocieszające jest widzieć, że dla ciebie rodzina stoi ponad wszystkim.";
			link.l1 = "Dręczy mnie to, że przez cały czas mojego pobytu tutaj, na Karaibach, nie zadałem sobie trudu, by wysłać mu choć jeden list; zamierzam więc naprawić to osobistą wizytą. Choćby mój sukces był najbardziej zawrotny, nie wolno mi zapominać o obowiązku wobec ojca. Co do barona — sądzę, że zdoła wybaczyć mi tak nagłe odejście: w końcu uczyniłem wiele dla dobra naszego wspólnego państwa.";
			link.l1.go = "SharlieEpilog_Benua_Governor_2";
			pchar.questTemp.SharlieEpilog_Governor = true;
		break;

		case "SharlieEpilog_Benua_Governor_2":
			dialog.text = "Dokonałeś trudnego, lecz godnego wyboru, mój synu. Władza i zaszczyty są przemijające, lecz obowiązek wobec rodziny i rodu jest wieczny. Uwierz mi, Pan widzi twoje intencje i nagradza nie tytuły, lecz wierność serca.";
			link.l1 = "Dziękuję, ojcze. Pańskie słowa wiele dla mnie znaczą.";
		...
		case "SharlieEpilog_Benua_Governor_4":
			dialog.text = "Doskonale. Podejmę wszelkie kroki, aby listy zostały dostarczone w jak najkrótszym czasie.";
			link.l1 = "...";
			link.l1.go = "SharlieEpilog_Benua_Delivery";
