	...
	switch(Dialog.CurrentNode)
	{
		...
		case "norman_fes":
			...
			switch (hrand(5))
			{
				...
				case 5:
					...
					link.l1 = "Oto jest pytanie! Czy zniosimy tę zniewagę i ruszymy dalej...";
		...
		case "norman_fes0_exit":
			...
			else
			{
				if (GetDataDay() == 23 && GetDataMonth() == 2)
				{
					...
					Log_Info("Otrzymałeś Muszkiet z Zamkiem Zapłonowym");
				}
				else
				{
					GiveItem2Character(pchar, "cartridge");
					Log_Info("Otrzymałeś Nabój Papierowy");
				}
			}
		break;
		...
		case "norman_fes1_exit":
			...
			if (GetDataDay() == 13)
			{
				...
				Log_Info("Otrzymałeś Złoty Dublon");
			}
			else
			{
				if (GetDataDay() == 1 && GetDataMonth() == 4)
				{
					...
					Log_Info("Otrzymałeś Skrzynie z Dublonami");
				}
				else
				{
		...
		case "norman_fes2_exit":
			...
			if (GetDataDay() == 14)
			{
				...
				Log_Info("Otrzymałeś Amulet 'Cudotwórca'");
			}
			else
			{
				if (GetDataDay() == 25 && GetDataMonth() == 12)
				{
					...
					Log_Info("Otrzymałeś Talizman Wspólnika");
				}
				else
				{
					GiveItem2Character(pchar, "Mineral3");
					Log_Info("Otrzymałeś świecę woskową");
				}
			}
		break;
		
		case "norman_fes3_fight":
			dialog.text = "Wszyscy do mnie! Złapałem szpiega, który wygląda dokładnie jak mój syn! Jestem pewien, że Thomas go przysłał, żeby wybić mi drugie oko!";
		...
		case "norman_fes3_exit":
			...
			else
			{
				if (GetDataDay() == 19 && GetDataMonth() == 6)
				{
					...
					Log_Info("Otrzymałeś Amulet 'Skorpion'");
				}
				else
				{
					GiveItem2Character(pchar, "potion1");
					Log_Info("Otrzymałeś miksture leczniczą")
				}
			}
		break;
		...
		case "norman_fes4_exit":
			...
			if (GetDataDay() == 17)
			{
				...
				Log_Info("Dostałeś skórzaną kurtkę");
			}
			else
			{
				if (GetDataDay() == 1 && GetDataMonth() == 1)
				{
					...
					Log_Info("Otrzymałeś kirys paradny");
				}
				else
				{
					GiveItem2Character(pchar, "Mineral18");
					Log_Info("Dostałeś trochę tytoniu");
				}
			}
		break;
		...
		case "norman_fes5_fight":
			...
			link.l1 = "Karamba! Ten twój kawaleryjski miecz to wcale nie jest atrapa!";
			link.l1.go = "fight";
			NextDiag.TempNode = "norman_fight_again";
		break;
		...
		case "norman_fes5_exit":
			...
			if (GetDataDay() == 16)
			{
				...
				Log_Info("Otrzymałeś butelkę rumu");
			}
			else
			{
				if (GetDataDay() == 28 && GetDataMonth() == 11)
				{
					...
					Log_Info("Otrzymałeś Talizman 'Gardło z Żeliwa'");
				}
				else
				{
					GiveItem2Character(pchar, "Mineral17");
					Log_Info("Otrzymałeś szklaną butelkę");
				}
			}
		break;
		...
		case "tieyasal":
			...
			link.l2 = "Co masz na myśli, mówiąc po co mi on jest? Miguel to mój stary druh, można rzec przyjaciel. On ma...";
			link.l2.go = "tieyasal_2";
			link.l3 = "Chcę, żeby przyholował dla mnie nową łajbę, tak jak to kiedyś zrobił dla Sharka Dodsona.";
		...
		case "tieyasal_7":
			dialog.text = "Słuchaj, "+pchar.name+". Dichoso zabił biednego Blaze'a. Widziałem to na własne oczy, tak-tak. Ale oni mi nie wierzą. Widzisz, myślą, że jestem szalony. Jeden z nich, ale szalony. A ja byłem najlepszym abordażystą w załodze samego Nicolasa Sharpa! Dobre czasy! Ale w końcu straciłem swoje pazury, kopyto i soczewkę i zostałem uziemiony.\nCo? Ach, racja! Wszyscy myśleli, że to wina Sharka, ale tak nie było. Nie-nie-nie. Był tam ząb rekina, talizman Dodsona, ale to Dichoso go tam podłożył.\nMusiał go ukraść Sharkowi, gdy razem pływali na fregacie zwanej 'Fortuna'. Tego dnia źle spałem i wyszedłem na spacer. Widziałem, jak Dichoso zakradł się do rezydencji Sharpa, słyszałem, jak ktoś krzyczał, a potem jęczał. Wkrótce po tym, jak Dichoso opuścił rezydencję, jego ręce były we krwi.\nNie uwierzyli mi, nie-nie. Bo tego dnia nie było Dichoso na wyspie, ale był Shark. I znaleźli jego ząb. Ale widziałem Dichoso, jestem tego pewien. Nikt nie wiedział, że tam był tego dnia. 'Szczęśliwy' to bardzo przebiegły i budzący strach mężczyzna...";
		...
		case "tieyasal_8":
			...
			link.l1 = "Miguel Szczęśliwy?... Hm. To mi coś przypomina. Hm. Ta fraza brzmi tak znajomo...";
		...
		case "tieyasal_11":
			...
			link.l1 = "To nie jest język Indian. To łacina. Łacina... Łacina?! Jasny gwint!";
			link.l1.go = "tieyasal_12";
		break;
		
		case "tieyasal_12":
			dialog.text = "Czemu taki nerwowy, człowieku? Czemu krzyczysz? Ugryzł cię wąż czy co? Gdzie się podział?";
			link.l1 = "Czemu tego nie zauważyłem na początku! Wszystko było tuż przed moim nosem! Łacina! Miguel... Miguel Szczęśliwy!!!";
		...
		case "tieyasal_17":
			dialog.text = "Cóż, zdecydowanie nie po to, by odwiedzić karczme i sączyć rum z tobą. On zamierzał cię pojmać. Tak, tak! Pojmać cię! Jesteś w głębokim bagnie, kapitanie!";
		...
		case "tieyasal_22":
			...
			Log_Info("Otrzymałeś miecz Barbarigo");
			PlaySound("interface\important_item.wav");
