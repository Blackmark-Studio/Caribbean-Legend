...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "prepare_convoy_quest_3":
			...
			dialog.text = "Muszę być eskortowany do "+XI_ConvertString("Colony"+pchar.quest.destination+"Gen")+", który znajduje się na "+XI_ConvertString(GetIslandByCityName(pchar.quest.destination)+"To")+", w "+FindRussianDaysString(int(pchar.ConvoyQuest.iDay))+", a za to zapłacę ci "+FindRussianMoneyString(int(pchar.ConvoyQuest.convoymoney))+". Więc, twoja decyzja?";
