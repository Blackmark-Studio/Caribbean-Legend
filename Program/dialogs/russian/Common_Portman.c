...
// <-- эскорт
		      ...
		      /* if (rand(3) <= 2)
			  {		
			    if (ChangeCharacterNationReputation(pchar, int(NPChar.nation), 0) <= -20)
			    {
...
//			if(rand(1) == 0) PChar.GenQuest.ChurchQuest_1.CapWaitOnTavern = true;
		...
		case "ShipLetters_Portman1_1":
			...
			dialog.text = s1 + "Скажем, " + int(pchar.questTemp.different.GiveShipLetters.price1) + " песо Вы сочтёте достойной оплатой?";
...
///Jason-----------------------новые генераторы накопительного типа для портмана----------------------------
		case "Work_check"://фейс-контроль - общая проверка профпригодности ГГ и выбор задания
			...
			if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") >= 2)//проверка повтора
				{
					...
					{
						dialog.text = RandPhraseSimple("Здесь нет работы для тех, кто ходит под флагом " + NationNameGenitive(int(pchar.nation)) + "! Покиньте мой кабинет немедленно!", "Я не намерен сотрудничать с " + NationNameAblative(int(pchar.nation)) + ". Вон отсюда!");
					...
					link.l2 = "Я хочу подзаработать эскортом купцов.";
					Link.l2.go = "escort_begin";	//эскорт
					link.l3 = "Я хочу подзаработать доставкой почты.";
					Link.l3.go = "cureer_begin";	//доставка почты
					Link.l4 = "Ну, может вы что-нибудь сами предложите?";
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Вы бы ещё на тартане порывались конвои охранять. Для выполнения такой работы нужно судно с достойной огневой мощью — военное или рейдерское.";
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Сожалею, но для вас у меня ничего нет. Для выполнения заданий подобного рода нужен юркий рейдерский корабль, а на вашем лучше взять фрахт.";
				link.l1 = "Хорошо, я понял.";
				link.l1.go = "exit";
			}
		}
...
///--> ------фрахт со свободным выбором пункта назначения, оплаты и вида груза из предложенного списка---------
		...
		case "Fraht_choise":
			...
			if (hrand(5) < 4)//три варианта
			{
				dialog.text = "Есть следующие варианты:\n" +
					"груз " + GetGoodsNameAlt(iFrahtGoods1)+ " в количестве " + FindRussianQtyString(iFrahtGoodsQty1) + " в город " + XI_ConvertString("Colony" + pchar.questTemp.WPU.Fraht.City1) + ", за " +  FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1) + ". Оплата - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1)) + "\nгруз " + GetGoodsNameAlt(iFrahtGoods2)+ " в количестве " + FindRussianQtyString(iFrahtGoodsQty2) + " в город " + XI_ConvertString("Colony" + pchar.questTemp.WPU.Fraht.City2) + ", за " +  FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2) + ". Оплата - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2)) + "\nгруз " + GetGoodsNameAlt(iFrahtGoods3)+ " в количестве " + FindRussianQtyString(iFrahtGoodsQty3) + " в город " + XI_ConvertString("Colony" + pchar.questTemp.WPU.Fraht.City3) + ", за " +  FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty3) + ". Оплата - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money3)) + "\nЧто выбираете?";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (amount > 0)
			{
				dialog.text = "Давайте посмотрим. Вы должны были привезти груз " + GetGoodsNameAlt(iFrahtGoods)+ " в количестве " + FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty) + ". Но я вижу, что вы привезли не весь груз -  не хватает " + FindRussianQtyString(int(amount)) + " товара.";
		...
		case "Fraht_complete_bad_1":
			...
			dialog.text = "Стоимость товара в трёхкратном размере. Это составит " + FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost)) + ".";
		...
		case "Fraht_complete_fail":
			dialog.text = "Значит, вот так... Ну что же, тогда более даже не смейте обращаться ни по каким вопросам в портовые управления " + NationNameGenitive(int(pchar.questTemp.WPU.Fraht.Nation)) + "! Убирайтесь!";
...
/// -------------------------------работа курьером по доставке почты-------------------------------------
		...
		case "Postcureer":
			...
			dialog.text = "Необходимо доставить вот этот пакет в портовое управление города " + XI_ConvertString("Colony" + pchar.questTemp.WPU.Postcureer.City) + ", за " +  FindRussianDaysString(pchar.questTemp.WPU.Postcureer.DaysQty) + ", и как можно быстрее. Оплата - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
		...
		case "Postcureer_complete":
			if (pchar.questTemp.WPU.Postcureer == "late")//опоздали
			{
				...
				dialog.text = "Хм... Но вы опоздали на " + FindRussianDaysString(iTime)+ ". Поэтому я вынужден снизить размер оплаты за работу. Согласно договору, штраф составляет 10 процентов от суммы за каждый день просрочки. Получите, пожалуйста, ваши " + FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money)) + ", и впредь постарайтесь выполнять условия договора в точности.";
				link.l1 = "Спасибо и на этом.";
				link.l1.go = "Postcureer_complete_1";
				break;
			}
			dialog.text = "Спасибо за проделанную работу. Получите вашу оплату - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money)) + ".";
			link.l1 = "Спасибо!";
			link.l1.go = "Postcureer_complete_1";
		break;
...
/// ---------------------------работа в качестве эскорта для торговых кораблей----------------------------
		...
		case "escort":
			...
			dialog.text = "Итак, необходимо сопроводить корабли '" + pchar.questTemp.WPU.Escort.ShipName1 + "' и '" + pchar.questTemp.WPU.Escort.ShipName2 + "' до города " + XI_ConvertString("Colony" + pchar.questTemp.WPU.Escort.City) + ", за " +  FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty) + ". Оплата - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+". Оба корабля должны достичь пункта назначения в целости и сохранности. Корабли сдадите тамошнему служащему портового управления, оплату получите у него же.";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				...
				if (amount > 0)
				{
					dialog.text = "Так, замечательно. Вы должны были сопроводить корабли и привезти груз " + GetGoodsNameAlt(iGoods)+ " в количестве " + FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty)) + ". Но я вижу, что вы привезли не весь груз -  не хватает " + FindRussianQtyString(int(amount)) + " товара.";
					link.l1 = "Тысяча акул! Проклятые крысы сожрали! Не беспокойтесь, " + GetAddress_FormToNPC(NPChar) + ", я докуплю недостающее количество и сдам груз в полном комплекте.";
					link.l1.go = "Fraht_complete_3";
					break;
				}
			...
			if (pchar.questTemp.WPU.Escort == "late")//опоздали
			{
				...
				dialog.text = "Хм... Но вы опоздали на " + FindRussianDaysString(iTime)+ ". Поэтому я вынужден снизить размер оплаты за работу. Согласно договору, штраф составляет 10 процентов от суммы за каждый день просрочки. Получите, пожалуйста, ваши " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money)) + ", и впредь постарайтесь выполнять условия договора в точности.";
				link.l1 = "Спасибо и на этом.";
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				dialog.text = "Так, замечательно. Вы должны были сопроводить корабли и привезти груз " + GetGoodsNameAlt(iGoods)+ " в количестве " + FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty)) + ". Спасибо за проделанную работу. Получите вашу оплату - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money)) + ".";
			}
			else
			{
				dialog.text = "Спасибо за проделанную работу. Получите вашу оплату - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money)) + ".";
			}
			link.l1 = "Спасибо!";
			link.l1.go = "Escort_complete_1";
		}
		...
		case "escort_bonus":
			...
			dialog.text = "Итак, необходимо сопроводить корабли '" + pchar.questTemp.WPU.Escort.ShipName1 + "' и '" + pchar.questTemp.WPU.Escort.ShipName2 + "' до города " + XI_ConvertString("Colony" + pchar.questTemp.WPU.Escort.City) + ", за " +  FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty) + ". Оплата - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+ "\nТакже на вашем корабле я размещаю груз " + GetGoodsNameAlt(iGoods)+ " в количестве " + FindRussianQtyString(iGoodsQty) + ". Оплата за его доставку - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+".";
			link.l1 = "Задача ясна. Приступаю к выполнению?";
		...
		case "EscortBonus_complete_fail":
			...
			if (pchar.questTemp.WPU.Escort == "begin")
			{
				...
				if (amount > 0 && amount <= 100)
				{
					...
					dialog.text = "Невероятно! А зачем же тогда нужны были вы? Как вы допустили такое?! Я думаю, вам понятно, что ни о каком вознаграждении за сопровождение речи быть не может... Да ещё и груза у вас недостача - " + FindRussianQtyString(int(amount)) + "\nВ общем, я плачу вам вознаграждение только за фрахт, за вычетом стоимости недостающего товара, за то, что вы хоть груз доставили в приемлемом виде. Получите ваши " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+ " и можете быть свободны.";
					link.l1 = "Спасибо и на этом.";
				...
				if (amount > 100)
				{
					dialog.text = "Невероятно! А зачем же тогда нужны были вы? Как вы допустили такое?! Я думаю, вам понятно, что ни о каком вознаграждении за сопровождение речи быть не может... Да ещё и груза у вас огромная недостача\nУ меня нет возможности ждать, пока вы докупите недостающее количество. Поэтому вам придётся компенсировать стоимость груза деньгами в полном объёме - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+ ". Сам же груз оставьте себе.";
					if (int(pchar.money) >= int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))
					{
				...
				dialog.text = "Невероятно! А зачем же тогда нужны были вы? Как вы допустили такое?! Я думаю, вам понятно, что ни о каком вознаграждении за сопровождение речи быть не может\nВ общем, я плачу вам вознаграждение только за фрахт, за то, что вы хоть груз доставили. Получите ваши " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+ " и можете быть свободны.";
				link.l1 = "Спасибо и на этом.";
				link.l1.go = "EscortBonus_complete_fail_1";
				break;
			}
			if (pchar.questTemp.WPU.Escort == "late")
			{
				...
				if (amount > 0)
				{
					dialog.text = "Невероятно! А зачем же тогда нужны были вы? Как вы допустили такое?! Я думаю, вам понятно, что ни о каком вознаграждении за сопровождение речи быть не может... Корабль потеряли, опоздали на " + FindRussianDaysString(iTime)+ ", да ещё и груза у вас недостача - " + FindRussianQtyString(int(amount)) + "\nУ меня нет возможности ждать, пока вы докупите недостающее количество. Поэтому вам придётся компенсировать стоимость груза деньгами в полном объёме - " + FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+ ". Сам же груз оставьте себе.";
					if (int(pchar.money) >= int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))
					{
						link.l1 = "Хорошо-хорошо, не кипятитесь, вот ваши деньги.";
						link.l1.go = "EscortBonus_complete_fail_2";
				...
				dialog.text = "Невероятно! А зачем же тогда нужны были вы? Как вы допустили такое?! Я думаю, вам понятно, что ни о каком вознаграждении за сопровождение речи быть не может... Да ещё и опоздали на " + FindRussianDaysString(iTime)+ "\nПоэтому я вынужден снизить размер оплаты за фрахт. Согласно договору, штраф составляет 10 процентов от суммы за каждый день просрочки. Получите, пожалуйста, ваши " + FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney)) + ", и можете быть свободны.";
				link.l1 = "Спасибо и на этом.";
				link.l1.go = "EscortBonus_complete_fail_1";
				break;
			}
		...
		case "EscortBonus_complete_fail_3"://потеряли корабль и груз, время норма или нет, война
			dialog.text = "Значит, вот так... Ну что же, тогда более даже не смейте обращаться ни по каким вопросам в портовые управления " + NationNameGenitive(int(pchar.questTemp.WPU.Escort.Nation)) + "! Убирайтесь!";
			link.l1 = "Невелика потеря...";
			link.l1.go = "exit";
		...
		case "Escort_fail_complete":
			...
			if (amount > 0)
			{
				dialog.text = "Давайте посмотрим. Вы должны были привезти груз " + GetGoodsNameAlt(iFrahtGoods)+ " в количестве " + FindRussianQtyString(pchar.questTemp.WPU.Escort.GoodsQty) + ". Но я вижу, что вы привезли не весь груз - не хватает " + FindRussianQtyString(int(amount)) + " товара.";
				link.l1 = "Тысяча акул! Проклятые крысы сожрали! Не беспокойтесь, " + GetAddress_FormToNPC(NPChar) + ", я докуплю недостающее количество и сдам груз в полном комплекте.";
				link.l1.go = "Fraht_complete_3";
				break;
			}
...
//-------------------------------------задания 2 уровня для курьера---------------------------------------------
		case "Postcureer_LevelUp":
			...
			{
				case 0:
					dialog.text = "Необходимо срочно доставить важную государственную депешу для губернатора города "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" не позже, чем за "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Соблюдайте меры предосторожности - за этой депешей может вестись охота. Оплата за доставку - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Я соглас"+GetSexPhrase("ен","на")+"!";
				...
				case 1:
					dialog.text = "Необходимо доставить эти бумаги для хозяина магазина "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" не позже, чем за "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Это последняя сводка цен в магазинах всего архипелага - ценнейший коммерческий документ. Следует проявить осторожность - конкуренты вполне могут устроить охоту. Оплата за доставку - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Я соглас"+GetSexPhrase("ен","на")+"!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "Нет, откажусь, пожалуй.";
					link.l2.go = "Postcureer_LevelDown";
				...
				case 2:
					dialog.text = "Необходимо доставить этот пакет документов для ростовщика города "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" не позже, чем за "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Здесь очень важная деловая информация о курсах валют и ценных бумаг. Будьте начеку - этим пакетом уже интересовались какие-то тёмные личности. Оплата за доставку - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Я соглас"+GetSexPhrase("ен","на")+"!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "Нет, откажусь, пожалуй.";
					link.l2.go = "Postcureer_LevelDown";
				...
				case 3:
					dialog.text = "Необходимо доставить этот свёрток хозяину верфи города "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" не позже, чем за "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". В нём находится строительный чертёж нового "+RandPhraseSimple(LinkRandPhrase("брига","корвета","фрегата"), LinkRandPhrase("флейта","пинаса","галеона"))+", содержащий уникальные технические разработки. Думаю, понятно, что этим чертежом могут интересоваться и другие... заинтересованные лица, так что учтите это. Оплата за доставку - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "Я соглас"+GetSexPhrase("ен","на")+"!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "Нет, откажусь, пожалуй.";
					link.l2.go = "Postcureer_LevelDown";
...
//----------------------------------------задания 2 уровня фрахт------------------------------------------
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		{
			dialog.text = "Так, вы должны были привезти груз " + GetGoodsNameAlt(iGoods)+ " в количестве " + FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty) + ". Замечательно! Благодарю вас. Получите вашу оплату - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
...
//----------------------------------------задания 2 уровня эскорт ------------------------------------------
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		dialog.text = "Отлично! Вы уже в который раз доказываете, что на вас можно положиться. Спасибо за проделанную работу. Получите ваше вознаграждение - "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+". Не забывайте периодически навещать меня - работа для такого капитана, как вы, есть практически всегда.";
...
//-------------------------------- квест доставки судового журнала до рассеяного кэпа ---------------------
		...
		case "SeekShip_good_1":
			dialog.text = "Я готов выплатить вам вознаграждение. Оно составляет " + int(npchar.quest.money) + " дублонов. Больше я заплатить не могу, к сожалению.";
		...
		case "CapitainList":
			if (int(npchar.quest.qty) > 0)
			{
				...
				for (i=0; i<int(npchar.quest.qty); i++)
				{
					...
					attrL = "l" + i;
					link.(attrL) = GetFullName(sld) + ", капитан " + GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName + "Gen")) + " '" + sld.Ship.name + "'.";
		...
		case "ShipStock_2":
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName) + " '" + chref.Ship.Name + "', класс " + RealShips[int(chref.Ship.Type)].Class +
                     ", стоимость стоянки " + FindRussianMoneyString(int(NPChar.MoneyForShip)) + " в месяц, оплата за месяц вперёд.";
			dialog.Text = dialog.Text + " Если оставите с офицером и командой, тогда вместе с ними " + FindRussianMoneyString(int(NPChar.MoneyForShip) + int(NPChar.MoneyForCrew));
			...
			Link.l2 = "Да. Это меня устраивает. Оставляем с капитаном и командой";
			if (int(Pchar.Money) >= (int(NPChar.MoneyForShip) + int(NPChar.MoneyForCrew))) Link.l2.go = "ShipStock_4";
			else Link.l2.go = "ShipStock_NoMoney";

			Link.l3 = "Нет, я передумал"+ GetSexPhrase("","а") +".";
			Link.l3.go = "exit";
		break;
					...
					for(i=1; i<MAX_CHARACTERS; i++)
					{
						...
						{    
							if (chref.ShipInStockMan == NPChar.id)
							{
								...
								{  
								Link.(attrL)	= XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName) + " '" + chref.Ship.Name + "' и мой офицер " + GetFullName(chref) + ".";
		...
		case "ShipStockManBack":
			...
			{
			    dialog.Text = "Забираете? С вас за хранение ещё " + FindRussianMoneyString(int(NPChar.MoneyForShip)) + ".";
			}
			else
			{
					...
					if (_curCharIdx!=-1)
					{
						...
						if (!CheckAttribute(sld, "CompanionDisable"))
					    {
							if (!ok && GetRemovable(&characters[_curCharIdx]))
							{
								...
								if (int(pchar.Fellows.Passengers.navigator) == int(sld.index)) sProf += " (штурман)";
								if (int(pchar.Fellows.Passengers.boatswain) == int(sld.index)) sProf += " (боцман)";
								if (int(pchar.Fellows.Passengers.cannoner) == int(sld.index)) sProf += " (канонир)";
								if (int(pchar.Fellows.Passengers.doctor) == int(sld.index)) sProf += " (врач)";
								if (int(pchar.Fellows.Passengers.carpenter) == int(sld.index)) sProf += " (плотник)";
							    if (int(pchar.Fellows.Passengers.treasurer) == int(sld.index)) sProf += " (казначей)";
...
string findCurrentCity2(ref NPChar)//выбираем целевой город 2
{
	...
	if (howStore == 0) return "none";
	nation = storeArray[abs(hrand(howStore-1)-3)]; // todo: ???
	return colonies[nation].id;
}
...
string findCurrentCity3(ref NPChar)//выбираем целевой город 3
{
	...
	if (howStore == 0) return "none";
	nation = storeArray[abs(hrand(howStore-1)-7)]; // todo: ???
	return colonies[nation].id;
}
