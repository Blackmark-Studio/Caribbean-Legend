...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":			
			...
			if (false)
			{
				...
				else
				{
					if (DeceptionChance > (10+rand(150)+rand(150)))
					{
						...
						link.l1 = "Я бросил"+ GetSexPhrase("","а") +" якорь у " + XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen") + " под флагом " + NationNameGenitive(int(npchar.nation)) + ". Что тебе ещё не ясно?(Вероятность обмана "+DeceptionChance+"%%)";
			...
			else
			{
				...
				if (false)
				{
					...
					if (CheckNationLicence(HOLLAND))
					{
						...
						{
							link.l2 = "Ты что, не видишь, как на моём корабле развевается флаг " + NationNameGenitive(int(pchar.nation)) + "?! (Вероятность обмана "+DeceptionChance+"%%)";
						}
						else //если причалил не в городе
						{
							link.l2 = "Я бросил"+ GetSexPhrase("","а") +" якорь у " + XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen") + " под флагом " + NationNameGenitive(int(pchar.nation)) + ". Что тебе ещё не ясно? (Вероятность обмана "+DeceptionChance+"%%)";
						}
					...
					else
					{
						...
						if (findsubstr(pchar.location.from_sea, "_town" , 0) != -1) //если причалил в городе
						{
							link.l1 = "Ты что, не видишь, как на моём корабле развевается флаг " + NationNameGenitive(int(pchar.nation)) + "?! (Вероятность обмана "+DeceptionChance+ "%%)";
						}
						else //если причалил не в городе
						{
							link.l1 = "Я бросил"+ GetSexPhrase("","а") +" якорь у " + XI_ConvertString(GetIslandByCityName(npchar.city)+"Gen") + " под флагом " + NationNameGenitive(int(pchar.nation)) + ". Что тебе ещё не ясно? (Вероятность обмана "+DeceptionChance+"%%)";
						}
						if (DeceptionChance < (10+rand(50)+rand(50)))
						{
							notification("Недостаточно развит навык", SKILL_SNEAK);
				...
				else
				{
					...
					if(isMainCharacterPatented() && int(Items[int(pchar.EquipedPatentId)].TitulCur) > 4 && npchar.nation == int(Items[int(pchar.EquipedPatentId)].Nation))
					{
						switch(rand(2))
						{
							...
							case 2: 
								...
								link.l1 = "Служи " + NationNameGenitive(int(npchar.nation)) + ", солдат! Это лучшее, что ты можешь сделать.";
								link.l1.go = "exit";
							break;
						}
						break;
					...
					if(IsOfficerFullEquip() && npchar.nation == int(Items[int(pchar.EquipedPatentId)].Nation))
					{
						...
						{
							case 0: 
							dialog.text = "Приветствую, капитан! Если у вас есть для меня какие-то указания, то прошу простить: я хотя и на службе " + NationNameGenitive(int(npchar.nation)) + ", но подчиняюсь только коменданту и губернатору.";
							link.l1 = "Для указаний у меня есть свои офицеры и команда. Неси службу, солдат.";
							link.l1.go = "exit";
							break;

							case 1:
								dialog.text = "Капитан, может возьмёте меня к себе на корабль служить? Та же служба " + NationNameGenitive(int(npchar.nation)) + ", а море мне больше по душе.";
								link.l1 = "Ты нужен там, куда тебя определили, так что неси свою службу с честью. Глядишь, и в море возьмут.";
								link.l1.go = "exit";
							break;

							case 2: 
								dialog.text = "Эх, вам вот повезло: вы целый капитан корабля на службе " + NationNameGenitive(int(npchar.nation)) + "... А я тут торчу целый день.";
			...
			{
				dialog.text = "Приветствую в Бриджтауне, "+GetAddress_Form(NPChar)+"...";
				link.l1 = "Я думал, моё лицо известно не только во французских колониях... Меня зовут "+GetFullName(pchar)+".";
		...
		case "WildRose_Soldier_1":
			...
			{
				dialog.text = "А, это вы, капитан... Разморило меня малость под солнцем... Проклятая жара!";
			}
			else
			{
				dialog.text = "С этим не поспоришь. Однако, ночью все люди на одно лицо, знаете ли.";
			}
			link.l1 = "Чем обязан, солдат? Обычно ваша братия не донимает меня разговорами.";
			link.l1.go = "WildRose_Soldier_2";
		break;
		
		case "WildRose_Soldier_2":
			dialog.text = "Кхм, да, "+GetAddress_Form(NPChar)+". Однако, у меня приказ от лорда Уиллогби: я должен предупредить всех вновьприбывших на остров, что несколько дней назад с плантации Бишопа дала дёру кучка рабов. Проклятые надзорщики небось спали себе спокойно, пока те кандалы разбивали. А ведь сегодня я должен был быть в обьятиях моей милой Анжелики...";
			link.l1 = "Дай угадаю: на розыск собственности плантатора отправили солдат гарнизона?";
			link.l1.go = "WildRose_Soldier_3";
		break;
		
		case "WildRose_Soldier_3":
			dialog.text = "Ещё бы... Бишоп так же отрядил в сельву несколько отрядов своих наёмников, но ему и этого мало. Он пообещал хорошую награду тем, кто приведёт рабов к нему. Живыми. Обычно, у него спины гнут чёрномазые, но не так давно, подвезли партию дезертиров флотских - толку на каменоломне от них мало, зато улизнуть с плантации мозгов хватило...";
			link.l1 = "Так это были белые?";
			link.l1.go = "WildRose_Soldier_4";
		break;
		
		case "WildRose_Soldier_4":
			dialog.text = "Белее не бывает, разрази меня гром! Негры бы просто сбежали, а эти прихватили несколько мушкетов да сабель, что с зарезанных наёмников сняли - и уже успели пустить их в дело. Пришили одного из наших, и пару прихвостней Бишопа в придачу - от чего остальные словно озверели\nЕдва по ошибке не перестреляли наших же солдат, спутав их с беглецами. Да ещё и несколько гражданских бесследно сгинули в сельве.";
			link.l1 = "Городские ворота заперты?";
			link.l1.go = "WildRose_Soldier_5";
		break;
		
		case "WildRose_Soldier_5":
			dialog.text = "Нет, конечно - город ведь не на осадном положении. Вы вольны идти куда пожелаете. Но если захотите поохотиться на беглецов и попадёте под перекрёстный огонь, пеняйте только на себя. Если уцелеете, не стоит потом обивать пороги резиденции, чтобы пожаловаться Его Светлости - в конце концов, он проявил достаточно доброй воли, чтобы позаботиться не только о жителях Бриджтауна, но и его гостях.";
			link.l1 = "Как это... любезно с его стороны. Да хранит Господь Фрэнсиса Уиллогби!";
			link.l1.go = "WildRose_Soldier_6";
			link.l2 = "Эх, если бы все губернаторы проявляли такую заботу о новоприбывших, как сэр Уиллогби!..";
			link.l2.go = "WildRose_Soldier_7";
		break;
		
		case "WildRose_Soldier_6":
			dialog.text = "Очень смешно. Можете идти, капитан.";
			link.l1 = "Ну бывай, солдат.";
			link.l1.go = "WildRose_Soldier_8";
			AddCharacterExpToSkill(pchar, "Leadership", 100);
		break;
		
		case "WildRose_Soldier_7":
			dialog.text = "Да уж, у нашего брата было бы на порядок меньше работы. Не стану вас более задерживать, капитан.";
			link.l1 = "Удачи, солдат.";
			link.l1.go = "WildRose_Soldier_8";
