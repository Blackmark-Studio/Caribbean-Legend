	...
	switch(Dialog.CurrentNode)
	{
	  ...
	  // to_do
		...
		case "SitySpy_1":
			...
			dialog.text = "Кто угодно в этом городе! " + NPCharSexPhrase(npchar, GetFullName(&characters[int(pchar.GenQuest.SeekSpy.Idx)]) + " может без проблем, благо " + NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.Idx)], "он", "она") + " в двух шагах.",
				"Ой, Господи, да вот, хотя бы " + NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.Idx)], GetAddress_FormToNPC(NPChar) + " " + characters[int(pchar.GenQuest.SeekSpy.Idx)].lastname, GetFullName(&characters[int(pchar.GenQuest.SeekSpy.Idx)])) + " может.");
		...
		case "SeekSpy_Checking":
			dialog.text = "Да, я подтверждаю, " + NPCharSexPhrase(&characters[int(pchar.GenQuest.SeekSpy.BaseIdx)], "он действительно местный житель.", "она действительно местная жительница.");
		...
		case "CitizenFindItem_2":
			dialog.text = ""+ GetSexPhrase("Капитан","Девушка") +", это же прекрасно! Пожалуйста, найдите это кольцо, ведь через " + int(NPChar.LifeDay) + " дня приезжает мой муж и я не хотела бы, чтобы он меня увидел без обручального кольца.";
		...
		case "CitizenFindItem_4":
			dialog.text = "Да, ступайте. И поторопитесь. Помните, у вас есть всего " + int(NPChar.LifeDay) + " дня.";
			link.l1 = "Помню.";
			link.l1.go = "exit";
