...
//navy <--
	        ...
	        if(!bSeaBattle && PChar.location != "Deck_Near_Ship" && findsubstr(PChar.location, "_shipyard" , 0) == -1 && PChar.location != "CommonPackhouse_2" && !CheckAttribute(pchar, "GenQuest.CannotWait") && !CheckAttribute(loadedLocation, "CannotWait") && !CheckAttribute(loadedLocation, "CannotWait"))
	        {
...
//navy <--
		...
		case "FMQN_2":
			dialog.text = "Нет уж, всё-таки останусь. Но чёрта с два я буду сидеть и ждать у моря погоды - отправляюсь на корабль, пойду в порт Филипсбурга и разнюхаю обстановку. Я не намерен"+GetSexPhrase("","а")+" становиться слепым орудием в их руках.";
			link.l1 = "";
			link.l1.go = "exit";
