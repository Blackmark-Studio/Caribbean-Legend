...
// ------------------------------------------блок angry-----------------------------------------------
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
            ...
            link.l0 = "Пастор, я бы хотел" + GetSexPhrase("","а") + " обсудить свою безопасность в море. Парни из Берегового братства проявляют чрезмерный интерес к моей скромной персоне. Не мог бы ты их немного приструнить?";
        ...
        case "I_know_you_good":
			if (sti(pchar.GenQuest.Piratekill) > 20)
			{
				dialog.text = RandPhraseSimple("У тебя крыша поехала? Вообразил" + GetSexPhrase("","а") + " себя мясником? Все пираты злы на тебя, " + GetSexPhrase("приятель","подруга") + ", так что лучше тебе убраться отсюда побыстрее.", "Ты, " + GetSexPhrase("приятель","подруга") + ", похоже, из ума выжил" + GetSexPhrase("","а") + ". Руки чесались сильно? Теперь не обессудь - здесь тебе делать нечего. Вали отсюда побыстрее!");
				link.l1 = RandPhraseSimple("Послушай, я хочу исправить ситуацию...", "Помоги мне решить эту проблему...");
				link.l1.go = "pirate_town";
				break;
			}
...
//--------------------------------------------Похититель------------------------------------------------------
        ...
        case "pirate_threat":
            ...
            if (iGPThreat != 0)
            {
                ...
                dialog.text = "Ха! Достаётся тебе от нашей братии, а, " + GetSexPhrase("приятель", "подруга") + "? Конечно, я могу на какое-то время придержать их в узде. Только тебе придётся знатно раскошелиться. " + FindRussianDublonString(iMarlowTotalTemp) + " на бочку, и считай, что мы договорились.";
