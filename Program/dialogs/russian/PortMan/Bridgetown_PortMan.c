...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
            ...
            {
                link.l1 = "Я направляюсь в Блювельд, паприку продавать. Скажите, мистер, нет ли у вас сейчас пассажиров до Блювельда? Ну, или до Порт-Ройяла. Было бы неплохо подвезти, глядишь, покрыли бы убытки на оплату команде. Я и так её сократил до минимума, но всё равно, канальи, денег-то просят...";
		...
		//Голландский гамбит
		case "PortofficeDone":
			dialog.text = "Пассажиров ни до Блювельда, ни до Порт-Ройяла нет, к сожалению. Зайдите завтра, или послезавтра - может, появятся.";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Как вы заметили, капитан, у нас очень тревожная обстановка - город в осаде. Готовимся к атаке испанцев, мобилизуем все силы. Но регата продолжается\nТак, отмечаем: капитан "+ GetFullName(pchar) +", корабль - "+pchar.Ship.Name+"... Затрачено времени от старта регаты в часах - "+int(pchar.questTemp.Regata.FourthTransitionTime)+". Всё, ваш результат зафиксирован, можете продолжать путь.";
