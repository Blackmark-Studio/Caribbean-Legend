void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Himenes_5":
			...
			link.l1 = nationManName[int(pchar.BaseNation)] + ". Это так важно? А имя моё пока давайте не будем трогать.";
		...
		case "Himenes_6":
			...
			if (int(pchar.BaseNation) == SPAIN) dialog.text = "Кха-ха-ха-ха-ха-х! Вот она, наша удача. Ну ладно, терять всё равно нам нечего. " + dialog.text;
		...
		case "Himenes_60":
			dialog.text = "Разумеется, фактория потребует содержание. С её постройкой этот остров получит экономику с основным торговым партнёром в вашем лице. Рабов мы прокормим сами, но охрану и производство вам придётся оплатить из своего кармана. Поселенцы возьмут 20 дублонов и пять бутылок вина за каждую партию. По вопросам охраны договаривайтесь с сеньором Янссеном.";
			link.l1 = "";
		...
		case "Himenes_bacaut":
			dialog.text = "Конечно, капитан, забирайте. У нас готово следующее число партий: "+int(pchar.questTemp.IslaMona.Factory.Part)+", это "+int(pchar.questTemp.IslaMona.Factory.Goods)+" брёвен. Следовательно, с вас "+int(pchar.questTemp.IslaMona.Factory.Dublon)+" дублонов, "+int(pchar.questTemp.IslaMona.Factory.Bottle)+" бутылок рома и "+int(pchar.questTemp.IslaMona.Factory.Bottle)+" бутылок вина. У вас всё это есть?";
		...
		case "Himenes_church_10_3":
			...
			{
				sAdd += " "+int(pchar.questTemp.IslamonaChurch.Candle)+ " свечей,"
			}
			if(CheckAttribute(pchar, "questTemp.IslamonaChurch.Amber"))
			{
				sAdd += " "+int(pchar.questTemp.IslamonaChurch.Amber)+ " янтаря,"
			}
			if(CheckAttribute(pchar, "questTemp.IslamonaChurch.GoldNugget"))
			{
				sAdd += " "+ int(pchar.questTemp.IslamonaChurch.GoldNugget) + " золотых слитков,"
			}
		...
		case "FraOfficer_5":
			dialog.text = "Нет, но вас можно отправить к бандитам в качестве парламентёра. Ваш приезд означает, что их логово раскрыто, а у солдат короля теперь есть корабль и подкрепление. Ну, а пока они будут уши развешивать и оценивать всю глубину своей глубины, вы подберётесь как можно ближе к орудию, бросите под него пехотную бомбу и не дадите себя убить до подхода наших с вами людей. Если вы не уверены, что справитесь, то попробуйте отправить кого-нибудь из своих вместо себя.";
