...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "seafox_2":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+", это вас волновать не должно. Вам ничего не угрожает. Вы просто высадите нас в бухте и всё. Ну как, по рукам?";		
			link.l1 = "Хорошо, я соглас"+GetSexPhrase("ен","на")+", здесь действительно недалеко. Что насчёт оплаты?";
			link.l1.go = "seafox_3";
		...
		case "seafox_6":
			...
			link.l1 = "Гм... Это не совсем то, о чём мы договаривались...";
			link.l1.go = "seafox_7";
		break;
		...
		case "seafox_9":
			...
			link.l1 = "Лейтенант Каспар Грэттон в мундире голландских колониальных войск - весьма неожиданно!.. Я искал"+GetSexPhrase("","а")+" вас, сэр.";
		...
		case "seafox_11":
			...
			link.l1 = "После нашего с вами расставания мне довелось побывать в Филипсбурге и пообщаться с жителями. Голландцы знают о том, что на Синт-Маартен должна прибыть диверсионная группа англичан для того, чтобы вытащить из тюрьмы нескольких высокопоставленных английских офицеров.";
		...
		case "seafox_15":
			dialog.text = "Отнюдь... Капитан, мне нужна ваша помощь. Раз мы уже оказались в одной лодке - давайте выпутываться вместе, ведь если голландцы схватят нас, то и вам может не поздоровиться, как соучастни"+GetSexPhrase("ку","це")+".";
		...
		case "seafox_21":
			...
			link.l1 = "Я только что из церкви. И мне там ответили, что никакого служки по имени Филип Якобсен в их приходе нет.";
			link.l1.go = "seafox_22";
		break;
		
		case "seafox_22":
			dialog.text = "Он там бывает по утрам, я же говорил...";
			link.l1 = "Карамба! А мне сказали, что его нет там, ни по утрам, ни по вечерам! "+RandSwear()+" И зачем я только связал"+GetSexPhrase("ся","ась")+" с вами! "+RandSwear()+"";
		...
		case "seafox_25":
			if (pchar.sex == "man") {PlaySound("VOICE\Russian\LE\SeaFox\SeaFox_04.wav");}
			dialog.text = "Чёрт побери, кто-то лезет сюда!.. Ты прив"+GetSexPhrase("ёл","ела")+" голландцев, каналья!";
			link.l1 = ""+RandSwear()+"!!";
			link.l1.go = "exit";
		...
		case "seafox_27":
			...
			link.l1 = "Эх... Подведёте вы меня под монастырь, лейтенант! Но ладно, раз я уже начал"+GetSexPhrase("","а")+" вам помогать - помогу.";
		...
		case "seafox_35":
			...
			dialog.text = "Спасибо за вашу помощь, капитан! Я прошу вас погостить один день в нашей колонии, пока я сделаю доклад полковнику Фоксу, а завтра прошу вас явиться к нему на приём. Если вы не знаете - штаб-квартира полковника Фокса находится в резиденции губернатора, налево.";
		...
		case "seafox_35x":
			PlaySound("VOICE\Russian\LE\SeaFox\SeaFox_05.wav");
			dialog.text = "Спасибо за вашу помощь, капитан! Вы "+GetSexPhrase("вели себя, как настоящий герой","очень храбрая девушка")+", "+GetAddress_Form(NPChar)+"! Очень жаль лейтенанта Каспара Грэттона, но он погиб, как настоящий солдат и патриот Англии. Я прошу вас погостить один день в нашей колонии, пока я сделаю доклад полковнику Фоксу, а завтра прошу вас явиться к нему на приём. Если вы не знаете - штаб-квартира полковника Фокса находится в резиденции губернатора, налево.";
			link.l1 = "Хорошо. Вы уж там постарайтесь... за меня перед мистером Фоксом. Мне ведь теперь проблемы с голландскими властями решать придётся.";
			link.l1.go = "seafox_36";
		break;
		
		case "seafox_36":
			dialog.text = "Не беспокойтесь, я опишу ваши действия в докладе в самом лучшем виде. Был рад знакомству, "+GetAddress_Form(NPChar)+"!";
			link.l1 = "...";
		...
		case "soldier_1":
			...
			link.l1 = "Что-то я не понимаю... Я арестован"+GetSexPhrase("","а")+"? С какой стати?";
			link.l1.go = "soldier_2";
		break;
		
		case "soldier_2":
			AddLandQuestMark(characterFromId("Marigo_Mayor"), "questmarkmain");
			dialog.text = "Никак нет, "+GetAddress_Form(NPChar)+" капитан, вы не арестованы. Губернатор отдал приказ приглашать к нему всех капитанов кораблей, отдающих якорь у нас в порту, в связи с особой ситуацией. Пройдёмте!";
		...
		case "hol_officer":
			...
			dialog.text = "Итак, "+GetAddress_Form(NPChar)+", показывайте дорогу туда, где вы видели подозрительных личностей.";
		...
		case "hol_officer_4":
			...
			link.l1 = "Хорошо. Ну... я пош"+GetSexPhrase("ёл","ла")+".";
			link.l1.go = "exit";
		...
		case "hol_officer_5":
			...
			dialog.text = "Крепкие, мерзавцы, оказались... Отлично сработали, капитан! Поздравляю. Диверсанты ликвидированы, комендант и губернатор будут довольны.";
		...
		case "monk_1":
			...
			link.l1 = "Безусловно. Иначе бы я тут не стоял"+GetSexPhrase("","а")+". Отряд лейтенанта Каспара Грэттона сейчас находится в джунглях. Все одеты в форму голландских военных, но в тюрьме ждёт засада. Меня прислали за помощью к вам.";
			link.l1.go = "monk_2";
		break;
		...
		case "monk_repeat":
			dialog.text = "Ты прин"+GetSexPhrase("ёс","есла")+" две дюжины дублонов, "+GetSexPhrase("сын мой","дочь моя")+"?";
			if (PCharDublonsTotal() >= 24)
			{
				link.l1 = "Да. Держите.";
				link.l1.go = "monk_4";
