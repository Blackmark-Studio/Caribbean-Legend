...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "officer":
			...
			link.l1 = "Уф... Офицер, я приш"+GetSexPhrase("ёл","ла")+" сюда по просьбе подавальщицы таверны, и увидел"+GetSexPhrase("","а")+", что...";
		...
		case "officer_5":
			PlaySound("VOICE\Russian\z_arest_02.wav");
			dialog.text = "Что здесь происходит?! Драка?!";
			link.l1 = "Уф... Офицер, я здесь по просьбе подавальщицы таверны - она утверждала что здесь кого-то убивают, что на деле подтвердилось - на этого господина напали вон те два негодяя. Увы, мне не удалось спасти беднягу. Мерзавцы расправились с ним.";
			link.l1.go = "officer_6";
		break;
		
		case "officer_6":
			dialog.text = "Так-так... Да этот дворянин - испанец! И что ему тут надо было? Гм. Ваше имя, "+GetAddress_Form(NPChar)+"?";
			link.l1 = ""+GetFullName(pchar)+". Капитан "+GetFullName(pchar)+".";
		...
		case "noble":
			dialog.text = "Позвольте мне всё объяснить, офицер! Я, дон Доминго Ортис Альбалате из Санто-Доминго, прибыл в Порт-о-Пренс по приглашению одной юной особы. В ожидании встречи я остановился в таверне. Однако не прошло и двух часов, как ко мне в комнату вломились dos canallas malditas, и если бы не помощь "+GetSexPhrase("этого доброго сеньора, появившегося","этой прекрасной сеньориты, появившейся")+"  как раз вовремя, мне пришлось бы туго.";
			link.l1 = "Дон Доминго Ортис Альбалате? Испанец?";
			link.l1.go = "noble_1";
		break;
		
		case "noble_1":
			dialog.text = "Именно так, сеньор! Прошу вас учесть, что я нахожусь во французской колонии на законных основаниях и что я испанский дворянин благородного происхождения, а не какой-то там pirata socio!";
			link.l1 = "Дон Доминго, вам следует пройти со мной в комендатуру для выяснения! Мы во всём разберемся. А вы, "+GetAddress_Form(NPChar)+"...";
			link.l1.go = "noble_2";
		break;
		...
		case "noble_3":
			if (pchar.sex == "man") {PlaySound("VOICE\Russian\LE\Domingo\Domingo_01.wav");}
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+" капитан! Клянусь честью моего доброго отца, вы благородный человек! Я бы хотел с вами увидеться, позже, и отблагодарить, как положено. Окажите честь посетить моё скромное жилище в Санто-Доминго! Мой дом находится совсем рядом с городскими воротами. Буду ждать вас "+GetSexPhrase("my buen amigo","mi salvadora")+"! Запомните моё имя - дон Доминго Ортис Альбалате!";
			link.l1 = "...";
			link.l1.go = "noble_4";
		break;
		...
		case "noble_5":
			dialog.text = "Капитан "+pchar.lastname+"?! О! Какой ужас! Да тут настоящая скотобойня! Что здесь произошло, "+GetSexPhrase("mi buen amigo","mi salvadora")+"? Кто эти люди? ";
			link.l1 = "Рад"+GetSexPhrase("","а")+" вас видеть, дон Доминго! Эти люди... Я здесь по вашему приглашению, на пороге меня встретил некий господин, который во-он там лежит, видите? Он сказал, что его зовут Уго, он ваш слуга и...";
			link.l1.go = "noble_6";
		break;
		
		case "noble_6":
			dialog.text = "Уго?! О, нет, вас подло обманули, капитан!";
			link.l1 = "Мне это стало ясно слишком поздно! Три мерзавца проникли в ваш дом, убили вашего верного слугу Уго - его труп я наш"+GetSexPhrase("ёл","ла")+" наверху уже потом - и устроили засаду. Караулили, вне сомнения, вас, сеньор Альбалате. Моё появление спутало им планы - мне захотелось остаться, ну... они и начали махать клинками. Но не на "+GetSexPhrase("того","ту")+" напали!";
			link.l1.go = "noble_7";
		break;
		
		case "noble_7":
			dialog.text = "Oh, Dios! Вы владеете шпагой, как профессиональный кондотьер! Хорошо, что вы на моей стороне! О, мой бедный Уго! Он нянчил меня с детства, а теперь... убит безродными псами!";
			link.l1 = "Дон Доминго, осмелюсь предположить, что кто-то сильно жаждет вашей смерти! Ваш добрый слуга знал об опасности, грозящей вам. Я наш"+GetSexPhrase("ёл","ла")+" на столе записку. Прочтите.";
			link.l1.go = "noble_8";
		break;
		...
		case "noble_13":
			...
			if (SandBoxMode)    
			{
				...
				link.l1 = "Знаете, "+npchar.name+", мне приходилось иметь дело с людьми, которые хладнокровно убили бы всех своих родственников за гораздо менее весомую награду.";
				link.l1.go = "noble_14";
			}
			else
			{
		...
		case "noble_14":
			dialog.text = "Мой верный Уго в своей предсмертной записке упомянул про бумаги дяди... Это прямое указание. О, как я не хочу верить в это! Но... моя кузина, Маргарита Ортис, проживающая в Испании, недавно была помолвлена с инфантом весьма видного дворянского семейства. Не в этом ли дело, "+GetAddress_Form(NPChar)+" капитан, как вы считаете?";
		...
		case "noble_15":
			...
			if (SandBoxMode)    
			{
				link.l1 = "Какое коварство со стороны брата матери!";
				link.l1.go = "noble_16";
			}
			else
			{
		...
		case "noble_16":
			dialog.text = "Постойте, "+GetAddress_Form(NPChar)+"! Это ведь не первая попытка! С пару недель назад, как раз вскоре после объявления о помолвке, был один пьяный паршивец, имевший наглость обхамить меня так, что пришлось преподать ему урок хороших манер. Когда дошло до клинков, этот мужлан оказался не последним фехтовальщиком! Он даже умудрился задеть меня шпагой... перед собственной смертью...";
			link.l1 = "Ха! Поделом досталось наглецу! А как вы оказались в таверне Порт-о-Пренса? Вы что-то говорили о юной особе?";
			link.l1.go = "noble_17";
		break;
		
		case "noble_17":
			dialog.text = "(краснеет) О, да... Да, капитан, была там и милая особа... но только теперь я понимаю, что это была ловушка, а юная прелестница - морская сирена! О, кругом столько мерзавцев, столько мерзавцев, "+GetAddress_Form(NPChar)+", я никому не могу верить! Верный Уго... и тот мёртв!";
			if (SandBoxMode)    
			{
				link.l1 = "Может, вам стоит тайком отплыть в Испанию и навестить вашу кузину?";
				link.l1.go = "noble_18";
			}
			else
			{
		...
		case "noble_20":
			...
			link.l1 = "Было бы нелепо отказывать вам в помощи сейчас, дон Доминго, учитывая, насколько далеко мы уже зашли. Мой долг - быть на вашей стороне в этой гнусной истории!";
			link.l1.go = "noble_21";
		break;
		
		case "noble_21":
			AddMoneyToCharacter(pchar, 20000);
			dialog.text = "Я знал, что вы мне поможете, "+GetAddress_Form(NPChar)+"! Вот, возьмите - здесь двадцать тысяч песо. Это компенсация ваших расходов на переход к Порто-Белло. Когда мы отправляемся? Надо спешить, пока дядюшка не вернулся из Картахены!";
		...
		case "noble_22":
			dialog.text = "Вы позволите мне подняться на борт вашего судна немедленно, капитан?";
		...
		case "noble_25":
			dialog.text = "Кхм... Увы, "+GetAddress_Form(NPChar)+", к своему стыду должен признаться, что на этой плантации я последний раз был очень давно, ещё в совсем юном возрасте... но я намерен исправить сие досадное упущение!";
		...
		case "noble_27":
			dialog.text = "Что это у вас, "+GetAddress_Form(NPChar)+"?";
		...
		case "noble_36":
			if (pchar.sex == "man") {PlaySound("VOICE\Russian\LE\Domingo\Domingo_05.wav");}
			dialog.text = "Дядя Луис отправился туда, где ему самое место - в преисподнюю... Вас послали мне сами Небеса, "+GetAddress_Form(NPChar)+"! Я рад, что вы не поверили словам этого негодяя! На какой-то момент я даже испугался - что вы уйдёте и оставите меня в лапах этого мерзавца\nДа, я не безупречен, но эта гнусность -  мол, я нанял пиратов для убийства своих родителей!.. О нет, каким же подонком надо быть, чтобы обвинить меня в таком, в таком! Ну ничего, он своё получил!";
			link.l1 = "Рад, что мы вышли целыми из этой передряги, дон Доминго. Что нам делать дальше? Надеюсь, нам не предъявят обвинение?";
			link.l1.go = "noble_37";
		break;
		
		case "noble_37":
			dialog.text = "Обвинение?! Ха-ха! Я покажу любому обвинителю дядюшкин гроссбух с контрабандными сделками! И скажу, что во время ревизии негодяй попытался убить владельца плантации, благо, свидетелей того, как он напал на нас со своими клевретами - предостаточно. Не извольте волноваться, "+GetAddress_Form(NPChar)+". Мне пора показать наконец, кто здесь хозяин!";
			link.l1 = "Правильно! И обязательно выясните, кто пособничал вашему дяде в воровстве, и примерно накажите мерзавцев!";
			link.l1.go = "noble_38";
		break;
		
		case "noble_38":
			dialog.text = "Я так и поступлю, капитан! И если кто задумает чинить мне козни - пощады не будет! А теперь позвольте выразить вам свою глубочайшую признательность и благодарность, "+GetSexPhrase("кабальеро","сеньорита")+"! Я так рад, "+pchar.name+", что встретил вас на Эспаньоле! Сейчас вам нужно отдохнуть - ступайте в таверну Порто-Белло, выпейте чашу доброго вина, поспите, а завтра приходите сюда, в дом дядюшки\nЯ бы с удовольствием присоединился к вам, но... мне и в самом деле пора входить в курс дел на плантации. Жду вас завтра, капитан, я приготовлю для вас достойную награду!";
		...
		case "noble_39":
			...
			if (pchar.sex == "man") {PlaySound("VOICE\Russian\LE\Domingo\Domingo_05.wav");}
			dialog.text = "О, а вот и вы, мой "+GetAddress_Form(NPChar)+"!";
		...
		case "noble_40":
			...
			link.l1 = "Когда я помогал"+GetSexPhrase("","а")+" вам в сражениях, мной руководствовала вовсе не жажда получить награду, сеньор!..";
			link.l1.go = "noble_41";
		break;
		...
		case "noble_41":
			...
			link.l1 = "Это весьма щедро с вашей стороны, дон Доминго!";
			link.l1.go = "noble_42";
		break;
		
		case "noble_42":
			dialog.text = "Кроме того, у меня есть несколько вещей, которые мне точно не понадобятся, а вам, как капитану, очень пригодятся: это хорошая подзорная труба и три амулета. Возьмите, это всё ваше.";
			link.l1 = "Спасибо!";
			link.l1.go = "noble_43";
		break;
		...
		case "noble_43":
			...
			link.l1 = "Благодарствую, сеньор! Это было неожиданно...";
			link.l1.go = "noble_44";
		break;
		...
		case "noble_45":
			dialog.text = "Это точно. Был очень рад знакомству, "+GetAddress_Form(NPChar)+"! Удачи вам море!";
		...
		case "noble_48":
			...
			{
				dialog.text = "Ну а теперь я, к величайшему сожалению, вынужден с вами проститься. Мне нужно отправляться в Порто-Белло - назначена аудиенция у губернатора, затем отплывать в Картахену... В общем, моя прежняя весёлая жизнь подошла к концу, пора начинать жить по-новому. Мне так жаль с вами расставаться, "+pchar.name+"! Надеюсь, мы с вами ещё встретимся...";
				link.l1 = "Если так распорядится судьба, caballero! Желаю вам успехов в тяжбе с дядюшкой, дон Доминго! Уверена - вы победите!";
				link.l1.go = "noble_49";
			}
			else
			{
		...
		case "uncle":
			...
			if (pchar.sex == "woman") { dialog.text = "Кажется, я успел как раз вовремя. Мой непутёвый племянник и его новая пассия капитан "+GetFullName(pchar)+"... Да-да, "+GetSexPhrase("молодой человек","сеньорита")+", не удивляйтесь - я навёл о вас справки.";}
			else {dialog.text = "Кажется, я успел как раз вовремя. Мой непутёвый племянник и его ангел-хранитель капитан "+GetFullName(pchar)+"... Да-да, "+GetSexPhrase("молодой человек","сеньорита")+", не удивляйтесь - я навёл о вас справки.";}
		...
		case "uncle_3":
			if (pchar.sex == "woman") { dialog.text = "Мой дорогой племянник, как обычно - в своём репертуаре. Много крика и шума, звона шпаг и бокалов, но мало толка. Не знаю, что вы в нём нашли, сеньорита, он ведь...";}
			else { dialog.text = "Мой дорогой племянник, как обычно - в своём репертуаре. Много крика и шума, звона шпаг и бокалов, но мало толка. Не знаю, чем он вас так подкупил, что вы стали помогать этому недостойному юнцу...";}
			link.l1 = "Ха! Не найдя доказательств вины вашего племянника, вы решили совершить самосуд над ним, натравив на него убийц?";
			link.l1.go = "uncle_4";
		break;
		
		case "uncle_4":
			dialog.text = "Я? Помилуйте! Ваш "+GetSexPhrase("друг","... кхм... друг...")+" за недолгое время нажил себе столько врагов - удивительно, что он вообще до сих пор жив. Спросите у него - сколько свадеб он расстроил? Скольких оскорблённых юношей ранил или даже убил на дуэлях?";
			link.l1 = "Вообще-то его слуга Уго...";
			link.l1.go = "uncle_5";
		break;
		
		case "uncle_5":
			dialog.text = "Ах, "+GetSexPhrase("молодой человек","сеньорита")+", вы ещё ничего не смыслите в этой жизни! Уго души не чаял в этом выродке и на все его проделки смотрел, как на невинные шалости. Запомните: лезть в личные дела чужой семьи - неблагодарное дело, и зачастую очень опасное...";
		...
		case "uncle_6":
			...
			dialog.text = "Предупреждение. Вот что, капитан: "+GetSexPhrase("вы зашли слишком далеко, но ","")+"поскольку я считаю вас не своим врагом, а всего лишь жертвой заблуждения, то вот вам моё предложение: поворачивайтесь и уходите. Я не желаю кровопролития. Свои семейные дела мы решим как-нибудь без вас. "+GetSexPhrase("Я готов компенсировать вам ваши расходы, связанные с моим племянником:","А чтобы думалось вам легче, я готов заплатить вам:")+" "+FindRussianMoneyString(iTotalTemp)+" наличными. Берите их и забудьте сюда дорогу. Иначе мои охранники попросту убьют вас.";
		...
		case "uncle_8":
			if (pchar.sex == "woman") {dialog.text = "Я всё понял. Вы такая же как и мой племянник - глупая, безрассудная и заносчивая особа. Как раз ему под стать. Пора проучить вас, обоих!";}
			else {dialog.text = "Я всё понял. Вы ничем не отличаетесь от моего племянника - такой же глупый, вздорный юнец! Пора проучить вас, обоих!";}
			link.l1 = "...";
			link.l1.go = "exit";
		...
		case "ugo":
			...
			dialog.text = TimeGreeting()+", "+GetAddress_Form(NPChar)+". Чем могу служить?";
		...
		case "ugo_2":
			dialog.text = "Увы, досточтим"+GetSexPhrase("ый кабальеро","ая сеньорита")+", дона Доминго нет дома. Я сам бы очень хотел знать, где носит этого шалопая... ох, простите - молодого сеньора... Позвольте полюбопытствовать - кем вы ему приходитесь?";
			link.l1 = "Ну... так получилось, что я помог"+GetSexPhrase("","ла")+" дону Доминго. В таверне Порт-о-Пренса на него напали две тёмные личности. Я оказал помощь вашему сеньору в ликвидации этих... как он сказал? - dos maldita canalla. После дон Доминго пригласил меня в гости, однако сам был задержан французским офицером для разбирательств. Я надеял"+GetSexPhrase("ся","ась")+" - он уже всё уладил и вернулся домой...";
			link.l1.go = "ugo_3";
		break;
		
		case "ugo_3":
			dialog.text = "О, ужас! Какие страшные вещи вы рассказываете! Я немедленно должен бежать к коменданту - доложить, что дон Доминго в опасности! "+UpperFirst(GetAddress_Form(NPChar))+", вам нужно уйти, а мне - поторопиться!";
		...
		case "ugo_4":
			dialog.text = "Ох! Да как же вы можете? Этот наивный мальчуган - в лапах французов! А ещё ему угрожают бандиты! Нет-нет, немедленно к коменданту! "+GetSexPhrase("Сударь","Сударыня")+", ступайте, пожалуйста, и позвольте мне пройти!";
			link.l1 = "Хорошо-хорошо, не переживайте вы так. Я ухожу.";
			link.l1.go = "ugo_exit";
			link.l2 = "Сеньор, да что вы так разволновались? Говорю же вам - опасности нет. Позвольте мне остаться и подождать сеньора Доминго - мне нужно отдохнуть с дороги, а сами, если хотите, идите на доклад к коменданту.";
			link.l2.go = "ugo_5";
		break;
		...
		case "ugo_5":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+", я попрошу вас немедленно покинуть этот дом!";
			link.l1 = "Уго, не думаю, что слуге позволено разговаривать со мной таким тоном! Дону Доминго не понравится подобное обращение с человеком, спасшим ему жизнь!";
			link.l1.go = "ugo_6";
		break;
