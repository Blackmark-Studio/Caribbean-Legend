void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "FalseTrace_1":
			...
			dialog.text = "Кха! Приятно иметь дело с быстро мыслящим человеком, капитан... Суть дела такова: буквально на днях из одного порта должен выйти флейт с приличным грузом серебра. Корабль пойдёт один, без охранения - такая вот глупость, хе-хе\nЗахватить его - плёвое дело. Я сообщаю вам, откуда и куда идёт корабль, вы его захватываете, я получаю свою долю. Всё просто.";
			link.l1 = "И вы, конечно, хотите получить её наличными прямо сейчас?";
			link.l1.go = "FalseTrace_2";
		break;
		
		case "FalseTrace_2":
			dialog.text = "Я что, похож на мелкого жулика, кэп? Нет. Я поднимусь на борт вашей посудины и мы вместе пойдём искать добычу. Отсиживаться во время боя в трюме я не стану - я прошу назначить меня в абордажную команду офицером, временно, конечно\nУмений владеть саблей и пистолем мне не занимать. Так что разделю с вами все опасности пути... кхе... кха! А после того, как захватим флейт, вы выдадите мне мою долю серебром.";
			link.l1 = "И на какую часть добычи вы претендуете?";
			link.l1.go = "FalseTrace_3";
		break;
		
		case "FalseTrace_3":
			dialog.text = "Я не жадный, кэп. Моя доля составит столько, сколько влезет в трюм моего старого баркаса, это примерно 200 мер серебра. По моим сведениям, на флейте будет не меньше двух тысяч мер серебра, так что мне достанется десятая часть. Согласитесь, это справедливо.";
		...
		case "FalseTrace_7":
			...
			link.l1 = "Думаю, для тебя это не было неожиданностью... Где серебро, чёрт возьми?! Где серебро?! Я тебя спрашиваю! Капитан флейта заявил, что это просто фуражное военное судно! Ты обманул меня!";
			link.l1.go = "FalseTrace_8";
		break;
		
		case "FalseTrace_8":
			dialog.text = "Капитан, прошу вас, успокойтесь. Я вам сейчас постараюсь всё объяснить.";
			link.l1 = "Да, да! Я уже споко"+GetSexPhrase("ен","йна")+"! Ты до сих пор не болтаешься на нок-рее лишь потому, что я не могу понять мотивов твоего странного поступка - ты сам отправился со мной на охоту за этим флейтом, зная, что у него в трюмах нет никакого серебра. Так что уж изволь объясниться.";
			link.l1.go = "FalseTrace_9";
		break;
		
		case "FalseTrace_9":
			dialog.text = "Я именно это и хочу сделать, кэп. Да, я действительно обманул вас. Но в противном случае я бы никогда не смог убедить вас сделать то, что мне нужно, а именно - захватить... кхе... кха! Захватить этот треклятый флейт\nНо обещаю, что в долгу я не останусь, и сполна расплачусь за вашу помощь. Теперь к сути вопроса - зачем мне это было нужно. Вы нашли на флейте женщину? Я видел, как матросы вели её на ваше судно...";
		...
		case "FalseTrace_13":
			...
			link.l1 = "Что-то ваша история попахивает очередным 'флейтом с серебром'. Один раз вы меня уже обманули. Где гарантии, что вы и сейчас не лжёте?";
		...
		case "FalseTrace_25":
			...
			link.l1 = "По твоей фальшивой наводке я захватил"+GetSexPhrase("","а")+" пустой флейт и убил"+GetSexPhrase("","а")+" его капитана. Я сожалею, что сделал"+GetSexPhrase("","а")+" это, хотя уже поздно. Твоя жена, как я понял"+GetSexPhrase("","а")+", любила этого моряка. Поэтому она заказала тебя. Мне. Вот почему мы здесь втроём, в этой бухте.";
		...
		case "FalseTrace_abordage_2":
			dialog.text = "Повторяю - у меня нет никакого груза. Этот флейт - военное фуражное судно, и оно направлялось в "+XI_ConvertString("Colony"+pchar.questTemp.FalseTrace.TargetCity)+" за продовольствием для солдат и за пушечными ядрами.";
		...
		case "FalseTrace_wife_6":
			if (CheckAttribute(pchar, "questTemp.FalseTrace.KillCaptain"))
			{
				...
				link.l1 = "Полно, милочка. Никто никуда тебя бросать не собирается. Я вообще хочу прояснить ситуацию, поскольку нападением на флейт я обязан"+GetSexPhrase("","а")+" именно тебе, и никому другому.";
		...
		case "FalseTrace_wife_10":
			...
			link.l1 = "В общем, получилось именно так. Адам солгал мне о грузе серебра, чтобы я захватил"+GetSexPhrase("","а")+" флейт '"+pchar.questTemp.FalseTrace.ShipName+"', с очевидной целью вернуть тебя. Четверть часа назад он дал мне полный отчёт о своих действиях...";
		...
		case "FalseTrace_wife_14":
			dialog.text = "Поначалу он был другим. А его прошлое казалось мне тогда таким романтичным... Но потом всё изменилось. И я не хочу его теперь даже видеть, не то, что жить с ним, понимаете - не хочу! Особенно после того, как он организовал нападение на флейт!";
		...
		case "FalseTrace_Remove1":
			dialog.text = "Ну и растяпа же вы, капитан! Прозевать флейт, полный серебра... Зря я связался с вами. Прощайте!";
			link.l1 = "Прощай, Адам.";
