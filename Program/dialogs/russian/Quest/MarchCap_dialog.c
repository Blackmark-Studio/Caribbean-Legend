	...
	switch(Dialog.CurrentNode)
	{
		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				case 0: //конвой у острова, установка параметров
					...
					dialog.text = "Мне удалось разузнать, что через "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" небольшой торговый конвой под флагом "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", два купеческих судна и корабль охранения, будут проходить у "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+", это у "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+". В трюмах купцов полно "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+". Было бы глупо не воспользоваться этой информацией, капитан\nВдвоём мы легко разделаемся с сопровождением, а содержимое трюмов торгашей станет нашим. Ну как, вы согласны отправиться со мной в этот поход?";
					link.l1 = "Звучит заманчиво. Пожалуй, я соглас"+GetSexPhrase("ен","на")+"!";
					link.l1.go = "MarchCap_1_1";
					link.l2 = "Из-за какого-то груза "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" портить отношения с "+NationNameAblative(int(pchar.GenQuest.MarchCap.Nation))+"? Нет, я на такое не подпишусь! Прощайте, сударь, и даже не пытайтесь меня задерживать!";
				...
				case 1: //энкаунтер на глобалке, установка параметров
					...
					dialog.text = "Мне удалось разузнать, что через "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+" небольшой торговый конвой под флагом "+NationNameGenitive(int(pchar.GenQuest.MarchCap.Nation))+", два купеческих судна и корабль охранения, выйдут из "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Startcity+"Gen")+" и отправятся в "+XI_ConvertString("Colony"+pchar.GenQuest.MarchCap.Finishcity+"Acc")+". В трюмах купцов полно "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+". Было бы глупо не воспользоваться этой информацией, капитан\nВдвоём мы легко разделаемся с сопровождением, а содержимое трюмов торгашей станет нашим. Основная трудность - отыскать их в море между этими двумя городами... Ну как, вы согласны отправиться со мной в этот поход?";
					link.l1 = "Звучит заманчиво. Пожалуй, я соглас"+GetSexPhrase("ен","на")+"!";
				...
				case 2: //одиночный пиратский кулсейлор у бухты, установка параметров
					...
					dialog.text = "Я получил достоверные сведения об одном пирате по кличке "+GetName( NAMETYPE_ORIG, pchar.GenQuest.MarchCap.PirateName, NAME_NOM)+". Так вот, этот головорез недавно хорошенько пощипал "+RandPhraseSimple(RandPhraseSimple("англичан","французов"), RandPhraseSimple("испанцев","голландцев"))+" и загрузил себе полный трюм "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+". Но вот ведь незадача: в бою его "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.ShipType), "Name")+"Acc"))+" так сильно потрепали, что он со всем этим грузом был вынужден встать на ремонт\nСейчас он находится у "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+", это у "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+", устанавливает новые мачты и латает пробоины в корпусе. По моим расчётам, у него на это уйдёт не больше, чем "+FindRussianDaysString(int(pchar.GenQuest.MarchCap.DaysQty))+". Если постараемся - то сможем успеть к нему на свидание, и тогда весь груз "+GetGoodsNameAlt(int(pchar.GenQuest.MarchCap.Goods))+" станет нашим\nСправиться в одиночку с этим пиратом мне не под силу - он весьма искушен в ведении морского боя, но вдвоём мы сумеем разделаться с ним, несмотря на весь его опыт. Ну как, вы согласны отправиться со мной в этот поход?";
					link.l1 = "Звучит заманчиво. Пожалуй, я соглас"+GetSexPhrase("ен","на")+"!";
		...
		case "MarchCap_AfterBattle":
			...
			if (iTemp > 50 && iTemp < 500)
			{
				...
				link.l1 = "Если бы вы мне помогали, как следует, а не бакланов в небе считали, рейд был бы удачным... Забирайте свою долю - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+", и проваливайте!";
				link.l1.go = "MarchCap_Talk_exit";
			...
			{
				dialog.text = "Неплохо сработали, капитан! Как я вам и говорил - мы без труда опустошим трюмы этих торгашей, хе-хе... Добыча, как я вижу, составила "+FindRussianQtyString(iQty)+" товара. Моя доля - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			...
			dialog.text = "Отличная работа, капитан! Как я вам и говорил - мы без труда опустошим трюмы этих торгашей, хе-хе... Добыча превзошла все мои ожидания и, как я вижу, составила "+FindRussianQtyString(iQty)+" товара. Моя доля - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
		...
		case "MarchCap_AfterBattleGold":
			...
			{
				dialog.text = "Неплохо сработали, капитан! Как я вам и говорил - вдвоём мы справились с этим пиратом! Добыча, как я вижу, составила "+FindRussianQtyString(iQty)+" товара. Моя доля - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
				link.l1 = "Вы всё верно подсчитали, "+npchar.name+". У меня нет возражений. Шлюпки уже грузятся. Возможно, ещё когда-нибудь встретимся с вами...";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "good";
				break;
			}
			dialog.text = "Отличная работа, капитан! Как я вам и говорил - вдвоем мы справимся с этим пиратом! Добыча превзошла все мои ожидания и, как я вижу, составила "+FindRussianQtyString(iQty)+" товара. Моя доля - "+FindRussianQtyString(int(pchar.GenQuest.MarchCap.CapPart))+".";
			link.l1 = "Вы всё верно подсчитали, "+npchar.name+". У меня нет возражений. Шлюпки уже грузятся. Может, ещё как-нибудь сходим в совместный рейд, а? Вы мне кажетесь вполне подходящим компаньоном.";
			link.l1.go = "MarchCap_Deck_continue";
			pchar.GenQuest.MarchCap = "exellent";
		break;
