void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Regata_Head_8":
			dialog.text = "Победитель в регате только один - капитан, чей корабль вернётся в Порт-Ройял первым. Награды весьма солидные. Победитель получает крупную денежную сумму - 250 000 песо. Также ему будет вручён комплект ценных призов.";
		...
		case "Regata_Start":
			dialog.text = "Итак, "+GetSexPhrase("джентльмены","джентльмены... и одна леди")+", вы все собрались. Я объявляю парусную регату открытой. Правила вы знаете. Побеждает тот, чей корабль вернётся в Порт-Ройял первым. Дамы и господа, пожелаем нашим отважным капитанам удачи!\n"+GetSexPhrase("Джентльмены","Участники регаты")+"! Вы готовы? На старт! Внимание! Марш!!";
			link.l1 = "...";
		...
		case "Regata_Contra_3":
			if (int(pchar.questTemp.Regata.Index != 1)) sTemp = "а вы тем временем опередите его и займёте его место";
		...
		case "Regata_Spyglass_2":
			...
			dialog.text = "Вот, посмотрите. Она мне досталась от отца, он тоже был моряком, как и вы. Мне она не нужна, а вам может пригодиться. И прошу я за неё всего лишь "+FindRussianMoneyString(int(pchar.questTemp.Regata.SpyPrice))+".";
			if (int(pchar.money) >= int(pchar.questTemp.Regata.SpyPrice))
			{
				link.l1 = "Так-так... "+sTemp+"! Очень интересно. Пожалуй, я куплю её у вас. В том числе, и за ваши красивые глаза. Вы хотите за неё "+FindRussianMoneyString(int(pchar.questTemp.Regata.SpyPrice))+"? Пожалуйста, получите.";
		...
		case "Regata_skiper":
			...
			dialog.text = "Ваш путь лежит в Порт-Ройял? Самый короткий путь - через Кюрасао. Вернитесь в Бриджтаун. Там найдите человека по имени Вудро Доггерти - он старый моряк и опытный лоцман. Он как свои пять пальцев знает воды между Барбадосом и Кюрасао и проведёт ваш корабль кратчайшим путём\nВы сэкономите на этом время, столь необходимое вам для победы. Найти Вудро вы скорее всего сможете в церкви. Скажете, что вас прислал"+ NPCharSexPhrase(NPChar, "","а") +" " + GetFullName(npchar) + " - он вам не откажет. Высадите его потом в Порт-Рояле - он всё равно туда собирался.";
		...
		case "Regata_SiegeSkiper_1":
			...
			link.l1 = "Мне известно, что вы опытный штурман и хорошо знаете воды между Барбадосом и Кюрасао. Также мне известно, что вам нужно в Порт-Ройял. Я - участник регаты, мне предстоит последний переход как раз до Порт-Ройяла, и я хочу попросить вас войти в мою команду в качестве штурмана и лоцмана на этот переход.";
		...
		case "Regata_SiegeSkiper_3"://а тут - как уж свезет
			if(hrand(2) != 2)
			{
				dialog.text = "От "+pchar.questTemp.Regata.Name+"? Это, конечно, всё проясняет. Хорошо, капитан, я помогу проложить вам кратчайший путь от Бриджтауна до Порт-Ройяла. Вы отправляетесь прямо сейчас?";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				if (n==1)
				{
					dialog.text = "Капитан "+GetFullName(pchar)+", поздравляю вас с победой! Вы затратили на регату "+int(pchar.questTemp.Regata.FifthTransitionTime)+" часов, оставив позади всех противников. Это отличный результат!";
				...
				else
				{
					if (n==6)
					{
						dialog.text = "Капитан, увы - но ваш результат последний - вы затратили на регату "+int(pchar.questTemp.Regata.FifthTransitionTime)+" часов. Не огорчайтесь, у вас были действительно сильные противники.";
					...
					{
						dialog.text = "Капитан, вы заняли "+n+" место. Поздравляю с успешным прохождением маршрута, вы показали достойное время - "+int(pchar.questTemp.Regata.FifthTransitionTime)+" часов. И хоть вы и не выиграли, я всё равно поздравляю вас!";
		...
		case "Give_advantage_1":
			dialog.text = "С учётом сделанных ставок, ваш выигрыш составляет "+FindRussianMoneyString(int(pchar.questTemp.Regata.Advantage))+". Пожалуйста, получите. Поздравляю ещё раз с победой!";
			link.l1 = "Спасибо!";
		...
		case "Give_advantage_2":
			...
			dialog.text = "Безусловно, ваша популярность среди жителей Порт-Ройяла выросла. Думаю, вы вполне сможете это заметить... правда, память у людей короткая, увы. Так что купайтесь в лучах славы, пока они ещё яркие!";
