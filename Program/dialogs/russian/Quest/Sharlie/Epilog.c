void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SharlieEpilog_Cureer_2":
			dialog.text = "Велено передать вам прямо в руки. За оплату не беспокойтесь - аббат заблаговременно покрыл все мои расходы.";
			link.l1 = "Ну что ж, давайте её сюда. Благодарю вас.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Pikar_2":
			if (IsStateTitle())
			  ...
			  {
				dialog.text = "Ха-ха! Ну это ты зря. Как видишь - живее всех живых. Вот, решил себе 'чистенький' кораблик прикупить. Ну а ты... рассказывай: какими судьбами тебя сюда занесло, а, дружище?";
			  }
		...
		case "SharlieEpilog_Mary_9":
			dialog.text = "Возможно, стоит устроить на прощание небольшой пир в таверне?";
		...
		case "SharlieEpilog_Dassen":
			dialog.text = "Какие будут приказания, "+GetTitle(NPChar, true)+"?";
			link.l1 = ""+npchar.name+", не буду тянуть - я оставляю архипелаг. Дела семейные требуют моего присутствия в поместье моего отца.";
			link.l1.go = "SharlieEpilog_Dassen_1";
		break;
		
		case "SharlieEpilog_Dassen_1":
			dialog.text = "Если я правильно понимаю, мы с вами расстаёмся?";
			link.l1 = "Увы... Я не могу поступить иначе.";
		...
		case "SharlieEpilog_Dassen_nothing":
			dialog.text = "Я искренне разделяю эту надежду, "+GetTitle(NPChar, true)+".";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "Зовите меня просто Шарлем. И, кстати, у меня для вас есть кое-что.";
		...
		case "SharlieEpilog_Dassen_salary":
			dialog.text = "Благодарю вас, "+GetTitle(NPChar, true)+".";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "Зовите меня просто Шарлем. И, кстати, у меня для вас есть ещё кое-что.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
			}
			else
			{
		...
		case "SharlieEpilog_Dassen_salaryX3":
			dialog.text = "Весьма признателен, "+GetTitle(NPChar, true)+", вы слишком щедры ко мне.";
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_IslaMona_France"))
			{	
				link.l1 = "Зовите меня просто Шарлем. И, кстати, у меня для вас есть ещё кое-что.";
				link.l1.go = "SharlieEpilog_Dassen_Governor";
		...
		case "SharlieEpilog_Dassen_Governor":
			dialog.text = "Вы меня заинтриговали, "+pchar.name+".";
			link.l1 = "Я рекомендовал вас на должность губернатора Исла-Моны. Скоро из канцелярии придёт соответствующее письмо, и я очень надеюсь, что вы не ответите на него отказом. Этот остров очень много для меня значит, и я был бы счастлив видеть вас во главе его управления.";
			link.l1.go = "SharlieEpilog_Dassen_Governor_1";
		break;
		
		case "SharlieEpilog_Dassen_Governor_1":
			dialog.text = ""+UpperFirst(GetAddress_Form(NPChar))+"... "+pchar.name+"... Это весьма неожиданно...";
			link.l1 = "И тем не менее. Каков будет ваш ответ?";
			link.l1.go = "SharlieEpilog_Dassen_Governor_2";
		break;
		
		case "SharlieEpilog_Dassen_Governor_2":
			dialog.text = "Это предложение, от которого не отказываются. Я с удовольствием приму его и обещаю сделать всё для процветания этого острова и благополучия его жителей.";
			link.l1 = "Превосходно, мой друг! Тогда жду вас в таверне, на празднестве в честь моего отплытия. Алонсо сообщит вам, когда всё будет готово.";
			link.l1.go = "SharlieEpilog_Dassen_Governor_3";
		break;
		
		case "SharlieEpilog_Dassen_Governor_3":
			dialog.text = "Ни за что этого не пропущу, монсеньор.";
			link.l1 = "";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		
		case "SharlieEpilog_Dassen_Tavern":
			dialog.text = "Ни за что этого не пропущу, "+pchar.name+".";
			link.l1 = "Превосходно, мой друг. Тогда встретимся там. Алонсо даст вам знать, когда мы будем готовы начать.";
			link.l1.go = "SharlieEpilog_Dassen_Leave";
		break;
		...
		case "SharlieEpilog_PikarOnUliss_C_3":
			...
			link.l1.go = "SharlieEpilog_PikarOnUliss";
			AddDialogExitQuestFunction("SharlieEpilog_LetterFromBenua");
		break;
		...
		case "SharlieEpilog_PikarOnUliss_D_1":
			if (CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarAlive") || CheckAttribute(pchar, "questTemp.SharlieEpilog_PikarResurrected")) {dialog.text = "Твоя правда.";}
		...
		case "SharlieEpilog_Sailor_1":
			...
			if (!CheckAttribute(npchar, "SpeaksOnce"))
			{
				PlaySound("Voice\" + LanguageGetLanguage() + "\officer05.wav");
				npchar.SpeaksOnce = true;
			}
		break;
		...
		case "SharlieEpilog_KomendantFort_5":
			dialog.text = "Превосходный план, Ваша Светлость! Однако... вы человек, без сомнения, закалённый в морских боях, и всё же людям вашего положения не пристало рисковать жизнью. Флот вполне может возглавить и...";
			link.l1 = "Как не вынес я тихой жизни в Европе, так не вынесу и праздного ожидания на берегу, пока решается исход боя. От судьбы не уйти, "+GetAddress_Form(NPChar)+" "+npchar.lastname+". ";
			link.l1.go = "SharlieEpilog_KomendantFort_6";
		break;
			
		case "SharlieEpilog_KomendantFort_6":
			dialog.text = "Да будет по-вашему. Разрешите отдать приказ о начале приготовлений?";
			link.l1 = "Приступайте. И да прибудет с нами Господь.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Guard":
			dialog.text = "";
			link.l1 = "Стража, вызовите Алонсо.";
			link.l1.go = "exit";
		...
		case "SharlieEpilog_Mishelle_3":
			if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer")) sStr = " и женой";
			dialog.text = "Так ты ещё не понял? Ты умер. Вместе со своей командой"+sStr+". Очередной торговец, которого ты решил ограбить, подорвал крюйт-камеру - лишь бы ни гроша не досталось таким, как ты\nSpectra sunt memoria peccatorum. Это ты призрак, "+pchar.name+"... Карибская легенда о человеке, которого пожрали алчность и слепое тщеславие. Скольких ты убил ради золота?";
