...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "guide_61": // побили третий раз
			dialog.text = "Да, что-то слабоваты вы, "+pchar.name+". Но не огорчайтесь сильно: у вас ещё будет масса времени и возможностей отточить своё мастерство. Но будьте предельно осторожны и не лезьте попусту на рожон, пока не научитесь владеть шпагой получше.";
