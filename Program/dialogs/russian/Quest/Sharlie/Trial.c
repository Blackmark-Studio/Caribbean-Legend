...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "lecrua_1":
			...
			{
				link.l1 = "У меня "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(RealShips[int(pchar.ship.type)].basetype), "Name")))+", называется - '"+pchar.ship.name+"'.";
		...
		case "florian_deck_5":
			...
			dialog.text = "Но это ещё не всё, капитан. Хотите заработать ещё 75 монет?";
