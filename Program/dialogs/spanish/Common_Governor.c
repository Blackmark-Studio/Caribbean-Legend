...
void ProcessDialogEvent()
{
	...
	case "First time":
		...
		{
			dialog.text = "He oído que has sido muy persistente al solicitar una audiencia. Mi nombre es " + GetFullName(npchar) + ". Soy el gobernador general de las colonias de " + NationNameGenitive(int(NPChar.nation)) + ", delegado de la Corona de " + NationKingsName(npchar) + " en estas aguas. Ahora, por favor, sea tan amable de decirme, ¿cuál es el propósito de su visita, " + GetAddress_Form(NPChar) + "?";
		...
		{
			dialog.text = RandPhraseSimple("¿Oh, eres tú de nuevo? Bueno, ¿qué quieres del gobernador general de " + NationNameGenitive(int(NPChar.nation)) + "¿esta vez?", "¿Y una vez más, me estás distrayendo de asuntos de estado importantes? ¿Qué quieres, " + GetAddress_Form(NPChar) + "?");
			link.l1 = "Quiero hablar contigo sobre trabajar en nombre de la Corona de " + NationNameGenitive(int(NPChar.nation));
			link.l1.go = "work";
	...
	case "node_1":
		...
		link.l1 = "Quería hablar contigo sobre trabajar en nombre de la Corona de " + NationNameGenitive(int(NPChar.nation));
		link.l1.go = "work";
