	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (LAi_grp_playeralarm > 0)
			{
				...
				link.l1 = NPCharRepPhrase(pchar,RandPhraseSimple("Je, una alarma nunca es un problema para mí...","Ellos nunca me atraparán."),RandPhraseSimple("Cierra la boca, "+GetWorkTypeOfMan(npchar,"")+", ¡y tal vez no te arranque la sucia lengua!","¡Hey, "+GetWorkTypeOfMan(npchar," ")+", todos atrapen a los piratas! Eso es lo que te digo, compañero: mantente callado y vivirás..."));
				link.l1.go = "fight";
				break;
			}
			//Jason, фрахт
...
//-------------------------------------------//Jason, эскорт---------------------------------------------------	
				...
				{
					dialog.text = "Excelente. Estoy a su servicio, "+GetFullName(PChar)+".";
					Link.l1 = "Fui enviado aquí por el maestro del puerto "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.StartCity+"Gen")+". Me pidieron escoltar un barco que lleva un cargamento de armas y municiones. Aquí están mis papeles.";
				...
				{
					dialog.text = "Excelente. Estoy a su servicio, "+GetFullName(PChar)+".";
					Link.l1 = "He encontrado la embarcación desaparecida en las costas de la isla de "+XI_ConvertString(pchar.questTemp.WPU.Current.TargetIslandID)+". Bueno, no el barco, en realidad, sino más bien alrededor de medio centenar de hombres de su tripulación y el capitán. Ahora están a salvo, la tripulación ha subido a mi embarcación y el capitán está en tu ciudad, supongo que ya te ha hecho una visita.";
...
// <-- эскорт
		...
		case "Church_GenQuest1_Node_CapOnThisColony_1":
			dialog.text = "¿Qué quieres saber, "+GetSexPhrase("señor","señorita")+" "+GetFullName(PChar)+"?";
...
//			if(rand(1) == 0) PChar.GenQuest.ChurchQuest_1.CapWaitOnTavern = true;
		...
		case "ShipLetters_Portman1_1":
			...
			dialog.text = s1+"Decid, ¿consideraréis una suma de "+int(pchar.questTemp.different.GiveShipLetters.price1)+"¿pesos una recompensa adecuada?";
...
///Jason-----------------------новые генераторы накопительного типа для портмана----------------------------
		case "Work_check"://фейс-контроль - общая проверка профпригодности ГГ и выбор задания
			...
			if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") >= 2)//проверка повтора
				{
					...
					{
						dialog.text = RandPhraseSimple("No hay trabajo para aquellos que navegan bajo la bandera de "+NationNameGenitive(int(pchar.nation))+"¡Sal de mi oficina de inmediato!","No colaboraré con "+NationNameAblative(int(pchar.nation))+"¡Fuera!");
					...
					link.l2 = "Quiero ganar dinero extra escoltando comerciantes.";
					Link.l2.go = "escort_begin";	//эскорт
					link.l3 = "Quiero ganar dinero extra entregando correo.";
					Link.l3.go = "cureer_begin";	//доставка почты
					Link.l4 = "Bueno, ¿quizás podrías ofrecer algo?";
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "¿También pretende escoltar convoyes en una simple tartana? Para este trabajo se necesita un buque con suficiente potencia de fuego — un warrior o un raider.";
				link.l1 = "Vale, lo entiendo";
				link.l1.go = "exit";
			}
		}
		else
		...
		if (hrand(5) > 1)
		{
			...
			{
				//не тот тип корабля
				dialog.text = "Lo siento, pero no tengo nada para ti. Este tipo de encargo requiere un barco raider ágil. Con el tuyo es mejor dedicarse al flete.";
				link.l1 = "Vale, lo entiendo";
				link.l1.go = "exit";
			}
		}
...
///--> ------фрахт со свободным выбором пункта назначения, оплаты и вида груза из предложенного списка---------
		...
		case "Fraht_begin":
			if (hrand(4) < 4)
			{
				...
				{
					dialog.text = "Tienes muy poco espacio en tu bodega. Necesito un barco más espacioso para un flete. Te sugiero que revises en la tienda: el comerciante fleta embarcaciones frecuentemente para entregar pequeños lotes de carga.";
		...
		case "Fraht_choise":
			...
			{
				dialog.text = "Hay disponibles las siguientes variantes:\n"+"carga "+GetGoodsNameAlt(iFrahtGoods1)+" en cantidad de "+FindRussianQtyString(iFrahtGoodsQty1)+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+", en "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\ncarga "+GetGoodsNameAlt(iFrahtGoods2)+" en cantidad de "+FindRussianQtyString(iFrahtGoodsQty2)+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+", en "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"\ncarga "+GetGoodsNameAlt(iFrahtGoods3)+" en cantidad de "+FindRussianQtyString(iFrahtGoodsQty3)+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City3)+", en "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty3)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money3))+"¿Cuál es tu elección?";
				Link.l1 = "Elegiré la segunda opción, charter a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+" cargado con "+GetGoodsNameAlt(iFrahtGoods1)+".";
				Link.l1.go = "Fraht_Choise_1";
				Link.l2 = "Escogeré la tercera opción, charter hacia la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+" cargado con "+GetGoodsNameAlt(iFrahtGoods2)+".";
			...
			{
				dialog.text = "Hay las siguientes variantes disponibles:\n"+"carga "+GetGoodsNameAlt(iFrahtGoods1)+" en cantidad de "+FindRussianQtyString(iFrahtGoodsQty1)+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+", en "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty1)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money1))+"\ncarga "+GetGoodsNameAlt(iFrahtGoods2)+" en cantidad de "+FindRussianQtyString(iFrahtGoodsQty2)+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+", en "+FindRussianDaysString(pchar.questTemp.WPU.Fraht.DaysQty2)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money2))+"¿Cuál es tu elección?";
				Link.l1 = "Elegiré la primera opción, carta de navegación a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City1)+" cargado con "+GetGoodsNameAlt(iFrahtGoods1)+".";
				Link.l1.go = "Fraht_Choise_1";
				Link.l2 = "Elegiré la segunda opción: chárter a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Fraht.City2)+" cargado con "+GetGoodsNameAlt(iFrahtGoods2)+".";
				Link.l2.go = "Fraht_Choise_2";
			}
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (amount > 0)
			{
				dialog.text = "Veamos, entonces. Se suponía que debías entregar una carga de "+GetGoodsNameAlt(iFrahtGoods)+" en cantidad de "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+". Pero como puedo ver, no entregaste todo el cargamento, eres "+FindRussianQtyString(int(amount))+" corto.";
		...
		case "Fraht_complete_1":
			if (pchar.questTemp.WPU.Fraht == "late")//опоздали
			{
				...
				dialog.text = "Mm... Pero tú estabas "+FindRussianDaysString(iTime)+" tarde. Por lo tanto, debo reducir tu pago por el flete. Según el contrato, tu multa es del cinco por ciento del monto total por cada día de retraso. Ahora, por favor, toma tu "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+", y trata de ceñirte al contrato en el futuro.";
				link.l1 = "Bueno, es algo al menos.";
				link.l1.go = "Fraht_complete_2";
				break;
			}
			dialog.text = "Gracias por tu trabajo. Por favor, toma tu pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
			link.l1 = "¡Gracias!";
			link.l1.go = "Fraht_complete_2";
		break;
		...
		case "Fraht_complete_bad_1":
			...
			dialog.text = "Tres veces el costo de las mercancías. Esto asciende a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+".";
		...
		case "Fraht_complete_fail":
			dialog.text = "Oh, ya veo... Bueno, en ese caso nunca deberías mostrar tu cara de nuevo en ninguna autoridad portuaria de "+NationNameGenitive(int(pchar.questTemp.WPU.Fraht.Nation))+"¡Lárgate!";
...
/// -------------------------------работа курьером по доставке почты-------------------------------------
		...
		case "Postcureer":
			...
			dialog.text = "Este paquete debe ser entregado a las autoridades portuarias de la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City)+", para "+FindRussianDaysString(pchar.questTemp.WPU.Postcureer.DaysQty)+", y tan pronto como sea posible. Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
		...
		case "Postcureer_complete":
			if (pchar.questTemp.WPU.Postcureer == "late")//опоздали
			{
				...
				dialog.text = "Mm... Pero tú estabas "+FindRussianDaysString(iTime)+" tarde. Por lo tanto, debo reducir su pago por el trabajo. Según el contrato, su penalización es del cinco por ciento del monto total por cada día de retraso. Ahora por favor, tome su "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+", y trate de ceñirse al contrato en el futuro.";
				link.l1 = "Bueno, es algo al menos.";
				link.l1.go = "Postcureer_complete_1";
				break;
			}
			dialog.text = "Gracias por tu trabajo. Por favor, toma tu pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
			link.l1 = "¡Gracias!";
			link.l1.go = "Postcureer_complete_1";
		break;
...
/// ---------------------------работа в качестве эскорта для торговых кораблей----------------------------
		...
		case "escort":
			...
			dialog.text = "Entonces, necesitarás escoltar barcos '"+pchar.questTemp.WPU.Escort.ShipName1+"' y '"+pchar.questTemp.WPU.Escort.ShipName2+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+", para "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"Ambos barcos deben llegar a su destino sanos y salvos. Entregarás los barcos al funcionario de la autoridad portuaria. Él también será quien te pague por el trabajo.";
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				...
				if (amount > 0)
				{
					dialog.text = "Está bien, entonces. Se suponía, que escoltarías barcos y entregarías la carga de "+GetGoodsNameAlt(iGoods)+" en cantidad de "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+". Pero veo que no has entregado todo el cargamento, te faltan "+FindRussianQtyString(int(amount))+".";
					link.l1 = "¡Mil tiburones! ¡Deben ser esas malditas ratas! Pero no te preocupes, "+GetAddress_FormToNPC(NPChar)+", compraré la cantidad faltante y entregaré todo el lote.";
					link.l1.go = "Fraht_complete_3";
					break;
				}
			...
			if (pchar.questTemp.WPU.Escort == "late")//опоздали
			{
				...
				dialog.text = "Mm... Pero tú estabas "+FindRussianDaysString(iTime)+" tarde. Por lo tanto, debo reducir tu pago por el trabajo. Según el contrato, tu multa es del cinco por ciento del monto total por cada día de retraso. Ahora, por favor, toma tu "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+", y trate de ceñirse al contrato en el futuro.";
				link.l1 = "Bueno, es algo al menos.";
			...
			if (CheckAttribute(pchar, "questTemp.WPU.Escort.Bonus")) 
			{
				dialog.text = "Está bien, entonces. Se suponía, que debías escoltar barcos y entregar la carga de "+GetGoodsNameAlt(iGoods)+" en cantidad de "+FindRussianQtyString(int(pchar.questTemp.WPU.Escort.GoodsQty))+". Gracias por tu trabajo. Por favor, toma tu pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			else
			{
				dialog.text = "Gracias por tu trabajo. Por favor, toma tu pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+".";
			}
			link.l1 = "¡Gracias!";
			link.l1.go = "Escort_complete_1";
		}
		...
		case "escort_bonus":
			...
			dialog.text = "Entonces, necesitarás escoltar barcos '"+pchar.questTemp.WPU.Escort.ShipName1+"y"+pchar.questTemp.WPU.Escort.ShipName2+" a la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.City)+", por "+FindRussianDaysString(pchar.questTemp.WPU.Escort.DaysQty)+". Pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"\nTambién en tu barco, coloco la carga de "+GetGoodsNameAlt(iGoods)+" en cantidad de "+FindRussianQtyString(iGoodsQty)+". Pago por la entrega, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+".";
			link.l1 = "Todo parece claro. ¿Debería proceder?";
		...
		case "EscortBonus_complete_fail":
			...
			if (pchar.questTemp.WPU.Escort == "begin")
			{
				...
				if (amount > 0 && amount <= 100)
				{
					...
					dialog.text = "¡Increíble! ¿Y para qué te necesitábamos? ¿Cómo pudiste permitir que esto sucediera? Espero que entiendas que cualquier recompensa por la escolta ahora está fuera de discusión... No solo eso, sino que también te falta carga, "+FindRussianQtyString(int(amount))+"\nEn general, te pagaré solo por el flete de las mercancías que fueron entregadas en condiciones aceptables, menos el valor de los artículos faltantes. Consigue tu "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+" y puedes ir a donde quieras.";
					link.l1 = "Bueno, es algo al menos.";
				...
				dialog.text = "¡Increíble! ¿Y para qué te necesitábamos? ¿Cómo pudiste permitir que esto sucediera? Espero que entiendas que cualquier recompensa por la escolta ahora está fuera de discusión. En general, solo pagaré por el flete de las mercancías que has tomado. Consigue tu "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.BonusMoney))+" y puedes ir a donde quieras.";
				link.l1 = "Bueno, es algo al menos.";
				link.l1.go = "EscortBonus_complete_fail_1";
				break;
			}
			if (pchar.questTemp.WPU.Escort == "late")
			{
				...
				if (amount > 0)
				{
					dialog.text = "¡Increíble! ¿Y para qué te necesitábamos? ¿Cómo pudiste permitir que esto sucediera? Espero que entiendas que cualquier recompensa por la escolta ahora está fuera de discusión... Has perdido un barco, llegaste "+FindRussianDaysString(iTime)+" tarde, y además de eso, también te falta carga, "+FindRussianQtyString(int(amount))+"\nNo tengo tiempo para esperar a que adquieras las mercancías faltantes. Así que tienes que compensar el costo por completo "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))+". En cuanto a la carga, puedes quedártela.";
					if (int(pchar.money) >= int(pchar.questTemp.WPU.Fraht.GoodsAverigeCost))
					{
		...
		case "EscortBonus_complete_fail_3"://потеряли корабль и груз, время норма или нет, война
			dialog.text = "Oh, ya veo... Bueno, en ese caso nunca deberías mostrar tu rostro de nuevo en ninguna autoridad portuaria de "+NationNameGenitive(int(pchar.questTemp.WPU.Escort.Nation))+"¡Lárgate!";
			link.l1 = "Seguramente no me perdí de mucho...";
			link.l1.go = "exit";
		...
		case "Escort_fail_complete":
			...
			if (amount > 0)
			{
				dialog.text = "Veamos, entonces. Se suponía que debías entregar una carga de "+GetGoodsNameAlt(iFrahtGoods)+" en cantidad de "+FindRussianQtyString(pchar.questTemp.WPU.Escort.GoodsQty)+". Pero veo que no has entregado todo el cargamento, estás "+FindRussianQtyString(int(amount))+" corto.";
...
//-------------------------------------задания 2 уровня для курьера---------------------------------------------
		case "Postcureer_LevelUp":
			...
			{
				case 0:
					dialog.text = "Necesitarás entregar un despacho oficial urgente al gobernador de la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" no más tarde de "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Ten mucho cuidado, nuestros enemigos podrían estar buscando ese despacho también. Pago por la entrega, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "¡Estoy de acuerdo!";
				...
				case 1:
					dialog.text = "Necesitarás entregar estos documentos al dueño de la tienda de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" no más tarde de "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Este es el último resumen de los precios en las tiendas de todo el archipiélago, un valioso documento comercial. Hay que tener cuidado, los competidores pueden organizar una caza. Pago por la entrega, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "¡De acuerdo!";
					link.l1.go = "Postcureer_LevelUp_Go";
...
//					pchar.questTemp.WPU.Current.Add = "сводку цен для хозяина магазина";
				...
				case 2:
					dialog.text = "Necesitarás entregar este paquete de documentos al prestamista en el pueblo de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" no más tarde de "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Es una información muy importante sobre las tasas de cambio y los precios de las acciones. Ten mucho cuidado, ya que algunos personajes sospechosos ya están interesados en obtener este paquete. El pago por la entrega, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "¡Estoy de acuerdo!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "No, supongo que pasaré.";
					link.l2.go = "Postcureer_LevelDown";
				...
				case 3:
					dialog.text = "Deberías entregar este paquete al maestro del muelle en la ciudad de "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+" no más tarde de "+FindRussianDaysString(int(pchar.questTemp.WPU.Postcureer.DaysQty))+". Contiene planos para el nuevo "+RandPhraseSimple(LinkRandPhrase("bergantín","corbeta","fragata"),LinkRandPhrase("filibote","pinaza","galeón"))+", empleando diseños tecnológicos únicos. Espero que entiendas que hay otras... partes interesadas, así que tendrás que actuar con discreción. El pago por la entrega, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Postcureer.Money))+".";
					link.l1 = "¡Estoy de acuerdo!";
					link.l1.go = "Postcureer_LevelUp_Go";
					link.l2 = "No, supongo que pasaré.";
					link.l2.go = "Postcureer_LevelDown";
...
//----------------------------------------задания 2 уровня фрахт------------------------------------------
	...
	case "Fraht_LevelUp":
		...
		switch (rand(4))//для тестов
		{
			case 0:
				...
				dialog.text = "Últimamente, la demanda de cierta mercancía ha aumentado drásticamente en nuestra colonia. Las tiendas locales ya no la tienen en stock. Por lo tanto, me gustaría que me proporcionaras "+GetGoodsNameAlt(iGoods)+" en cantidad de "+FindRussianQtyString(iGoodsQty)+", y estoy dispuesto a pagar el doble, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" por artículo. Esto ascenderá a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Ah, y trata de cumplir con el plazo de un mes, no podré esperar más por las mercancías.";
			...
			case 2:
				...
				// 08Mar17 "month" was "moths"
				dialog.text = "El arsenal de nuestra colonia carece drásticamente "+GetGoodsNameAlt(iGoods)+". Las tiendas locales ya no lo tienen en stock. Por lo tanto, me gustaría que me proporcionaras "+GetGoodsNameAlt(iGoods)+" en cantidad de "+FindRussianQtyString(iGoodsQty)+", y estoy dispuesto a pagar el triple, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.GoodsAverigePrice))+" por artículo. Esto ascenderá a "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+". Oh, y trata de cumplir con el plazo de un mes, no podré esperar más por las mercancías.";
				link.l1 = "Está bien, aceptaré esta misión.";
				link.l1.go = "Fraht_LevelUp_Go";
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		{
			dialog.text = "Está bien, se suponía que debías entregar un cargamento de "+GetGoodsNameAlt(iGoods)+" en cantidad de "+FindRussianQtyString(pchar.questTemp.WPU.Fraht.GoodsQty)+". Excelente, muchas gracias. Aquí está su pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Fraht.Money))+".";
...
//----------------------------------------задания 2 уровня эскорт ------------------------------------------
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		...
		dialog.text = "¡Excelente! Una vez más, has demostrado que se puede confiar en ti. Gracias por un trabajo bien hecho. Por favor, acepta tu pago, "+FindRussianMoneyString(int(pchar.questTemp.WPU.Escort.Money))+"Y no olvides visitarme de vez en cuando, siempre hay un trabajo o dos para un capitán como tú.";
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		dialog.text = "Sí, ya fui notificado. Excelente trabajo, "+GetAddress_Form(NPChar)+"¡ Has demostrado una vez más que eres en quien puedo confiar. Toma tu recompensa de 30,000 pesos.\nRecuerda visitarme de vez en cuando, personas como tú son invaluables y siempre encontraré un trabajo adecuado para ti.";
	...
	if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
	{
		dialog.text = "Sí, ya me contó sobre la batalla con los piratas y lo heroico que fuiste defendiendo su embarcación. Excelente trabajo, "+GetAddress_Form(NPChar)+"¡Has demostrado una vez más que eres alguien en quien puedo confiar! Recibe tu recompensa de 50,000 pesos.\nRecuerda visitarme de vez en cuando, personas como tú son invaluables y siempre encontraré un trabajo adecuado para ti.";
	...
	case "Escort_LU1VSP_completeSink_1":
		dialog.text = "Creo, "+GetAddress_Form(NPChar)+", que habías hecho todo lo posible para salvar el barco y a esos desafortunados marineros. Pero ay... Bueno, por favor, toma tu recompensa de 10000 pesos y gracias por tu servicio.";
	...
	break;
		...
		case "BurntShip15":
			...
			dialog.text = "¡Oh, capitán, estoy tan feliz de que no me hayas defraudado! ¡Qué "+GetSexPhrase("buen joven","joven dama")+" eres tú, literalmente me salvaste la vida... Por favor, ¿podrías venir a recoger tu dinero en un par de días? Sabes, necesito arreglar las formalidades con el seguro... Mientras tanto, podrías hacer pequeñas reparaciones al barco, renovar las velas, tapar los agujeros, un carenado no vendría mal, tampoco...";
		...
		case "BurntShip18":
			dialog.text = "¿Podría recordarme su nombre? Mi memoria no es como solía ser, todo ese ajetreo...";
...
//-------------------------------- квест доставки судового журнала до рассеяного кэпа ---------------------
		...
		case "SeekShip_good_1":
			dialog.text = "Estoy listo para pagarte tu recompensa. Consiste en " + int(npchar.quest.money) + " in doubloons. Desafortunadamente, no puedo pagarte más que eso.";
		...
		case "CapitainList":
			if (int(npchar.quest.qty) > 0)
			{
				...
				for (i=0; i<int(npchar.quest.qty); i++)
				{
					...
					attrL = "l" + i;
					link.(attrL) = GetFullName(sld) + ", capitán del " + GetStrSmallRegister(XI_ConvertString(RealShips[int(sld.Ship.Type)].BaseName + "Gen")) + " '" + sld.Ship.name + "'.";
...
*/		
		case "ShipStock_2":
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName)+" ' "+chref.Ship.Name+", clase "+RealShips[int(chref.Ship.Type)].Class+", el costo de amarre es "+FindRussianMoneyString(int(NPChar.MoneyForShip))+" al mes, pago por un mes por adelantado.";
			...
			dialog.Text = XI_ConvertString(RealShips[int(chref.Ship.Type)].BaseName) + " '" + chref.Ship.Name + "', clase " + RealShips[int(chref.Ship.Type)].Class +
								 ", coste de atraque " + FindRussianMoneyString(int(NPChar.MoneyForShip)) + " por mes, pago por adelantado.";
			dialog.Text = dialog.Text + " Si lo dejas con un oficial y la tripulación, entonces con ellos " + FindRussianMoneyString(int(NPChar.MoneyForShip) + int(NPChar.MoneyForCrew));

			Link.l1 = "Sí. Me parece bien.";
			if (int(Pchar.Money) >= int(NPChar.MoneyForShip)) Link.l1.go = "ShipStock_3";
			else  Link.l2.go = "ShipStock_NoMoney";

			Link.l2 = "Sí. Me parece bien. Lo dejamos con el capitán y la tripulación.";
		...
		case "ShipStock_3":
			...
			dialog.text = "Bien. Lo recogerás cuando lo necesites.";
			Link.l1 = "Gracias.";
			Link.l1.go = "exit";
		break;
        ...
        case "ShipStockManBack":
			...
			if (AttributeIsTrue(NPChar, "StoreWithOff") && FindFreeRandomOfficer() < 1 ) {  
				dialog.text = "Capitán, parece que no hay espacio en su tripulación para otro oficial.";  
				link.l1 = "Hmm... Volveré más tarde.";  
				link.l1.go = "exit";  
				break;  
			}
			...
			{
			    dialog.Text = "¿Quieres recoger tu barco? Por el amarre, aún debes "+FindRussianMoneyString(int(NPChar.MoneyForShip))+".";
			}
			else
			{
