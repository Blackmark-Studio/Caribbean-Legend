void ProcessDialogEvent()
{
	...
	case "First time":
		...
		{
			dialog.text = "Mmm. Esta historia no parece correcta. Creo que esa balandra de St. John's nunca debió existir. El teniente me está arrastrando a su operación militar. ¿Por qué necesitaría eso? No estoy en guerra con los holandeses... ¿Qué debería hacer? Irme de aquí y mandarlos todos al infierno, ¿o quedarme?";
			link.l1 = "Vete.";
			link.l1.go = "FMQN_1";
			link.l2 = "Quédate.";
			link.l2.go = "FMQN_2";
	...
	case "TalkSelf_SlavesToCrew_1":
		...
		pchar.Ship.Crew.Exp.Sailors = (float(pchar.Ship.Crew.Exp.Sailors) * GetCrewQuantity(pchar) +
									   float(pchar.Ship.Crew.Exp.Sailors) * 0.3 * int(pchar.GenQuest.SlavesToCrew)) /
									  fTemp;
		pchar.Ship.Crew.Exp.Cannoners = (float(pchar.Ship.Crew.Exp.Cannoners) * GetCrewQuantity(pchar) +
										 float(pchar.Ship.Crew.Exp.Cannoners) * 0.3 * int(pchar.GenQuest.SlavesToCrew)) /
										fTemp;
		pchar.Ship.Crew.Exp.Soldiers = (float(pchar.Ship.Crew.Exp.Soldiers) * GetCrewQuantity(pchar) +
										float(pchar.Ship.Crew.Exp.Soldiers) * 0.3 * int(pchar.GenQuest.SlavesToCrew)) /
									   fTemp;
		// падение опыта <--
	...
	case "FMQN_2":
		dialog.text = "Nah, de ninguna manera, me quedo. Pero diablos, no me quedaré aquí esperando a Dios sabe qué. Debería navegar al puerto de Philipsburg e investigar la situación. No pienso convertirme en una herramienta ciega en sus manos.";
		link.l1 = "";
		link.l1.go = "exit";
