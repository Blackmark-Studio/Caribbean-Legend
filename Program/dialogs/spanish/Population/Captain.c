...
void ProcessDialogEvent()
{
	...
	case "First time":
		...
		if (npchar.quest.meeting == "0")
		{
			...
			{
				dialog.text = "¡Alto! Navegas bajo la bandera de " + NationNameGenitive(int(pchar.nation)) + ", grumete. No tengo ningún deseo de hablar contigo y no quiero problemas... ¡Lárgate!";
		...
		else
		{
			...
			{
				dialog.text = "¿Debería repetirlo? No quiero que sospechen que soy amigo de " + NationNameAblative(int(pchar.nation)) + "¡Lárgate o llamaré a los guardias!";
	...
	case "march_1":
		...
		dialog.text = "Mi barco es " + GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.shiptype), "Name"))) + " '" + pchar.GenQuest.MarchCap.shipname + "Está en el puerto ahora mismo. ¡No llegues tarde, capitán, me iré en seis horas!";
