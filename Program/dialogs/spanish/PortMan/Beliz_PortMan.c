	...
	case "Regata_check":
		...
		if (int(Pchar.Ship.Type) != SHIP_NOTUSED && ok) // проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Vamos a registrarnos: capitán " + GetFullName(pchar) + ", el barco es " + pchar.Ship.Name + "... el tiempo de la regata en horas es " + int(pchar.questTemp.Regata.FirstTransitionTime) + ". Listo, tu resultado está registrado, puedes continuar tu camino.";
