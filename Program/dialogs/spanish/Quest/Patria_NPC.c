	...
	switch(Dialog.CurrentNode)
	{
		...
		case "BladeUpg":
			...
			for(n=0; n<iItemsNum; n++)
			{
				...
				{
					attrL = "l" + i;
					Link.(attrL) = GetItemName(rItem) + ": Ataque, " + FloatToString(stf(rItem.attack),1) + ", equilibrio, " + FloatToString(stf(rItem.Balance),1) + ", peso, " + FloatToString(stf(rItem.Weight),1);
