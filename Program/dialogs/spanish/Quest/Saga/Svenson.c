...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "saga_55":
			...
			link.l1 = "He encontrado "+FindRussianQtyString(int(npchar.quest.bakaut))+" en el 'Oyster'.";
			link.l1.go = "saga_56";
		break;
		...
		case "saga_60":
			...
			link.l1 = "En total, tengo dos mil de oro para Loxley y "+int(npchar.quest.bakaut_sum)*2+"doblones encima de eso.";
			link.l1.go = "saga_61";
		break;
		...
		case "saga_61":
			...
			if (PCharDublonsTotal() > 0)
			{
				...
				link.l1 = "Aquí tienes. He conseguido "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
		...
		case "saga_61_1":
			...
			{
				npchar.quest.bakaut_pay = func_min(int(npchar.quest.bakaut_sum), PCharDublonsTotal()); // дублоны в кармане
				link.l1 = "Aquí tienes. Tengo "+FindRussianQtyString(int(npchar.quest.bakaut_pay))+".";
				link.l1.go = "bakaut_pay";
			}
			else
			{
		...
		case "bakaut_pay": // оплата
			...
			Log_Info("Has dado "+int(npchar.quest.bakaut_pay)+" doblones");
			PlaySound("interface\important_item.wav");
			if (iTemp == 0)
			{
			...
			{
				dialog.text = "Estupendo. ¿Traerás el resto "+FindRussianQtyString(int(npchar.quest.bakaut_sum))+"¿doblones después?";
