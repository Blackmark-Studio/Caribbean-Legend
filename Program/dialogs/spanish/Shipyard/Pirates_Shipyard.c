...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	case "Al_ShipLetters_2":
		...
		s1 = s1 + "En realidad necesitaba un paquete como este. Yo, por mi parte, le ofreceré " + int(pchar.questTemp.different.GiveShipLetters.price5) + " pesos. ¿Qué me dices?";
		dialog.text = s1;
	...
	case "FDM_cabin":
		dialog.text = "¿Por qué no? Mis manos todavía son firmes.";
		link.l1 = "...";
	...
	case "FDM_cabin_s":
		dialog.text = "Siete arcones de doblones y estará hecho en cinco días.";
		link.l1 = "¡Trato hecho!";
	...
	case "FDM_cabin_pay":
		...
		if (amount <= 0) // всё принес
		{
			...
			dialog.text = "Entonces, tenías que traerme " + FindRussianQtyString(int(npchar.quest.FDM_cabin.chest)) + " cofres, has traído " + FindRussianQtyString(iTemp) + ". Tengo el pago. Ahora es mi turno. Ve y da un paseo. No te preocupes, haré lo mejor que pueda.";
			link.l1 = "Bueno...";
		...
		else
		{
			...
			dialog.text = "Entonces, tenías que traerme " + FindRussianQtyString(int(npchar.quest.FDM_cabin.chest)) + " cofres, has traído " + FindRussianQtyString(iTemp) + ". " + FindRussianQtyString(amount) + " cofres restantes.";
	...
	case "FDM_hull_01":
		dialog.text = "Bueno, hagamos cuentas... Para una reconstrucción completa del casco del barco voy a necesitar 150 unidades de guayacán, jarcia, seda naval y resina de cada una. Y además 10000 doblones de oro. Y no me miréis así: la mayor parte del dinero se irá en materiales. Este trabajo equivale a desmontar el barco pieza por pieza y volver a construirlo desde cero.";
		link.l1 = "No, creo que no. No estoy listo para traer tantos recursos y entregar un montón de oro por la reconstrucción. Me quedaré con lo que tengo.";
	...
	case "FDM_hull_givemoney":
		...
		if(PCharDublonsTotal() >= 10000)
		{
			link.l2 = "Sí, lo traje. 10 000 doblones de peso completo, como acordamos.";
