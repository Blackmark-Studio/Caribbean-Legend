	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "Hostess_NotChoice":
			...
			else
			{
				...
				// <-- legendary edition
				dialog.text = ""+GetSexPhrase("Pekâlâ, harika, benim aslanım!","Hepsi işlerinde gerçekten çok yetenekli, bundan hiç şüphen olmasın.")+"Sana çok güzel bir kız sunabilirim, adı "+GetFullName(sld)+", ve şu anda müsait. Bu zevkin bedeli "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+". Kabul mü?";
        ...
        case "Hostess_NotChoice_agree":
			...
			{
				dialog.text = "Ah, bakın, sorun şu ki "+sld.name+" ucuz bir kız değil, onun fiyatı "+FindRussianMoneyString(int(sld.quest.price)+charWhoreCost)+". Ve görüyorum ki şu anda onu karşılayacak durumda değilsiniz. Zengin olduğunuzda tekrar gelin"+GetSexPhrase(", sevgili","")+"...";
		...
		// belamour ночной приключенец
		case "NA_Girl":
            dialog.text = "Oo, demek burada bir uzmanımız var, hı hı ... Pekâlâ, tam sana göre bir kızım var. Ama ucuz değildir: "+int(pchar.GenQuest.NightAdventure_money)+" peso.";
			link.l1 = "Anlaştık.";
