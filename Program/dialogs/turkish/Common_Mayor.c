	...
	switch (Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			else
			{
				...
				if (CheckAttribute(&Colonies[FindColony(npchar.City)],"Siege"))
                {
                    ...
                    {
                        dialog.text = "Sizi üzmek istemem ama şu anda sizinle konuşacak vaktim yok. Bildiğiniz gibi, kolonimize saldırıldı "+NationNamePeople(int(aData.nation))+". Savunmamızla ilgilenmeye devam ediyorum.";
                ...
                {//->
                    ...
                    && CheckAttribute(aData, "HelpColony") && int(aData.HelpColony) == true )
                    {
                        ...
                        dialog.text = "Ah, sensin demek, "+GetAddress_Form(NPChar)+" "+GetFullName(pchar)+",   seni   görmekten   memnunum.   Kolonimiz   için   iyi   haberlerim   var   –   saldırıyı   püskürttük. "+NationNameSK(int(aData.nation))+" işgalciler. Bu işte senin de payın var, bu yüzden ödülün burada - "+(int(aData.iSquadronPower)*1500)+" sekiz parça. Al bakalım.";
                    ...
                    else
                    {
						...
						{
							link.l1 = "Sizin için çalışmak hakkında konuşmak istiyorum "+NationNameGenitive(int(NPChar.nation))+".";
							link.l1.go = "work";
						}
						...
						if(CheckAttribute(pchar,"GenQuest.CaptainComission") && (pchar.GenQuest.CaptainComission.variant == "A2") && (pchar.GenQuest.CaptainComission.City == NPChar.city))
						{
							...
							{
								link.l12 = "Devriyenin başında bir kaptan var "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" named "+pchar.GenQuest.CaptainComission.Name+", onu nasıl görebilirim?";
								link.l12.go = "CapComission_Mayor1";
							}	
							if(CheckAttribute(pchar,"GenQuest.CaptainComission.PrisonFree"))
							{
								link.l12 = "Devriye birliğinin eski kaptanı "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+"   yetkililere gizli hazinesinin yerini açıklamayı kabul etti.";
								link.l12.go = "CapComission_Mayor9";
							}
							if(pchar.GenQuest.CaptainComission == "GetGoodsSuccess")
							{
								link.l12 = "Devriye birliğinin eski kaptanı "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" bana gizli hazinesini gösterdi.";
        				...
        				//link.l7 = " Valilik Genel Valisi'nin nerede olduğunu öğrenmek isterim "+NationNameGenitive(int(NPChar.nation))+"is.";
		...
		case "node_1":
			...
			if (CheckAttribute(&Colonies[FindColony(npchar.City)],"Siege"))
            {
                ...
                {
                    dialog.text = "Sizi üzmek istemem ama şu anda sizinle konuşacak vaktim yok. Bildiğiniz gibi, kolonimize saldırıldı "+NationNamePeople(int(aData.nation))+". Savunmamızla ilgilenmem gerek.";
            ...
            {//->
                ...
                && CheckAttribute(aData, "HelpColony") && int(aData.HelpColony) == true )
                {
                    ...
                    DeleteAttribute(aData, "PlayerHelpMayor");
                    DeleteAttribute(aData, "HelpColony");
                    dialog.text = "Ah, sensin demek, "+GetAddress_Form(NPChar)+" "+GetFullName(pchar)+",   seni   görmekten   memnun   oldum.   İyi   haberlerim   var – saldırıyı püskürttük "+NationNameSK(int(aData.nation))+" istilacılar. Sen de bunda rolünü oynadın, işte ödülün - "+(int(aData.iSquadronPower)*1500)+" sekiz parça. Al bakalım.";
                ...
                else
                {
					...
					if (npchar.city != "Panama") // Addon-2016 Jason
					{
						link.l1 = "Benim için çalışmak istedim "+NationNameGenitive(int(NPChar.nation))+".";
						link.l1.go = "work";
					}
					// Warship Генер "Пираты на необитайке" - сдаём пиратов властям
					if(CheckAttribute(PChar, "Quest.PiratesOnUninhabited_OnShore") || CheckAttribute(PChar, "Quest.PiratesOnUninhabited_ShipSink"))
					...
					if(CheckAttribute(pchar,"GenQuest.CaptainComission") && (pchar.GenQuest.CaptainComission.variant == "A2") && (pchar.GenQuest.CaptainComission.City == NPChar.city))
					{
						if(pchar.GenQuest.CaptainComission == "Begin_1" && !CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakMayor")) 
						{
							link.l12 = "Devriyenin başında bir kaptan var "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" named "+pchar.GenQuest.CaptainComission.Name+", onunla nasıl görüşebilirim?";
							link.l12.go = "CapComission_Mayor1";
						}	
						if(CheckAttribute(pchar,"GenQuest.CaptainComission.PrisonFree"))
						{
							link.l12 = "Devriye komutasında eski bir kaptan "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+"   yetkililere   sakladığı   yeri   açıklamayı   kabul   etti.";
							link.l12.go = "CapComission_Mayor9";
						}
						if(pchar.GenQuest.CaptainComission == "GetGoodsSuccess")
						{
							link.l12 = "Devriyenin başındaki eski kaptan "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" bana gizli hazinesini gösterdi.";
							link.l12.go = "CapComission_Mayor13";
						}
					}
					if(CheckAttribute(pchar,"GenQuest.ShipWreck") && pchar.GenQuest.ShipWreck == "SailorsOnShore" && GetQuestPastDayParam("GenQuest.ShipWreck") < 25 && int(npchar.nation) == int(pchar.GenQuest.ShipWreck.Nation))
		...
		case "Helen_node_1":
            ...
            {//->
                ...
                && CheckAttribute(aData, "HelpColony") && int(aData.HelpColony) == true )
                {
                    ...
                    DeleteAttribute(aData, "PlayerHelpMayor");
                    DeleteAttribute(aData, "HelpColony");
                    dialog.text = "Ah, sensin demek, "+GetAddress_Form(NPChar)+" "+GetFullName(pchar)+",   seni   görmek   güzel.   İyi   haberlerim   var   –   saldırıyı   püskürttük   "+NationNameSK(int(aData.nation))+" istilacılar. Sen de bunda rolünü oynadın ve işte ödülün - "+(int(aData.iSquadronPower)*1500)+" sekiz parça. Al bakalım.";
                    link.l1 = "Teşekkür ederim, "+GetAddress_FormToNPC(NPChar)+", sizinle iş yapmak bir zevkti!";
                    link.l1.go = "exit";
                }
                else
                {
					...
					if(CheckAttribute(pchar,"GenQuest.CaptainComission") && (pchar.GenQuest.CaptainComission.variant == "A2") && (pchar.GenQuest.CaptainComission.City == NPChar.city))
					{
						if(pchar.GenQuest.CaptainComission == "Begin_1" && !CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakMayor")) 
						{
							link.l12 = "Devriyenin başında bir kaptan var "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" named "+pchar.GenQuest.CaptainComission.Name+", onunla nasıl tanışabilirim?";
							link.l12.go = "CapComission_Mayor1";
						}	
						if(CheckAttribute(pchar,"GenQuest.CaptainComission.PrisonFree"))
						{
						...
						if(pchar.GenQuest.CaptainComission == "GetGoodsSuccess")
						{
							link.l12 = "Devriyenin başında olan eski kaptan "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" bana gizli hazinesini gösterdi.";
							link.l12.go = "CapComission_Mayor13";
						}
					}
					if(CheckAttribute(pchar,"GenQuest.ShipWreck") && pchar.GenQuest.ShipWreck == "SailorsOnShore" && GetQuestPastDayParam("GenQuest.ShipWreck") < 25 && int(npchar.nation) == int(pchar.GenQuest.ShipWreck.Nation))
		...
		case "work": 
			...
			{
				if(int(pchar.reputation.nobility) > 41) {dialog.text = " Bana emanet edilen topraklarda kanunsuzluğa karşı elimden gelen her şeyi yapıyorum ve sizin gibi onurlu kaptanlar için her zaman görevlerim var. Ne tür görevler üstlenmek istersiniz? ";}
				...
				link.l3 = "Uluslararası ilişkileri etkileyebilecek ve   konumunu güçlendirebilecek yüksek riskli görevlerle ilgileniyorum "+NationNameGenitive(int(NPChar.nation))+".";
		...
		case "CAPTURE_Main":
            ...
            else
            {
	            ...
	            if (isMainCharacterPatented() && bOk && !CheckAttribute(&colonies[iColony], "notCaptured")) //не даем захватить колонию
	            {
					...
					dialog.Text = "Eşi benzeri görülmemiş bir küstahlık!   Nasıl cüret edersin   bir koloniye saldırmaya, üstelik   "+NationNameGenitive(int(NPChar.nation))+"?! Dökülen kanın ve devletimize verdiğin zarar için ağır bedel ödeyeceksin.";
	                Link.l1 = "Bu andan itibaren, bu koloni bana ait. Otoriteme karşı çıkan herkes doğrudan cehenneme gider.";
	                Link.l1.go = "City_patent";
	                Link.l2 = "Geri çekil.  Ben  adına hareket ediyorum. "+NationNameGenitive(int(PChar.nation))+". Bundan böyle, bu koloni artık  "+NationKingsCrown(PChar)+"!";
		...
		case "DestroyGang":
			...
			dialog.text = "Elbette. Bu görevi yerine getirmen için sana veriyorum "+FindRussianDaysString(int(pchar.GenQuest.DestroyGang.Terms))+", ve başarılı olursanız ödülünüz de "+FindRussianMoneyString(int(pchar.GenQuest.DestroyGang.Money))+".";
		...
		case "TakePostcureer":
			...
			dialog.text = "Elbette.  Bir kurye gemisini bulmanız gerekecek "+NationNameGenitive(int(pchar.GenQuest.TakePostcureer.Nation))+" adında '"+pchar.GenQuest.TakePostcureer.ShipName+"', gemiye çık ve kaptanın kamarasında bulman gereken evrakları bana getir. Bu gemi şu civardan geçecek "+XI_ConvertString("Colony"+pchar.GenQuest.TakePostcureer.City+"Gen")+" yaklaşık olarak "+FindRussianDaysString(pchar.GenQuest.TakePostcureer.Terms)+".";
		...
		case "TakePostcureer_agree":
			...
			dialog.text = "Posta. Benim için çok önemli bazı evraklar var. Ama belgeleri tek tek aramana gerek yok, paketin tamamını bana getir yeter. Sana ödeme yapacağım "+FindRussianMoneyString(int(pchar.GenQuest.TakePostcureer.Money))+"\nO halde, sizi ve yaptığınız işin sonuçlarını konutumda bekliyor olacağım.";
		...
		case "TakeArsenalship":
			...
			npchar.GenQuest.Governor.Group3.day = GetDataDay();
			dialog.text = "Elbette. Bir askeri nakliye gemisini bulmanız gerekecek "+NationNameGenitive(int(pchar.GenQuest.TakeArsenalship.Nation))+", with gunpowder and ammunition on board; the name of the vessel is '"+pchar.GenQuest.TakeArsenalship.ShipName+"', find and destroy it. We'll weaken our enemy by doing that\nThe transport will sail with an escort to the colony "+XI_ConvertString("Colony"+pchar.GenQuest.TakeArsenalship.City)+", and will be approximately in "+FindRussianDaysString(pchar.GenQuest.TakeArsenalship.Terms)+", so you should hurry.";
		...
		case "TakeArsenalship_agree":
			...
			dialog.text = "Umurumda değil, sana kalmış. Önemli olan bu yükün hedefine ulaşmaması. Başarırsan, sana ödeme yapacağım "+FindRussianMoneyString(int(pchar.GenQuest.TakeArsenalship.Money))+"... Artık sizi daha fazla oyalamıyorum, kaptan.";
			link.l1 = "Sizi fazla bekletmeyeceğim, "+GetAddress_FormToNPC(NPChar)+".";
			link.l1.go = "exit";
		...
		case "CustomPatrol_agree":
			...
			dialog.text = "Kaynağım bana bir anlaşmanın hazırlandığını ve yakında tamamlanacağını söyledi. Hepsi bu. Yani, sularımızdaki şüpheli gemilere daha yakından bak. Başarırsan, sana şu kadar ödeme yapacağım "+FindRussianMoneyString(int(pchar.GenQuest.CustomPatrol.Money))+"\nAyrıca, mevcut koşulları göz önünde bulundurarak, bulabildiğiniz tüm kaçak mallara el koymanıza izin veriyorum. Elbette bu gayriresmî bir yetki; yeter ki malları şehrimizde satmayın.";
		...
		case "Intelligence":
			npchar.GenQuest.Governor.Group3.day = GetDataDay();
			dialog.text = "Anladım. Belki de bir ödül olarak "+FindRussianMoneyString(int(pchar.GenQuest.Intelligence.Money))+" sizin için iyi bir teşvik olacak.";
		...
		case "Intelligence_ExecuteLate":
			...
			dialog.text = RandPhraseSimple("Fena değil. Ama teslim tarihini kaçırdın, bu yüzden ödülünü biraz azaltmam gerekecek. Şimdi alacağın ücret "+FindRussianMoneyString(int(pchar.GenQuest.(QuestName).Money))+". Buyurun.","Güzel. Ama teslim tarihine uymadın ve bu mektupları bekliyordum. Planlarımı bozmuş oldun, bu yüzden ödülünü biraz azaltmak zorundayım. Yine de hak ettiğin ödülü vermekten memnunum - "+FindRussianMoneyString(int(pchar.GenQuest.(QuestName).Money))+". Buyurun.");
		...
		case "All_Execute":
			QuestName = pchar.GenQuest.questName;
			dialog.text = RandPhraseSimple("Harika haber! Şimdi, her şeyi toparlama vakti. Ücretiniz ise "+FindRussianMoneyString(int(pchar.GenQuest.(QuestName).Money))+". Buyurun.","Harika! Bu herkes için en iyisi... Pekâlâ, hak ettiğiniz ödülü size vermekten memnuniyet duyarım - "+FindRussianMoneyString(int(pchar.GenQuest.(QuestName).Money))+". Buyurun.");
