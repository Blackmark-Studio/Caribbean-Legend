...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("Kocam yabancılarla konuşmama izin vermiyor!","Konuşacak hiçbir şeyimiz yok.");
				Link.l1 = "Önemli bir şey değil.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Bana söyle, yerel meyhanede hangi dedikodular dolaşıyor?","Burada neler olup bittiğini bana anlatmak ister misin?","Hey, dostum, bana karada hayatın nasıl olduğunu anlatır mısın?");
		...
		case "sovet":
			...
			link.l10 = LinkRandPhrase("Bana söyle, yerel meyhanede hangi dedikodular dolaşıyor?","Bana burada neler olup bittiğini anlatmak ister misin?","Hey, dostum, bana karada hayatın nasıl olduğunu anlatır mısın?");
			link.l10.go = "rumours_citizen"; //повесил на горожанина, если нужны отедельные слухи, то пишем соответствующую реализацию
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("Ah evet, memnunum"+NPCharSexPhrase(NPChar,"","")+"Seni görmek neredeyse bir kadeh içki gibi. Ne istersen sor.","Konuşmayı seviyorsun, kaptan? Ben de... Özellikle bir bardak rom eşliğinde.","Evet, "+PChar.name+"?"),LinkRandPhrase("Ne istiyorsun?","Anladım, Kaptan "+PChar.name+", sen bayağı gevezesin, değil mi?","Sıkıcı sorular, Kaptan. Bana biraz rom ısmarlasan daha iyi olurdu.")),PCharRepPhrase(LinkRandPhrase("Ne bilmek istiyordun "+GetAddress_Form(NPChar)+"?","Sizi dinliyorum, kaptan.","Gitmem lazım, o yüzden lütfen çabuk ol kaptan."),LinkRandPhrase("Her zaman hoş bir dostluğa sahip olmaktan mutluluk duyarım, "+GetAddress_Form(NPChar)+" "+PChar.lastname+" . Söyle bakalım, ne düşünüyorsun.","Evet, "+GetAddress_Form(NPChar)+"?","Sohbet etmeyi seviyorsun, kaptan? Eh, ben de severim...")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("Yerel meyhanede hangi dedikodular popüler?","Bu topraklarda neler oluyor?","Burada karada neler yeni?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("Hayır, hayal görüyorsun. Ben artık gidiyorum.","Hayır, hiçbir şey. Sadece biraz dolaşmaya çıktım."),RandPhraseSimple("Hayır, hiçbir şey yok. Sana bol şans!","Sadece biraz dolaşıyorum. Hoşça kal."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Dinle, bu kasabanın bir vatandaşı olarak senden kınsını çekilmiş bir şekilde dolaşmamandan rica ediyorum.");
				link.l1 = LinkRandPhrase("Pekala.","Pekala.","Dediğiniz gibi...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"Sakin ol, "+GetSexPhrase("dostum","kız")+", elinde silahla ortalıkta dolaşırken. Bu beni biraz tedirgin edebilir...","Hoşlanmıyorum, ne zaman "+GetSexPhrase("adamlar","insanlar")+" silahları ellerinde hazır şekilde dolaşıyorlar. Bu beni korkutuyor...");
				link.l1 = RandPhraseSimple("Anladım.","Onu kaldıracağım.");
			}
			link.l1.go = "exit";
