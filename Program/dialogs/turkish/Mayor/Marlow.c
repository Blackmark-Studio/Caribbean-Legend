...
//--------------------------------------------Похититель------------------------------------------------------
		...
		case "pirate_threat":
			...
			if (iGPThreat != 0)
			{
				...
				dialog.text = "Ha! Kardeşlik tayfası seni fena hırpalıyor ha, " + GetSexPhrase("dostum", "dostum") + "? Onları bir süreliğine dizginleyebilirim. Ama bunun bir bedeli var. " + FindRussianDublonString(iMarlowTotalTemp) + " ver, işi halledelim.";
