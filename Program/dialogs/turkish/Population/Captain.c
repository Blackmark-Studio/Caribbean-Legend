	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Dur! Hangi bayrak altında yelken açıyorsun? "+NationNameGenitive(int(pchar.nation))+", çaylak. Seninle konuşmak istemiyorum ve başıma dert açmanı da istemem... Defol git!";
			...
			else
			{
				...
				{
					dialog.text = "Tekrar mı edeyim? Birinin dostu olduğumdan şüphelenilmek istemem "+NationNameAblative(int(pchar.nation))+"! Defol git yoksa muhafızları çağırırım!";
		...
		//квестовая часть
			...
			case "march_1":
				...
				dialog.text = "Gemi m   "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.MarchCap.shiptype),"Name")))+" '"+pchar.GenQuest.MarchCap.shipname+" . Şu anda limanda. Geç kalma kaptan, altı saat sonra ayrılmış olacağım!";
