    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			else
			{
				...
				dialog.text = "Kaptan, şehrin kuşatma altında olduğunu fark etmiş olabilirsiniz. İspanyolların her an saldırmasını bekliyoruz ve kuvvetlerimizi seferber ediyoruz. Ama regatta hâlâ devam ediyor\nHaydi kaydolalım, kaptan "+GetFullName(pchar)+", gemi - "+pchar.Ship.Name+"... Regatta süresinin saat cinsinden ifadesi "+int(pchar.questTemp.Regata.FourthTransitionTime)+". Tamam, sonucun kaydedildi, yoluna devam edebilirsin.";
