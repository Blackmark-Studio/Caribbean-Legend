...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		...
		if(int(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			dialog.text = "Kayıt olalım: Kaptan "+GetFullName(pchar)+", gemi "+pchar.Ship.Name+"... Regatta'nın süresi saat olarak "+int(pchar.questTemp.Regata.ThirdTransitionTime)+". Tamam, sonucun kaydedildi, yoluna devam edebilirsin.";
