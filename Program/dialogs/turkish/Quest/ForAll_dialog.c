void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		//встретил в городе
		case "PortmansCap":
			dialog.text = "Günaydın. Benim adım "+GetFullName(npchar)+". Ben   kaptanıyım "+GetStrSmallRegister(XI_ConvertString(RealShips[int(npchar.Ship.Type)].BaseName+"Acc"))+" '"+npchar.Ship.name+"'.";
		...
		case "PortmansCap_3":
			...
			{
				dialog.text = "Tam zamanında geldin, henüz yeni bir tane başlatmadım, bu yüzden elimden geldiğince ödeyeceğim. "+FindRussianMoneyString(int(npchar.quest.money))+" ve birkaç parça kişisel takımımdan kabul et.";
			...
			else
			{
				if (int(npchar.quest.stepsQty) < 5)
				{
					...
					dialog.text = "Hm, biliyor musun, yeni bir seyir defteri tutmaya başladım bile. Yine de, eskisinin benim için hâlâ bir değeri var. O yüzden sana ödeme yapacağım "+FindRussianMoneyString(int(npchar.quest.money))+" ve birkaç parça kişisel takımımdan kabul et.";
		...
		case "PortmansCap_inDeck_3":
			dialog.text = "Sana ödeme yapabilirim "+FindRussianMoneyString(int(npchar.quest.money))+" ve birkaç mücevherim. Elimden gelen sadece bu kadar.";
...
//<-- поисковые квесты дворян
		...
		case "SCQ_Slave_1":
			...
			dialog.text = ""+npchar.quest.SeekCap.capName+" of the "+GetStrSmallRegister(XI_ConvertString(npchar.quest.SeekCap.shipTapeName+"Voc"))+" named "+npchar.quest.SeekCap.shipName+" . Sana ödeyeceğim  "+FindRussianMoneyString(int(npchar.quest.money))+", artı tüm mücevherlerim."; // belamour gen
		...
		case "SCQ_RapeWife_1":
			...
			dialog.text = "Zengin değilim, ama sahip olduğum tüm değerli eşyaları size vermekten memnuniyet duyarım! Eğer eşimi kurtarıp bana getirirseniz, size ödeme yapacağım "+FindRussianMoneyString(int(npchar.quest.money))+", bir de tüm mücevherlerim.";
		...
		case "SCQ_Friend_1":
			...
			dialog.text = "Sana ödeme yapabilirim "+FindRussianMoneyString(int(npchar.quest.money))+" ve tüm mücevherlerimi sana vereceğim. Şu anda elimden gelen sadece bu.";
		...
		case "SCQR_manSlave":
			dialog.text = "Harika, senin hakkında yanılmamışım! Söz verdiğim gibi, işte senin "+FindRussianMoneyString(int(npchar.quest.money))+" ve mücevherler. Yardımınız için teşekkür ederim.";
			link.l1 = "Rica ederim...";
			link.l1.go = "SCQ_exit";
			sTemp = "SCQ_" + npchar.index;
			pchar.quest.(sTemp).over = "yes"; //снимаем прерывание смерть горожанина-квестодателя
		...
		case "SCQR_manRapeWife":
			dialog.text = "Aman Tanrım! Bizi kurtardınız! Lütfen, alın şu "+FindRussianMoneyString(int(npchar.quest.money))+" ve mücevherler. Ve bil ki, sonsuza dek senin için dua edeceğiz!";
		...
		break;
		case "SCQR_manFriend":
			dialog.text = "Harika!... Al, buyur "+FindRussianMoneyString(int(npchar.quest.money))+" ve mücevherler. Ve teşekkür ederim, Kaptan.";
		...
		case "SCQ_Hasband_1":
			...
			SetSeekCapCitizenParam(npchar, int(npchar.nation));
			dialog.text = "Kesinlikle! Ne demek istediğimi anlıyorsun. Sen de tam bir "+GetSexPhrase("cesur kaptan","cesur kız")+", bu yüzden senden kocamı bulmanı istiyorum. Sana ödeme yapmaya hazırım "+FindRussianMoneyString(int(npchar.quest.money))+", ayrıca tüm mücevherlerimi de sana vereceğim.";
		...
		case "SCQ_Revenge_7":
			...
			SetSeekCapCitizenParam(npchar, PIRATE);
			dialog.text = "Her şeyim senin olsun. Mücevherlerimi ve "+FindRussianMoneyString(int(npchar.quest.money))+"! Anlaştık mı?";
		...
		case "SCQ_Pirates_5":
			...
			SetSeekCapCitizenParam(npchar, PIRATE);
			dialog.text = "Pek bir şey değil, "+FindRussianMoneyString(int(npchar.quest.money))+" ve tüm mücevherlerim... Ama ruhun için sonsuza dek dua edeceğim!";
		...
		case "SCQR_womanHasband":
			dialog.text = "Tanrım, sana minnettarım! Ve evet, al şunu "+FindRussianMoneyString(int(npchar.quest.money))+". Ve tekrar teşekkür ederim!";
		...
		case "SCQR_womanRevenge":
			dialog.text = "Mükemmel! Peki, al bakalım "+FindRussianMoneyString(int(npchar.quest.money))+" ve mücevherler. Elveda...";
			link.l1 = "Elveda. ";
			link.l1.go = "SCQ_exit";
			sTemp = "SCQ_" + npchar.index;
			pchar.quest.(sTemp).over = "yes"; //снимаем прерывание смерть горожанина-квестодателя
		...
		case "SCQR_womanPirates":
			dialog.text = "Eminim o! Tanrım kaptan, size minnettarım. Alın sizin "+FindRussianMoneyString(int(npchar.quest.money))+". Hayatımın her günü senin için dua edeceğim!";
