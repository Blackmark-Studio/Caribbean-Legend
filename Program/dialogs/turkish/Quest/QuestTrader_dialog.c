void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Escort_choice"://первые развилки - выбираем между городом, бухтой и необитайкой
			...
			switch (hrand(2))
			{
				case 0://город дружественный
					...
					dialog.text = "Beni   oraya kadar   eşlik etmeni   rica ediyorum "+XI_ConvertString("Colony"+pchar.GenQuest.Escort.Trader.City+"Gen")+" içinde "+FindRussianDaysString(int(pchar.GenQuest.Escort.Trader.DaysQty))+".   Sana   ödeme   yapacağım "+FindRussianMoneyString(int(pchar.GenQuest.Escort.Trader.Money))+".";
				...
				case 1://бухта
					...
					dialog.text = "Beni   oraya   kadar   eşlik   etmeni   rica   ediyorum "+XI_ConvertString(pchar.GenQuest.Escort.Trader.Shore+"Gen")+",   çok   uzakta   olmayan "+XI_ConvertString("Colony"+pchar.GenQuest.Escort.Trader.City+"Gen")+" içinde "+FindRussianDaysString(int(pchar.GenQuest.Escort.Trader.DaysQty))+". Sana ödeme yapacağım "+FindRussianMoneyString(int(pchar.GenQuest.Escort.Trader.Money))+".";
					link.l1 = "Ve neden olmasın? Birlikte yelken açmak her zaman daha güvenlidir, bunu anlıyorum. Katılıyorum.";
					link.l1.go = "EscortType";
				...
				case 2://необитайка
					...
					dialog.text = "Beni   oraya   kadar   eşlik   etmeni   rica   edeceğim "+XI_ConvertString(pchar.GenQuest.Escort.Trader.Shore+"Gen")+"   şu anda   "+XI_ConvertString(pchar.GenQuest.Escort.Trader.Island)+", in "+FindRussianDaysString(int(pchar.GenQuest.Escort.Trader.DaysQty))+". Sana ödeme yapacağım "+FindRussianMoneyString(int(pchar.GenQuest.Escort.Trader.Money))+".";
		...
		case "EscortTrader_complete_2":
			dialog.text = "Evet, çok akıllıca bir gözlem... Al bakalım, işte senin "+FindRussianMoneyString(int(pchar.GenQuest.Escort.Trader.Money))+" ve elveda.";
