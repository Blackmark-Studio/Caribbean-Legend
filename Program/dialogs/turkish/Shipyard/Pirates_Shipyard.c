	...
	// <-- хронометр Алекса
		...
		case "FDM_cabin_pay":
			...
			if (amount <= 0) // всё принес
			{
				...
				dialog.text = "Yani, beni buraya getirmek zorunda mıydın "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" sandıkları getirdiniz "+FindRussianQtyString(iTemp)+"Parayı getirdim. Şimdi sıra bende. Git biraz dolaş. Merak etme, elimden gelenin en iyisini yapacağım.";
				link.l1 = "İyi...";
			...
			else
			{
				...
				dialog.text = "Demek ki, beni buraya getirmek zorunda kaldın "+FindRussianQtyString(int(npchar.quest.FDM_cabin.chest))+" sandıkları getirdiniz "+FindRussianQtyString(iTemp)+". "+FindRussianQtyString(amount)+" sandık kaldı.";
				link.l1 = "Pekala, usta.";
				link.l1.go = "FDM_cabin_wait";
		...
		case "FDM_hull_01":
			dialog.text = "Haydi, hesaplayalım... Geminin gövdesini baştan aşağı yeniden kurmak için bana demirağacı, halat, gemi ipeği ve reçineden ayrı ayrı 150’şer birim gerekecek. Bir de üstüne 10000 altın doblon. Bana öyle bakmayın — paranın büyük kısmı malzemeye gidecek. Bu iş, gemiyi parça parça söküp en baştan yeniden yapmakla eşdeğer.";
