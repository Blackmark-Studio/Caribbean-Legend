	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "Bugün iş yok. Her körfezde devriyeler var, valimiz bir kaptan tarafından el konulan bir yükü arıyor"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" ve onu bir yere sakladı.";
		...
		case "SM_ShipLetters_1":
			...
			else
			{
				...
				{
					dialog.text = "Neden olmasın? Belki 'yükün' için bir kullanım bulurum. Al "+int(pchar.questTemp.different.GiveShipLetters.price2)+" paraları al ve unut gitsin.";
				...
				else
				{
					...
					{
						dialog.text = "Kesinlikle! Şimdi gerçekten merak ettim. Sanırım eğer sana ödeme yaparsam "+int(pchar.questTemp.different.GiveShipLetters.price3)+" parayı alınca, o kağıtlar umurunuzda bile olmayacak.";
						link.l1 = "Hayır, teşekkürler.";
						link.l1.go = "SM_ShipLetters_2";
					...
					{
						dialog.text = "Şimdi, o küstah herifi yakalamak için bir fırsat çıktı! Gerçekten, gerçekten merak ediyorum. Bak, sana ödeme yapacağım "+int(pchar.questTemp.different.GiveShipLetters.price4)+" altın karşılığında, masadaki belgeleri tamamen unutacaksın, öyle mi?";
						link.l1 = "Yapmam.";
						link.l1.go = "SM_ShipLetters_2";
		...
		case "Meeting":
			if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 2)
			{
				...
				{
					pchar.GenQuest.CaptainComission.SpeakContra = true;
					dialog.text = "Bugün iş yok. Her koyda devriyeler var, valimiz bir kaptan tarafından el konulan bir yükü arıyor"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(int(pchar.GenQuest.CaptainComission.ShipType),"Name")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" ve onu bir yere sakladı.";
					link.l1 = "Ve bu kaptan nerede"+pchar.GenQuest.CaptainComission.Name+"?";
					link.l1.go = "CapComission_1";
					break;
				}
		...
		case "Meeting_2":
			...
			Dialog.Text = "Ne için?";
			Link.l1 = "Belki özel bazı mallarla ilgilenirsiniz? Ya da belki bana sunacak bir şeyiniz vardır?";
			Link.l1.go ="Meeting_3";				
		break;
...
////////////////////////////////////////////////////////////////////////////////
		...
		//Jason --> генератор сбыта бакаута
		case "bakaut":
			dialog.text = "Öyle mi? Peki. Bay Rosenkraft'ın bir miktar demir ağacına ihtiyacı var, ama en azından "+FindRussianQtyString(int(pchar.GenQuest.Bakaut.Value))+" . Yeterince yükün var mı?";
