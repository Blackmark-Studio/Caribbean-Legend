...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Sorular, "+GetAddress_Form(NPChar)+"?","Size nasıl yardımcı olabilirim, "+GetAddress_Form(NPChar)+"?"),""+GetSexPhrase("Hm, neyin peşindesin, "+GetAddress_Form(NPChar)+"? ","Yine tuhaf sorular mı? Tatlım, git biraz rom falan iç...")+"","Bugün boyunca bu soruyu üçüncü kez gündeme getiriyorsun..."+GetSexPhrase(" Bunlar bir tür yakınlık göstergesi mi?","")+"","Sanırım daha fazla sorunuz var, "+GetAddress_Form(NPChar)+"?","block",1,npchar,Dialog.CurrentNode);
