...
void FlagsProcess()
{
	...
	bool disableByMapEnter = bDisableMapEnter;
	if (CheckAttributeEqualTo(pchar, "quest.FMQN_to_port.function", "FMQN_ArriveToPort")) disableByMapEnter = false;  // костыль мышеловки

	if (disableByMapEnter || !bTmpBool)
	{
        if (!bBettaTestMode)
        {
