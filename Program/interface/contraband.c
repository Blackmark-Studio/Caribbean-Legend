...
void InitInterface_RR(string iniName, ref ContraTrader , ref pStore)
{
	...
	XI_SetArrowsInputHandler("TABLE_LIST", &ADD_BUTTON, &REMOVE_BUTTON);
	XI_SetArrowsInputHandler("QTY_EDIT", &ADD_BUTTON, &REMOVE_BUTTON);
	XI_SetArrowsInputHandler("QTY_BUYSELL_BUTTON", &ADD_BUTTON, &REMOVE_BUTTON);
...
void IDoExit(int exitCode)
{
	...
	if (int(Pchar.quest.Contraband.Counter) > 0) Achievment_SetStat(31, 1);
...
void CS_TableSelectChange()
{
	...
	SetShipWeight();
	SetVariable();
    ShowGoodsInfo(int(GameInterface.TABLE_LIST.(sRow).index));
}
...
void ShowGoodsInfo(int iGoodIndex)
{
    ...
    if (bBettaTestMode)
	{
		...
		goodsDescr += NewStr() + "NormPrMod " + FloatToString(float(refStore.Goods.(GoodName).NormPriceModify), 4) + " CurPrMod " +
		                        FloatToString(float(refStore.Goods.(GoodName).RndPriceModify), 4);
		goodsDescr += NewStr();
	}
