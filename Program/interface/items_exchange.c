...
int iTableAddAllBtnY = 313;
int iTableAddBtnX = 610;
int iTableAddBtnY = 313;

int iTableRemoveBtnX = 670;
int iTableRemoveBtnY = 313;
int iTableRemoveAllBtnX = 800;
int iTableRemoveAllBtnY = 313;

int iTableAddAllBtnX2 = 1010;
int iTableAddAllBtnY2 = 313;
int iTableAddBtnX2 = 1545;
int iTableAddBtnY2 = 313;

int iTableRemoveBtnX2 = 1605;
int iTableRemoveBtnY2 = 313;
int iTableRemoveAllBtnX2 = 1240;
int iTableRemoveAllBtnY2 = 313;
...
void InitInterface_RS(string iniName, ref itemsRef, string faceID)
{
	...
	else if (CheckAttribute(GameInterface, "TABLE_LIST.tr1.index"))
	{
	...
	AddTakeAllHint() // Подсказку вниз по хоткею взять всё
}

void AddTakeAllHint()
{
	if (GetSelectable("GETALL_BUTTON") != 1) return; // не доступна кнопка взять всё = нет хоткея = нет подсказки
	if (!CheckAttribute(&objControlsState, "keygroups.primaryland.InterfaceTakeAll")) return;
...
void ShowItemsWeight()
{
	...
	else if (sInterfaceType == INTERFACETYPE_CHEST)
	{
		sText2 = FloatToString(GetItemsWeightEx(refToChar), 1);
		SetNodeUsing("WEIGHT_ICON2",true);
	}
	else
	{
