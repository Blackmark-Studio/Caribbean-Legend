...
int g_nCurrentSaveIndex = 0;	// индекс выделенного сейва
int g_nFirstSaveIndex = -1;		// индекс первого сейва на странице
int g_nCurrentSaveSlot = -1;	// номер выделенного слота
...
void InitInterface_BB(string iniName, bool isSave, bool isMainMenu)
{
	...
	SetEventHandler("ReadScreenshot_Start", "ReadScreenshot_Start", 1);
	SetEventHandler("ChooseProfile", "ProcChooseProfile", 0);
	SetMenuLogo();
...
void IDoExit(int exitCode)
{
	...
	DelEventHandler("ReadScreenshot_Start", "ReadScreenshot_Start");
	DelEventHandler("ChooseProfile", "ProcChooseProfile");
...
void ProcessCommandExecute()
{
	...
	int iSlot;
	string saveFrameTag = "SAVEFRAME";
	string saveButtonTag = "SAVEBUTTON";
	string deleteButtonTag = "DELETEBUTTON";
	bool bFrame = IsStrLeft(sNode, saveFrameTag);
	bool bSaveButton = IsStrLeft(sNode, saveButtonTag);
	if(bFrame || bSaveButton)
	{
		if(sCommand == "click" || sCommand == "dblclick" || sCommand == "activate")
		{
			...
			if(bSaveButton || sCommand == "dblclick" || sCommand == "activate")
			{
				...
				else if(bGameProcess || CheckAttributeHasValue(&g_oSaveList[iSlot], "bad"))
			...
			ScrollStepDown();
		else if(sCommand == "leftstep" && !XI_IsWindowEnable("PROFILE_WINDOW"))
	...
	if(sNode == "BUTTON_1" && !XI_IsWindowEnable("PROFILE_WINDOW"))
	{
		if(sCommand == "rightstep")
		{
			iSlot = g_nCurrentSaveSlot;
	...
	if(sNode == "BUTTON_2" && !XI_IsWindowEnable("PROFILE_WINDOW"))
	{
		if(sCommand == "rightstep")
		{
			iSlot = g_nCurrentSaveSlot;
	...
	if(sNode == "BUTTON_3" && !XI_IsWindowEnable("PROFILE_WINDOW"))
	{
		if(sCommand == "rightstep")
		{
			iSlot = g_nCurrentSaveSlot;
...
void ProcessInterfaceControls()
{
	...
	if(controlName == "ICancel")
	{
		if(XI_IsWindowEnable("PROFILE_WINDOW"))
			ProcExitProfile();
		else if(XI_IsWindowEnable("CONFIRM_WINDOW"))
			DoConfirm(false);
		else if(XI_IsWindowEnable("CUSTOM_SAVE_WINDOW"))
			ExitSaveCustomGame();
		else
			ProcessCancelExit();
	}
...
void ScrollSaveSlot(bool bDown)
{
	int diff;
	if(bDown)
		diff = 1;
...
void SetCurrentProfile(string sProfileName)
{
	...
	if(sProfileName == "")
	{
		g_nSaveQuantity = 0;
		ResetSlots();
		StringCollection_SetText("PROFILE_INFO", PROFILE_NAME, "#" + XI_ConvertString("ProfileName") + ": " + "");
		return;
	}
	int nSaveNum = 0;
	string saveName;
	int nSaveSize;
	string sAutoTag = ".auto";
	string sQuickTag = ".quick";
	string attr;
	GameInterface.SavePath = "SAVE\" + currentProfile;
	int i = 0;
	while(FindSaveFile(i, &saveName, nSaveSize))	// ищем файлы, заполняем контейнер
	{
		i++;
		if(!bAuto && IsStrRight(saveName, sAutoTag))
			continue;
		if(!bQuick && IsStrRight(saveName, sQuickTag))
			continue;
		attr = "s" + nSaveNum;
		Screenshoter_Release(saveName);
	...
	ResetSlots();
	SetNodeUsing("SHOW_AUTOSAVES", g_nSaveQuantity > 0);
	SetNodeUsing("SHOW_QUICKSAVES", g_nSaveQuantity > 0);
	
	DelEventHandler("ReadScreenshot", "ReadScreenshot");
	PostEvent("ReadScreenshot_Start", 2);
...
{
	SetNodeUsing("SAVEFRAME" + (iSlot + 1), false);
	SetNodeUsing("SAVEIMG" + (iSlot + 1), false);
	SetNodeUsing("SAVEBUTTON" + (iSlot + 1), false);
	SetNodeUsing("DELETEBUTTON" + (iSlot + 1), false);
	StringCollection_ChangeColor("SAVELIST_INFO", iSlot * 3 + LIST_NAME, argb(0,0,0,0));
	StringCollection_ChangeColor("SAVELIST_INFO", iSlot * 3 + LIST_SYSTEMTIME, argb(0,0,0,0));
	StringCollection_ChangeColor("SAVELIST_INFO", iSlot * 3 + LIST_PLAYTIME, argb(0,0,0,0));
}

void ReadScreenshot_Start()
{
...
void ReadScreenshot()
{
	...
	ReadScreenshotStep(iSave);
	ReadScreenshotStep(iSave + 1);
	PostEvent("ReadScreenshot", 0, "l", iSave + 2);
}

void ReadScreenshotStep(int iSave)
{
...
void FillSaveList(int nFirstSaveIndex)
{
	...
	{
		iSaveIdx = nFirstSaveIndex + iSlot;
		attr = "s" + iSaveIdx;
		ClearSaveInfoByIndex(iSlot);
		if(CheckAttribute(&g_oSaveContainer, attr))
		{
	...
	if(iSlot < g_SaveSlots - 1)
	{
		for(int i = iSlot; i < g_SaveSlots; i++)
		{
			g_oSaveList[i].empty = 1;
			ResetSlot(i);
		}
	}
...
void LoadInfo(int iSlot, int iSaveIdx, string sSaveName, int iTexture, string sData)
{
	g_oSaveList[iSlot].saveidx = iSaveIdx;
	g_oSaveList[iSlot].savefile = sSaveName;
	ReadSaveData(iSlot, iTexture, sData);	// загружаем данные сейва
}

void ReadSaveData(int iSlot, int iTexture, string sData)
{
...
void SelectSlot(int iSlot)
{
	...
	int i, textColor, pictureColor;
	for(i = 0; i < g_SaveSlots; i++)
	{
		if(CheckAttribute(&g_oSaveList[i], "empty"))
			continue;
		if(i == iSlot)
		{
			if(CheckAttributeHasValue(&g_oSaveList[i], "bad"))
			{
				pictureColor = argb(255,255,64,64);
				textColor = argb(255,255,64,64);
			}
			else
			{
				pictureColor = argb(255,128,128,128);
		...
		else
		{
			if(CheckAttributeHasValue(&g_oSaveList[i], "bad"))
			{
				pictureColor = argb(255,160,32,32);
				textColor = argb(255,160,32,32);
			}
			else
			{
				pictureColor = argb(255,70,70,70);
				textColor = argb(255,148,148,148);
			}
			SetNodeUsing("SAVEBUTTON" + (i + 1), false);
			SetNodeUsing("DELETEBUTTON" + (i + 1), false);
		}
		Picture_SetColor("SAVEFRAME" + (i + 1), pictureColor);
		StringCollection_ChangeColor("SAVELIST_INFO", i * 3 + LIST_NAME, textColor);
		StringCollection_ChangeColor("SAVELIST_INFO", i * 3 + LIST_SYSTEMTIME, textColor);
		StringCollection_ChangeColor("SAVELIST_INFO", i * 3 + LIST_PLAYTIME, textColor);
	}
	if(bSave)
	{
		...
		if(bNewSave)
		{
			...
			StringCollection_ChangeColor("SAVELIST_INFO", (MAX_SAVE_SLOTS - 1) * 3 + 1, argb(255,255,255,255));
			SetNodeUsing("SAVEBUTTON_NEW", true);
		}
		else
		{
			Picture_SetColor("SAVEFRAME_NEW", argb(255,70,70,70));
			StringCollection_ChangeColor("SAVELIST_INFO", (MAX_SAVE_SLOTS - 1) * 3 + 1, argb(255,148,148,148));
	...
	if(CheckAttribute(saveInfo, "locname") && saveInfo.locname != "")
	{
		...
		StringCollection_ChangeColor("SAVE_INFO", SAVE_NAME, argb(255,255,255,255));
	}
	else
		StringCollection_ChangeColor("SAVE_INFO", SAVE_NAME, argb(0,128,128,128));
	...
	if(CheckAttribute(saveInfo, "timeStr") && saveInfo.timeStr != "")
	{
		...
		StringCollection_ChangeColor("SAVE_INFO", SAVE_GAMETIME, argb(255,255,255,255));
	}
	else
		StringCollection_ChangeColor("SAVE_INFO", SAVE_GAMETIME, argb(0,128,128,128));
	if(CheckAttribute(saveInfo, "rank") && saveInfo.rank != "")
	{
		...
		StringCollection_ChangeColor("SAVE_INFO", SAVE_RANK, argb(255,255,255,255));
	}
	else
		StringCollection_ChangeColor("SAVE_INFO", SAVE_RANK, argb(0,128,128,128));
	if(CheckAttribute(saveInfo, "SquadPower") && saveInfo.SquadPower != "")
	{
		...
		StringCollection_ChangeColor("SAVE_INFO", SAVE_SQUADPOWER, argb(255,255,255,255));
	}
	else
	...
	if(CheckAttribute(saveInfo, "questUniqueID") && CheckAttribute(saveInfo, "questID"))
	{
		...
		bool needDeleteQuest = false;
		bool needDeleteData = false;
		bool needChangeData = false;
		string sOldData = "";
		if(CheckAttribute(saveInfo, "questUserData") && saveInfo.questUserData != "")
		{
			...
			{
				needDeleteQuest = true;
				needDeleteData = true;
			}
			else if(!CheckAttribute(pchar, "QuestInfo." + idQuest + ".UserData"))
				needDeleteData = true;
			else if(pchar.QuestInfo.(idQuest).UserData != saveInfo.questUserData)
			{
				needChangeData = true;
				sOldData = pchar.QuestInfo.(idQuest).UserData;
			}
		}
		...
		if(needChangeData)
			pchar.QuestInfo.(idQuest).UserData = sOldData;
		if(needDeleteData)
			DeleteAttribute(pchar, "QuestInfo." + idQuest + ".UserData");
		if(needDeleteQuest)
	...
	if(CheckAttribute(saveInfo, "HeroName") && saveInfo.HeroName != "")
	{
		...
		StringCollection_ChangeColor("PROFILE_INFO", PROFILE_HERONAME, argb(0,128,128,128));
	...
	if(CheckAttribute(saveInfo, "difficulty") && saveInfo.difficulty != "")
	{
		...
		StringCollection_ChangeColor("PROFILE_INFO", PROFILE_DIFFICULTY, argb(0,128,128,128));
	if(CheckAttribute(saveInfo, "GameMode") && saveInfo.GameMode != "")
	{
		...
		StringCollection_ChangeColor("PROFILE_INFO", PROFILE_GAMEMODE, argb(0,128,128,128));
	if(bBettaTestMode && CheckAttribute(saveInfo, "last_commit") && saveInfo.last_commit != "")
	{
		string sBranch = "";
		string sCommit = "";
		string sDirty = "";
		ParseCommitString(saveInfo.last_commit, &sBranch, &sCommit, &sDirty);
		SetFormatedText("SAVE_COMMIT", "Branch: " + sBranch + NewStr() + "Commit: " + sCommit + NewStr() + "Dirty: " + sDirty);
	}
	else
		SetFormatedText("SAVE_COMMIT", "");
...
void ParseCommitString(string sData, ref sBranch, ref sCommit, ref sDirty)
{
	sBranch = FindStringBeforeChar(sData, "(");
	sData = FindStringAfterChar(sData, "(");
	sCommit = FindStringBeforeChar(sData, ")");
	sData = FindStringAfterChar(sData, ")");
	sData = FindStringAfterChar(sData, "(");
	sDirty = FindStringBeforeChar(sData, ")");
}
...
void ProcessSave()
{
	...
	PostEvent("evntSave", 0, "ss", "SAVE\" + currentProfile + "\" + saveName + ".manual", sSaveDescriber);
	object OptionsList;
...
void ShowConfirmWindow(int iMode)
{
	...
	XI_WindowDisable("PROFILE_WINDOW", true);
...
void LaunchCustomSaveGame()
{
	...
	string saveName;
	saveName = GetClampedSaveName(curLocName, 0) + ".manual";
	int idx = 0;
	while(SendMessage(&GameInterface, "ls", MSG_INTERFACE_NEW_SAVE_FILE_NAME, saveName) == 1)
	{
		saveName = GetClampedSaveName(curLocName, idx + 1) + ".manual";
		idx++;
	}
	g_sReturnWindow = "MAIN_WINDOW";
	if(XI_IsWindowEnable("PROFILE_WINDOW"))
	...
	GameInterface.CUSTOM_SAVE_NAME.str = GetSaveNameWithoutExtension(saveName);
	if(GetCurSaveName() != "" && !HasSubStr(GetCurSaveName(), "QuickSave"))
	{
		saveName = GetSaveNameWithoutExtension(GetCurSaveName());
...
string GetSaveNameWithoutExtension(string saveName)
{
	string res = saveName;
	if(IsStrRight(saveName, ".auto"))
		res = strleft(saveName, strlen(saveName) - strlen(".auto"));
	else if(IsStrRight(saveName, ".quick"))
		res = strleft(saveName, strlen(saveName) - strlen(".quick"));
	else if(IsStrRight(saveName, ".manual"))
		res = strleft(saveName, strlen(saveName) - strlen(".manual"));
	return res;
}