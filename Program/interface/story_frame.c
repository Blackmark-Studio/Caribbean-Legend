...
{
	// JOKERTODO включить для всех, когда будет готов туториал
...
void ProcCommand()
{
	...
	if (comName=="downstep") _SF_SelectButtonByKeyboard(nodeName,1);
	else if (comName=="upstep") _SF_SelectButtonByKeyboard(nodeName,-1);
...
string _SF_GetHumanReadableFactor(string factorName)
{
	...
	string locValue = DLG_ConvertE("Factor" + factorName, "StoryFrames\Common.txt", &storyObject);
	if (locValue != "") return locValue; // нашли в общих ключах сторей

	if (CheckAttribute(&storyObject, "module"))
	{
		locValue = DLG_ConvertE("Factor" + factorName, "StoryFrames\" + storyObject.module + ".txt", &storyObject);
