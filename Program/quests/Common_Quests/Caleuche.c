...
void Caleuche_StartTotal(string qName) // этот долбоебизм вызван необходимостью типа не начинать новую игру... 
{
	...
	switch (int(pchar.questTemp.Caleuche.Tile))
	{
...
void Caleuche_CreateGhostship(string qName)//подгружаем в море Калеуче
{
	...
	Group_SetTaskAttack("Caleuche_Attack", PLAYER_GROUP);
	Ship_SetTaskAttack(SECONDARY_TASK, int(sld.index), GetMainCharacterIndex());
...
{
	PlaySound("interface\key.wav");
	switch (int(pchar.questTemp.Caleuche.Lever.Count))
	{
		case 0: return; break;
...
void Caleuche_CheckThreeLeverTurn() // проверям правильность поворота левых рычагов
{
	bool bOk = false;
	pchar.questTemp.Caleuche.Lever.Count = 0;
	switch (int(pchar.questTemp.Caleuche.LeftLevers))
	{
		case 0: 
...
void Caleuche_CheckSixLeverTurn() // проверям правильность поворота правых рычагов
{
	bool bOk = false;
	pchar.questTemp.Caleuche.Lever.Count = 0;
	switch (int(pchar.questTemp.Caleuche.RightLevers))
	{
		case 0: 
...
void Caleuche_LeverAttack(string qName) // ставим чавинави при неправильной последовательности
{
	...
	for (int i=1; i<=2; i++)
	{
		...
		sld.MultiFighter = 1.0+MOD_SKILL_ENEMY_RATE/20; // мультифайтер
		LAi_SetActorType(sld);
		if (int(pchar.questTemp.Caleuche.LeverType) == 0)
		{
...
{
	sld = GetCharacter(NPC_GenerateCharacter("Caleuche_Alonso", "Alonso", "man", "man", int(pchar.rank), FRANCE, 0, false, "soldier"));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	else if (sQuestName == "Caleuche_GhostshipBoardingDeck") // расстановка абордажных групп
	{
		...
		InterfaceStates.Buttons.Save.enable = false;//запретить сохраняться
		// делаем расчёт сил
		int pCrew = int(pchar.Ship.Crew.Quantity);
		int eRank = 20+MOD_SKILL_ENEMY_RATE;
		int pRank = int(pCrew*30/666);
		if (pRank < 1) pRank = 1;
		int pScl = int(pCrew*70/666)+(20-MOD_SKILL_ENEMY_RATE*2);
		if (pScl < 10) pScl = 10;
		n = 20;
		if (pCrew < 20) n = pCrew;
		// наши 
	...
	else if (sQuestName == "Caleuche_ReloadCoast") // на берег
	{
		...
		pchar.ship.HP = int(pchar.ship.HP) / 5;
		pchar.Ship.Crew.Quantity = int(pchar.ship.Crew.Quantity) / 5;
	...
	else if (sQuestName == "Caleuche_LeverFightOver") // прибили чавинави у рычагов
	{
		LAi_group_Delete("EnemyFight");
		chrDisableReloadToLocation = false;
		if (int(pchar.questTemp.Caleuche.LeverType) == 0) pchar.questTemp.Caleuche.LeverL = "true";
		if (int(pchar.questTemp.Caleuche.LeverType) == 1) pchar.questTemp.Caleuche.LeverR = "true";
	...
	else if (sQuestName == "Caleuche_ChangeShip") // меняем харки калеуче
	{
		...
		shTo = &RealShips[int(sld.Ship.Type)];
		ref refKaleuche;
