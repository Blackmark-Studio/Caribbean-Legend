...
void Regata_Start(string qName)//старт регаты
{
	...
	for (i=1; i <=5; i++)
	{
		...
		if (i==4) sTemp = "d";
		if (i==5) sTemp = "e";
		pchar.questTemp.Regata.AdversaryFirstTransition.Time.(sTemp) = 216*float(pchar.questTemp.Regata.AdversarySpeed.(sTemp));//время 1 перехода в часах
		pchar.questTemp.Regata.AdversarySecondTransition.Time.(sTemp) = 366*float(pchar.questTemp.Regata.AdversarySpeed.(sTemp));//время 1+2 переходов в часах
...
void RegataCheck(string qName)//проверку ставим в локации острова, чтобы не жульничал
{
	...
	bOk = int(RealShips[int(pchar.ship.type)].basetype) == SHIP_LUGGER || int(RealShips[int(pchar.ship.type)].basetype) == SHIP_CAREERLUGGER;
...
void RegataAttack_Lugger(string qName)//засада из 2 люггеров
{
	...
	int iRank = int(pchar.rank)+MOD_SKILL_ENEMY_RATE;
	int iScl = 5*int(pchar.rank);
	int iNation = int(pchar.questTemp.Regata.AttackNation);
	for (int i=1; i<=2; i++)
	{
...
void RegataAttack_Brander(string qName)//люггер-брандер
{
	...
	Ship_SetTaskMove(SECONDARY_TASK, int(sld.Index), -1548.87, 559.55);
...
void BranderGo(string qName)//относится к функции выше - таск брандера не действует на дрейфующий корабль
{
	...
	Ship_SetTaskBrander(SECONDARY_TASK, int(sld.Index), GetMainCharacterIndex());
...
void RegataFinishSea(string qName)//здесь пересчитаем время для установки кораблей соперников
{
	...
	Group_FindOrCreateGroup("Regata");
	int n=0;
	pchar.questTemp.Regata.FifthTransitionTime = GetPastTime("hour", int(pchar.questTemp.Regata.StartYear), int(pchar.questTemp.Regata.StartMonth), int(pchar.questTemp.Regata.StartDay), float(pchar.questTemp.Regata.StartTime), GetDataYear(), GetDataMonth(), GetDataDay(), GetTime());//истратил ГГ в часах на всю регату
	for (i=1; i <=5; i++)
	{
		if (i==1) sTemp = "a";
		if (i==2) sTemp = "b";
	...
	Log_TestInfo(FindRussianDaysString(int(pchar.questTemp.Regata.FifthTransitionTime)));
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	else if (sQuestName == "RegataSiegeOpen")
	{
		...
		shTo.SpeedRate = (float(shTo.SpeedRate)/1.2);
		shTo.TurnRate = (float(shTo.TurnRate)/1.2);
	...
	else if (sQuestName == "RegataShoreAfterBattle")	
	{
		...
		shTo = &RealShips[int(Pchar.Ship.Type)];
		shTo.SpeedRate = (float(pchar.questTemp.Regata.Shipspeed));
		shTo.TurnRate = (float(pchar.questTemp.Regata.Shipturnrate));
		n = FindIsland("Barbados");	
