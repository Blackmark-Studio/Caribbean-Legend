...
{
	pchar.questTemp.FMQ.Success = int(pchar.questTemp.FMQ.Success)+1;
	if (int(pchar.questTemp.FMQ.Success) == 5)
	{
...
{
	int iRank = int(pchar.rank)+MOD_SKILL_ENEMY_RATE-2; // Addon 2016-1 Jason пиратская линейка
	...
	pchar.quest.FMQG_Juansail.win_condition.l1.date.hour  = int(GetTime() + rand(6));
...
void FMQG_ArriveMoskitos(string qName) // 
{
	...
	chrDisableReloadToLocation = true;//закрыть локацию
	LAi_LocationFightDisable(&Locations[FindLocation(pchar.location)], true);//запретить драться
	sld = GetCharacter(int(Pchar.questTemp.FMQG.PrisonerIDX));
	ReleasePrisoner(sld);
	sld.Dialog.Filename = "Quest\LineMiniQuests\FMQ_Guadeloupe.c";
...
void FMQM_ConvoyStart(string qName) // конвой со смолой
{
	...
	for (int i = 1; i <= 3; i++)
    {
		...
		iSpace = int(iSpace/2 + hrand(iSpace/2));
		Fantom_SetCharacterGoods(sld, GOOD_SUGAR, iSpace, true);
		if (i == 2) 
		{
...
void FMQM_ArriveGuadeloupe(string qName) // 
{
	...
	realships[int(sld.ship.type)].SpeedRate = 4.0;
	sld.DontRansackCaptain = true;
	sld.AnalizeShips = true;
	...
	pchar.quest.FMQM_Tartane.win_condition.l1.date.hour  = int(GetTime() + 1);
...
void FMQM_TartaneSail(string qName) // 
{
	...
	pchar.quest.FMQM_Tartane1.win_condition.l1.date.hour  = int(GetTime() + 1);
...
void FMQM_AbandonGuadeloupe(string qName) // 
{
	...
	pchar.quest.FMQM_Tartane2.win_condition.l1.date.hour  = int(GetTime() + 3);
...
void FMQM_AbandonCoast(string qName) // в бухте Морн л'О
{
	...
	int i, n;
	n = int(MOD_SKILL_ENEMY_RATE/3 + 6);
	...
	for (i=1; i<=5; i++)
	{
		...
		sld = GetCharacter(NPC_GenerateCharacter("FMQM_Our_crew_"+i, model, "man", ani, iRank, int(pchar.nation), 0, false, "soldier"));
...
void FMQM_PeaceFinal(string qName) // 
{
	...
	bQuestDisableMapEnter = false;//открыть карту
	DeleteAttribute(pchar, "GenQuest.MapClosedNoBattle");
	int i = int(int(pchar.questTemp.FMQM.Qty)/2);
...
void FMQM_SeaBattleTartane(string qName) // 
{
	...
	SetCharacterRelationBoth(int(sld.index), GetMainCharacterIndex(), RELATION_ENEMY);
	realships[int(sld.ship.type)].SpeedRate = 12.5;
	RefreshBattleInterface();
	UpdateRelations();
	Group_LockTask("FMQM_Tartane");
...
void FMQN_ReloadToGovernor() // релоад к губеру
{
	...
	if (float(environment.time) < 10.0) WaitDate("",0,0,0,4,5);
...
void FMQN_EnglandInShoreAttack(string qName) // рубилово в бухте
{
	...
	int iRank = int(pchar.rank)/2+MOD_SKILL_ENEMY_RATE;
	int iScl = 20 + int(pchar.rank);
	LAi_group_Delete("EnemyFight");
...
void FMQN_EnglandSeaAttack(string qName) // атака в море
{
	...
	if(int(RealShips[int(pchar.ship.type)].Class) == 5) iShipType = SHIP_SCHOONER_W;
...
void FMQT_Activation(string qName) // 
{
	...
	if (int(pchar.reputation.nobility) > 41) sld.Dialog.currentnode = "girl";
...
void FMQT_MercenEnter(string qName) // входит наемник 
{
	...
	int i = int(MOD_SKILL_ENEMY_RATE/3);
	int n = 1+i;
...
void FMQT_FinalWithSex() // 
{
	...
	else
	{
		...
		if (int(Pchar.money) >= 50000) Pchar.money = int(Pchar.money)-25000;
		else Pchar.money = int(Pchar.money)/2;
...
void FMQP_InSantoDomingo(string qName) // 
{
	...
	int iRank = MOD_SKILL_ENEMY_RATE/2+int(pchar.rank)/2;// Addon 2016-1 Jason пиратская линейка
	int iScl = 15 + int(pchar.rank);
...
{
	if(bImCasual) return;
	int iRank = MOD_SKILL_ENEMY_RATE/2+int(pchar.rank);// Addon 2016-1 Jason пиратская линейка
	int iScl = 10 + 2*int(pchar.rank);
	int iShipType = GetRandomShipType(GetClassFlag(int(RealShips[int(pchar.ship.type)].Class)), FLAG_SHIP_TYPE_RAIDER, FLAG_SHIP_NATION_ANY);
...
void FMQP_CreateUncleLuis(string qName) // 
{
	...
	// дядюшка
	int iRank = MOD_SKILL_ENEMY_RATE+int(pchar.rank)-2;// Addon 2016-1 Jason пиратская линейка
	int iScl = 15 + 2*int(pchar.rank); // may-16
...
void FMQL_AbandonBdlS(string qName) // 
{
	...
	chrDisableReloadToLocation = true;
	int oRank = int(pchar.rank)+5;
	int eRank = int(pchar.rank)-1;
	int oScl = 20 + 2*int(pchar.rank);
	int eScl = 10 + 2*int(pchar.rank);// Addon 2016-1 Jason пиратская линейка
	// ставим наших
	sld = CharacterFromID("Guide_y");
	RemoveCharacterCompanion(pchar, sld);
...
{
	int iRank = int(pchar.rank)-1+MOD_SKILL_ENEMY_RATE/2;// Addon 2016-1 Jason пиратская линейка
...
{
	chrDisableReloadToLocation = true;
	int iRank = int(pchar.rank)+8-int(MOD_SKILL_ENEMY_RATE/2);
	int iScl = 25 + 2*int(pchar.rank);
...
{
	int iRank = int(pchar.rank)+1+MOD_SKILL_ENEMY_RATE/2;
	int iScl = 35 + 3*int(pchar.rank);
	LAi_group_MoveCharacter(sld, "EnemyFight");
	// вождь
...
void FMQL_OnLamantenBeach(string qName) // на самом пляже
{
	...
	TakeNItems(sld, "potion2", int(MOD_SKILL_ENEMY_RATE/2)); // may-16
	sld.SaveItemsForDead = true; 
	sld.DontClearDead = true;
...
void FMQL_CreateGuidesWarriors() // ставим кодлу гида
{
	...
	for (int i=1; i<=2; i++)
	{
		...
		TakeNItems(sld, "potion2", int(MOD_SKILL_ENEMY_RATE/3)); // may-16
		//sld.cirassId = Items_FindItemIdx("cirass4");
		sld.LSC_clan = true; // may-16
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	else if (sQuestName == "FMQT_PirateInTavern") // пирата с ключом - в таверну
	{
		...
		iScl = 15 + 2*int(pchar.rank);// Addon 2016-1 Jason пиратская линейка
	...
	else if (sQuestName == "FMQT_MercenHire") // 
	{
		...
		LAi_SetImmortal(sld, false);
		sld.quest.OfficerPrice = int(pchar.rank)*2000;
	...
	{
		iRank = MOD_SKILL_ENEMY_RATE+int(pchar.rank)-2;// Addon 2016-1 Jason пиратская линейка
	...
	{
		iRank = int(pchar.rank) + MOD_SKILL_ENEMY_RATE + 2; // Addon 2016-1 Jason пиратская линейка
		iScl = 30 + 2 * int(pchar.rank);
		// наши
		sld = CharacterFromID("Guide_y");
	...
	else if (sQuestName == "FMQL_CoastClear") // всех перебили
	{
		...
		AddQuestUserData("FMQ_Lesson", "sSanl", FindRussianQtyString(int(pchar.questTemp.FMQL.Sanl)));
		AddQuestUserData("FMQ_Lesson", "sSilk", FindRussianQtyString(int(pchar.questTemp.FMQL.Silk)));
		AddQuestUserData("FMQ_Lesson", "sRope", FindRussianQtyString(int(pchar.questTemp.FMQL.Rope)));
		AddQuestUserData("FMQ_Lesson", "sOil", FindRussianQtyString(int(pchar.questTemp.FMQL.Oil)));
	...
	else if (sQuestName == "FMQL_AfterBattleOnBeach") // закончили бой с кодлой Валинье
	{
		...
		RemoveGeometryFromLocation("Shore38", "smg");
		LAi_group_Delete("EnemyFight");
		SetCharacterGoods(pchar, GOOD_SANDAL, GetCargoGoods(pchar, GOOD_SANDAL) + int(pchar.questTemp.FMQL.Sanl));
		SetCharacterGoods(pchar, GOOD_SHIPSILK, GetCargoGoods(pchar, GOOD_SHIPSILK) + int(pchar.questTemp.FMQL.Silk));
		SetCharacterGoods(pchar, GOOD_ROPES, GetCargoGoods(pchar, GOOD_ROPES) + int(pchar.questTemp.FMQL.Rope));
		SetCharacterGoods(pchar, GOOD_OIL, GetCargoGoods(pchar, GOOD_OIL) + int(pchar.questTemp.FMQL.Oil));
		DeleteAttribute(pchar, "GenQuest.CannotWait");
		LAi_LocationDisableOfficersGen("Shore38", false);
		...
		if (CheckAttribute(pchar, "questTemp.FMQL.Cpy"))
		{
			...
			{
				sld = CharacterFromID("Duran");
				if (!CheckAttributeEqualTo(sld, "OfficerImmortal", "Injury")) // пират не ранен
				{
					sld.dialog.FileName = "Quest\LineMiniQuests\FMQ_Lesson.c";
