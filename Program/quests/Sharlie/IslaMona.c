...
void IslaMona_HimenesWait() // ждать сутки на Исла Моне
{
	...
	pchar.quest.islamona_wait.win_condition.l1.date.hour  = int(GetTime());
...
void IslaMona_FactoryReActivar() // перезапуск фактории после защиты Исла Моны
{
	...
	pchar.questTemp.IslaMona.Factory.Part = int(pchar.questTemp.IslaMona.RecordPart);
	DeleteAttribute(pchar, "questTemp.IslaMona.RecordPart");
	pchar.questTemp.IslaMona.Factory.Goods = 20*int(pchar.questTemp.IslaMona.Factory.Part);
	pchar.questTemp.IslaMona.Factory.Dublon = 20*int(pchar.questTemp.IslaMona.Factory.Part);
	pchar.questTemp.IslaMona.Factory.Bottle = 5*int(pchar.questTemp.IslaMona.Factory.Part);
	for(int i=1; i<=5; i++)
	{
...
{
	if (int(pchar.questTemp.IslaMona.Factory.Part) > 5) return;
	//log_info("Родриго Хименес подготовил очередную партию бакаута на Исла Мона");
	LaunchMessage(StringFromKey("IslaMona_4"));
	pchar.questTemp.IslaMona.Factory.Part = int(pchar.questTemp.IslaMona.Factory.Part)+1;
	pchar.questTemp.IslaMona.Factory.Goods = 20*int(pchar.questTemp.IslaMona.Factory.Part);
	pchar.questTemp.IslaMona.Factory.Dublon = 20*int(pchar.questTemp.IslaMona.Factory.Part);
	pchar.questTemp.IslaMona.Factory.Bottle = 5*int(pchar.questTemp.IslaMona.Factory.Part);
}
...
{
	PlaySound("interface\notebook.wav");
	RemoveDublonsFromPCharTotal(int(pchar.questTemp.IslaMona.Factory.Dublon));
	RemoveItemsFromPCharTotal("potionrum", int(pchar.questTemp.IslaMona.Factory.Bottle));
	RemoveItemsFromPCharTotal("potionwine", int(pchar.questTemp.IslaMona.Factory.Bottle));
	notification(StringFromKey("IslaMona_5", int(pchar.questTemp.IslaMona.Factory.Bottle)), "None");
	notification(StringFromKey("IslaMona_6", int(pchar.questTemp.IslaMona.Factory.Bottle)), "None");
	//Log_Info("Вы отдали "+int(pchar.questTemp.IslaMona.Factory.Bottle)+" бутылок рома");
	//Log_Info("Вы отдали "+int(pchar.questTemp.IslaMona.Factory.Bottle)+" бутылок вина");
	sld = &stores[29];
	AddStorageGoods(sld, GOOD_SANDAL, int(pchar.questTemp.IslaMona.Factory.Goods));
	//Log_Info("На ваш склад загружено "+int(pchar.questTemp.IslaMona.Factory.Goods)+" бревен бакаута");
	LaunchMessage(StringFromKey("IslaMona_7", int(pchar.questTemp.IslaMona.Factory.Goods)));
	pchar.questTemp.IslaMona.Factory.Part = 0;
...
void IslaMona_ChurchComplete(string qName) // церковь построена - начало защиты Исла Моны
{
	...
	pchar.questTemp.IslaMona.RecordPart = int(pchar.questTemp.IslaMona.Factory.Part);
