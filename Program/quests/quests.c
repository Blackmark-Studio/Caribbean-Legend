...
void QuestsInit()
{
	...
	for(int i=0; i<q; i++)
	{
		...
		DoQuestCheckDelay(arTmp.qname, float(arTmp.delay));
	}*/
...
void CharacterDeadProcess()
{
	...
	ref chref = GetCharacter(int(charef.index));
	...
	switch(charType)
	{
		...
		case FELLOWTRAVEL_OFFICER:
			...
			CheckForReleaseOfficer(int(chref.index)); // boal 25/06/06 убираем совместителя  RemoveOfficersIndex(pchar,int(chref.index));
...
void ReOpenQuestHeader(string idQuest)
{
	...
	if (CheckAttribute(mainCh,"QuestInfo."+idQuest))// && int(mainCh.QuestInfo.(idQuest).Complete) == true)
	{
        object refNewAttr;
        aref   curQ, newQ,oldQ;
...
{
	return GetTimeStringEx(int(worldMap.date.hour), int(worldMap.date.min));
}
// boal <--
...
void AddQuestUserData(string idQuest, string strID, string strData)
{
	...
	aref arCurQuest;
	makearef(arCurQuest, pchar.QuestInfo.(idQuest));
	if (CheckAttribute(arCurQuest, "UserData"))
	{
...
float GetLocatorSqrDistanceToMe(aref locatorRef)
{
	...
	x = float(locatorRef.x)-x;
	y = float(locatorRef.y)-y;
	z = float(locatorRef.z)-z;
	return (x*x+y*y+z*z);
}
...
bool SetCameraToShipView(string characterName, string questCheckName, float viewTimeInSec)
{
	...
	float xship = float(chrRef.ship.pos.x);
	float zship = float(chrRef.ship.pos.z);
	...
	PostEvent("evntQuestCameraRestore",int(viewTimeInSec*1000),"s",questCheckName);
...
bool CheckCurLocator(string group,string locator,float x,float y,float z)
{
	...
	float xloc = float(curloc.x) - x;
	float yloc = float(curloc.y) - y;
	float zloc = float(curloc.z) - z;
	float rad = float(curloc.radius);
...
{
	bool deleteOldQuest = CheckAttribute(pchar,"quest."+questCheckName);
	...
	PostEvent("QuestDelayExit", int(delayInSecond*1000), "sl", questCheckName, deleteOldQuest);
}
...
void DoQuestDelayExit()
{
	...
	if (CheckAttribute(pchar, "PostEventQuest.questDelay."+stmp))
	{
		if (stmp != "")
		{
			if (deleteOldQuest)
...
void ExitActAnimation()
{
	...
	ref chref = GetCharacter(int(apchar.index));
...
bool RestorePassengers(string idCharacter)
{
	...
	for(i=0; i<GetAttributesNum(arTmp); i++)
	{
		...
		idx = int(GetAttributeValue(arCur));
...
int	GetCharacterFromFantom(ref chref)
{
	...
	int retVal = -1;
	ref fantRef;
	int startVal = int(chref.index)+1;
...
void CompleteQuestName(string sQuestName, string qname)
{
	...
	{
		Event("qprocTaskEnd","a",GetCharacter(int(objQuestScene.list.(sQuestName).chrIdx)));
	}
	else
	{
...
void FoundAndDeleteAllQuestForQuestNumberExceptCurrentQuest(int iQuest, string sCurrentQuest)
{
	...
	for(int i=0; i < q; i++)
	{
		...
		if(int(aQuestsDetailed.quest) == iQuest && sCheckQuest != sCurrentQuest)
		{
...
void FoundAndDeleteAllQuestForQuestNumber(int iQuest)
{
	...
	for(int i=0; i<q; i++)
	{
		...
		{
			if(int(aQuestsDetailed.quest) == iQuest)
			{
				aQuestsDetailed.over = "yes";
			}
		}
...
int TransferGoods_CalculateWeight(ref _character)
{
	...
	for(int i = 0; i < GetArraySize(&Goods); i++)
	{
		...
		{
			weight += GetGoodWeightByType(i, int(PChar.TransferGoods.(characterId).(goodName)) - GetCargoGoods(_character, i));
		}
	}
...
int TransferGoods_StartTransfer(ref rChar, string sColony) // rChar - кому будет ложить товар в трюм, sColony - в какой колонии закупаем
{
	...
	for(i = 0; i < GetArraySize(&Goods); i++)
	{
		...
		iNeedGoodsQty = int(PChar.TransferGoods.(characterId).(sGood)); // Сколько нужно ВСЕГО данного товара (не докупить!)
		...
		if(iCurGoodQty > iNeedGoodsQty) // продаем
		{
			...
			iCost = GetStoreGoodsPrice(rStore, i, 1, PChar, 1)*iNeedGood/int(rGood.Units); // Цена товара для продажи
		...
		if(iCurGoodQty < iNeedGoodsQty) // докупаем
		{
			...
			iCost = GetStoreGoodsPrice(rStore, i, 0, PChar, 1)*iNeedGood/int(rGood.Units); // Цена товара для покупки
			if(int(PChar.Money) >= iCost)
			{
	...
	if(iMoneyQty != 0) // Если хоть что-то продали или купили
	{
		...
		AddCharacterExpToSkill(rTreasurer, "Commerce", int(abs(iMoneyQty) / 800) + rand(1) + 2); // Экспа в навык торговли
	}
...
bool CheckQuestInfo()
{
	...
	for(i=0; i<GetAttributesNum(arQuestInfo); i++)
    {
		...
		{
			if(int(pchar.QuestInfo.(attributeName).Complete) == false && CheckAttribute(pchar,"QuestInfo."+attributeName+ ".color")) return true;
		}
	}
	return false;
}
...
void CaleucheLeversAction()
{
	...
	{
		pchar.questTemp.Caleuche.Lever.Count = int(pchar.questTemp.Caleuche.Lever.Count)+1;
