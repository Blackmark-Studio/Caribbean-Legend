...
{
    int yy = int(Environment.date.year);
    int mm = int(Environment.date.month);
    int dd = int(Environment.date.day);
...
void SelectAction(ref rid) //CASE с действиями для слухов
{
	...
	switch(CurrentRumour.event) // если слух с действием, то выполняем:
	{
		...
		{
			//////////////////////////////////////////
			EndTime_GoldFleet(rid.id$int(0));
			///////////////////////////////////////////
		}
		break;
...
string SelectRumour() // Получить рандомный слух из очереди
{
    ...
    int st;
    ref CurrentRumour;
    for(int Rumour_Index = rand(MAX_RUMOURS - 1); Rumour_Index<MAX_RUMOURS; Rumour_Index++)
    {
        makeref(CurrentRumour, Rumour[Rumour_Index]);
        st=int(CurrentRumour.state);

        if ((int(CurrentRumour.actualtime) >= DateToInt(0))
        && (st > 0) && CurrentRumour.text != "") // Слух не просрочен
        {                                                                    // Не выпадал до этого несколько раз

            if (int(CurrentRumour.starttime) <= DateToInt(0))
            {
                st--;
                CurrentRumour.state = st;  //n раз сказал и все!
                rez = CurrentRumour.text;
                AddRumourLogInfo(CurrentRumour.id$int(0)); // fix
        ...
        else
        {
            ...
            DeleteRumor(FindRumour(CurrentRumour.id$int(0))); // просроченные сразу трем
        }
...
bool RumourCheker(ref rRumour, string key, aref arPrm)
{
	...
	if (CheckAttribute(rRumour, "nonation"))
	{
		...
		{
			string svar = "n"+i;
            if (CheckAttribute(aNation, svar) && int(aNation.(svar)) == iNation)
			{
                return false;
			}
		}
    ...
    if (int(rRumour.starttime) > DateToInt(0))
    {
        return false;
    }
	...
	if ((CheckAttribute(rRumour, "onlynation")) && int(rRumour.onlynation) != iNation)
    {
        return false;
    }
...
{                              // key - ключ спец. слуха
    ...
    for(Rumour_Index = 0; Rumour_Index < MAX_RUMOURS; Rumour_Index++)
    {
        ...
        {
            if (int(CurrentRumour.actualtime) >= DateToInt(0) && st > 0 && CurrentRumour.text != "")  // непросроченный
            {
    ...
    {
        rnd = rand(i - 1);
        int pin = FindRumour(TEMP[rnd].id$int(0));
        Rumour[pin].LastNPC = arChr.id;
        AddRumourLogInfo(TEMP[rnd].id$int(0));
        SelectAction(&TEMP[rnd]); // Если слух с действием, то выполняем
        st = TEMP[rnd].state$int(0);
...
int AddRumor(string Text, string Status, string Key, string Repa, string Start, string Period, string action, string Next)//Добавляем слух в очередь слухов
{
 ref tmp1;
    ...
    tmp1.tip = Key;
    tmp1.rep = Repa;
    tmp1.starttime = DateToInt(int(Start));
    tmp1.actualtime = DateToInt(int(Period)+int(Start));
    tmp1.event = action;
    tmp1.next = Next;
...
int AddRumorR(ref rum)
{
	...
	tmp1 = rum;
	tmp1.starttime = DateToInt(int(rum.starttime));
	tmp1.actualtime = DateToInt(int(rum.starttime)+int(rum.actualtime)); //fix
	...
	for(Rumour_Index = 0; (Rumour_Index < MAX_RUMOURS) && (add == false); Rumour_Index++)
	{
		...
		if (nextORevent == true &&	CheckAttribute(CurrentRumour, "care") && 
			int(CurrentRumour.care) > 0 && Rumour_Index == (MAX_RUMOURS - 1))
			//navy -- CurrentRumour.care > 0, тоже не стработает, т.к. строка с целым.. приводим типы!
		{
			SelectAction(int(CurrentRumour.id));
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////
		//navy -- а вот это не понял? .tip строка!!! никогда не сработает
		if (int(CurrentRumour.tip) != 0 &&				//если слух специальный
			int(tmp1.tip) > int(CurrentRumour.tip) &&	//и свежий
...
void  ReplaceRumorR(int rep, ref rum)
{
	...
	if (rep != -1)
	{
		...
		rum.id = id_counter;
		rum.starttime = DateToInt(int(rum.starttime));
		rum.actualtime = DateToInt(int(rum.actualtime)+int(rum.actualtime));
...
void  ReplaceRumor(int rep, string Text, string Status, string Key, string Repa, string Start, string Period, string action, string Next)
{
	...
	if (rep != -1)
	{
		...
		CurRumour.tip = Key;
		CurRumour.rep = Repa;
		CurRumour.starttime = DateToInt(int(Start));
		CurRumour.actualtime = DateToInt(int(Period)+int(Start));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
int AddSimpleRumourEx(string stext, string City, int terms, int qty, string sEvent, string AddString)
{
    object tmp;
    ref mc;
	mc = GetMainCharacter();
