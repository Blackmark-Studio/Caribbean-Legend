...
string DLG_RunKnownFunction(string functionName, ref context, ref args, int argsCount)
{
  ...
  switch (functionName)
  {
    ...
    case "modifier":        return HumanModifierValue(args, context);                         break;
    ...
    case "ctx":             return DLG_GetObjectAttributeSafe(context, DLG_A0(args));         break;
    case "goodI":           return DLG_GoodName(DLG_A0(args));                                break;
...
string DLG_RunReplace(string key, ref context)
{
  switch(key)
  {
    ...
    case "$playerNationAbl": return NationNameSK(GetBaseHeroNation()); break;
    case "$red":         return SF_Indent();                         break;
    case "$br": return NewStr()                                      break;
  }
...
{
  DLG_Replace(input, context, "$br");
  DLG_Replace(input, context, "$sir");
  DLG_Replace(input, context, "$red");
