...
{
	if (_STH_IsFortDestroyed(sColony)) return true;
	if (CheckAttributeEqualTo(pchar, "GenQuestFort.StartAttack", "1")) return true; // идёт штурм с суши
	if (!bSeaLoaded && !CheckAttributeEqualTo(loadedLocation, "type", "jungle")) return true; // не из джунглей
...
// Разрушен ли форт колонии
bool _STH_IsFortDestroyed(string sColony)
{
	int fortIdx = GetCharacterIndex(sColony + " Fort Commander");
	if (fortIdx < 0) return false;

	ref fortChr = GetCharacter(fortIdx);
	int mode = GetAttributeInt(fortChr, "fort.mode");
	return mode == FORT_ABORDAGE || mode == FORT_DEAD;
}

int STH_GetColonyStatus(string sColony)
{
	if (sColony == "LosTeques") return STH_LEGAL; // не знаю, почему есть такая колония, но допустим
