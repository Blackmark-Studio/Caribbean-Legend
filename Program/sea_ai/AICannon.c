...
bool Cannon_LoadBall()
{
	...
	int iBallType = int(aCharacter.Ship.Cannons.Charge.Type);
...
void Cannon_UnloadBall()
{
	...
	{
		AddCharacterGoodsCannon(aCharacter, int(aCharacter.Ship.Cannons.Charge.Type), 1);
	}	
	// boal -->
	AddCharacterGoodsCannon(aCharacter, GOOD_POWDER, 1);
	// boal <--
...
void AddCharacterGoodsCannon(aref aCharacter, int iGood, int iQuantity)
{
	...
	aCharacter.Ship.Cargo.Goods.(sGoodName) = int(aCharacter.Ship.Cargo.Goods.(sGoodName)) + iQuantity;
...
{
	if (int(Characters[iCharacterIndex].Ship.Cannons.Type) == CANNON_TYPE_NONECANNON) { return; }

	ref	rCharacter = GetCharacter(iCharacterIndex);
	ref	rCannon = GetCannonByType(int(rCharacter.Ship.Cannons.Type));
	ref	rBall = GetGoodByType(int(rCharacter.Ship.Cannons.Charge.Type));
	ref realShip = GetRealShip(GetCharacterShipType(rCharacter));
	if (CheckAttribute(rCharacter, "TmpPerks.LongRangeShoot"))
	{
		rCharacter.Ship.Cannons.SpeedV0 = float(rCannon.SpeedV0) * float(rBall.SpeedV0) * AIShip_isPerksUse(int(rCharacter.TmpPerks.LongRangeShoot), 1.0, PERK_VALUE_LONG_RANGE_SHOOT); //slib
	}
	else
	{
	    rCharacter.Ship.Cannons.SpeedV0 = float(rCannon.SpeedV0) * float(rBall.SpeedV0);
	}
	...
	if(IsMainCharacter(rCharacter) && ShipBonus2Artefact(rCharacter, SHIP_GALEON_SM) && GetCharacterEquipByGroup(rCharacter, BLADE_ITEM_TYPE) == "lacrima_patris")
	{
		...
		if(CheckAttribute(Blade,"KillerBonus.RangeBonus")) RangeBonusMtp += float(Blade.KillerBonus.RangeBonus)/100.0;
	}
	RangeBonusMtp += GetAttributeFloat(realShip, "tuning.modifiers." + M_SHIP_FIRE_DISTANCE);
	rCharacter.Ship.Cannons.SpeedV0 = float(rCharacter.Ship.Cannons.SpeedV0) * RangeBonusMtp;
...
float Cannon_GetFireHeight()
{
	...
	ref rBallType = GetGoodByType(int(aCharacter.Ship.Cannons.Charge.Type));
	ref rEnemyShip = GetRealShip(int(aEnemy.Ship.Type));

	string sBallName = rBallType.Name;

	float Y,DY;
	Y = float(rEnemyShip.Height.(sBallName).Y);
	DY = float(rEnemyShip.Height.(sBallName).DY);
	...
	ref	rCannon = GetCannonByType(int(aCharacter.Ship.Cannons.Type));
	if (sBallName == "Bombs" || sBallName == "Grapes")
	{
	   Y = Y + 0.6;
	}
	if (int(aEnemy.Ship.Type) == SHIP_FORT)
	{
	   Y = 40;
	   DY = 16;
	}
	float RY = abs(Y * (dist / ( float(rCannon.FireRange) / 2)) + (frnd()-0.5) * DY / 4);
	if (RY < 0.8)
	{
	   RY = 0.8;
	}
...
float Cannon_GetRechargeTime()
{
	...
	float	fCannonSkill = 1.0 - float(aCharacter.TmpSkill.Cannons) / 3.0;
	...
	fMultiply = fMultiply * AIShip_isPerksUse(int(aCharacter.TmpPerks.FastReload), 1.0, 0.9);
	fMultiply = fMultiply * AIShip_isPerksUse(int(aCharacter.TmpPerks.ImmediateReload), 1.0, 0.5);
	...
	{
		fMultiply *= 1.0 - float(aCharacter.Tmp.LastBortFire.Bonus) / 100.0;
	}
	...
	if (int(refBaseShip.BaseType) != SHIP_FORT)
	{
		...
		if (int(aCharacter.index) != GetMainCharacterIndex())
		{
	    ...
	    {
            float  fExp;
			fExp = 0.001 + float(GetCrewExp(aCharacter, "Cannoners") * crewQty) / float(OptCrew * GetCrewExpRate());
			if (fExp > 1) fExp = 1;
...
float Cannon_GetFireTime()
{
	...
	float fCannonSkillNormalized = 1.8 - 1.0*pow(float(aCharacter.TmpSkill.Cannons)/100, 0.65); //нормализация задержки по скиллу дает от +40% до -40% в зависимости от навыка, а не фиксу
	...
	float crewMax = float(refBaseShip.MaxCrew);
	float crewOpt = float(refBaseShip.OptCrew);
...
void Cannon_FireCannon()
{
	...
	bool bMode = bool(GetEventData());
	string bort = GetEventData();
	
	//log_testinfo("Ядро для борта " + 	bort);
	 // boal навел порядок по оптимизации
	Ball_AddBall(aCharacter, fX, fY, fZ, fSpeedV0, fDirAng, fHeightAng, fCannonDirAng, fMaxFireDistance, bMode, bort);
}
...
float Cannon_DamageEvent()
{
	...
	{
		if(int(aCharacter.chr_ai.immortal) == 1)
		{
			return 0.0;
		}
	}
	...
	if (aCharacter.index == GetMainCharacterIndex() && GetCharacterIndex("Knippel") != -1 && int(aCharacter.Fellows.Passengers.cannoner) != -1)
	{
		ref rKnippel = CharacterFromID("Knippel");
		if (IsCharacterPerkOn(rKnippel, "Bombardier") && int(aCharacter.Fellows.Passengers.cannoner) == int(rKnippel.index)) fCurDamage = fCurDamage/2.0;
	}
    
	//fCurDamage = fCurDamage - fCurDamage/100*(int(aCharacter.ship.upgrades.cannons)-1)*20;
	if (fCurDamage >= 1.0)
	{
		fCurDamage = 1.0;
		CreateBlast(x,y,z);
...
int GetBortIntactCannonsNum(ref rCharacter, string sBort, int iNumCannonsOnBort)
{
	float fDamage  = 0.0;
        ...
        if(int(rCharacter.ship.type) == SHIP_NOTUSED) return 0;
	...
	{
		fDamage = float(GetAttributeValue(GetAttributeN(arDamages, i)));
...
int GetBortCannonsQtyMax(ref rCharacter, string sBort)
{
	...
	int 	nShipType = int(rCharacter.ship.type);
	ref 	refBaseShip = GetRealShip(nShipType);	
	
	iCannons = int(refBaseShip.(sBort));	// это теоретический максимум !
...
bool SetHeel_XZ(ref rCharacter,  ref float_x, ref float_z)
{
	...
	ang_z = MAX_HEEL_ANGLE * float(iDelta)/float(iRCannon);
	ang_x = 0.0; // TODO
