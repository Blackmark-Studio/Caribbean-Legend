...
void Cannon_RecalculateParameters(int iCharacterIndex)
{
	...
	float vSpeedMtp = float(rBall.SpeedV0) * AIShip_isPerksUse(int(rCharacter.TmpPerks.LongRangeShoot), 1.0, PERK_VALUE_LONG_RANGE_SHOOT);
	...
	for (int _i, ref bortName: borts)
	{
		...
		rCharacter.Ship.Cannons.(bortName).FireAngMax = rCannon.FireAngMax;
		rCharacter.Ship.Cannons.(bortName).FireAngMin = rCannon.FireAngMin;
	}

	CAN_CashCannonsHealth(rCharacter);
}
...
{
	aref	aCharacter = GetEventData();
	string bortName = GetEventData();

	float	fCannonSkill = 1.0 - float(aCharacter.TmpSkill.Cannons) / 3.0;

	ref		rCannon = GetCannonOnBort(aCharacter, bortName);
	...
	if (int(refBaseShip.BaseType) != SHIP_FORT)
	{
	    ...
	    {
            float  fExp;
			fExp = 0.001 + float(GetCrewExp(aCharacter, "Cannoners") * crewQty) / float(MaxCrew * GetCrewExpRate());
			if (fExp > 1) fExp = 1;
	...
	return func_fmax(0.1, fMultiply) * fReloadTime * fCannonSkill;
}
...
float Cannon_GetFireTime()
{
	...
	float crewQ = func_fmin(crewMax, GetCrewQuantity(aCharacter));
	float fCannonersExp = GetCrewExp(aCharacter, "Cannoners");
	float fCrew = crewQ / crewMax;
...
{
	aref	aCharacter = GetEventData();
	if (LAi_IsImmortal(aCharacter)) return 0.0;
	...
	if (IsMainCharacter(aCharacter)) notification(XI_ConvertString("Cannon_DamageEvent"), "Cannons");
	aCharacter.Ship.Cargo.RecalculateCargoLoad = true;
	return 1.0;
}
