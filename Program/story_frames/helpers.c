...
{
	int mode = GetAttributeIntOrDefault(&StoryObject, "checkResults", SF_NORMAL_MODE);
	switch (mode)
	{
		case SF_SUCCESS_MODE: return value; break;
		case SF_FAIL_MODE:   return 0; break;
	}
...
bool SF_ChanceLock(int percent)
{
	...
	switch (mode)
	{
		case SF_SUCCESS_MODE: return true; break;
		case SF_FAIL_MODE: return false; break;
	}
...
// Инит общего модуля сторей, чтобы локализация работала
void SF_InitModule(string moduleName)
{
	storyObject.module = moduleName;
}
...
void SF_FailEx(string letterIndex)
{
	...
	_SF_Log(SF_Convert(letterIndex + "fail"), false);
	action = SF_AddAction(letterIndex + "fail", "", "", SF_Icon("story", "fail"));
