...
void CalcLocalTime(float _inc) // TO_DO: DEL
{
	if (locTmpTime >= _inc)
	{
		/*int hour = GetHour();
...
string GetDataFromSave(string save_data, string data_type)
{
	object saveAttributes = DeserializeAttributes(save_data);
	if(CheckAttribute(&saveAttributes, data_type))
		return saveAttributes.(data_type);
	return "unknown";
}
